...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "key_version", sa.Integer(), unique=True, nullable=False, comment="Version number of the encryption key"
        ),
        sa.Column(
            "encrypted_key", sa.LargeBinary(), nullable=False, comment="Encrypted key data (encrypted with master key)"
        ),
        sa.Column(
            "created_at",
        ...
        sa.Column(
            "rotated_at", sa.DateTime(timezone=True), nullable=True, comment="When key was rotated (deactivated)"
        ),
        sa.Column(
            "is_active",
            sa.Boolean(),
        ...
        sa.Column("created_by", sa.String(100), nullable=True, comment="User/system that created this key"),
        sa.Column("notes", sa.Text(), nullable=True, comment="Additional notes about key creation/rotation"),
    )
    ...
    op.create_table(
        ...
        sa.Column("id", sa.Integer(), primary_key=True, autoincrement=True),
        sa.Column("key_version", sa.Integer(), nullable=True, comment="Version of encryption key used"),
        sa.Column(
            "operation", sa.String(50), nullable=False, comment="Type of operation (encrypt, decrypt, rotate, migrate)"
        ),
        sa.Column("table_name", sa.String(100), nullable=True, comment="Database table involved in operation"),
        sa.Column("column_name", sa.String(100), nullable=True, comment="Database column involved in operation"),
        sa.Column("user_id", sa.String(100), nullable=True, comment="User ID associated with operation"),
        sa.Column("session_id", sa.String(100), nullable=True, comment="Session ID for request tracking"),
        sa.Column(
            "timestamp",
            sa.DateTime(timezone=True),
            nullable=False,
            server_default=text("NOW()"),
            comment="When operation occurred",
        ),
        sa.Column(
            "field_type", sa.String(50), nullable=True, comment="Type of field (email, tax_id, personal_data, etc.)"
        ),
        sa.Column(
            "success", sa.Boolean(), nullable=False, default=True, comment="Whether operation completed successfully"
        ),
        sa.Column("error_message", sa.Text(), nullable=True, comment="Error message if operation failed"),
        sa.Column("ip_address", sa.String(45), nullable=True, comment="IP address of client (for audit)"),
        sa.Column("user_agent", sa.Text(), nullable=True, comment="User agent string (for audit)"),
        sa.Column("processing_time_ms", sa.Float(), nullable=True, comment="Time taken for operation in milliseconds"),
        sa.Column("data_size_bytes", sa.Integer(), nullable=True, comment="Size of data processed in bytes"),
    )
    ...
    op.create_index("idx_encryption_audit_log_user_timestamp", "encryption_audit_log", ["user_id", "timestamp"])
    op.create_index("idx_encryption_audit_log_table_operation", "encryption_audit_log", ["table_name", "operation"])
    op.create_index("idx_encryption_audit_log_timestamp_success", "encryption_audit_log", ["timestamp", "success"])
    ...
    op.create_table(
        "encryption_config",
        sa.Column("id", sa.Integer(), primary_key=True, autoincrement=True),
        sa.Column("config_key", sa.String(100), unique=True, nullable=False, comment="Configuration parameter name"),
        sa.Column("config_value", sa.Text(), nullable=True, comment="Configuration parameter value"),
        sa.Column(
            "config_type",
            sa.String(50),
            nullable=False,
        ...
        sa.Column("description", sa.Text(), nullable=True, comment="Description of configuration parameter"),
        sa.Column(
            "is_sensitive",
            sa.Boolean(),
            nullable=False,
        ...
        sa.Column("updated_by", sa.String(100), nullable=True, comment="User who last updated this config"),
    )
    ...
    op.create_table(
        ...
        sa.Column("id", sa.Integer(), primary_key=True, autoincrement=True),
        sa.Column("table_name", sa.String(100), nullable=False, comment="Name of the database table"),
        sa.Column("column_name", sa.String(100), nullable=False, comment="Name of the encrypted column"),
        sa.Column(
            "field_type", sa.String(50), nullable=False, comment="Type of field (email, tax_id, personal_data, etc.)"
        ),
        sa.Column(
            "encryption_enabled",
            sa.Boolean(),
        ...
        sa.Column("key_version", sa.Integer(), nullable=True, comment="Key version used for this field"),
        sa.Column(
            "migration_status",
            sa.String(50),
            nullable=False,
        ...
        sa.Column("total_records", sa.Integer(), nullable=True, comment="Total number of records in table"),
        sa.Column(
            "encrypted_records",
            sa.Integer(),
            nullable=False,
        ...
        sa.Column(
            "last_migration_at", sa.DateTime(timezone=True), nullable=True, comment="Last migration attempt timestamp"
        ),
        sa.Column(
            "created_at",
            sa.DateTime(timezone=True),
            nullable=False,
            server_default=text("NOW()"),
            comment="Registry entry creation timestamp",
        ),
        sa.Column(
            "updated_at",
            sa.DateTime(timezone=True),
    ...
    op.create_index(
        "idx_encrypted_field_registry_encryption_enabled", "encrypted_field_registry", ["encryption_enabled"]
    )
    ...
    op.execute(
        """
        INSERT INTO encryption_config (config_key, config_value, config_type, description, is_sensitive) VALUES
        ('encryption_enabled', 'true', 'boolean', 'Whether database encryption is enabled', FALSE),
        ('default_algorithm', 'AES-256-CBC', 'string', 'Default encryption algorithm', FALSE),
        ('key_rotation_interval_days', '90', 'integer', 'Number of days between key rotations', FALSE),
        ('audit_log_retention_days', '730', 'integer', 'Number of days to retain audit logs (2 years)', FALSE),
        ('max_key_versions', '10', 'integer', 'Maximum number of key versions to keep', FALSE),
        ('performance_monitoring_enabled', 'true', 'boolean', 'Whether to monitor encryption performance', FALSE),
        ('compliance_mode', 'italian_gdpr', 'string', 'Compliance mode (italian_gdpr, eu_gdpr, custom)', FALSE),
        ('emergency_decryption_enabled', 'false', 'boolean', 'Whether emergency decryption procedures are enabled', FALSE),
        ('backup_encryption_enabled', 'true', 'boolean', 'Whether database backups should be encrypted', FALSE),
        ('field_level_audit_enabled', 'true', 'boolean', 'Whether to audit individual field operations', FALSE)
    """
    )
    ...
    op.execute(
        ...
        INSERT INTO encrypted_field_registry (
            table_name, column_name, field_type, encryption_enabled, migration_status, encrypted_records, failed_records
        ) VALUES
        ('user', 'email', 'email', true, 'pending', 0, 0),
        ('user', 'phone', 'phone', true, 'pending', 0, 0),
        ('user', 'tax_id', 'tax_id', true, 'pending', 0, 0),
        ('query_logs', 'query', 'query', true, 'pending', 0, 0),
        ('faq_usage_logs', 'response_variation', 'personal_data', true, 'pending', 0, 0),
        ('faq_usage_logs', 'comments', 'personal_data', true, 'pending', 0, 0),
        ('subscription_data', 'stripe_customer_id', 'financial_data', true, 'pending', 0, 0),
        ('subscription_data', 'invoice_data', 'financial_data', true, 'pending', 0, 0),
        ('payment_logs', 'customer_details', 'financial_data', true, 'pending', 0, 0),
        ('document_processing_log', 'error_details', 'personal_data', true, 'pending', 0, 0)
    """
    )
...
def downgrade():
    ...
    # Drop triggers
    op.execute(
        "DROP TRIGGER IF EXISTS update_encrypted_field_registry_updated_at_trigger ON encrypted_field_registry;"
    )
    ...
    op.execute(
        "DROP FUNCTION IF EXISTS log_encryption_operation(INTEGER, VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR, BOOLEAN, TEXT, FLOAT, INTEGER);"
    )
