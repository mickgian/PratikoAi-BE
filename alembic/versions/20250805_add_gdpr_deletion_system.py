...
def upgrade():
    ...
    op.create_index('ix_gdpr_deletion_certificates_certificate_id', 'gdpr_deletion_certificates', ['certificate_id'], unique=True)
    ...
    op.execute("COMMENT ON TABLE gdpr_deletion_requests IS 'GDPR Article 17 deletion requests with 30-day deadline tracking'")
    op.execute("COMMENT ON TABLE gdpr_deletion_audit_log IS 'Audit trail for GDPR deletion operations across all systems'")
    ...
    op.execute("COMMENT ON COLUMN gdpr_deletion_audit_log.anonymized_user_id IS 'Anonymized identifier for audit trail preservation'")
    op.execute("COMMENT ON COLUMN gdpr_deletion_certificates.verification_details IS 'JSON containing detailed verification results'")
