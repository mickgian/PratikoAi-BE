...
revision: str = '9c4322e06e4d'
down_revision: Union[str, Sequence[str], None] = '18759d179eec'
...
def upgrade() -> None:
    ...
    sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
    sa.Column('updated_at', sa.DateTime(timezone=True), nullable=True),
    sa.ForeignKeyConstraint(['regione_id'], ['regioni.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    ...
    op.create_index('idx_regional_tax_rates_lookup', 'regional_tax_rates', ['regione_id', 'tax_type', 'valid_from', 'valid_to'], unique=False)
    ...
    op.create_table('comunal_tax_rates',
    sa.Column('id', sa.Uuid(), nullable=False),
    ...
    sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
    sa.Column('updated_at', sa.DateTime(timezone=True), nullable=True),
    sa.ForeignKeyConstraint(['comune_id'], ['comuni.id'], ),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('comune_id', 'tax_type', 'valid_from', name='uq_comunal_tax_rate_period')
    )
    op.create_index('idx_comunal_tax_rates_lookup', 'comunal_tax_rates', ['comune_id', 'tax_type', 'valid_from', 'valid_to'], unique=False)
