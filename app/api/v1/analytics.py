...
from app.models.usage import CostAlert, CostOptimizationSuggestion, UsageEvent, UsageQuota, UserUsageSummary
from app.services.usage_tracker import CostBreakdown, UsageMetrics, usage_tracker
...
async def get_current_usage(
    ...
    try:
        ...
        daily_cost_percentage = (
            (quota.current_daily_cost_eur / quota.daily_cost_limit_eur * 100) if quota.daily_cost_limit_eur > 0 else 0
        )
        monthly_cost_percentage = (
            (quota.current_monthly_cost_eur / quota.monthly_cost_limit_eur * 100)
            if quota.monthly_cost_limit_eur > 0
            else 0
        )
        ...
        return JSONResponse(
            {
                ...
                "current_usage": {
                    ...
                    "monthly": {
                        ...
                        "avg_daily_cost_eur": quota.current_monthly_cost_eur
                        / max(1, (datetime.utcnow() - quota.monthly_reset_at).days),
                    },
                },
                "metrics": {
        ...
        logger.error("current_usage_retrieval_failed", session_id=session.id, error=str(e), exc_info=True)
...
async def get_usage_history(
    ...
    try:
        ...
        logger.error("usage_history_retrieval_failed", session_id=session.id, error=str(e), exc_info=True)
...
async def get_cost_breakdown(
    ...
    start_date: datetime | None = Query(None, description="Start date for custom period"),
    end_date: datetime | None = Query(None, description="End date for custom period"),
    session: Session = Depends(get_current_session),
):
    ...
    try:
        ...
        logger.error("cost_breakdown_retrieval_failed", session_id=session.id, error=str(e), exc_info=True)
...
async def get_optimization_suggestions(
    ...
    try:
        ...
        logger.error("optimization_suggestions_retrieval_failed", session_id=session.id, error=str(e), exc_info=True)
...
async def get_cost_alerts(
    ...
    try:
        ...
        async with database_service.get_db() as db:
            query = select(CostAlert).where(CostAlert.user_id == user_id)
        ...
        logger.error("alerts_retrieval_failed", session_id=session.id, error=str(e), exc_info=True)
...
async def acknowledge_alert(
    ...
    try:
        ...
        async with database_service.get_db() as db:
            query = select(CostAlert).where(and_(CostAlert.id == alert_id, CostAlert.user_id == user_id))
            result = await db.execute(query)
            ...
            logger.info("cost_alert_acknowledged", user_id=user_id, alert_id=alert_id, alert_type=alert.alert_type)
        ...
        logger.error(
            "alert_acknowledgment_failed", session_id=session.id, alert_id=alert_id, error=str(e), exc_info=True
        )
