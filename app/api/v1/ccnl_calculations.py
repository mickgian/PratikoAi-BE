...
class LeaveBalanceRequest(BaseModel):
    ...
    used_days: Optional[Dict[str, int]] = Field(default=None)
...
async def calculate_leave_balances(
    ...
    if request.used_days:
        ...
        for leave_type_str, days in request.used_days.items():
            try:
                ...
                raise HTTPException(
                    status_code=400,
                    detail=f"Invalid leave type: {leave_type_str}"
                )
    ...
    if not balances:
        raise HTTPException(
            status_code=404,
            detail=f"No CCNL data found for sector {request.sector.value}"
        )
...
async def calculate_seniority_benefits(
    request: SeniorityBenefitsRequest,
    current_user: User = Depends(get_current_user)
...
async def answer_complex_query(
    ...
    if not result:
        raise HTTPException(
            status_code=404,
            detail=f"No CCNL data found for the specified parameters"
        )
...
async def get_overtime_scenarios(
    ...
    if not ccnl_db:
        raise HTTPException(
            status_code=404,
            detail=f"No CCNL data found for sector {sector.value}"
        )
    ...
    scenarios = calculator.calculate_overtime_scenarios(
        base_monthly_salary=Decimal(str(monthly_salary)),
...
async def get_geographic_differences(
    ...
    if not differences:
        raise HTTPException(
            status_code=404,
            detail=f"No salary data found for level {level_code}"
        )
...
async def calculate_career_progression(
    ...
    result = calculator.calculate_career_progression(
        ...
        progression_path=progression_tuples,
...
async def compare_sectors(
    ...
    if not ccnl1_db or not ccnl2_db:
        raise HTTPException(
            status_code=404,
            detail="CCNL data not found for one or both sectors"
        )
