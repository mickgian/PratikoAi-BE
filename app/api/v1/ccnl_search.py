...
class SearchFiltersRequest(BaseModel):
    ...
    # Sector filters
    sectors: list[CCNLSector] | None = Field(None, description="CCNL sectors to search")
    sector_keywords: list[str] | None = Field(None, description="Keywords for sector matching")

    # Geographic filters
    geographic_areas: list[GeographicArea] | None = Field(None, description="Geographic areas")
    regions: list[str] | None = Field(None, description="Italian regions")
    provinces: list[str] | None = Field(None, description="Italian provinces")

    # Worker category filters
    worker_categories: list[WorkerCategory] | None = Field(None, description="Worker categories")
    job_levels: list[str] | None = Field(None, description="Specific job level codes")

    # Salary filters
    min_salary: Decimal | None = Field(None, description="Minimum monthly salary")
    max_salary: Decimal | None = Field(None, description="Maximum monthly salary")
    ...
    # Experience filters
    min_experience_months: int | None = Field(None, description="Minimum experience in months")
    max_experience_months: int | None = Field(None, description="Maximum experience in months")

    # Working conditions
    max_weekly_hours: int | None = Field(None, description="Maximum weekly working hours")
    flexible_hours_required: bool | None = Field(None, description="Flexible hours required")
    part_time_allowed: bool | None = Field(None, description="Part-time work allowed")
    shift_work_allowed: bool | None = Field(None, description="Shift work allowed")

    # Leave filters
    min_vacation_days: int | None = Field(None, description="Minimum annual vacation days")
    leave_types_required: list[LeaveType] | None = Field(None, description="Required leave types")

    # Benefits filters
    required_allowances: list[AllowanceType] | None = Field(None, description="Required allowances")
    company_sizes: list[CompanySize] | None = Field(None, description="Company size categories")

    # Date filters
    valid_on_date: date | None = Field(None, description="CCNL valid on specific date")
    active_only: bool = Field(True, description="Only show currently active CCNLs")

    # Text search
    keywords: str | None = Field(None, description="Full-text search keywords")
...
class ComparisonRequest(BaseModel):
    ...
    comparison_aspects: list[str] = Field(
        ["salary", "vacation", "hours", "notice", "allowances"], description="Aspects to compare"
    )
...
async def search_ccnl(
    ...
    response = await ccnl_search_service.search(
        query=query, filters=filters, page=page, page_size=page_size, sort_by=sort_by, include_facets=include_facets
    )
    ...
    return SearchResponseModel(
        results=[
            SearchResultResponse(
                ...
                summary=r.summary,
                salary_range={"min": float(r.salary_range[0]), "max": float(r.salary_range[1])}
                if r.salary_range
                else None,
        ...
        facets={
            key: [FacetCountResponse(value=f.value, count=f.count, display_name=f.display_name) for f in facets]
...
async def natural_language_search(
    request: NaturalLanguageSearchRequest, current_user: User = Depends(get_current_user)
) -> SearchResponseModel:
    ...
    """
    response = await ccnl_search_service.search(
        query=request.query, page=request.page, page_size=request.page_size, include_facets=request.include_facets
    )
...
async def search_by_sector(
    ...
    response = await ccnl_search_service.search_by_sector(sectors=[sector], include_related=include_related)
...
async def search_by_geographic_area(
    ...
    regions: list[str] | None = Query(None, description="Italian regions"),
    provinces: list[str] | None = Query(None, description="Italian provinces"),
    page: int = Query(1, ge=1),
    page_size: int = Query(20, ge=1, le=100),
    current_user: User = Depends(get_current_user),
) -> SearchResponseModel:
    """Search CCNL agreements by geographic area."""
    response = await ccnl_search_service.search_by_geographic_area(areas=areas, regions=regions, provinces=provinces)
...
async def search_by_salary_range(
    min_salary: Decimal | None = Query(None, description="Minimum monthly salary"),
    max_salary: Decimal | None = Query(None, description="Maximum monthly salary"),
    ...
    response = await ccnl_search_service.search_by_salary_range(
        min_salary=min_salary, max_salary=max_salary, include_benefits=include_benefits
    )
...
async def search_by_job_category(
    ...
    experience_years: int | None = Query(None, description="Years of experience"),
    page: int = Query(1, ge=1),
    page_size: int = Query(20, ge=1, le=100),
    current_user: User = Depends(get_current_user),
) -> SearchResponseModel:
    ...
    response = await ccnl_search_service.search_by_job_category(
        categories=categories, experience_years=experience_years
    )
...
async def compare_sectors(
    request: ComparisonRequest, current_user: User = Depends(get_current_user)
) -> dict[str, Any]:
    ...
    comparison = await ccnl_search_service.compare_sectors(
        sector1=request.sector1, sector2=request.sector2, comparison_aspects=request.comparison_aspects
    )
...
async def get_popular_searches(current_user: User = Depends(get_current_user)) -> list[str]:
    """Get list of popular search queries."""
    return await ccnl_search_service.get_popular_searches()


@router.post("/autocomplete")
async def autocomplete(request: AutocompleteRequest, current_user: User = Depends(get_current_user)) -> list[str]:
    """Get autocomplete suggestions for search queries."""
    return await ccnl_search_service.autocomplete(partial_query=request.partial_query, limit=request.limit)


@router.get("/sectors")
async def get_all_sectors(current_user: User = Depends(get_current_user)) -> list[dict[str, str]]:
    """Get all available CCNL sectors."""
    return [
        {"code": sector.value, "name": sector.italian_name(), "priority": sector.priority_level()}
        for sector in CCNLSector
    ]


@router.get("/worker-categories")
async def get_worker_categories(current_user: User = Depends(get_current_user)) -> list[dict[str, str]]:
    """Get all worker categories."""
    return [
        {"code": category.value, "name": category.italian_name(), "hierarchy_level": category.hierarchy_level()}
        for category in WorkerCategory
    ]


@router.get("/geographic-areas")
async def get_geographic_areas(current_user: User = Depends(get_current_user)) -> list[dict[str, str]]:
    """Get all geographic areas."""
    return [{"code": area.value, "name": area.value.replace("_", " ").title()} for area in GeographicArea]
