...
from app.models.data_export import DataExportRequest, ExportFormat, ExportStatus, PrivacyLevel
...
class CreateExportRequest(BaseModel):
    ...
    date_from: date | None = Field(None, description="Export data from this date (DD/MM/YYYY)")
    date_to: date | None = Field(None, description="Export data until this date (DD/MM/YYYY)")
...
class UpdateExportRequest(BaseModel):
    ...
    max_downloads: int | None = Field(None, ge=1, le=50, description="Maximum download count")
    extend_expiry: bool | None = Field(None, description="Extend expiry by 24 hours (one time only)")
    ...
    **Security:**
    ...
    try:
        ...
        export_request = await service.create_export_request(
            user_id=current_user.id, format=request.format, options=options
        )
...
async def get_export_history(
    ...
    try:
        ...
        # Get export history
        exports, total_count = await service.get_user_export_history(current_user.id, limit=limit, offset=offset)
...
async def get_export_status(
    export_id: UUID, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)
) -> ExportRequestResponse:
    ...
    try:
        ...
        if export_request.user_id != current_user.id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Non autorizzato ad accedere a questo export"
            )
...
async def download_export(
    ...
    **Security Features:**
    ...
    try:
        ...
        if not success:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Impossibile scaricare l'export")
    ...
    try:
        ...
        if request.extend_expiry and not export_request.is_expired:
            ...
            if hasattr(export_request, "expiry_extended") and export_request.expiry_extended:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST, detail="La scadenza può essere estesa solo una volta"
                )
        ...
        if updated:
            ...
            await service._create_audit_log(
                ...
                current_user.id,
                "updated",
                {"max_downloads": request.max_downloads, "extend_expiry": request.extend_expiry},
            )
...
async def delete_export_request(
    ...
    try:
        ...
        # Create audit log
        await service._create_audit_log(export_request.id, current_user.id, "deleted", {"reason": "user_requested"})
    ...
    try:
        ...
        if not export_request.can_retry():
            if export_request.status != ExportStatus.FAILED:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST, detail="Solo gli export falliti possono essere ripetuti"
                )
            else:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
        ...
        # Create audit log
        await service._create_audit_log(
            export_request.id, current_user.id, "retried", {"retry_attempt": export_request.retry_count + 1}
        )
...
async def get_compliance_info() -> dict[str, Any]:
    ...
    return {
        "gdpr_compliance": {
            ...
            "jurisdiction": "Italia",
            "applicable_laws": ["GDPR (Regolamento UE 2016/679)", "Codice Privacy (D.Lgs. 196/2003 e s.m.i.)"],
        },
