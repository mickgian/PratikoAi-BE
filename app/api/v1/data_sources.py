...
from app.models.cassazione import CassazioneSearchQuery, CassazioneSection, DecisionType, LegalPrincipleArea
from app.models.ccnl_data import CCNLSector
from app.services.data_sources.base_source import DataSourceQuery, DataSourceStatus, DataSourceType
...
class DataSourceSearchRequest(BaseModel):
    ...
    sectors: list[CCNLSector] | None = Field(None, description="Filter by specific sectors")
    date_from: date | None = Field(None, description="Start date for document search")
    date_to: date | None = Field(None, description="End date for document search")
    ...
    source_types: list[DataSourceType] | None = Field(None, description="Filter by source types")
...
async def get_data_source_details(source_id: str = Path(..., description="Data source identifier")):
    ...
    try:
        ...
        return {
            ...
            "connection_status": {"connected": is_connected, "last_test": datetime.utcnow().isoformat()},
            "statistics": stats,
        }
...
async def get_sources_by_type(source_type: DataSourceType = Path(..., description="Type of data sources to retrieve")):
    ...
    try:
        ...
        return {"source_type": source_type.value, "total_sources": len(sources_info), "sources": sources_info}
...
async def get_sources_by_sector(sector: CCNLSector = Path(..., description="CCNL sector")):
    ...
    try:
        ...
        return {"sector": sector.value, "total_sources": len(sources_info), "sources": sources_info}
...
async def get_data_sources_health():
    ...
    try:
        ...
        for _source_id, status in health_status.items():
...
async def get_data_sources_statistics():
    ...
    try:
        ...
        total_reliability = sum(
            source.source_info.reliability_score for source in ccnl_data_sources_manager.registry.sources.values()
        )
        ...
        aggregate = {
            ...
            "average_reliability": total_reliability / len(ccnl_data_sources_manager.registry.sources)
            if ccnl_data_sources_manager.registry.sources
            else 0,
            "sources_by_type": {},
            "coverage_by_sector": {},
        }
        ...
        return {"aggregate": aggregate, "by_source": stats}
    ...
    try:
        ...
        return {
            ...
            "summary": {
                ...
                "average_reliability": sum(a["reliability_score"] for a in associations) / len(associations)
                if associations
                else 0,
            },
        }
...
async def search_sector_associations(
    ...
    keywords: list[str] | None = Query(None, description="Keywords to search for"),
    ...
    try:
        results = await ccnl_data_sources_manager.search_sector_specific(
            sectors=sectors, keywords=keywords, max_results=max_results
        )
    ...
    try:
        ...
        return {
            ...
            "statistics": {
                ...
                "average_sources_per_sector": sum(len(sources) for sources in coverage_dict.values())
                / len(coverage_dict)
                if coverage_dict
                else 0,
            },
...
class SectorAssociationSearchRequest(BaseModel):
    ...
    keywords: list[str] | None = Field(None, description="Keywords to search for")
    document_types: list[str] | None = Field(None, description="Filter by document types")
    date_from: date | None = Field(None, description="Start date for search")
    date_to: date | None = Field(None, description="End date for search")
...
async def search_sector_associations_detailed(request: SectorAssociationSearchRequest):
    ...
    try:
        ...
        for source in ccnl_data_sources_manager.registry.sources.values():
            if (
                source.source_info.source_type == DataSourceType.SECTOR_ASSOCIATION
                and request.sector in source.source_info.supported_sectors
            ):
        ...
        return {
            ...
            "search_summary": {
                ...
                "average_documents_per_association": sum(a.get("documents_found", 0) for a in associations_searched)
                / len(associations_searched)
                if associations_searched
                else 0,
            },
        }
    ...
    try:
        ...
        return {
            ...
            "summary": {
                ...
                "average_reliability": sum(s["reliability_score"] for s in government_sources)
...
async def search_government_sources(
    sectors: list[CCNLSector] | None = Query(None, description="Sectors to search within"),
    keywords: list[str] | None = Query(None, description="Keywords to search for"),
    max_results: int = Query(50, ge=1, le=100, description="Maximum number of results"),
):
...
async def get_government_source_priority(source_id: str = Path(..., description="Government source identifier")):
...
class GovernmentSourceSearchRequest(BaseModel):
    ...
    query_type: str = Field(
        "comprehensive", description="Type of search: comprehensive, legal, regulatory, statistical"
    )
    sectors: list[CCNLSector] | None = Field(None, description="Specific sectors to search within")
    keywords: list[str] | None = Field(None, description="Keywords to search for")
    document_types: list[str] | None = Field(None, description="Filter by document types")
    date_from: date | None = Field(None, description="Start date for search")
    date_to: date | None = Field(None, description="End date for search")
    ...
    try:
        ...
        return {
            ...
            "search_summary": {
                ...
                "average_documents_per_source": sum(s.get("documents_found", 0) for s in sources_searched)
                / len(sources_searched)
                if sources_searched
                else 0,
                "highest_confidence": max([d.confidence_score for d in documents_response])
...
class CassazioneSearchRequest(BaseModel):
    ...
    keywords: list[str] | None = Field(None, description="Legal keywords to search for")
    ...
    try:
        ...
        return {
            ...
            "metadata": {
                ...
                "endpoints_available": len(cassazione_source.endpoints)
                if hasattr(cassazione_source, "endpoints")
                else 0,
...
async def search_cassazione_jurisprudence(request: CassazioneSearchRequest):
    ...
    try:
        ...
        return {
            ...
            "results": {"total_decisions": len(decisions_response), "decisions": decisions_response},
            "metadata": {
                ...
                "legal_areas_covered": list(
                    {area for decision in decisions_response for area in decision.legal_areas}
                ),
            },
        }
...
async def get_cassazione_legal_principles(
    legal_area: LegalPrincipleArea | None = Query(None, description="Filter by legal area"),
    sector: CCNLSector | None = Query(None, description="Filter by CCNL sector"),
    precedent_value: str | None = Query(None, description="Filter by precedent value"),
    ...
    try:
        ...
        return {
            ...
            "results": {"total_principles": len(principles), "principles": principles},
            "metadata": {
...
async def validate_cassazione_connectivity():
    ...
    try:
        ...
        sample_documents = []
        try:
            test_query = DataSourceQuery(keywords=["lavoro", "ccnl"], max_results=5, include_content=False)
        ...
        if sample_documents:
            ...
            complete_documents = [
                doc for doc in sample_documents if doc.title and doc.publication_date and doc.extracted_data
            ]
        ...
        return {
            ...
            "functionality": {
                ...
                "legal_classification": "operational"
                if any(doc.extracted_data and doc.extracted_data.get("legal_areas") for doc in sample_documents)
                else "limited",
            },
            "data_quality": {
                "overall_rating": data_quality,
                "sample_size": len(sample_documents),
                "average_confidence": sum(doc.confidence_score for doc in sample_documents) / len(sample_documents)
                if sample_documents
                else 0,
                "completeness_indicators": {
                    "has_legal_keywords": sum(
                        1
                        for doc in sample_documents
                        if doc.extracted_data and doc.extracted_data.get("legal_keywords")
                    ),
                    "has_precedent_value": sum(
                        1
                        for doc in sample_documents
                        if doc.extracted_data and doc.extracted_data.get("precedent_value")
                    ),
                    "has_related_sectors": sum(
                        1
                        for doc in sample_documents
                        if doc.extracted_data and doc.extracted_data.get("related_sectors")
                    ),
                },
            },
...
def _generate_cassazione_recommendations(connection_test: bool, search_test: bool, data_quality: str) -> list[str]:
