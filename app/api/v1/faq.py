...
from app.models.faq import FAQAnalyticsSummary, FAQCategory, FAQEntry, FAQUsageLog, UpdateSensitivity
...
class FAQQueryRequest(BaseModel):
    ...
    context: str | None = Field(None, description="Additional context", max_length=200)
...
class FAQQueryResponse(BaseModel):
    ...
    faq_id: str | None = Field(None, description="FAQ entry ID if matched")
    ...
    category: str | None = Field(None, description="FAQ category")
    tags: list[str] | None = Field(None, description="FAQ tags")
...
class FAQUpdateRequest(BaseModel):
    ...
    question: str | None = Field(None, description="FAQ question", min_length=10, max_length=500)
    answer: str | None = Field(None, description="FAQ answer", min_length=20, max_length=2000)
    tags: list[str] | None = Field(None, description="FAQ tags")
    change_reason: str | None = Field(None, description="Reason for change", max_length=200)
...
class FAQFeedbackRequest(BaseModel):
    ...
    comments: str | None = Field(None, description="User comments", max_length=500)
...
async def query_faq(
    ...
    try:
        ...
        faq_response = await faq_service.handle_query(
            query=request.query, user_id=current_user.id if current_user else None, context=request.context or ""
        )
...
async def search_faqs(
    ...
    try:
        ...
        # Build search query
        query = select(FAQEntry).where(FAQEntry.needs_review is False)
...
async def get_categories(
    ...
    try:
        query = select(FAQCategory).where(FAQCategory.is_active is True).order_by(FAQCategory.sort_order)
        result = await db.execute(query)
...
async def create_faq(
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Error creating FAQ: {str(e)}")
...
async def update_faq(
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Error updating FAQ: {str(e)}")


@router.get("/admin/entries/{faq_id}", response_model=FAQEntryResponse)
async def get_faq(
    faq_id: str, current_user: User = Depends(get_current_admin_user), db: AsyncSession = Depends(get_async_db)
) -> FAQEntryResponse:
...
async def delete_faq(
    ...
    try:
        ...
        faq_entry.updated_at = datetime.now(UTC)

        await db.commit()

        return {"success": True, "message": "FAQ entry marked for review (soft delete)"}
...
async def list_faqs(
    ...
    try:
        ...
        return FAQSearchResponse(results=faq_responses, total_count=len(faq_responses), query_time_ms=query_time_ms)
