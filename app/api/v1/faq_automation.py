...
from app.models.faq_automation import (
    QueryCluster, FAQCandidate, GeneratedFAQ, RSSFAQImpact,
    FAQGenerationJob, FAQ_AUTOMATION_CONFIG
)
...
async def get_dashboard_stats(
    ...
    try:
        ...
        candidates_query = select(FAQCandidate).where(
            FAQCandidate.created_at >= since_date
        )
        ...
        faqs_query = select(GeneratedFAQ).where(
            GeneratedFAQ.created_at >= since_date
        )
        ...
        impacts_query = select(RSSFAQImpact).where(
            RSSFAQImpact.created_at >= since_date
        )
        ...
        rss_stats = {
            ...
            "pending_action": len([i for i in impacts if not i.processed and i.action_required in ["regenerate", "review"]]),
        ...
        performance_stats = {
            ...
            "success_rate": (candidates_stats["completed"] / candidates_stats["total"]) if candidates_stats["total"] > 0 else 0,
        ...
        if cost_savings["generation_costs"] > 0:
            ...
            cost_savings["break_even_months"] = cost_savings["generation_costs"] / cost_savings["realized_monthly_savings"] if cost_savings["realized_monthly_savings"] > 0 else float('inf')
...
async def get_faq_candidates(
    ...
    min_roi: Optional[float] = Query(None, description="Minimum ROI score"),
...
async def approve_faq(
    ...
    try:
        ...
        return {
            "message": f"FAQ {request.approval_status}",
            "faq_id": faq_id,
...
async def get_rss_impacts(
    ...
    try:
        query = select(RSSFAQImpact).order_by(
            desc(RSSFAQImpact.impact_score),
            desc(RSSFAQImpact.rss_published_date)
        )
...
async def check_rss_updates(
    background_tasks: BackgroundTasks,
    rss_integration: FAQRSSIntegration = Depends(get_rss_integration)
):
    ...
    try:
        ...
        return {
            "message": "RSS update check started",
            "status": "processing",
