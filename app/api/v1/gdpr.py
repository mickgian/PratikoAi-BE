...
class CreateDeletionRequestModel(BaseModel):
    ...
    priority: DeletionPriority | None = Field(default=DeletionPriority.NORMAL, description="Request priority level")
...
class AdminCreateDeletionRequestModel(CreateDeletionRequestModel):
    ...
    priority: DeletionPriority = Field(default=DeletionPriority.LOW, description="Request priority level")
...
async def create_user_deletion_request(
    ...
    try:
        ...
        deletion_request = await gdpr_service.create_deletion_request(
            user_id=current_user.id, initiated_by_user=True, reason=request_data.reason, priority=request_data.priority
        )
...
async def get_user_deletion_status(
    current_user: EncryptedUser = Depends(get_current_user), db: AsyncSession = Depends(get_async_db)
):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to get deletion status")
...
async def get_deletion_certificate(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to get certificate")
...
async def list_deletion_requests(
    status_filter: DeletionStatus | None = Query(None, description="Filter by status"),
    priority_filter: DeletionPriority | None = Query(None, description="Filter by priority"),
    user_initiated_only: bool | None = Query(None, description="Filter user-initiated requests"),
    overdue_only: bool | None = Query(False, description="Show only overdue requests"),
    ...
    try:
        ...
        if overdue_only:
            conditions.append(
                and_(
                    GDPRDeletionRequest.deletion_deadline <= datetime.now(UTC),
...
async def execute_overdue_deletions(
    current_admin: EncryptedUser = Depends(get_current_admin_user), db: AsyncSession = Depends(get_async_db)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to execute overdue deletions"
        )
...
async def verify_user_deletion(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to verify deletion")
...
async def check_deadline_compliance(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to check deadline compliance"
        )
...
async def execute_scheduled_deletions(db: AsyncSession = Depends(get_async_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to execute scheduled deletions"
        )
...
@router.get("/health", summary="GDPR service health check", description="Check health of GDPR deletion service")
async def gdpr_service_health(db: AsyncSession = Depends(get_async_db)):
