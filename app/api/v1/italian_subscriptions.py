...
class ItalianCustomerData(BaseModel):
    ...
    partita_iva: Optional[str] = Field(None, min_length=11, max_length=11, description="11-digit Italian VAT number")
    codice_fiscale: Optional[str] = Field(None, min_length=16, max_length=16, description="16-character Italian tax code")
...
class SubscriptionPlanResponse(BaseModel):
    ...
    monthly_equivalent_message: Optional[str] = None
...
class PlanChangeResponse(BaseModel):
    ...
    new_plan: Optional[SubscriptionPlanResponse]
...
async def download_invoice_xml(
    ...
    try:
        ...
        if not subscription.is_business:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Fattura elettronica disponibile solo per aziende"
            )
...
async def validate_partita_iva(
    partita_iva: str,
    db: AsyncSession = Depends(get_db)
) -> Dict[str, Any]:
    """
