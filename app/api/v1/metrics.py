...
from app.services.metrics_service import Environment, MetricsReport, metrics_service
...
async def send_email_report(request: EmailReportRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        background_tasks.add_task(email_service.send_metrics_report, request.recipient_emails, environments)
...
async def enable_scheduled_task(task_name: str):
    ...
    try:
        ...
        return {"message": f"Task '{task_name}' enabled", "timestamp": datetime.utcnow().isoformat()}
...
async def disable_scheduled_task(task_name: str):
    ...
    try:
        ...
        return {"message": f"Task '{task_name}' disabled", "timestamp": datetime.utcnow().isoformat()}
