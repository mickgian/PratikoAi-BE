...
async def create_customer(
    ...
    try:
        ...
        customer = await stripe_service.create_customer(
            user_id=user_id, email=customer_data.email, name=customer_data.name
        )
...
async def create_checkout_session(
    ...
    try:
        ...
        logger.error("checkout_session_creation_api_failed", session_id=session.id, error=str(e), exc_info=True)
...
async def get_user_subscription(
    ...
    try:
        ...
        logger.error("subscription_retrieval_api_failed", session_id=session.id, error=str(e), exc_info=True)
...
async def cancel_subscription(
    ...
    try:
        ...
        success = await stripe_service.cancel_subscription(
            subscription_id=subscription.stripe_subscription_id, user_id=user_id
        )
        ...
        logger.error("subscription_cancellation_api_failed", session_id=session.id, error=str(e), exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to cancel subscription")
...
async def get_user_invoices(
    ...
    try:
        ...
        logger.error("invoices_retrieval_api_failed", session_id=session.id, error=str(e), exc_info=True)
...
async def stripe_webhook(
    ...
    try:
        ...
        logger.error("stripe_webhook_failed", error=str(e), exc_info=True)
...
async def create_billing_portal_session(
    ...
    try:
        ...
        logger.error("billing_portal_creation_failed", session_id=session.id, error=str(e), exc_info=True)
