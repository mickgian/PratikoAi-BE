...
class LocationSearchRequest(BaseModel):
    ...
    region_filter: Optional[str] = Field(None, description="Filtro per regione")
...
class LocationResponse(BaseModel):
    ...
    provincia: str
    regione: str
    popolazione: Optional[int]
    is_capoluogo: bool
    is_capoluogo_provincia: bool
    area_kmq: Optional[float]
...
async def calculate_irap_tax(
    request: IRAPCalculationRequest,
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db)
    ...
    try:
        ...
        return IRAPCalculationResponse(
            region_info={
                ...
                "tipo_attivita": result["tipo_attivita"]
            },
            calculation=result,
            timestamp=datetime.now().isoformat()
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Errore nel calcolo IRAP"
        )
...
async def calculate_irpef_addizionali(
    ...
    try:
        ...
        result = await service.calculate_irpef_addizionali(
            reddito_imponibile=Decimal(str(request.taxable_income)),
            cap=request.cap
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Errore nel calcolo addizionali IRPEF"
        )
...
async def calculate_complete_regional_taxes(
    ...
    try:
        ...
        await service.log_calculation(
            user_id=current_user.id,
            tax_type="COMPLETE_REGIONAL",
            input_data=request.dict(),
            result=result
        )
        
        logger.info(
            f"Complete tax calculation for user {current_user.id}: "
            f"CAP {request.cap}, total €{result['total']}"
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Errore nel calcolo completo"
        )
...
async def compare_regional_taxes(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Errore nel confronto fiscale"
        )
...
async def search_locations(
    ...
    try:
        ...
        logger.info(
            f"Location search by user {current_user.id}: "
            f"query '{request.query}', {len(results)} results"
        )
        
        return LocationSearchResponse(
            results=[LocationResponse(**result) for result in results],
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Errore nella ricerca località"
        )
...
async def validate_italian_address(
    ...
    try:
        ...
        logger.info(
            f"Address validation by user {current_user.id}: "
            f"CAP {request.cap}, valid: {result['is_valid']}"
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Errore nella validazione indirizzo"
        )
...
async def get_nearby_locations(
    ...
    try:
        ...
        nearby = await service.get_nearby_locations(
            cap=cap,
            radius_km=radius_km,
            limit=limit
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Errore nella ricerca località vicine"
        )
...
async def get_regional_tax_overview(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Errore nella generazione panoramica"
        )
