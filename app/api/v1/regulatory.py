...
class TriggerCollectionRequest(BaseModel):
    ...
    sources: Optional[List[str]] = Field(
        default=None,
        description="Specific sources to collect from (e.g., ['agenzia_entrate', 'inps'])"
    )
    immediate: bool = Field(
        default=True,
        description="Whether to run collection immediately or schedule for next interval"
    )
...
class DocumentSearchRequest(BaseModel):
    ...
    source: Optional[str] = Field(default=None, description="Filter by source authority")
...
async def get_regulatory_documents(
    request: Request,
    source: Optional[str] = Query(None, description="Filter by source (agenzia_entrate, inps, etc.)"),
    ...
    try:
        ...
        query = select(RegulatoryDocument).where(
            RegulatoryDocument.status == status
        )
        ...
        logger.error(
            "regulatory_documents_retrieval_failed",
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to retrieve regulatory documents"
        )
...
async def get_regulatory_document(
    ...
    try:
        ...
        if include_content:
            ...
            doc_data["content_preview"] = document.content[:500] + "..." if len(document.content) > 500 else document.content
            doc_data["content_length"] = len(document.content)
        ...
        # Find newer versions
        newer_query = select(RegulatoryDocument).where(
            RegulatoryDocument.previous_version_id == document_id
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to retrieve regulatory document"
        )
...
async def trigger_document_collection(
    ...
    try:
        ...
        if collection_request.immediate:
            ...
            result = await scheduler_service.trigger_immediate_collection(
                sources=collection_request.sources
            )
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to trigger document collection"
        )
...
async def get_collection_status(
    ...
    try:
        ...
        recent_logs_query = select(DocumentProcessingLog).where(
            DocumentProcessingLog.created_at >= datetime.now(timezone.utc) - timedelta(hours=24)
        ).order_by(desc(DocumentProcessingLog.created_at)).limit(50)
        ...
        stats_query = select(
            ...
            func.sum(func.case((DocumentProcessingLog.status == 'success', 1), else_=0)).label('successful_operations'),
        ...
        logger.error(
            "collection_status_retrieval_failed",
            session_id=session.id,
            error=str(e),
            exc_info=True
        )
        raise HTTPException(
            status_code=500,
            detail="Failed to retrieve collection status"
        )


@router.get("/feeds/health",
           summary="Get feed health status",
           description="Get health status of all RSS feeds")
...
async def get_feeds_health(
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                "failed_to_get_recent_feed_health",
                error=str(e)
            )
            recent_health = None
        
        logger.info(
            "feeds_health_retrieved",
            user_id=session.user_id,
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to retrieve feed health status"
        )
...
async def get_regulatory_sources(
    ...
    try:
        ...
        sources_query = select(
            ...
            func.count(
                func.case((RegulatoryDocument.status == 'active', 1), else_=None)
        ...
        for row in sources_result:
            source_data = {
                ...
                "earliest_document_date": row.earliest_document_date.isoformat() if row.earliest_document_date else None
            }
            
            # Get document types for this source
            types_query = select(
                RegulatoryDocument.source_type,
                func.count(RegulatoryDocument.id).label('count')
            ).where(
                RegulatoryDocument.source == row.source
        ...
        logger.error(
            "regulatory_sources_retrieval_failed",
            session_id=session.id,
            error=str(e),
            exc_info=True
        )
        raise HTTPException(
            status_code=500,
            detail="Failed to retrieve regulatory sources"
        )


async def run_immediate_collection(
