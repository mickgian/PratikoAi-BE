...
from app.models.knowledge import KnowledgeFeedback, KnowledgeItem, KnowledgeQuery, KnowledgeSearchResponse
...
class SemanticSearchRequest(BaseModel):
    ...
    knowledge_type: str | None = Field(default=None, description="Filter by type: regulation, tax_rate, template")
    language: str = Field(default="italian", description="Language for search")
    top_k: int | None = Field(default=10, ge=1, le=50, description="Number of results")
...
class HybridSearchRequest(BaseModel):
    ...
    query: str = Field(..., min_length=3, max_length=500, description="Search query")
    keywords: list[str] | None = Field(default=None, description="Additional keywords for filtering")
    knowledge_type: str | None = Field(default=None, description="Filter by type")
...
async def semantic_search(
    ...
    try:
        ...
        if not vector_service.is_available():
            raise HTTPException(
                status_code=503, detail="Vector search service is not available. Please try keyword search."
            )

        # Perform semantic search
        results = vector_service.search_italian_knowledge(
            query=search_request.query, knowledge_type=search_request.knowledge_type, language=search_request.language
        )
        ...
        return JSONResponse(
            {
                ...
                "filters": {"knowledge_type": search_request.knowledge_type},
            }
        )
    ...
    try:
        ...
        if search_request.keywords:
            ...
            regulations = await italian_knowledge_service.search_regulations(
                keywords=search_request.keywords, subjects=None
            )
                ...
                keyword_results.append(
                    {
                        ...
                        "summary": reg.summary,
                        "type": "regulation",
                        "metadata": {"authority": reg.authority, "year": reg.year, "subjects": reg.subjects},
                    }
                )
        ...
        logger.error(
            "hybrid_search_failed", session_id=session.id, query=search_request.query[:50], error=str(e), exc_info=True
        )
...
async def index_document(
    ...
    try:
        # Check if vector service is available
        if not vector_service.is_available():
            raise HTTPException(status_code=503, detail="Vector indexing service is not available")
...
async def find_similar_documents(
    ...
    try:
        # Check if vector service is available
        if not vector_service.is_available():
            raise HTTPException(status_code=503, detail="Vector search service is not available")

        # This would retrieve the document and find similar ones
        # For now, return a placeholder response
        logger.info("similar_documents_search", user_id=session.user_id, document_id=document_id, top_k=top_k)
...
async def get_vector_index_stats(
    ...
    try:
        ...
        logger.info("vector_stats_retrieved", user_id=session.user_id, status=stats.get("status"))

        return JSONResponse(stats)

    except Exception as e:
        logger.error("vector_stats_retrieval_failed", session_id=session.id, error=str(e), exc_info=True)
...
async def reindex_italian_knowledge(
    ...
    try:
        ...
        logger.error("italian_knowledge_reindexing_failed", session_id=session.id, error=str(e), exc_info=True)
...
async def search_knowledge_fts(
    ...
    try:
        ...
        results = await search_service.search(
            query=q, limit=limit, offset=offset, category=category, min_rank=min_relevance
        )
        ...
        logger.error("fts_knowledge_search_failed", session_id=session.id, query=q, error=str(e), exc_info=True)
        raise HTTPException(status_code=500, detail="Knowledge search service temporarily unavailable")
...
async def get_knowledge_search_suggestions(
    ...
    try:
        ...
        logger.info("fts_suggestions_completed", user_id=session.user_id, query=q, suggestions_count=len(suggestions))

        return JSONResponse({"query": q, "suggestions": suggestions, "count": len(suggestions)})

    except Exception as e:
        logger.error("fts_suggestions_failed", session_id=session.id, query=q, error=str(e), exc_info=True)
        raise HTTPException(status_code=500, detail="Suggestions service temporarily unavailable")
...
async def submit_knowledge_feedback(
    ...
    feedback_text: str | None = Query(None, description="Optional feedback text"),
    feedback_type: str = Query(..., description="Feedback type: helpful, accurate, outdated, incorrect"),
    search_query: str | None = Query(None, description="Original search query"),
    session: Session = Depends(get_current_session),
    db: AsyncSession = Depends(get_async_session),
):
    ...
    try:
        ...
        return JSONResponse(
            {"status": "success", "message": "Feedback submitted successfully", "feedback_id": feedback.id}
        )

    except HTTPException:
        raise
    except Exception as e:
        logger.error("knowledge_feedback_failed", session_id=session.id, error=str(e), exc_info=True)
...
async def get_knowledge_categories(
    ...
    try:
        ...
        # Get category counts
        query = (
            select(KnowledgeItem.category, KnowledgeItem.subcategory, func.count(KnowledgeItem.id).label("count"))
            .where(KnowledgeItem.status == "active")
            .group_by(KnowledgeItem.category, KnowledgeItem.subcategory)
            .order_by(KnowledgeItem.category, KnowledgeItem.subcategory)
        )
        ...
        for row in rows:
            ...
            if category not in categories:
                categories[category] = {"name": category, "total_count": 0, "subcategories": {}}
        ...
        logger.info("knowledge_categories_retrieved", user_id=session.user_id, categories_count=len(categories))

        return JSONResponse({"categories": list(categories.values()), "total_categories": len(categories)})

    except Exception as e:
        logger.error("knowledge_categories_failed", session_id=session.id, error=str(e), exc_info=True)
...
async def reindex_knowledge_fts(
    ...
    try:
        ...
        logger.info("knowledge_fts_reindex_completed", user_id=session.user_id, message=message)

        return JSONResponse({"status": "success", "message": message})

    except Exception as e:
        logger.error("knowledge_fts_reindex_failed", session_id=session.id, error=str(e), exc_info=True)
        raise HTTPException(status_code=500, detail="Reindex operation failed")
...
async def clear_knowledge_search_cache(
    ...
    try:
        ...
        return JSONResponse({"status": "success", "message": "Search cache cleared successfully"})

    except Exception as e:
        logger.error("knowledge_cache_clear_failed", session_id=session.id, error=str(e), exc_info=True)
...
async def _update_knowledge_feedback_metrics(db: AsyncSession, knowledge_item_id: int):
    ...
    ).where(KnowledgeFeedback.knowledge_item_id == knowledge_item_id)
