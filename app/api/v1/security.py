...
class GenerateAPIKeyRequest(BaseModel):
    ...
    expires_in_days: Optional[int] = Field(default=30, ge=1, le=365, description="Expiration in days")
...
async def generate_api_key(
    ...
    try:
        ...
        if key_request.key_type in ["admin", "service"]:
            ...
            logger.warning(
                "admin_key_generation_attempted",
            ...
            raise HTTPException(
                status_code=403,
                detail="Insufficient permissions to create admin/service keys"
            )
        ...
        logger.error(
            "api_key_generation_endpoint_failed",
            user_id=session.user_id,
            error=str(e),
            exc_info=True
        )
...
async def rotate_api_keys(
    ...
    try:
        ...
        logger.error(
            "api_key_rotation_endpoint_failed",
            user_id=session.user_id,
            error=str(e),
            exc_info=True
        )
...
async def revoke_api_key(
    ...
    try:
        ...
        success = await api_key_manager.revoke_api_key(
            api_key=revoke_request.api_key,
        ...
        logger.error(
            "api_key_revocation_endpoint_failed",
            user_id=session.user_id,
            error=str(e),
            exc_info=True
        )
...
async def log_security_event(
    ...
    try:
        ...
        logger.error(
            "security_event_logging_endpoint_failed",
            user_id=session.user_id,
            error=str(e),
            exc_info=True
        )
...
async def get_security_monitoring_status(
    ...
    try:
        ...
        logger.error(
            "security_monitoring_status_endpoint_failed",
            user_id=session.user_id,
            error=str(e),
            exc_info=True
        )
...
async def resolve_security_threat(
    ...
    try:
        ...
        if not session.user_id.startswith("admin_"):
            raise HTTPException(
                status_code=403,
                detail="Admin privileges required to resolve threats"
            )
        ...
        logger.info(
            "security_threat_resolved_via_endpoint",
...
async def generate_compliance_report(
    ...
    try:
        ...
        if not session.user_id.startswith("admin_"):
            raise HTTPException(
                status_code=403,
                detail="Admin privileges required to generate compliance reports"
            )
        
        # Generate compliance report
        report = await security_audit_logger.generate_compliance_report(
        ...
        logger.info(
            "compliance_report_generated_via_endpoint",
            admin_user_id=session.user_id,
            report_type=report_type
        )
