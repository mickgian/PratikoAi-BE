...
class CalculationAccuracyTestResponse(BaseModel):
    ...
    calculated_result: Optional[float]
...
async def get_success_criteria_summary():
    ...
    try:
        ...
        query_success_rate = sum(1 for q in report.query_tests if q.success) / len(report.query_tests) if report.query_tests else 0
        query_capability_score = query_success_rate * 100
        
        calc_success_rate = sum(1 for c in report.calculation_tests if c.within_tolerance) / len(report.calculation_tests) if report.calculation_tests else 0
        calculation_accuracy_score = calc_success_rate * 100
        
        timeliness_rate = sum(1 for u in report.update_metrics if u.meets_48hour_requirement) / len(report.update_metrics) if report.update_metrics else 0
...
async def analyze_test_coverage():
    ...
    try:
        ...
        return {
            ...
            "grade": "A" if coverage_percentage >= 95 else "B" if coverage_percentage >= 90 else "C" if coverage_percentage >= 80 else "D" if coverage_percentage >= 70 else "F",
