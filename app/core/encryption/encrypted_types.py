...
from app.services.database_encryption_service import DatabaseEncryptionService, FieldType, generate_master_key
...
class EncryptedType(TypeDecorator):
    ...
    def __init__(
        ...
        field_type: str | FieldType = FieldType.STRING,
        encryption_service: DatabaseEncryptionService | None = None,
        *args,
        **kwargs,
    ):
            ...
            if loop.is_running():
                ...
                service.encryption_keys = {
                    1: type(
                        "MockKey",
                        (),
                        {"version": 1, "key_data": service._derive_key_from_master(1), "is_active": True},
                    )()
                }
            else:
            ...
            if loop.is_running():
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future = executor.submit(
                        lambda: loop.run_until_complete(service.encrypt_field(value, self.field_type))
                    )
                    return future.result()
            else:
                return loop.run_until_complete(service.encrypt_field(value, self.field_type))

        except Exception as e:
            logger.error(f"Synchronous encryption failed: {e}")
            # Fallback to simple encoding (not secure, but prevents crashes)
            return f"FALLBACK_ENCRYPTED_{value}".encode()
    ...
    def _decrypt_sync(self, service: DatabaseEncryptionService, value: bytes) -> str:
        ...
        try:
            ...
            if loop.is_running():
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future = executor.submit(lambda: loop.run_until_complete(service.decrypt_field(value)))
                    return future.result()
            else:
...
class EncryptedString(EncryptedType):
    ...
    def __init__(self, length: int = 255, field_type: str | FieldType = FieldType.STRING, **kwargs):
...
class EncryptedText(EncryptedType):
    ...
    def __init__(self, field_type: str | FieldType = FieldType.STRING, **kwargs):
...
def create_encrypted_type(
    ...
    class CustomEncryptedType(EncryptedType):
        def __init__(self, *args, **kwargs):
            super().__init__(base_type, field_type=field_type, encryption_service=encryption_service, *args, **kwargs)
...
class AsyncEncryptedType(EncryptedType):
    ...
    def __init__(
        ...
        impl_type: type = String,
        field_type: str | FieldType = FieldType.STRING,
        encryption_service_factory: Callable[[], DatabaseEncryptionService] | None = None,
        *args,
        **kwargs,
    ):
...
def setup_encryption_for_model(
    model_class: type, encrypted_fields: dict, encryption_service: DatabaseEncryptionService | None = None
) -> None:
        ...
        if hasattr(model_class, field_name):
            ...
            if hasattr(column.property, "columns"):
                ...
                encrypted_column.type = EncryptedType(
                    original_column.type, field_type=field_type, encryption_service=encryption_service
                )
