...
DEV-249: Integrates HallucinationGuard for citation validation.
Validates that law citations in LLM responses exist in KB context.
"""
...
def _validate_citations_in_response(
    response_text: str,
    kb_context: str,
    state: RAGState,
        ...
        if "error" in res and res["error"] not in ["", None]:
            ...
            if "response" in res or "llm_response" in res:
                ...
                if content:
                    ...
                    # DEV-249: Validate citations in response against KB context
                    hallucination_result = _validate_citations_in_response(display_content, kb_context, state)
                    if hallucination_result:
                        state["hallucination_check_result"] = hallucination_result.to_dict()
