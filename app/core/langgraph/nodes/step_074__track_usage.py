...
async def node_step_74(state: RAGState) -> RAGState:
    ...
    with rag_step_timer(STEP):
        # DEV-254/DEV-257: Extract correct field names from state for the orchestrator.
        ...
        # DEV-257: CheapProvider (step_051) stores as "provider_type", not "selected"
        if isinstance(provider_raw, dict):
            provider = provider_raw.get("selected") or provider_raw.get("provider_type") or provider_raw.get("name")
        else:
