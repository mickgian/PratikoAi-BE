...
from app.core.monitoring.metrics import track_ccnl_cache_hit, track_ccnl_query
...
class CCNLQueryInput(BaseModel):
    ...
    job_category: str | None = Field(
        default=None, description="Job category/level (e.g., 'worker', 'employee', 'manager', 'apprentice')"
    )
    geographic_area: str | None = Field(
        default=None, description="Geographic area (e.g., 'north', 'center', 'south', 'milan', 'rome')"
    )
    experience_years: int | None = Field(default=None, description="Years of work experience")
    company_size: str | None = Field(default=None, description="Company size ('small', 'medium', 'large')")
    search_terms: str | None = Field(
        default=None, description="Free text search terms for general information queries"
    )
    calculation_params: dict[str, Any] | None = Field(
        default=None, description="Additional parameters for calculations (salary base, hours, etc.)"
    )
...
class CCNLTool(BaseTool):
        ...
        try:
            ...
            if query_type == "search":
                ...
                result = await self._handle_notice_period_query(
                    sector=sector, job_category=job_category, experience_years=experience_years
                )
                ...
                result = await self._handle_sector_info_query(sector=sector, search_terms=search_terms)
            else:
                result = {
                    "success": False,
            ...
            logger.error("ccnl_query_failed", query_type=query_type, error=str(e), exc_info=True)
            return json.dumps(
                {
                    "success": False,
    ...
    async def _handle_search_query(
        self, search_terms: str | None, sector: str | None, job_category: str | None, geographic_area: str | None
    ) -> dict[str, Any]:
        ...
        try:
            ...
            if vector_service.is_available() and search_terms:
                semantic_results = vector_service.search_ccnl_semantic(
                    query=search_terms, sector_filter=sector.lower() if sector else None
                )
            ...
            return {
                ...
                "filters": {"sector": sector, "job_category": job_category, "geographic_area": geographic_area},
            }
    ...
    async def _handle_leave_calculation(
        ...
        try:
            ...
            return {
                ...
                "result": leave_result,
                "parameters": {"sector": sector, "job_category": job_category, "experience_years": experience_years},
            }
    ...
    async def _handle_notice_period_query(
        self, sector: str | None, job_category: str | None, experience_years: int | None
    ) -> dict[str, Any]:
    ...
    async def _handle_comparison_query(
        ...
        try:
            ...
            if not parsed_sectors:
                ...
                parsed_sectors = [CCNLSector.METALWORKING, CCNLSector.CONSTRUCTION, CCNLSector.COMMERCE]
            ...
            return {
                ...
                "parameters": {"sectors": sectors, "job_category": job_category, "geographic_area": geographic_area},
            }
    ...
    async def _handle_sector_info_query(self, sector: str | None, search_terms: str | None) -> dict[str, Any]:
        ...
        try:
            ...
            sector_info = await self.search_service.get_sector_overview(sector=parsed_sector, include_statistics=True)

            return {
                "success": True,
                "query_type": "sector_info",
                "result": sector_info,
                "parameters": {"sector": sector, "search_terms": search_terms},
            }
    ...
    def _parse_sector(self, sector_str: str | None) -> CCNLSector | None:
    ...
    def _parse_worker_category(self, category_str: str | None) -> WorkerCategory | None:
    ...
    def _parse_geographic_area(self, area_str: str | None) -> GeographicArea | None:
    ...
    def _parse_company_size(self, size_str: str | None) -> CompanySize | None:
