...
class KnowledgeSearchInput(BaseModel):
    ...
    category: str | None = Field(
        default=None, description="Filter by knowledge category (e.g., 'labor_law', 'tax', 'hr', 'payroll')"
    )
    search_mode: str | None = Field(
        default="hybrid", description="Search mode: 'hybrid' (default), 'bm25_only', or 'vector_only'"
    )
    ...
    async def _arun(
        ...
        **kwargs,
        ...
        try:
            ...
            # Prepare query data
            query_data = {"query": query, "max_results": max_results, "search_mode": mode.value, "filters": {}}
            ...
            logger.error(
                "knowledge_search_failed", query=query, error=str(e), duration_seconds=duration_seconds, exc_info=True
            )
