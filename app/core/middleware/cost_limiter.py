        ...
        try:
            ...
            if response.status_code < 400:
                try:
                    ...
                    response.headers["X-RateLimit-Remaining"] = str(max(0, quota.daily_requests_limit - quota.current_daily_requests))
                except Exception as e:
                    logger.error(
                        "cost_header_update_failed",
                        error=str(e)
                    )
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if current_time - self._last_pattern_check > self._pattern_check_interval:
            ...
            try:
                ...
                logger.error(
                    "optimization_suggestion_generation_failed",
                    error=str(e),
                    user_id=user_id
                )
        ...
        try:
            ...
            logger.error(
                "optimization_hint_generation_failed",
                error=str(e),
                user_id=user_id
            )
