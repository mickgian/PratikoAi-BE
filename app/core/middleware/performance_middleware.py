...
class PerformanceMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            await self._record_request_performance(
                request, response, response_time, request_size, response_size
            )
    ...
    def _should_monitor_db_performance(self, request: Request) -> bool:
        ...
        for monitored_path in self.db_monitored_paths:
            if path.startswith(monitored_path):
                return True
        
        return False
    
    async def _record_database_performance(
        self, 
        request: Request, 
        query_time: float
    ) -> None:
        ...
        try:
            ...
            logger.error(
                "database_performance_recording_failed",
                path=str(request.url.path),
                error=str(e)
            )
    ...
    async def _record_request_performance(
        ...
        try:
            ...
            logger.error(
                "request_performance_recording_failed",
                method=request.method,
                path=str(request.url.path),
                error=str(e)
            )
    
    async def _record_error_performance(
        self,
        request: Request,
        error: Exception,
        response_time: float
...
class DatabaseQueryMiddleware:
    ...
    async def record_execution(
        ...
        try:
            ...
            logger.error(
                "query_execution_recording_failed",
                execution_time=execution_time,
                error=str(e)
            )
