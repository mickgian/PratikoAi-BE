...
from app.core.security import security_audit_logger, security_monitor
...
class SecurityMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, enabled: bool = True):
        ...
        self.sensitive_endpoints = {"/api/v1/payments/webhook", "/api/v1/security/api-keys/", "/api/v1/auth/refresh"}
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            await self._post_request_monitoring(request, response, client_ip, user_agent, start_time)
    ...
    async def _pre_request_checks(self, request: Request, client_ip: str, user_agent: str) -> None:
        ...
        if security_monitor.is_ip_blocked(client_ip):
            logger.warning(
                "blocked_ip_request_attempted", client_ip=client_ip, path=str(request.url.path), method=request.method
            )
            ...
            raise HTTPException(status_code=403, detail="Access denied by security system")

        # Check rate limiting
        if security_monitor.is_rate_limited(client_ip):
            logger.warning("rate_limited_request_attempted", client_ip=client_ip, path=str(request.url.path))
    ...
    async def _check_suspicious_patterns(self, request: Request, client_ip: str, user_agent: str) -> None:
        ...
        for pattern in suspicious_patterns:
            if pattern in path_lower or pattern in query_string:
                ...
                await security_audit_logger.log_security_event(
                    ...
                    outcome="detected",
                    details={"pattern": pattern, "query_string": query_string[:200]},
                )
                ...
                await security_monitor.process_security_event(
                    ...
                    ip_address=client_ip,
                    outcome="detected",
                    details={"pattern": pattern, "path": path, "user_agent": user_agent[:100]},
                )
                break
    ...
    def _detect_bot_behavior(self, user_agent: str, request: Request) -> bool:
        ...
        return bool(not request.headers.get("accept-language") and request.headers.get("accept"))

    async def _post_request_monitoring(
        self, request: Request, response: Response, client_ip: str, user_agent: str, start_time: float
    ) -> None:
        ...
        if path in ["/api/v1/auth/login", "/api/v1/auth/register"]:
            await self._monitor_auth_endpoint(request, response, client_ip, user_agent, path)

        # Monitor payment endpoints
        elif path.startswith("/api/v1/payments/"):
            await self._monitor_payment_endpoint(request, response, client_ip, user_agent, path)

        # Monitor API key endpoints
        elif path.startswith("/api/v1/security/api-keys/"):
            await self._monitor_api_key_endpoint(request, response, client_ip, user_agent, path)
        ...
        if duration > 30.0:  # 30 seconds
            ...
            await security_audit_logger.log_security_event(
                ...
                action="slow_request",
                outcome="detected",
                details={"duration_seconds": duration, "status_code": status_code},
            )

        # Monitor high error rates
        if status_code >= 400:
            await self._monitor_error_response(request, response, client_ip, user_agent, path)

    async def _monitor_auth_endpoint(
        self, request: Request, response: Response, client_ip: str, user_agent: str, path: str
    ) -> None:
        ...
        if path == "/api/v1/auth/login":
            if status_code == 200:
                # Successful login
                await security_monitor.process_security_event(
                    event_type=SecurityEventType.LOGIN_SUCCESS, ip_address=client_ip, outcome="success"
                )
    ...
    async def _monitor_payment_endpoint(
        ...
        await security_monitor.process_security_event(
            ...
            ip_address=client_ip,
            outcome=outcome,
            details={"endpoint": path, "status_code": status_code},
        )
    ...
    async def _monitor_api_key_endpoint(
        ...
        await security_monitor.process_security_event(
            ...
            ip_address=client_ip,
            outcome=outcome,
            details={"endpoint": path, "method": method, "status_code": status_code},
        )
    ...
    async def _log_security_exception(
        self, request: Request, exception: HTTPException, client_ip: str, user_agent: str
    ) -> None:
        """Log security-related HTTP exceptions."""
        path = str(request.url.path)
        ...
        await security_audit_logger.log_security_event(
            ...
            action=request.method.lower(),
            outcome=outcome,
            details={"status_code": status_code, "detail": str(exception.detail)},
        )
