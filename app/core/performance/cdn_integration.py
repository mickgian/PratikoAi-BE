...
class CDNManager:
    ...
    async def _optimize_text_content(self, content: bytes, content_type: str) -> bytes:
        ...
        try:
            ...
            logger.error(
                "text_content_optimization_failed",
                content_type=content_type,
                error=str(e)
            )
            return content
    ...
    def purge_asset_cache(self, asset_ids: List[str]) -> Dict[str, bool]:
        ...
        try:
            ...
            for asset_id in asset_ids:
                if asset_id in self.assets:
                    ...
                    logger.info(
                        "cdn_asset_purged",
                        asset_id=asset_id,
                        original_url=self.assets[asset_id].original_url
                    )
                else:
            ...
            logger.error(
                "cdn_cache_purge_failed",
                asset_ids=asset_ids,
                error=str(e),
                exc_info=True
            )
            return {asset_id: False for asset_id in asset_ids}
    ...
    def get_cdn_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            bandwidth_saved = (
                ...
                if (self.stats.bytes_served + self.stats.bytes_saved) > 0 else 0
            )
                ...
                "top_assets": [
                    {
                        ...
                        "cache_efficiency": (
                            ...
                            if (asset.hit_count + asset.miss_count) > 0 else 0
                        )
                    }
            ...
            logger.error(
                "cdn_statistics_generation_failed",
                error=str(e),
                exc_info=True
            )
            return {"error": str(e)}
