...
class ResponseCompressor:
        ...
        try:
            ...
            if self.stats.total_bytes_original > 0:
                self.stats.avg_compression_ratio = (
                    self.stats.total_bytes_compressed / self.stats.total_bytes_original
                )
    ...
    def compress_json_response(
        ...
        try:
            ...
            logger.error(
                "json_response_compression_failed",
    ...
    def optimize_json_payload(self, data: Any) -> Any:
        ...
        try:
            if isinstance(data, dict):
                ...
                return [
                    self.optimize_json_payload(item)
                    for item in data
                    if item is not None
                ]
            ...
            logger.error(
                "json_payload_optimization_failed",
                data_type=type(data).__name__,
                error=str(e)
            )
            return data
    ...
    def create_optimized_response(
        ...
        try:
            ...
            logger.error(
                "optimized_response_creation_failed",
                error=str(e),
                exc_info=True
            )
        ...
        try:
            stats = {
                ...
                "avg_compression_time_ms": (
                    ...
                    if self.stats.compressed_requests > 0 else 0
                ),
                "bandwidth_saved_percent": (
                    round((1 - self.stats.avg_compression_ratio) * 100, 1)
                    if self.stats.avg_compression_ratio > 0 else 0
                ),
                "settings": {
...
class CompressionMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            should_compress, compression_type = self.compressor.should_compress(
                content,
                content_type,
                accept_encoding
            )
