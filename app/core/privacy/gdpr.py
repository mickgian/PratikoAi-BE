...
class ConsentManager:
    ...
    def withdraw_consent(
        ...
        for consent in self._consent_records[user_id]:
            if (consent.consent_type == consent_type and 
                consent.granted and 
                not consent.withdrawal_timestamp):
    ...
    def has_valid_consent(self, user_id: str, consent_type: ConsentType) -> bool:
        ...
        for consent in self._consent_records[user_id]:
            if (consent.consent_type == consent_type and
                consent.granted and
                not consent.withdrawal_timestamp and
                (not consent.expiry_date or consent.expiry_date > now)):
                return True
    ...
    def cleanup_expired_consents(self) -> int:
        ...
        for user_id in self._consent_records:
            ...
            expired_consents = [
                ...
                if c.expiry_date and c.expiry_date <= now and c.granted
            ]
...
class DataProcessor:
    ...
    def get_retention_period(self, data_category: DataCategory) -> Optional[timedelta]:
...
class GDPRCompliance:
    ...
    def get_compliance_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "retention_policies": {
                category.value: str(period) 
                for category, period in self.data_processor._retention_policies.items()
            },
            "compliance_features": [
                "consent_management",
