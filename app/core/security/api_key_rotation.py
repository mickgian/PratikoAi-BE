...
class APIKeyRotationManager:
    ...
    def generate_api_key(self, user_id: str, key_type: str = "user") -> str:
        ...
        try:
            ...
            prefix_map = {"user": "nai_user_", "admin": "nai_admin_", "service": "nai_svc_"}
            ...
            logger.error("api_key_generation_failed", user_id=user_id, key_type=key_type, error=str(e), exc_info=True)
    ...
    async def store_api_key(
        self, user_id: str, api_key: str, key_type: str = "user", expires_at: datetime | None = None
    ) -> bool:
    ...
    async def validate_api_key(self, api_key: str) -> dict[str, any] | None:
        ...
        try:
            ...
            logger.debug(
                "api_key_validated", key_prefix=api_key[:12] + "...", key_type=key_type, user_id=user_info["user_id"]
            )
            ...
            logger.error(
                "api_key_validation_failed", key_prefix=api_key[:12] + "..." if api_key else "empty", error=str(e)
            )
            return None
    ...
    async def revoke_api_key(self, api_key: str, reason: str = "manual_revocation") -> bool:
        ...
        try:
            ...
            logger.error("api_key_revocation_failed", reason=reason, error=str(e), exc_info=True)
            return False
    ...
    async def cleanup_expired_keys(self) -> int:
        ...
        try:
            ...
            logger.info("expired_keys_cleaned", cleaned_count=cleaned_count, cleanup_time=current_time.isoformat())
    ...
    async def get_key_statistics(self, user_id: str | None = None) -> dict[str, any]:
        ...
        try:
            ...
            stats = {
                ...
                "key_types": {"user": 140, "admin": 8, "service": 2},
            }
            ...
            logger.debug("key_statistics_retrieved", user_id=user_id, total_keys=stats["total_active_keys"])

            return stats

        except Exception as e:
            logger.error("key_statistics_failed", user_id=user_id, error=str(e), exc_info=True)
            return {}
