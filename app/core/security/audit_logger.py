...
class SecurityAuditLogger:
    ...
    async def log_security_event(
        ...
        try:
            ...
            audit_entry = {
                ...
                "retention_expires_at": (datetime.utcnow().timestamp() + (self.max_log_retention_days * 24 * 3600)),
            }

            # Log to structured logger
            logger.info("security_audit_event", **audit_entry)
    ...
    async def log_gdpr_event(
        ...
        event_details.update(
            {"data_type": data_type, "legal_basis": legal_basis, "gdpr_article": self._get_gdpr_article(action)}
        )
    ...
    async def log_payment_security_event(
        ...
        event_details = details or {}
        event_details.update({"payment_method": payment_method, "amount": amount, "currency": currency})
    ...
    async def get_security_events(
        ...
        try:
            ...
            logger.error("security_events_retrieval_failed", user_id=user_id, error=str(e), exc_info=True)
            return []

    async def generate_compliance_report(
        self, report_type: str = "gdpr", start_date: datetime | None = None, end_date: datetime | None = None
    ) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error("compliance_report_generation_failed", report_type=report_type, error=str(e), exc_info=True)
            return {}
