...
class RequestSigner:
    ...
    def verify_signature(
        ...
        try:
            ...
            if is_valid:
                logger.debug(
                    "signature_verified_successfully",
                    method=method,
                    path=path,
                    timestamp=timestamp
                )
    ...
    async def sign_outgoing_request(
        ...
        try:
            ...
            signature = self.generate_signature(
                method, path, body or b"", timestamp, secret_key
            )
            ...
            logger.error(
                "outgoing_request_signing_failed",
                method=method,
                url=url,
                error=str(e),
                exc_info=True
    ...
    def create_webhook_signature(self, payload: bytes, secret: str) -> str:
        ...
        try:
            ...
            logger.error(
                "webhook_signature_creation_failed",
                error=str(e),
                exc_info=True
            )
...
async def verify_request_signature(request: Request) -> bool:
    ...
    try:
        ...
        logger.error(
            "signature_verification_dependency_error",
            error=str(e),
            exc_info=True
        )
        raise HTTPException(
