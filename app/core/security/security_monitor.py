...
from app.core.security.audit_logger import security_audit_logger, SecurityEventType, SecuritySeverity
...
class SecurityMonitor:
    ...
    async def process_security_event(
        ...
        try:
            ...
            logger.error(
                "security_event_processing_failed",
    ...
    async def _check_security_rules(self, event: Dict[str, Any]) -> Optional[SecurityThreat]:
    ...
    def _matches_conditions(self, event: Dict[str, Any], conditions: Dict[str, Any]) -> bool:
        ...
        for key, expected_value in conditions.items():
            if key in event and event[key] != expected_value:
                return False
            elif key in event.get("details", {}) and event["details"][key] != expected_value:
                return False
        return True
    ...
    async def _execute_response_actions(self, threat: SecurityThreat) -> None:
        ...
        try:
            ...
            logger.error(
                "response_action_execution_failed",
                threat_id=threat.threat_id,
                error=str(e),
                exc_info=True
            )
    ...
    async def _cleanup_old_data(self) -> None:
        ...
        while self.monitoring_active:
            try:
                ...
                # Clean user tracking
                for user, events_by_type in self.user_tracking.items():
                    for event_type, events in events_by_type.items():
                        before_count = len(events)
                        events[:] = [e for e in events if e["timestamp"] >= cutoff_time]
                        cleaned_count += before_count - len(events)
                ...
                for session, events_by_type in self.session_tracking.items():
                    for event_type, events in events_by_type.items():
                        before_count = len(events)
                        events[:] = [e for e in events if e["timestamp"] >= cutoff_time]
                        cleaned_count += before_count - len(events)
                ...
                expired_ips = [
                    ip for ip, info in self.rate_limited_ips.items()
                    if datetime.utcnow() > info["reset_time"]
                ]
                ...
                logger.error(
                    "security_monitoring_cleanup_failed",
                    error=str(e),
                    exc_info=True
                )
