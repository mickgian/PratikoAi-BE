...
_write_stats: dict[str, dict[str, int]] = defaultdict(
    lambda: {"count": 0, "total_bytes": 0, "first_chunk_logged": False}
)


def write_sse(response, frame: str, request_id: str | None = None) -> str:
    ...
    if write_count <= 5:
        try:
            ...
            if line.startswith("data:"):
                ...
                logger.debug(
                    "SOCKET_WRITE [%s] chunk=%d seq=%s sha1=%s done=%s acc=%s raw=%s",
                    req_id,
                    write_count,
                    payload.get("seq"),
                    payload.get("sha1"),
                ...
                logger.debug("SOCKET_WRITE [%s] chunk=%d raw_frame_len=%d", req_id, write_count, frame_len)
        except Exception:
            logger.debug("SOCKET_WRITE [%s] chunk=%d raw_frame_len=%d (unparsed)", req_id, write_count, frame_len)

    # For streaming response, we return the frame for yielding
    return frame


def log_sse_summary(request_id: str | None = None) -> None:
