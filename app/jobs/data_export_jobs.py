...
from app.models.data_export import DataExportRequest, ExportAuditLog, ExportStatus
...
def process_export_request(self, export_id: str):
    ...
    try:
        ...
        if result["success"]:
            logger.info(
                f"Export {export_id} completed successfully in {result.get('processing_time_seconds', 0)} seconds"
            )
            return {
                "status": "completed",
                "export_id": export_id,
            ...
            return {"status": "failed", "export_id": export_id, "error": result["error"]}
        ...
        return {"status": "failed", "export_id": export_id, "error": str(e), "final_failure": True}
    ...
    try:
        ...
        def update_progress(current: int, total: int, description: str):
            task_instance.update_state(
                state="PROGRESS",
                meta={"current": current, "total": total, "description": description, "export_id": export_id},
            )
...
def cleanup_expired_exports():
    ...
    try:
        ...
        logger.info(
            f"Cleanup completed: {result['cleaned_count']} exports cleaned, {result['storage_mb_freed']} MB freed"
        )
...
async def _cleanup_expired_exports_async() -> dict:
    ...
    try:
        ...
        return {"cleaned_count": cleaned_count, "storage_mb_freed": round(storage_freed_bytes / 1024 / 1024, 2)}
...
async def _monitor_export_health_async() -> dict:
    ...
    try:
        ...
        stuck_stmt = select(DataExportRequest).where(
            and_(DataExportRequest.status == ExportStatus.PROCESSING, DataExportRequest.started_at < stuck_cutoff)
        )
        ...
        for status in ExportStatus:
            count_stmt = select(DataExportRequest).where(
                and_(DataExportRequest.status == status, DataExportRequest.requested_at >= recent_cutoff)
            )
...
async def _process_export_queue_async() -> dict:
    ...
    try:
        ...
        return {"processed_count": processed_count, "timestamp": datetime.utcnow().isoformat()}
...
async def _generate_export_metrics_async() -> dict:
    ...
    try:
        ...
        active_stmt = select(DataExportRequest).where(DataExportRequest.status == ExportStatus.PROCESSING)
...
async def _get_period_metrics(db: AsyncSession, start_time: datetime, end_time: datetime) -> dict:
    ...
    try:
        ...
        total_stmt = select(DataExportRequest).where(
            and_(DataExportRequest.requested_at >= start_time, DataExportRequest.requested_at <= end_time)
        )
        ...
        # Processing times
        processing_times = [e.processing_time_seconds for e in completed_exports if e.processing_time_seconds]

        # File sizes
        file_sizes = [e.file_size_bytes for e in completed_exports if e.file_size_bytes]
        ...
        return {
            ...
            "average_processing_time": round(sum(processing_times) / len(processing_times), 1)
            if processing_times
            else 0,
            "max_processing_time": max(processing_times) if processing_times else 0,
