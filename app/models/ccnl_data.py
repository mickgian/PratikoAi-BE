...
class CCNLSector(str, Enum):
    ...
    def priority_level(self) -> int:
        ...
        priority_1 = [
            self.METALMECCANICI_INDUSTRIA, self.METALMECCANICI_ARTIGIANI,
            self.COMMERCIO_TERZIARIO, self.EDILIZIA_INDUSTRIA,
            self.EDILIZIA_ARTIGIANATO, self.PUBBLICI_ESERCIZI,
            self.TURISMO, self.TRASPORTI_LOGISTICA,
            self.CHIMICI_FARMACEUTICI, self.TESSILI
        ]
        priority_2 = [
            self.TELECOMUNICAZIONI, self.CREDITO_ASSICURAZIONI,
            self.STUDI_PROFESSIONALI, self.SERVIZI_PULIZIA,
            self.VIGILANZA_PRIVATA, self.ICT,
            self.AGENZIE_VIAGGIO, self.CALL_CENTER,
            self.COOPERATIVE_SOCIALI, self.SERVIZI_EDUCATIVI
        ]
        priority_3 = [
            self.ALIMENTARI_INDUSTRIA, self.PANIFICAZIONE,
            self.AGRICOLTURA, self.FLOROVIVAISTI,
            self.LEGNO_ARREDAMENTO, self.CARTA_GRAFICA,
            self.ENERGIA_PETROLIO, self.GAS_ACQUA,
            self.GOMMA_PLASTICA, self.VETRO
        ]
        priority_4 = [
            self.SANITA_PRIVATA, self.CASE_DI_CURA,
            self.FARMACIE_PRIVATE, self.ENTI_LOCALI,
            self.MINISTERI, self.SCUOLA_PRIVATA,
            self.UNIVERSITA_PRIVATE, self.ENTI_DI_RICERCA
        ]
        priority_5 = [
            self.GIORNALISTI, self.GRAFICI_EDITORIALI,
            self.CINEMA_AUDIOVISIVO, self.TEATRO,
            self.RADIO_TV_PRIVATE
        ]
        priority_6 = [
            self.AUTOTRASPORTO_MERCI, self.AUTONOLEGGIO,
            self.AUTORIMESSE, self.POMPE_FUNEBRI,
            self.ACCONCIATURA_ESTETICA, self.IMPIANTI_SPORTIVI,
            self.DIRIGENTI_INDUSTRIA, self.DIRIGENTI_COMMERCIO,
            self.QUADRI
        ]
...
class GeographicArea(str, Enum):
    ...
    def includes_region(self, region: str) -> bool:
        ...
        region_mapping = {
            self.NORD: ["lombardia", "piemonte", "veneto", "liguria", "emilia-romagna", 
                       "trentino", "valle-aosta", "friuli"],
...
class CompanySize(str, Enum):
    ...
    def employee_range(self) -> Tuple[int, Optional[int]]:
...
class JobLevel:
    ...
    decision_making_level: Optional[str] = None
...
class WorkingHours:
    ...
    flexible_hours_range: Optional[Tuple[int, int]] = None  # (min, max) daily hours
    core_hours: Optional[Tuple[str, str]] = None  # ("09:00", "17:00")
    part_time_allowed: bool = True
    minimum_part_time_hours: Optional[int] = None
    ...
    def get_min_flexible_daily_hours(self) -> Optional[int]:
    ...
    def get_max_flexible_daily_hours(self) -> Optional[int]:
...
class OvertimeRules:
    ...
    maximum_daily_overtime: Optional[int] = None
    maximum_weekly_overtime: Optional[int] = None
    maximum_monthly_overtime: Optional[int] = None
    maximum_annual_overtime: Optional[int] = None
...
class LeaveEntitlement:
    ...
    base_annual_days: Optional[int] = None
    base_annual_hours: Optional[int] = None
...
class TFRRules:
    ...
    maximum_monthly_basis: Optional[Decimal] = None
...
class DisciplinaryRule:
    ...
    description: str
    sanctions: List[str] = field(default_factory=list)  # ["verbal_warning", "written_warning", "suspension", "dismissal"]
...
class WorkArrangementRules:
    ...
    ccnl_sector: CCNLSector
    part_time_allowed: bool = True
    part_time_minimum_hours: Optional[int] = None
...
class CCNLAgreement:
    ...
    overtime_rules: Optional[OvertimeRules] = None
    ...
    tfr_rules: Optional[TFRRules] = None
    disciplinary_rules: List[DisciplinaryRule] = field(default_factory=list)
    work_arrangement_rules: Optional[WorkArrangementRules] = None
    apprenticeship_rules: Optional[ApprenticeshipRules] = None
    training_rights: Optional[TrainingRights] = None
    union_rights: Optional[UnionRights] = None
    ...
    def get_salary_for_level(self, level_code: str, area: GeographicArea = GeographicArea.NAZIONALE) -> Optional[SalaryTable]:
...
class CCNLCalculator:
    ...
    def get_notice_period(self, worker_category: WorkerCategory, seniority_months: int) -> Optional[int]:
    ...
    def calculate_annual_leave(self, leave_type: LeaveType, seniority_months: int) -> Optional[int]:
    ...
    def get_probation_period(self, worker_category: WorkerCategory) -> Optional[int]:
    ...
    if provision_type == "leave_entitlements":
        ...
        for notice1 in ccnl1.notice_periods:
            for notice2 in ccnl2.notice_periods:
                if (notice1.worker_category == notice2.worker_category and
                    notice1.seniority_months_min == notice2.seniority_months_min):
...
def calculate_ccnl_coverage_percentage(covered_sectors: List[CCNLSector]) -> float:
    ...
    total_coverage = sum(
        sector_worker_percentages.get(sector, 0.0) 
