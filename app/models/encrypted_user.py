...
class EncryptedUser(BaseModel, table=True):
    ...
    phone: Optional[str] = Field(
        default=None,
        sa_column=Column(EncryptedPhone(50)),
        description="User's phone number (encrypted)"
    )
    
    tax_id: Optional[str] = Field(
        default=None,
        sa_column=Column(EncryptedTaxID(50)),
    ...
    full_name: Optional[str] = Field(
        default=None,
        sa_column=Column(EncryptedPersonalData(200)),
        description="User's full name (encrypted)"
    )
    
    address: Optional[str] = Field(
        default=None,
        sa_column=Column(EncryptedPersonalData(500)),
        description="User's address (encrypted)"
    )
    ...
    refresh_token_hash: Optional[str] = Field(
        default=None, 
        index=True,
        description="Hash of refresh token for secure validation"
    )
    ...
    gdpr_consent_date: Optional[datetime] = Field(
        ...
        description="When GDPR consent was given"
    )
    ...
    pii_last_updated: Optional[datetime] = Field(
        default=None,
        sa_column=Column(DateTime(timezone=True)),
        description="When PII was last updated"
    )
    encryption_key_version: Optional[int] = Field(
        default=None,
        description="Version of encryption key used for this user's data"
    )
    ...
    def update_pii_timestamp(self) -> None:
        ...
        self.pii_last_updated = datetime.now(timezone.utc)
...
class EncryptedQueryLog(BaseModel, table=True):
    ...
    # Encrypted fields
    query: str = Field(
        sa_column=Column(EncryptedPersonalData(2000)),
        description="User query content (encrypted)"
    )
    
    user_query_context: Optional[str] = Field(
        default=None,
        sa_column=Column(EncryptedPersonalData(1000)),
        description="Additional query context (encrypted)"
    )
    ...
    llm_tokens_used: Optional[int] = Field(default=None, description="LLM tokens consumed")
    estimated_cost_euros: Optional[float] = Field(default=None, description="Estimated cost in EUR")
...
class EncryptedSubscriptionData(BaseModel, table=True):
    ...
    stripe_customer_id: Optional[str] = Field(
        default=None,
        sa_column=Column(EncryptedPersonalData(100)),
        description="Stripe customer ID (encrypted)"
    )
    
    invoice_data: Optional[str] = Field(
        default=None,
        sa_column=Column(EncryptedPersonalData(2000)),
        description="Invoice and billing data (encrypted)"
    )
    ...
    monthly_cost_euros: Optional[float] = Field(
        default=None,
