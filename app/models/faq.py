...
from sqlmodel import SQLModel, Field, JSON, Column, DateTime, Text, Boolean, ARRAY
...
class FAQEntry(SQLModel, table=True):
    ...
    regulatory_refs: Dict[str, Any] = Field(
        ...
        description="References to regulatory documents"
    )
    ...
    last_used: Optional[datetime] = Field(
        ...
        description="Last time FAQ was accessed"
    )
    avg_helpfulness: Optional[float] = Field(
        default=None,
        description="Average helpfulness score (0.0-1.0)"
    )
    ...
    previous_version_id: Optional[str] = Field(
        default=None,
        description="Previous version ID for version history"
    )
...
class FAQUsageLog(SQLModel, table=True):
    ...
    # References
    faq_id: str = Field(
        foreign_key="faq_entries.id",
        description="FAQ entry that was used"
    )
    user_id: Optional[str] = Field(
        default=None,
        foreign_key="users.id",
        description="User who accessed the FAQ"
    )
    ...
    response_variation: str = Field(
        sa_column=Column(Text),
        description="The actual response sent to user"
    )
    from_cache: bool = Field(
        default=False,
        description="Whether variation came from cache"
    )
    
    # Cost tracking
    variation_cost_euros: float = Field(
        default=0.0003,
        description="Cost of variation generation in EUR"
    )
    variation_cost_cents: int = Field(
        default=3,
        description="Cost in 0.01 cent units (for precision)"
    )
    
    # User feedback
    was_helpful: Optional[bool] = Field(
        default=None,
        description="User feedback: was the response helpful?"
    )
    followup_needed: Optional[bool] = Field(
        default=None,
        description="User indicated need for followup"
    )
    comments: Optional[str] = Field(
        default=None,
        sa_column=Column(Text),
        description="User comments/feedback"
    )
...
class FAQVersionHistory(SQLModel, table=True):
    ...
    faq_id: str = Field(
        foreign_key="faq_entries.id",
        description="FAQ entry this version belongs to"
    )
    ...
    tags: List[str] = Field(
        default_factory=list,
        sa_column=Column(ARRAY(String)),
        description="Tags at this version"
    )
    regulatory_refs: Dict[str, Any] = Field(
        default_factory=list,
        sa_column=Column(JSON),
        description="Regulatory references at this version"
    )
...
class FAQObsolescenceCheck(SQLModel, table=True):
    ...
    faq_id: str = Field(
        foreign_key="faq_entries.id",
        description="FAQ entry that was checked"
    )
    ...
    is_potentially_obsolete: bool = Field(
        default=False,
        description="Whether FAQ might be obsolete"
    )
    confidence_score: float = Field(
        default=0.0,
        description="Confidence in obsolescence detection (0.0-1.0)"
    )
    reason: Optional[str] = Field(
        default=None,
        sa_column=Column(Text),
        description="Reason for potential obsolescence"
    )
    ...
    affecting_updates: Dict[str, Any] = Field(
        default_factory=list,
        sa_column=Column(JSON),
        description="Regulatory updates that might affect this FAQ"
    )
    
    # Action taken
    action_taken: Optional[str] = Field(
        default=None,
        description="Action taken based on check (review_flagged, auto_updated, etc.)"
    )
    reviewed_by: Optional[str] = Field(
        default=None,
        description="Admin who reviewed the obsolescence alert"
    )
    reviewed_at: Optional[datetime] = Field(
        default=None,
        sa_column=Column(DateTime(timezone=True)),
        description="When obsolescence alert was reviewed"
    )
...
class FAQCategory(SQLModel, table=True):
    ...
    avg_helpfulness: Optional[float] = Field(
        default=None,
        description="Average helpfulness across category"
    )
...
class FAQVariationCache(SQLModel, table=True):
    ...
    faq_id: str = Field(
        foreign_key="faq_entries.id",
        description="FAQ entry this variation is for"
    )
    user_id: Optional[str] = Field(
        default=None,
        foreign_key="users.id",
        description="User this variation was generated for"
    )
    cache_key: str = Field(
        unique=True,
        max_length=200,
        description="Unique cache key for this variation"
    )
    
    # Cached content
    original_answer: str = Field(
        sa_column=Column(Text),
        description="Original FAQ answer"
    )
    ...
    generation_cost_euros: float = Field(
        default=0.0003,
        description="Cost of generating this variation"
    )
    ...
    expires_at: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc) + datetime.timedelta(hours=24),
        sa_column=Column(DateTime(timezone=True)),
    ...
    last_used: Optional[datetime] = Field(
        default=None,
        sa_column=Column(DateTime(timezone=True)),
        description="Last time cached variation was accessed"
    )
...
class FAQAnalyticsSummary(SQLModel, table=True):
    ...
    top_categories: Dict[str, Any] = Field(
        default_factory=list,
        sa_column=Column(JSON),
        description="Most popular FAQ categories"
    )
    top_faqs: Dict[str, Any] = Field(
        default_factory=list,
        sa_column=Column(JSON),
        description="Most accessed FAQs"
    )
...
def calculate_cost_savings(
    ...
    actual_costs = (variations_needed * variation_cost_per_query) + \
                   (full_llm_responses * full_llm_cost_per_query)
