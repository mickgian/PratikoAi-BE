...
from sqlalchemy import Column, String, Integer, Float, Boolean, DateTime, Text, JSON, Enum, ForeignKey, Index, CheckConstraint
...
class ExpertProfile(Base):
    ...
    credential_types: Mapped[List[ExpertCredentialType]] = mapped_column(ARRAY(Enum(ExpertCredentialType)), default=list)
    ...
    professional_registration_number: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)
...
class ExpertFeedback(Base):
    ...
    expert_id: Mapped[UUID] = mapped_column(PostgreSQLUUID(as_uuid=True), ForeignKey("expert_profiles.id"), nullable=False)
    
    # Feedback details
    feedback_type: Mapped[FeedbackType] = mapped_column(Enum(FeedbackType), nullable=False)
    category: Mapped[Optional[ItalianFeedbackCategory]] = mapped_column(Enum(ItalianFeedbackCategory), nullable=True)
    ...
    # Expert input
    expert_answer: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    ...
    complexity_rating: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)  # 1-5 scale
...
class PromptTemplate(Base):
    ...
    # Metadata
    created_by: Mapped[Optional[UUID]] = mapped_column(PostgreSQLUUID(as_uuid=True), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now(), onupdate=func.now())
    ...
    __table_args__ = (
        ...
        CheckConstraint("overall_quality_score >= 0.0 AND overall_quality_score <= 1.0", name="overall_quality_score_range"),
...
class FailurePattern(Base):
    ...
    resolution_method: Mapped[Optional[str]] = mapped_column(String(200), nullable=True)
...
class SystemImprovement(Base):
    ...
    trigger_pattern_id: Mapped[Optional[UUID]] = mapped_column(PostgreSQLUUID(as_uuid=True), ForeignKey("failure_patterns.id"), nullable=True)
    ...
    actual_metrics: Mapped[Optional[Dict[str, Any]]] = mapped_column(JSON, nullable=True)
    ...
    planned_completion_date: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)
    ...
    expert_approved: Mapped[Optional[bool]] = mapped_column(Boolean, nullable=True)
    approving_expert_id: Mapped[Optional[UUID]] = mapped_column(PostgreSQLUUID(as_uuid=True), nullable=True)
...
class QualityMetric(Base):
    ...
    expert_specialization: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)
    user_segment: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)
    ...
    benchmark_percentile: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    ...
    calculation_method: Mapped[Optional[str]] = mapped_column(String(200), nullable=True)
...
class ExpertValidation(Base):
    ...
    # Final outcome
    validated_answer: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    ...
    __table_args__ = (
        ...
        CheckConstraint("consensus_confidence >= 0.0 AND consensus_confidence <= 1.0", name="consensus_confidence_range"),
        CheckConstraint("final_confidence_score >= 0.0 AND final_confidence_score <= 1.0", name="final_confidence_range"),
        CheckConstraint("expert_agreement_score >= 0.0 AND expert_agreement_score <= 1.0", name="agreement_score_range"),
...
class QualityAnalysisConfig:
    ...
    def __post_init__(self):
        if self.REQUIRED_EXPERT_CREDENTIALS is None:
            self.REQUIRED_EXPERT_CREDENTIALS = [
