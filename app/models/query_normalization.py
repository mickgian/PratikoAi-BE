...
from sqlmodel import SQLModel, Field, JSON, Column, DateTime, Text, Boolean
...
class QueryNormalizationLog(SQLModel, table=True):
    ...
    applied_rules: Dict[str, Any] = Field(
        ...
        description="List of applied normalization rules"
    )
    processing_time_ms: float = Field(description="Processing time in milliseconds")
    
    # Cache analytics
    cache_hit: Optional[bool] = Field(default=None, description="Whether query resulted in cache hit")
    cache_hit_after_normalization: Optional[bool] = Field(
        default=None,
        description="Cache hit after normalization"
    )
    
    # User context
    user_id: Optional[str] = Field(default=None, max_length=100, description="User who made the query")
...
class QueryNormalizationStats(SQLModel, table=True):
    ...
    rule_frequency: Dict[str, int] = Field(
        ...
        sa_column=Column(JSON),
        description="Frequency of each normalization rule"
    )
...
class QueryNormalizationPattern(SQLModel, table=True):
    ...
    example_queries: Dict[str, Any] = Field(
        default_factory=list,
        sa_column=Column(JSON),
        description="Example original queries"
    )
    ...
    category: Optional[str] = Field(
        default=None,
        max_length=50,
        description="Pattern category (tax, legal, general)"
    )
    complexity: str = Field(
        default="medium",
        max_length=20,
        description="Pattern complexity (simple, medium, complex)"
    )
