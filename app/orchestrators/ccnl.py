...
async def step_81__ccnlquery(*, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs) -> Any:
    ...
    with rag_step_timer(
        81, "RAG.ccnl.ccnltool.ccnl.query.query.labor.agreements", "CCNLQuery", request_id=request_id, stage="start"
    ):
        rag_step_log(
            step=81,
        ...
        try:
            ...
            try:
                ...
                ccnl_result = {
                    "success": False,
                    "error": "Failed to parse CCNL response",
                    "raw_response": str(ccnl_response),
                }
...
async def step_100__ccnlcalc(*, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs) -> Any:
    ...
    with rag_step_timer(
        100, "RAG.ccnl.ccnlcalculator.calculate.perform.calculations", "CCNLCalc", request_id=request_id, stage="start"
    ):
        rag_step_log(
            step=100,
        ...
        try:
            ...
            rag_step_log(
                ...
                request_id=request_id,
                calculation_success=calculation_result["success"],
                gross_total=str(calculation_result["compensation_breakdown"].get("gross_total", "0.00"))
                if calculation_result["success"]
                else None,
                net_total=str(calculation_result["compensation_breakdown"].get("net_total", "0.00"))
                if calculation_result["success"]
                else None,
                processing_stage="completed",
            )
...
async def _perform_ccnl_calculations(ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        from app.models.ccnl_data import CCNLAgreement, CCNLSector, CompanySize, GeographicArea
...
def _parse_company_size(size_str: str | None):
...
async def _handle_ccnl_calculation_error(ctx: dict[str, Any], error_msg: str) -> dict[str, Any]:
    ...
    return {
        **ctx,
        ...
        "error": error_msg,
        "compensation_breakdown": {},
        "calculation_metadata": {"error": error_msg, "fallback_applied": True, "calculation_failed": True},
        "tool_result_data": {
            "tool_type": "CCNL",
            "success": False,
            "error": error_msg,
