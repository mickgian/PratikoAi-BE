...
async def _display_feedback_ui_options(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        result = {
            ...
            'ui_config': ui_config,
            'ui_display_timestamp': datetime.now(timezone.utc).isoformat(),
...
async def step_113__feedback_ui(*, messages: Optional[List[Any]] = None, ctx: Optional[Dict[str, Any]] = None, **kwargs) -> Any:
    ...
    with rag_step_timer(113, 'RAG.feedback.feedbackui.show.options.correct.incomplete.wrong', 'FeedbackUI', stage="start"):
        ...
        result.update({
            'processing_stage': 'feedback_ui_ready' if ui_result.get('feedback_ui_displayed') else 'feedback_disabled'
        })
...
async def _evaluate_feedback_presence(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # 3. FAQ/General feedback data
            ...
            if (feedback_data.get('was_helpful') is not None or
                feedback_data.get('feedback_type') or
...
async def step_114__feedback_provided(
    ...
    **kwargs
    ...
    with rag_step_timer(114, 'RAG.feedback.user.provides.feedback', 'FeedbackProvided', stage="start"):
        ...
        # Add processing metadata
        result.update({
            'processing_stage': 'feedback_decision_completed'
        })
...
async def _finalize_feedback_pipeline(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        elif ctx.get('decision_reason') == 'feedback_ui_not_displayed':
...
async def step_116__feedback_type_sel(
    ...
    **kwargs
    ...
    with rag_step_timer(116, 'RAG.feedback.feedback.type.selected', 'FeedbackTypeSel', stage="start"):
        ...
        # Add processing metadata
        result.update({
            'processing_stage': 'feedback_routing_completed'
        })
...
async def _process_feedback_rejection(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    rejection_result = {
        'rejection_timestamp': datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z'),
...
async def step_122__feedback_rejected(*, messages: Optional[List[Any]] = None, ctx: Optional[Dict[str, Any]] = None, **kwargs) -> Any:
    ...
    with rag_step_timer(122, 'RAG.feedback.feedback.rejected', 'FeedbackRejected', stage="start"):
        rag_step_log(step=122, step_id='RAG.feedback.feedback.rejected', node_label='FeedbackRejected',
                     category='feedback', type='error', processing_stage="started")
...
async def step_123__create_feedback_rec(*, messages: Optional[List[Any]] = None, ctx: Optional[Dict[str, Any]] = None, **kwargs) -> Any:
    ...
    with rag_step_timer(123, 'RAG.feedback.create.expertfeedback.record', 'CreateFeedbackRec', stage="start"):
        rag_step_log(step=123, step_id='RAG.feedback.create.expertfeedback.record', node_label='CreateFeedbackRec',
                     category='feedback', type='process', processing_stage="started")
        ...
        if collection_result.get('success'):
            ...
            result = {
                ...
                'business_logic_delegation': {
                    'service': 'ExpertFeedbackCollector',
                ...
                'previous_step_decision': ctx.get('processing_metadata', {}).get('decision_outcome', 'trust_score_acceptable')
            }
