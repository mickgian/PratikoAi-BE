...
async def step_26__kbcontext_check(
    ...
    with rag_step_timer(
        ...
        try:
            ...
            if golden_updated:
                ...
                golden_dt = datetime.now(UTC) - timedelta(days=14)

            # Fetch recent KB articles (last 14 days by default)
            query_data = {"query": user_query, "since_date": golden_dt - timedelta(days=14), "max_results": 10}
        ...
        result = {
            **ctx,
            "kb_context": {
                ...
                "tags": list({tag for item in kb_context_items for tag in item.get("tags", [])}),
            },
...
async def step_80__kbquery_tool(
    *, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs
) -> Any:
    ...
    with rag_step_timer(
        80, "RAG.kb.knowledgesearchtool.search.kb.on.demand", "KBQueryTool", request_id=request_id, stage="start"
    ):
        rag_step_log(
            step=80,
        ...
        result = {
            **ctx,
            ...
            "search_query": query,
            "result_count": result_count,
            "search_metadata": {"query": query, "result_count": result_count, "tool_call_id": tool_call_id},
...
async def step_118__knowledge_feedback(
    *, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs
) -> Any:
...
async def step_132__rssmonitor(
    *, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs
) -> Any:
    ...
    with rag_step_timer(132, "RAG.kb.rss.monitor", "RSSMonitor", stage="start"):
        ...
        try:
            ...
            return {"step": 132, "status": "error", "error": str(e), "monitor_type": monitor_type}
...
async def _coordinate_rss_monitoring(monitor_type: str = "all") -> dict[str, Any]:
    ...
    if not monitoring_tasks:
        return {"status": "no_monitors", "has_new_items": False, "feeds": [], "new_item_count": 0, "feed_sources": []}
    ...
    for _i, result in enumerate(monitoring_results):
...
async def _monitor_ccnl_feeds() -> dict[str, Any]:
    ...
    try:
        ...
        return {"status": "success", "feeds": ccnl_updates, "new_item_count": len(ccnl_updates), "sources": ["ccnl"]}
