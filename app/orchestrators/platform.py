...
async def step_2__validate_request(
    *, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs
    ...
    with rag_step_timer(
        2, "RAG.platform.chatbotcontroller.chat.validate.request.and.authenticate", "ValidateRequest", stage="start"
    ):
        rag_step_log(
            step=2,
        ...
        try:
            ...
            logger.error(
                "Request validation failed with exception", error=str(e), request_id=request_id, exc_info=True
            )
            rag_step_log(
                step=2,
                step_id="RAG.platform.chatbotcontroller.chat.validate.request.and.authenticate",
...
async def step_1__start(*, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs) -> Any:
...
async def step_5__error400(*, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs) -> Any:
    ...
    with rag_step_timer(5, "RAG.platform.return.400.bad.request", "Error400", stage="start"):
        ...
        if error_type == "validation_failed" and validation_errors:
            ...
            error_response = {"detail": detail, "status_code": status_code, "errors": validation_errors}
        elif error_message:
            error_response = {"detail": error_message, "status_code": status_code}
        else:
            # Default error response
            error_response = {"detail": "Bad request", "status_code": status_code}
        ...
        if log_level == "error":
            logger.error(
                ...
                user_id=user_id,
                validation_errors=validation_errors,
                extra={"error_event": "bad_request_returned", "status_code": status_code, "error_type": error_type},
            )
        else:
            logger.warning(
                log_message,
...
async def step_9__piicheck(*, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs) -> Any:
    ...
    with rag_step_timer(9, "RAG.platform.pii.detected", "PIICheck", stage="start"):
        ...
        if pre_detected is not None:
            ...
            high_confidence_matches = [match for match in matches if match.get("confidence", 0) >= pii_threshold]
...
async def step_10__log_pii(*, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs) -> Any:
...
async def step_11__convert_messages(
    *, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs
) -> Any:
        ...
        try:
                ...
                try:
                    ...
                    if converted_msg:
                        ...
                        if await _validate_message(converted_msg):
                            ...
                            logger.warning("Message validation failed", message_index=i, request_id=request_id)
                    else:
                        conversion_errors.append(f"Message {i}: conversion failed")
                        logger.warning("Message conversion failed", message_index=i, request_id=request_id)

                except Exception as e:
                    conversion_errors.append(f"Message {i}: {str(e)}")
                    logger.warning("Message conversion error", error=str(e), message_index=i, request_id=request_id)
            ...
            if enable_deduplication and converted_messages:
                ...
                if len(converted_messages) < original_count:
                    logger.info(
                        f"Deduplicated {original_count - len(converted_messages)} messages",
                        original_count=original_count,
                        final_count=len(converted_messages),
                        request_id=request_id,
                    )
            ...
            logger.error("Message conversion failed", error=str(e), request_id=request_id, exc_info=True)
...
async def _validate_message(message: "Message") -> bool:
    ...
    try:
        ...
        return message.role in ["user", "assistant", "system"]
    except Exception:
        return False
...
async def _deduplicate_messages(messages: list["Message"]) -> list["Message"]:
    ...
    for message in messages[1:]:
        ...
        if message.role != deduplicated[-1].role or message.content != deduplicated[-1].content:
            deduplicated.append(message)

    return deduplicated


async def step_13__message_exists(
    *, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs
) -> Any:
...
async def step_38__use_rule_based(
    *, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs
) -> Any:
    ...
    with rag_step_timer(38, "RAG.platform.use.rule.based.classification", "UseRuleBased", stage="start"):
        ...
        try:
            ...
            logger.error(
                ...
                extra={"request_id": request_id, "error": error, "step": 38},
            )
...
async def step_50__strategy_type(
    *, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs
) -> dict[str, Any]:
    ...
    with rag_step_timer(50, "RAG.platform.routing.strategy", "StrategyType", stage="start"):
        ...
        try:
            ...
            logger.info(
                "STEP 50: Routing strategy decision completed",
                extra={
                    "step": 50,
                    "decision": decision,
            ...
            error_msg = str(e)
            logger.error(
                "STEP 50: Failed to determine routing strategy",
                extra={"step": 50, "error": error_msg, "routing_strategy": routing_strategy_value},
            )
...
async def step_69__retry_check(
    *, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs
) -> Any:
    ...
    max_retries = int(
        os.getenv(
            "RAG_MAX_RETRIES",
            str(kwargs.get("max_retries") or ctx.get("max_retries") or settings.MAX_LLM_CALL_RETRIES),
        )
    )

    kwargs.get("error") or ctx.get("error")
    ...
    try:
        ...
        logger.error("step_69_decision_error", extra={"request_id": request_id, "step": 69, "error": error_message})
...
async def step_70__prod_check(
    *, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs
    ...
    try:
        ...
        logger.error("step_70_decision_error", extra={"request_id": request_id, "step": 70, "error": error_message})
...
async def step_71__error500(*, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs) -> Any:
...
async def step_76__convert_aimsg(
    *, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs
) -> Any:
    ...
    try:
        ...
        # )
        ai_message = AIMessage(content=llm_response.content, tool_calls=llm_response.tool_calls)
        ...
        logger.info(
            "aimessage_with_tool_calls_created",
            extra={
                "request_id": request_id,
                "step": 76,
                "conversion_successful": True,
        ...
        logger.error("step_76_conversion_error", extra={"request_id": request_id, "step": 76, "error": error})
...
async def step_77__simple_aimsg(
    *, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs
) -> Any:
    ...
    try:
        ...
        logger.error("step_77_conversion_error", extra={"request_id": request_id, "step": 77, "error": error})
...
async def step_78__execute_tools(
    *, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs
) -> Any:
    ...
    try:
        ...
        logger.error("step_78_execution_error", extra={"request_id": request_id, "step": 78, "error": error})
...
async def step_86__tool_error(
    *, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs
    ...
    with rag_step_timer(86, "RAG.platform.return.tool.error.invalid.file", "ToolErr", stage="start"):
        ...
        if tool_call_id:
            tool_message = ToolMessage(content=f"Error: {error_message}", tool_call_id=tool_call_id)
    ...
    try:
        ...
        if existing_error:
            ...
            return {
                ...
                "tool_message_data": {"content": error_content, "name": tool_name, "tool_call_id": tool_call_id},
                "metadata": {
                    "tool_name": tool_name,
        ...
        tool_message_data = {"content": formatted_content, "name": tool_name, "tool_call_id": tool_call_id}
...
async def step_99__tool_results(
    *, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs
) -> Any:
    ...
    with rag_step_timer(99, "RAG.platform.return.to.tool.caller", "ToolResults", request_id=request_id, stage="start"):
        rag_step_log(
            step=99,
...
async def step_103__log_complete(
    *, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs
) -> Any:
    ...
    with rag_step_timer(103, "RAG.platform.logger.info.log.completion", "LogComplete", stage="start"):
        ...
        if classification:
            if hasattr(classification, "domain"):
                domain = (
                    ...
                    if hasattr(classification.domain, "value")
                    else str(classification.domain)
                )
            if hasattr(classification, "action"):
                action = (
                    classification.action.value
                    if hasattr(classification.action, "value")
                    else str(classification.action)
                )
        ...
        log_message = (
            ...
            if success
            else f"RAG processing completed with error: {error_message}"
        )
        ...
        result.update(
            {
                ...
                "generator_timestamp": datetime.now(UTC).isoformat(),
            }
        )
...
async def step_120__validate_expert(
    *, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs
) -> Any:
...
async def step_126__determine_action(
    *, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs
) -> Any:
    ...
    with rag_step_timer(126, "RAG.platform.determine.action", "DetermineAction", request_id=request_id, stage="start"):
        rag_step_log(
            step=126,
...
async def step_133__fetch_feeds(
    *, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs
) -> Any:
    ...
    with rag_step_timer(133, "RAG.platform.fetch.and.parse.sources", "FetchFeeds", stage="start"):
        rag_step_log(
            ...
            type="process",
            processing_stage="started",
            attrs={"feeds_to_process": len(rss_feeds), "expected_items": new_item_count, "feed_sources": feed_sources},
        )

        try:
            # Fetch and parse RSS feeds
            processing_result = await _fetch_and_parse_feeds(rss_feeds=rss_feeds, feed_sources=feed_sources)
            ...
            rag_step_log(
                ...
                node_label="FetchFeeds",
                processing_stage="error",
                attrs={"error": str(e), "feeds_to_process": len(rss_feeds), "feed_sources": feed_sources},
            )

            return {"step": 133, "status": "error", "error": str(e), "feeds_fetched": 0, "total_items_parsed": 0}
...
async def _fetch_and_parse_feeds(rss_feeds: list[dict[str, Any]], feed_sources: list[str]) -> dict[str, Any]:
    ...
    if not rss_feeds:
        return {
            ...
            "parsed_feeds": [],
            "processing_summary": {"successful_feeds": 0, "failed_feeds": 0, "processing_time_seconds": 0},
        }
    ...
    for _i, result in enumerate(parsing_results):
...
async def _parse_individual_feed(feed_data: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if not feed_url:
            return {"feed_name": feed_name, "parsing_status": "error", "error": "Missing feed URL", "items_parsed": 0}
...
def _determine_priority_level(confidence_score: float, trust_score: float, frequency: int) -> str:
    ...
    if combined_score >= 0.85 and frequency >= 20 or combined_score >= 0.9 and frequency >= 10:
        return "high"
    elif combined_score >= 0.8 and frequency >= 5 or combined_score >= 0.7:
        return "medium"
    else:
        return "low"
...
async def _handle_action_determination_error(ctx: dict[str, Any], error_msg: str) -> dict[str, Any]:
    ...
    return {
        **ctx,
        ...
        "route_to_golden_candidate": False,
        "error": error_msg,
        "decision_metadata": {"error": error_msg, "fallback_applied": True, "action_determination_failed": True},
