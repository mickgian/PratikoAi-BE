...
async def step_8__init_agent(*, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs) -> Any:
    ...
    with rag_step_timer(8, "RAG.response.langgraphagent.get.response.initialize.workflow", "InitAgent", stage="start"):
        ...
        try:
            ...
            logger.info(
                ...
                extra={"workflow_event": "initialized", "agent_ready": True, "message_count": len(processed_messages)},
            )
...
async def step_30__return_complete(
    *, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs
    ...
    with rag_step_timer(
        30, "RAG.response.return.chatresponse", "ReturnComplete", request_id=request_id, stage="start"
    ):
        rag_step_log(
            step=30,
...
async def _format_chat_response(ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        chat_response = {"messages": messages, "metadata": response_metadata}
        ...
        return {"success": True, "chat_response": chat_response, "formatting_metadata": formatting_metadata}
        ...
        return {
            ...
            "chat_response": {
                ...
                "metadata": {"source": "error", "formatted_at": datetime.now(UTC).isoformat()},
            },
            "formatting_metadata": {
                "formatted_at": datetime.now(UTC).isoformat(),
...
def _build_response_metadata(ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    # Add timestamp
    metadata["formatted_at"] = datetime.now(UTC).isoformat()
    ...
    try:
        ...
        if llm_response and llm_response.tool_calls:
            ...
            logger.info(
                ...
                extra={"request_id": request_id, "step": 75, "has_tool_calls": False, "next_step": next_step},
            )
        ...
        logger.error("step_75_decision_error", extra={"request_id": request_id, "step": 75, "error": error_message})
...
async def step_101__final_response(
    *, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs
) -> Any:
        ...
        result.update(
            {
                ...
                "final_response_timestamp": datetime.now(UTC).isoformat(),
            }
        )
...
async def step_102__process_msg(
    *, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs
) -> Any:
    ...
    """
    with rag_step_timer(
        102, "RAG.response.langgraphagent.process.messages.convert.to.dict", "ProcessMsg", stage="start"
    ):
        ctx = ctx or {}
        ...
        result.update(
            {
                ...
                "message_processing_timestamp": datetime.now(UTC).isoformat(),
            }
        )
    ...
    try:
        ...
        for message in openai_style_messages:
            if (
                ...
                processed.append({"role": message["role"], "content": str(message["content"])})
...
def _validate_response_delivery(ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    performance_fields = ["response_time_ms", "total_tokens", "cost", "cache_hit", "provider", "model", "health_score"]
    for field in performance_fields:
        if ctx.get(field) is not None:
            status[field] = ctx.get(field)

    # Include feedback metadata if available
    feedback_fields = ["feedback_enabled", "feedback_options", "expert_feedback_available"]
...
async def step_112__end(*, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs) -> Any:
