...
async def step_79__tool_type(*, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs) -> Any:
    ...
    with rag_step_timer(79, "RAG.routing.tool.type", "ToolType", request_id=request_id, stage="start"):
        rag_step_log(
            step=79,
...
async def _handle_tool_type_error(ctx: dict[str, Any], error_msg: str) -> dict[str, Any]:
    ...
    return {
        **ctx,
        ...
        "route_to": "ErrorHandling",
        "previous_step": ctx.get("rag_step"),
        "tool_routing_metadata": {"error": error_msg, "fallback_applied": True, "detection_failed": True},
        "request_id": ctx.get("request_id", "unknown"),
    }
