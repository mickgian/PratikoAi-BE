...
async def step_104__stream_check(
...
def _determine_streaming_preference(ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    if "requested" in streaming_state:
        return {"requested": streaming_state["requested"], "source": "state"}
    ...
    if stream_param is not None:
        ...
        return {"requested": streaming_requested, "source": "stream_parameter"}

    # Check HTTP Accept header
    accept_header = http_headers.get("Accept", "").lower()
    if "text/event-stream" in accept_header:
        return {"requested": True, "source": "http_headers"}

    # Default to non-streaming
    return {"requested": False, "source": "default"}
...
async def step_105__stream_setup(
    *, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs
) -> Any:
        ...
        result.update(
            {
                ...
                "formatting_timestamp": datetime.now(UTC).isoformat(),
            }
        )
...
def _create_streaming_response(ctx: dict[str, Any]) -> Any:
    ...
    default_headers = {"Content-Type": "text/event-stream", "Cache-Control": "no-cache", "Connection": "keep-alive"}
    ...
    return StreamingResponse(content=sse_stream, media_type=media_type, headers=response_headers)
...
async def step_109__stream_response(
    *, messages: list[Any] | None = None, ctx: dict[str, Any] | None = None, **kwargs
) -> Any:
