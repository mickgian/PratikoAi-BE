...
from app.models.quality_analysis import PromptTemplate, ExpertFeedback, QUALITY_ANALYSIS_CONFIG
...
class AdvancedPromptEngineer:
    ...
    async def generate_enhanced_prompt(
        ...
        try:
            ...
            if self.cache:
                cache_key = f"generated_prompt:{hashlib.md5(f'{template_id}:{str(context_data)}'.encode()).hexdigest()}"
    ...
    async def select_template_variant(self, query_category: str) -> Dict[str, Any]:
        ...
        try:
            ...
            variants_query = select(PromptTemplate).where(
                and_(
            ...
            if not variants:
                ...
                default_query = select(PromptTemplate).where(
                    and_(
                        PromptTemplate.category == query_category,
                        PromptTemplate.is_active == True
                    )
    ...
    def _calculate_completeness_score(self, text: str) -> float:
        ...
        essential_components = [
            'domanda', 'quesito', 'problema',  # Question identification
            'normativa', 'legge', 'decreto',   # Regulatory references
            'interpretazione', 'analisi',      # Analysis section
            'conclusione', 'risposta'          # Conclusion
        ]
        
        components_found = sum(1 for component in essential_components 
    ...
    def _calculate_italian_tax_domain_score(self, text: str) -> float:
        ...
        tax_terms = [
            'iva', 'irpef', 'ires', 'irap', 'partita iva',
            'dichiarazione', 'fattura', 'regime forfettario',
            'detrazione', 'deduzione', 'contributi',
            'codice fiscale', 'f24', 'modello 730',
        ...
        professional_phrases = [
            'ai sensi dell\'art', 'in base alla normativa',
            'secondo quanto previsto', 'come stabilito dal',
    ...
    async def _analyze_feedback_patterns(self, feedback_data: List[Dict[str, Any]]) -> List[str]:
        ...
        feedback_text = " ".join([fb.get('expert_answer', '') + ' ' + ' '.join(fb.get('improvement_suggestions', [])) 
                                for fb in feedback_data])
        
        # Pattern analysis
        if 'normativa' in feedback_text.lower() and ('obsoleta' in feedback_text.lower() or 'aggiorna' in feedback_text.lower()):
    ...
    async def _generate_improved_template(
        ...
        for suggestion in suggestions:
            if "regulatory references" in suggestion.lower():
                ...
                improved_template = improved_template.replace(
                    "**Risposta Finale:**",
                    "**Risposta Specifica e Dettagliata:**"
                )
...
{additional_resources}
    ...
    async def get_template_analytics(self, template_id: Optional[str] = None, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            analytics = {
                ...
                'avg_quality_score': sum(t.overall_quality_score for t in templates) / len(templates) if templates else 0,
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'quality_thresholds': {
                ...
                'target': self.target_quality_score
            },
            'base_templates': list(self.base_templates.keys()),
            'performance_metrics': {
                'templates_per_improvement': self.stats['improvements_applied'] / max(self.stats['templates_created'], 1),
