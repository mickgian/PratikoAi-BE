...
from app.models.quality_analysis import (
    SystemImprovement, FailurePattern, ExpertFeedback, PromptTemplate,
...
class AutomaticImprovementEngine:
    ...
    async def update_knowledge_base(self, knowledge_update: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not all([category, updated_content, source]):
                return {
                    'success': False,
                    'error': 'Missing required fields: category, updated_content, source'
                }
    ...
    async def measure_improvement_impact(
        ...
        try:
            ...
            accuracy_improvement = impact_analysis['metrics_comparison'].get('accuracy_score', {}).get('improvement_percentage', 0)
            satisfaction_improvement = impact_analysis['metrics_comparison'].get('user_satisfaction', {}).get('improvement_percentage', 0)
            failure_rate_change = impact_analysis['metrics_comparison'].get('failure_rate', {}).get('improvement_percentage', 0)
        ...
        try:
            ...
            for change in specific_changes:
                if 'Update regime forfettario references to 2024' in change:
                    updated_template_text = updated_template_text.replace(
                        '€65.000', '€85.000'
                    ).replace(
                        '2023', '2024'
                    )
            ...
            improvement_result = await self.prompt_engineer.improve_template_from_feedback({
                ...
                'feedback_data': [],
                'performance_metrics': {'accuracy_score': 0.8}
            })
    ...
    async def _apply_knowledge_base_update(self, update_request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        updated_content = update_request.get('updated_content', '')
    ...
    async def _create_improvement_record(self, update_request: Dict[str, Any], results: Dict[str, Any]) -> None:
        ...
        try:
            improvement = SystemImprovement(
                ...
                description=f"Applied automatic improvement based on pattern analysis",
                category='automatic',
    ...
    async def _perform_knowledge_update(self, category: str, outdated: str, updated: str, source: str) -> Dict[str, Any]:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'performance_metrics': {
                'recommendations_per_analysis': self.stats['recommendations_generated'] / max(self.stats['improvements_applied'], 1),
                'automatic_application_rate': self.stats['automatic_updates'] / max(self.stats['improvements_applied'], 1),
                'expert_validation_rate': self.stats['expert_validations_requested'] / max(self.stats['improvements_applied'], 1)
            }
        }