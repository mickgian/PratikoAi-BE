...
from app.models.ccnl_database import CCNLAgreementDB, CCNLSectorDB, JobLevelDB, SalaryTableDB, SpecialAllowanceDB
...
class SearchIndex:
    ...
    def get_tsvector_expression(self) -> str:
        ...
        for column in self.columns:
            ...
            expressions.append(f"setweight(to_tsvector('{self.language}', COALESCE({column}, '')), '{weight}')")
...
class CCNLIndexingService:
        ...
        try:
            with database_service.get_session_maker() as session:
                ...
                sector_stats = session.exec(
                    ...
                    .join(SalaryTableDB, CCNLAgreementDB.id == SalaryTableDB.agreement_id)
                ...
                aggregations["geographic_stats"] = {
                    stat[0]: {"agreement_count": stat[1], "avg_salary": float(stat[2]) if stat[2] else 0}
                ...
                aggregations["category_stats"] = {
                    stat[0]: {"agreement_count": stat[1], "level_count": stat[2]} for stat in category_stats
                }

                # Cache aggregations
                for key, value in aggregations.items():
                    cache_key = f"{self.cache_prefix}aggregation:{key}"
                    await cache_service.set(cache_key, json.dumps(value), ttl=self.aggregation_cache_ttl)
    ...
    async def get_cached_aggregation(self, aggregation_type: str) -> dict[str, Any] | None:
    ...
    async def get_index_health(self) -> dict[str, Any]:
        ...
        health = {"status": "healthy", "indexes": {}, "issues": []}
