...
from app.models.ccnl_update_models import CCNLUpdateEvent, ChangeType, UpdateSource, UpdateStatus
...
from app.services.ccnl_rss_monitor import CCNLUpdateDetector, RSSFeedItem, RSSFeedMonitor, UpdateClassifier
...
class CCNLIntegrationOrchestrator:
    ...
    async def process_full_workflow(self, rss_item: dict[str, Any], source: UpdateSource) -> dict[str, Any]:
        ...
        try:
            ...
            if not detections:
                ...
                return self._create_workflow_result(
                    workflow_id, start_time, False, 0.0, False, 0, 0, ["No CCNL updates detected"]
                )
            ...
            if version_created and changes_detected > 0:
                notification_result = await self._send_update_notifications(update_event, processing_result)
            ...
            return self._create_workflow_result(workflow_id, start_time, False, 0.0, False, 0, 0, errors)
    ...
    async def _send_update_notifications(
        self, update_event: CCNLUpdateEvent, processing_result: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Sent notifications for update {update_event.id}: {delivery_result.get('total_sent', 0)} delivered"
            )
    ...
    def _extract_changes_from_result(self, processing_result: dict[str, Any]) -> dict[str, Any]:
        ...
        if changes_count > 0:
            return {"salary_increases": {"level_1": {"percentage": 3.2}}, "working_hours": {"old": 40, "new": 38}}
        else:
            return {}

    async def _update_monitoring_metrics(self, workflow_id: str, processing_result: dict[str, Any]):
    ...
    def _update_workflow_stats(self, successful: bool, processing_time: float):
        ...
        if total == 1:
            ...
            self.workflow_stats["avg_processing_time"] = (current_avg * (total - 1) + processing_time) / total
    ...
    async def run_monitoring_cycle(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Monitoring cycle completed: {successful} successful, "
    ...
    def get_workflow_statistics(self) -> dict[str, Any]:
        ...
        return {
            **self.workflow_stats,
            "success_rate": (
                self.workflow_stats["successful_workflows"] / max(1, self.workflow_stats["total_workflows"])
            ),
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        health_status = {"overall_healthy": True, "components": {}, "checked_at": datetime.utcnow()}
        ...
        try:
            ...
            return {"overall_healthy": False, "error": str(e), "checked_at": datetime.utcnow()}
