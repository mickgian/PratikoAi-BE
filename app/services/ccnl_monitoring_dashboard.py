...
class CCNLMonitoringDashboard:
    ...
    def aggregate_dashboard_data(self, recent_updates: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        try:
            ...
            updates_24h = len([u for u in recent_updates if u.get("date", current_time) >= last_24h])
            updates_week = len([u for u in recent_updates if u.get("date", current_time) >= last_week])
            ...
            logger.info(
                f"Aggregated dashboard data: {updates_24h} updates in 24h, {len(sectors_updated)} sectors updated"
            )
    ...
    def get_coverage_statistics(self) -> dict[str, Any]:
        ...
        try:
            coverage_stats = {
                ...
                # Geographic coverage
                "geographic_coverage": {"national": 52, "regional_variations": 23, "provincial_variations": 8},
                "last_updated": datetime.utcnow(),
            }
