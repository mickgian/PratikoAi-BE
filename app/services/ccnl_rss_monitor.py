...
class RSSFeedMonitor:
    ...
    def _initialize_default_feeds(self):
        ...
        default_feeds = [
            ...
            {
                "url": "https://www.confindustria.it/feed/",
                "name": "Confindustria",
                "source": UpdateSource.CONFINDUSTRIA_NEWS,
            },
            {
                "url": "https://www.confcommercio.it/rss",
                "name": "Confcommercio",
                "source": UpdateSource.CONFCOMMERCIO_NEWS,
            },
            {
                "url": "https://www.confartigianato.it/feed/",
                "name": "Confartigianato",
                "source": UpdateSource.CONFARTIGIANATO_NEWS,
            },
            ...
            {
                "url": "https://www.lavoro.gov.it/rss",
                "name": "Ministry of Labor",
                "source": UpdateSource.MINISTRY_LABOR,
            },
        ]
    ...
    async def fetch_all_updates(self) -> list[RSSFeedItem]:
        ...
        logger.info(
            f"Fetched {len(unique_items)} unique CCNL items from {len([f for f in self.feeds if f['is_active']])} feeds"
        )
...
class CCNLUpdateDetector:
    ...
    def classify_sector(self, title: str, content: str = "") -> CCNLSector | None:
    ...
    def calculate_priority(self, title: str, content: str = "", sector: CCNLSector | None = None) -> float:
    ...
    def calculate_confidence(self, matched_keywords: list[str], sector: CCNLSector | None, update_type: str) -> float:
...
class UpdateClassifier:
    ...
    async def classify_update(self, feed_item: RSSFeedItem) -> dict[str, Any]:
        ...
        if self.ai_classification_enabled:
            ai_result = await self.detector.ai_classify_update(feed_item.title, feed_item.description)
