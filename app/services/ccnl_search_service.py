...
from app.models.ccnl_data import (
    CCNLSector, WorkerCategory, GeographicArea, LeaveType, 
    AllowanceType, CompanySize
)
from app.models.ccnl_database import (
    CCNLAgreementDB, CCNLSectorDB, JobLevelDB, SalaryTableDB,
    WorkingHoursDB, LeaveEntitlementDB, NoticePeriodsDB, 
    SpecialAllowanceDB
)
...
class SearchFilters:
    ...
    # Geographic filters
    geographic_areas: Optional[List[GeographicArea]] = None
    ...
    shift_work_allowed: Optional[bool] = None
    
    # Leave filters
    min_vacation_days: Optional[int] = None
    leave_types_required: Optional[List[LeaveType]] = None
    
    # Benefits filters
    required_allowances: Optional[List[AllowanceType]] = None
...
class SearchResult:
    ...
    # Key information
    salary_range: Optional[Tuple[Decimal, Decimal]] = None
    ...
    def __init__(self):
        ...
        self.sector_keywords = {
            CCNLSector.METALMECCANICI_INDUSTRIA: [
                "metalmeccanici", "metalmeccanico", "metal", "meccanica", 
                "industria meccanica", "operaio metalmeccanico"
            ],
            CCNLSector.COMMERCIO_TERZIARIO: [
                "commercio", "commerciale", "negozio", "vendita", "terziario",
                "commesso", "cassiere", "retail"
            ],
            CCNLSector.EDILIZIA_INDUSTRIA: [
                "edilizia", "edile", "costruzione", "costruzioni", "cantiere",
                "muratore", "carpentiere", "construction"
            ],
            CCNLSector.TURISMO: [
                "turismo", "turistico", "hotel", "albergo", "ristorante",
                "cameriere", "receptionist", "hospitality"
            ],
            CCNLSector.TRASPORTI_LOGISTICA: [
                "trasporti", "trasporto", "logistica", "autista", "spedizioni",
                "magazzino", "corriere", "delivery"
            ]
        }
        ...
        self.category_patterns = {
            WorkerCategory.OPERAIO: [
                r"\boperai[oi]?\b", r"\blavorator[ei]\b", r"\baddett[oi]\b"
            ],
            WorkerCategory.IMPIEGATO: [
                r"\bimpiegat[oi]?\b", r"\bufficio\b", r"\bamministrativ[oi]\b"
            ],
            WorkerCategory.QUADRO: [
                r"\bquadr[oi]?\b", r"\bresponsabil[ei]\b", r"\bcoordinator[ei]\b"
            ],
            WorkerCategory.DIRIGENTE: [
                r"\bdirigent[ei]?\b", r"\bdirettor[ei]\b", r"\bmanager\b"
            ]
        }
        ...
        self.experience_pattern = re.compile(
            r"(\d+)\s*ann[oi]?\s*(?:di\s*)?(?:esperienza|anzianità)",
            re.IGNORECASE
        )
        self.hours_pattern = re.compile(
            r"(\d+)\s*ore\s*(?:settimanal[ei]|a\s*settimana)",
            re.IGNORECASE
        )
    ...
    def parse_query(self, query: str) -> SearchFilters:
        ...
        if salary_match:
            ...
            single_salary_pattern = re.compile(
                r"(?:stipendi[oi]?|salari[oi]?|paga).*?(\d+).*?(?:euro|€)?",
                re.IGNORECASE
            )
...
class CCNLSearchService:
    ...
    async def search(
        ...
        try:
            ...
            suggestions = self.nlp_processor.suggest_query_improvements(
                query or filters.keywords or "", 
                total_count
            )
    ...
    def _build_search_query(self, filters: SearchFilters) -> Select:
        ...
        query = select(
            ...
            CCNLSectorDB, CCNLAgreementDB.sector_code == CCNLSectorDB.sector_code
        ).outerjoin(
            SalaryTableDB, CCNLAgreementDB.id == SalaryTableDB.agreement_id
        ).group_by(
            CCNLAgreementDB.id, CCNLSectorDB.id
        )
        ...
        if filters.active_only:
            ...
            conditions.append(
                and_(
                    ...
                    or_(
                        CCNLAgreementDB.valid_to.is_(None),
                        CCNLAgreementDB.valid_to >= filters.valid_on_date
                    )
                )
            )
    ...
    async def _calculate_facets(self, filters: SearchFilters) -> Dict[str, List[FacetCount]]:
        ...
        with database_service.get_session_maker() as session:
            # Sector facets
            sector_query = select(
                CCNLSectorDB.sector_code,
                CCNLSectorDB.italian_name,
                func.count(CCNLAgreementDB.id)
            ).join(
                CCNLAgreementDB, CCNLSectorDB.sector_code == CCNLAgreementDB.sector_code
            ).group_by(
                CCNLSectorDB.sector_code, CCNLSectorDB.italian_name
            )
            ...
            facets["sectors"] = [
                ...
                for code, name, count in sector_results
            ]
            
            # Worker category facets (from job levels)
            category_query = select(
                JobLevelDB.worker_category,
                func.count(func.distinct(JobLevelDB.agreement_id))
            ...
            facets["worker_categories"] = [
                FacetCount(
                    value=category,
                    count=count,
                    display_name=WorkerCategory(category).italian_name()
                )
            ...
            area_query = select(
                ...
                func.count(func.distinct(SalaryTableDB.agreement_id))
    ...
    async def _calculate_statistics(self, filters: SearchFilters) -> Dict[str, Any]:
        ...
        with database_service.get_session_maker() as session:
            ...
            query = select(
                ...
                CCNLAgreementDB, SalaryTableDB.agreement_id == CCNLAgreementDB.id
            )
    ...
    def _merge_filters(self, filters1: SearchFilters, filters2: SearchFilters) -> SearchFilters:
        ...
        merged.max_salary = min(
            filters1.max_salary or Decimal('999999'), 
            filters2.max_salary or Decimal('999999')
        ) if filters1.max_salary or filters2.max_salary else None
    ...
    async def compare_sectors(
        ...
        comparison = {
            "sector1": {
                "code": sector1.value,
                "name": sector1.italian_name()
            },
            "sector2": {
                "code": sector2.value,
                "name": sector2.italian_name()
            },
            "differences": {},
            "similarities": {}
        }
        ...
        with database_service.get_session_maker() as session:
            ...
            agreement1 = session.exec(
                select(CCNLAgreementDB).where(
                    CCNLAgreementDB.sector_code == sector1.value
                ).order_by(CCNLAgreementDB.valid_from.desc())
            ).first()
            
            agreement2 = session.exec(
                select(CCNLAgreementDB).where(
                    CCNLAgreementDB.sector_code == sector2.value
                ).order_by(CCNLAgreementDB.valid_from.desc())
            ).first()
            ...
            if "hours" in comparison_aspects:
                hours1 = session.exec(
                    select(WorkingHoursDB).where(
                        WorkingHoursDB.agreement_id == agreement1.id
                    )
                ).first()
                
                hours2 = session.exec(
                    select(WorkingHoursDB).where(
                        WorkingHoursDB.agreement_id == agreement2.id
                    )
                ).first()
    ...
    async def autocomplete(self, partial_query: str, limit: int = 10) -> List[str]:
        ...
        # Common search terms
        common_terms = [
            "stipendio", "salario", "ferie", "permessi", "orario",
            "turni", "straordinari", "tredicesima", "quattordicesima",
