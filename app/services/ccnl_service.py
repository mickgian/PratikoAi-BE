...
class CCNLQueryFilters:
    ...
    worker_categories: Optional[List[WorkerCategory]] = None
    geographic_area: Optional[GeographicArea] = None
    company_sizes: Optional[List[CompanySize]] = None
    valid_on_date: Optional[date] = None
    min_monthly_salary: Optional[Decimal] = None
    max_monthly_salary: Optional[Decimal] = None
    ...
    has_apprenticeship: Optional[bool] = None
    has_part_time: Optional[bool] = None
    has_flexible_hours: Optional[bool] = None
    minimum_leave_days: Optional[int] = None
    maximum_notice_days: Optional[int] = None
    has_union_rights: Optional[bool] = None
    has_training_provisions: Optional[bool] = None
    small_company_rules: Optional[bool] = None  # <15 employees
    priority_levels: Optional[List[int]] = None
    search_text: Optional[str] = None  # Full-text search
    
    def is_valid(self) -> bool:
        """Validate filter constraints."""
        if (self.min_monthly_salary and self.max_monthly_salary and 
            self.min_monthly_salary > self.max_monthly_salary):
            return False
        
        if (self.min_experience_months and self.max_experience_months and
            self.min_experience_months > self.max_experience_months):
            return False
...
class CCNLService:
    ...
    async def get_ccnl_by_id(self, ccnl_id: int) -> Optional[CCNLAgreementDB]:
    ...
    async def get_ccnl_by_sector(
        self, 
        sector: CCNLSector, 
        current_only: bool = True
    ) -> List[CCNLAgreementDB]:
        """Get all CCNL agreements for a specific sector."""
        try:
            with database_service.get_session_maker() as session:
                query = select(CCNLAgreementDB).where(
                    CCNLAgreementDB.sector_code == sector.value
                )
                ...
                if current_only:
                    ...
                    query = query.where(
                        and_(
                            ...
                            or_(
                                CCNLAgreementDB.valid_to.is_(None),
                                CCNLAgreementDB.valid_to >= today
                            )
                        )
                    )
    ...
    async def get_current_ccnl_by_sector(self, sector: CCNLSector) -> Optional[CCNLAgreementDB]:
            ...
            with database_service.get_session_maker() as session:
                ...
                if filters.valid_on_date:
                    ...
                    date_filter = and_(
                        ...
                        or_(
                            CCNLAgreementDB.valid_to.is_(None),
                            CCNLAgreementDB.valid_to >= check_date
                        )
                    )
        ...
        try:
            ...
            for salary in agreement.salary_tables:
                if (salary.level_code == level_code and 
                    salary.geographic_area == geographic_area.value):
                    salary_table = salary
                    break
    ...
    async def calculate_notice_period(
        ...
        try:
            ...
            for notice in agreement.notice_periods:
                if (notice.worker_category == worker_category.value and
    ...
    def _calculate_salary_statistics(self, matrix: Dict[str, Dict[str, float]], level_codes: List[str]) -> Dict[str, Any]:
            ...
            for i, salary_data in enumerate(data["salary_tables"]):
                if "salary" in salary_data:
                    try:
                        ...
                        if salary <= 0:
                            result.add_error(f"salary_tables[{i}].salary", 
                                           "Salary must be positive", "NEGATIVE_SALARY")
                    except (ValueError, TypeError):
                        result.add_error(f"salary_tables[{i}].salary",
                                       "Invalid salary format", "INVALID_SALARY")
    ...
    async def generate_coverage_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            worker_coverage_estimate = self._estimate_worker_coverage([s for s in all_sectors if len(await self.get_ccnl_by_sector(s, current_only=True)) > 0])
    ...
    ) -> Optional[CompensationBreakdown]:
    ...
    def _matches_advanced_filters(self, agreement: CCNLAgreement, filters: CCNLQueryFilters) -> bool:
        ...
        if filters.geographic_area:
            ...
            geographic_match = any(
                salary.geographic_area == filters.geographic_area
                for salary in agreement.salary_tables
            )
        ...
        if filters.worker_categories:
            ...
            category_match = any(
                level.category in filters.worker_categories
        ...
        if filters.small_company_rules is not None:
            ...
            has_small_company_rules = any(
                CompanySize.MICRO in salary.company_size_adjustments
                for salary in agreement.salary_tables
            )
        ...
        if filters.has_remote_work is not None:
            has_remote = (agreement.work_arrangement_rules and 
                         agreement.work_arrangement_rules.remote_work_allowed)
            if filters.has_remote_work != has_remote:
                return False
        
        if filters.has_part_time is not None:
            has_part_time = (agreement.work_arrangement_rules and
                           agreement.work_arrangement_rules.part_time_allowed)
            if filters.has_part_time != has_part_time:
                return False
        
        if filters.has_flexible_hours is not None:
            has_flexible = (agreement.working_hours and
                          agreement.working_hours.flexible_hours_allowed)
        ...
        if filters.minimum_leave_days:
            max_leave = max(
                (leave.base_annual_days for leave in agreement.leave_entitlements 
                 if leave.base_annual_days), 
                default=0
            )
        ...
        if filters.maximum_notice_days:
            max_notice = max(
                (notice.notice_days for notice in agreement.notice_periods),
                default=0
            )
    ...
    async def _compare_salary_ranges(
        self, 
        sector_agreements: Dict[CCNLSector, CCNLAgreement],
        result: Dict[str, Any],
        criterion: str
    ):
    ...
    async def search_by_company_size(
        ...
        include_special_rules: bool = True
    ) -> CCNLSearchResult:
    ...
    async def search_by_worker_category(
        self,
        worker_category: WorkerCategory,
        include_career_progression: bool = True
    ) -> Dict[str, Any]:
    ...
    async def search_by_geographic_area(
        self,
        geographic_area: GeographicArea,
        include_regional_variations: bool = True
    ) -> Dict[str, Any]:
    ...
    async def _analyze_regional_variations(
        ...
        for agreement in agreements:
            ...
            regional_salaries = [
                salary for salary in agreement.salary_tables
                if salary.geographic_area == geographic_area
            ]
    ...
    async def get_ccnl_coverage_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            stats["total"]["agreements_count"] = (
                len(priority1_data) + len(priority2_data) + 
                len(priority3_data) + len(priority4_data) + len(priority5_data)
            )
