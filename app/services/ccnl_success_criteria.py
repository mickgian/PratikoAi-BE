...
class CCNLSuccessCriteriaService:
    ...
    async def _verify_coverage_requirements(self) -> CoverageAnalysis:
        ...
        try:
            ...
            await self.ccnl_service.get_all_ccnl_data()

            # Analyze coverage by priority
            sectors_by_priority = {
                "Priority 1": [
                    sector.value for sector in CCNLSector if "INDUSTRIA" in sector.value or "COMMERCIO" in sector.value
                ][:10],
                "Priority 2": [
                    sector.value for sector in CCNLSector if "PRIVATA" in sector.value or "TERZIARIO" in sector.value
                ][:10],
                "Priority 3": [
                    sector.value for sector in CCNLSector if "ARTIGIANI" in sector.value or "ICT" in sector.value
                ][:10],
                "Priority 4": [
                    sector.value for sector in CCNLSector if "PUBBLICA" in sector.value or "SANITA" in sector.value
                ][:8],
                "Priority 5": [
                    sector.value for sector in CCNLSector if "SPETTACOLO" in sector.value or "SPORT" in sector.value
                ][:5],
                "Priority 6": [
                    sector.value for sector in CCNLSector if "TRASPORTO" in sector.value or "BELLEZZA" in sector.value
                ][:9],
            }
            ...
            covered_workers = sum(
                worker_count
                for sector, worker_count in self.major_sectors.items()
            ...
            all_major_sector_names = {sector.value for sector in self.major_sectors.keys()}
    ...
    def _determine_overall_success(
        ...
        # Coverage criteria
        coverage_success = (
            coverage.major_ccnls_covered >= 50
            and coverage.estimated_worker_coverage_percentage >= 90
            and len(coverage.coverage_gaps) == 0
        )
        ...
        calc_accuracy_rate = (
            sum(1 for c in calculations if c.within_tolerance) / len(calculations) if calculations else 0
        )
        ...
        return all(
            [coverage_success, query_criteria_met, calc_criteria_met, timeliness_criteria_met, test_coverage_met]
        )
    ...
    def _generate_recommendations(
        ...
        if coverage.estimated_worker_coverage_percentage < 90:
            recommendations.append(
                f"Improve worker coverage from {coverage.estimated_worker_coverage_percentage:.1f}% to 90%+ by adding missing sectors"
            )

        if coverage.major_ccnls_covered < 50:
            recommendations.append(
                f"Add {50 - coverage.major_ccnls_covered} more major CCNLs to reach minimum 50 requirement"
            )
        ...
        if slow_sources:
            recommendations.append(
                f"Improve update frequency for {len(slow_sources)} data sources exceeding 48-hour limit"
            )

        # Test coverage recommendations
        if test_coverage < 90:
            recommendations.append(
                f"Increase test coverage from {test_coverage:.1f}% to 90%+ by adding tests for uncovered code"
            )
