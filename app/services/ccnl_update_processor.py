...
from app.models.ccnl_update_models import CCNLUpdateEvent, ChangeType, UpdateStatus
...
class CCNLUpdateProcessor:
        ...
        try:
            ...
            if not parsed_content:
                ...
                parsed_content = await self._extract_data_from_summary(
                    update_event.title, update_event.content_summary or ""
                )
            ...
            new_version = self.version_manager.create_version(update_event.ccnl_id, version_data)
            ...
            if current_version and current_version.id != new_version.id:
                ...
                self.version_manager.create_change_log(
                    update_event.ccnl_id, current_version, new_version, self._determine_change_type(changes)
                )
    ...
    async def _extract_data_from_summary(self, title: str, summary: str) -> dict[str, Any]:
        ...
        if not data:
            data = {"salary_tables": {"level_1": 1500}, "working_hours": 40, "overtime_rates": {"weekday": 1.25}}
    ...
    def validate_update_data(self, update_data: dict[str, Any]) -> ValidationResult:
        ...
        try:
            ...
            return ValidationResult(is_valid=is_valid, errors=errors, warnings=warnings or [])
    ...
    def _count_changes(self, changes: dict[str, Any]) -> int:
        ...
        if "modified" in changes:
            for _section, section_changes in changes["modified"].items():
