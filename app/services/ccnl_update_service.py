...
class CCNLUpdateService:
    ...
    async def _check_renewal_availability(self, sector: CCNLSector) -> dict[str, Any] | None:
    ...
    async def get_active_alerts(self) -> list[CCNLAlert]:
        ...
        active = [alert for alert in self.active_alerts if not alert.expires_at or alert.expires_at > current_time]
        return active
