...
class MultiSourceContextBuilder:
    ...
    def __init__(
        ...
        self.preserve_phrases = [
            'in base all\'art', 'ai sensi dell\'art', 'secondo quanto previsto',
            'come stabilito dal', 'in conformità a', 'ai fini dell\'applicazione'
        ]
    ...
    async def build_context(
        ...
        source_types: List[str] = ['faq', 'regulation', 'circular', 'knowledge'],
        ...
        try:
            ...
            context_parts = await self._build_context_parts(
                grouped_results,
                query,
                token_limit,
                boost_recent
    ...
    async def _process_source_results(
        ...
        for result in results:
            ...
            relevant_content = await self._extract_relevant_portion(
                result.content,
                query,
                min(available_tokens - tokens_used, self.max_excerpt_length)
            )
            ...
            boosted_score = self._apply_context_boosting(
                result,
                relevant_content,
                query,
                boost_recent
    ...
    async def _extract_relevant_portion(
        self,
        content: str,
        query: str,
        max_tokens: int
        ...
        term_positions.sort()
        best_start, best_end = self._find_best_span(
            term_positions,
            max_length,
            len(content)
        )
    ...
    def _apply_context_boosting(
        ...
        professional_terms = [
            'ai sensi', 'in base', 'secondo', 'previsto', 'stabilito',
            'decreto', 'legge', 'articolo', 'comma', 'circolare'
        ]
    ...
    def configure_token_allocation(
        self,
        faq_percentage: Optional[float] = None,
        regulation_percentage: Optional[float] = None,
        circular_percentage: Optional[float] = None,
        knowledge_percentage: Optional[float] = None
    ) -> Dict[str, float]:
