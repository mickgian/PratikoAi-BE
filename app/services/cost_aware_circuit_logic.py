...
class CostAwareCircuitLogic:
    ...
    async def should_allow_request_cost_check(self, 
                                            provider: str, 
    ...
    async def record_actual_cost(self, 
        ...
        try:
            ...
            self.cost_history[provider] = [
                record for record in self.cost_history[provider]
    ...
    async def set_cost_budget(self, 
                            provider: str,
                            period: BudgetPeriod,
                            limit: float,
                            alert_thresholds: List[float] = None,
                            hard_limit: bool = True) -> bool:
    ...
    async def get_cost_metrics(self, provider: str) -> Optional[CostMetrics]:
    ...
    def _check_budget_limits(self, budget: CostBudget, estimated_cost: float) -> Dict[str, Any]:
        ...
        for threshold in budget.alert_thresholds:
            if utilization >= threshold:
                ...
                asyncio.create_task(self._create_cost_alert(
                    budget.provider,
    ...
    def _calculate_italian_cost_adjustment(self, base_cost: float) -> Dict[str, Any]:
        ...
        if now.month in [7, 11]:
            ...
            reasons.append(f"Tax deadline period (+{(self.italian_cost_factors['tax_deadline_multiplier'] - 1) * 100:.0f}%)")
        ...
        alternatives = [
            {
                ...
                "cost_savings": max(0, self.cost_history.get(current_provider, [{"cost": 0}])[-1]["cost"] - ranking["average_cost"])
            }
    ...
    async def _create_cost_alert(self, 
                               provider: str,
                               alert_type: CostBreakingReason,
                               message: str,
                               cost_impact: float,
                               threshold_exceeded: float) -> str:
        """Create cost-related alert"""
        
        alert_id = str(uuid4())
        
        # Determine severity
        if threshold_exceeded > 2.0 or alert_type in [CostBreakingReason.BUDGET_EXCEEDED, CostBreakingReason.DAILY_LIMIT_REACHED]:
            severity = "critical"
