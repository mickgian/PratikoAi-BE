...
class DataExportService:
    ...
    async def create_export_request(
        self, user_id: UUID, format: ExportFormat = ExportFormat.JSON, options: dict[str, Any] | None = None
    ) -> DataExportRequest:
        ...
        try:
            ...
            if recent_exports >= self.max_exports_per_day:
                raise ExportLimitExceeded(
                    f"Massimo {self.max_exports_per_day} export al giorno raggiunti. Riprova tra 24 ore."
                )
            ...
            await self._create_audit_log(
                export_request.id, user_id, "requested", {"format": format.value, "options": options}
            )

            # Queue for background processing
            await self._queue_export_job(export_request.id)

            logger.info(f"Data export requested: {export_request.id} by user {user_id}, format: {format.value}")
    ...
    async def process_export(self, export_id: UUID) -> None:
        ...
        try:
            ...
            # Create audit log
            await self._create_audit_log(export_request.id, export_request.user_id, "started")

            # Initialize progress tracking
            progress_tracker = ExportProgressTracker(self.redis)
            await progress_tracker.update_progress(str(export_id), "Inizializzazione export", 1, 8)

            # Collect all user data
            await progress_tracker.update_progress(str(export_id), "Raccolta dati utente", 2, 8)
            user_data = await self._collect_user_data(export_request)

            # Generate export files
            await progress_tracker.update_progress(str(export_id), "Generazione file export", 3, 8)
            files = await self._generate_export_files(user_data, export_request)

            # Create ZIP if multiple files or large size
            await progress_tracker.update_progress(str(export_id), "Creazione archivio", 4, 8)
            final_file = await self._create_final_export_file(files, export_request)

            # Upload to secure storage
            await progress_tracker.update_progress(str(export_id), "Caricamento sicuro", 5, 8)
            download_url = await self._upload_export_file(final_file, export_request)

            # Update request status
            await progress_tracker.update_progress(str(export_id), "Finalizzazione", 6, 8)
            ...
            await progress_tracker.update_progress(str(export_id), "Invio notifica", 7, 8)
            await self._send_export_ready_email(export_request)

            # Complete progress
            await progress_tracker.update_progress(str(export_id), "Export completato", 8, 8)
            ...
            logger.info(f"Data export completed: {export_request.id}, size: {export_request.file_size_bytes} bytes")
        ...
        if hasattr(user, "is_business") and user.is_business:
            profile["business_info"] = {
                ...
                "partita_iva": self._process_sensitive_field(
                    getattr(user, "partita_iva", None), export_request, "partita_iva"
                ),
            }
        ...
        if hasattr(user, "codice_fiscale") and user.codice_fiscale:
            if export_request.mask_codice_fiscale:
                ...
                profile["codice_fiscale"] = self._process_sensitive_field(
                    user.codice_fiscale, export_request, "codice_fiscale"
                )
    ...
    async def _collect_document_metadata(
        self, user_id: UUID, export_request: DataExportRequest
    ) -> list[dict[str, Any]]:
    ...
    async def _collect_tax_calculations(
        self, user_id: UUID, export_request: DataExportRequest
    ) -> list[dict[str, Any]]:
    ...
    async def _collect_subscription_history(
        self, user_id: UUID, export_request: DataExportRequest
    ) -> list[dict[str, Any]]:
    ...
    async def _collect_invoice_data(self, user_id: UUID, export_request: DataExportRequest) -> list[dict[str, Any]]:
        ...
        stmt = (
            select(Invoice).where(Invoice.subscription_id.in_(subscription_ids)).order_by(desc(Invoice.invoice_date))
        )
    ...
    async def _collect_usage_statistics(self, user_id: UUID, export_request: DataExportRequest) -> dict[str, Any]:
        ...
        queries_stmt = select(
            ...
            func.sum(func.case((QueryHistory.response_cached is True, 1), else_=0)).label("cached_queries"),
    ...
    async def _collect_faq_interactions(
        self, user_id: UUID, export_request: DataExportRequest
    ) -> list[dict[str, Any]]:
    ...
    async def _collect_knowledge_searches(
        self, user_id: UUID, export_request: DataExportRequest
    ) -> list[dict[str, Any]]:
        ...
        stmt = (
            select(KnowledgeBaseSearch).where(and_(*query_conditions)).order_by(desc(KnowledgeBaseSearch.searched_at))
        )
        result = await self.db.execute(stmt)
    ...
    async def _count_recent_exports(self, user_id: UUID, hours: int = 24) -> int:
        ...
        stmt = select(func.count(DataExportRequest.id)).where(
            and_(DataExportRequest.user_id == user_id, DataExportRequest.requested_at >= since)
        )
    ...
    async def _get_export_request(self, export_id: UUID) -> DataExportRequest | None:
    ...
    async def _create_audit_log(
        self, export_request_id: UUID, user_id: UUID, activity_type: str, activity_data: dict | None = None
    ) -> None:
    ...
    async def _create_final_export_file(
        self, files: dict[str, bytes], export_request: DataExportRequest
    ) -> dict[str, Any]:
    ...
    async def _upload_export_file(self, file_data: dict[str, Any], export_request: DataExportRequest) -> str:
        ...
        try:
            ...
            download_url = self.s3_client.generate_presigned_url(
                "get_object",
                Params={"Bucket": settings.EXPORT_S3_BUCKET, "Key": secure_filename},
...
class ExportProgressTracker:
    ...
    async def update_progress(self, export_id: str, step: str, current: int, total: int) -> None:
    ...
    async def get_progress(self, export_id: str) -> dict[str, Any]:
        ...
        return {"step": "In coda", "percentage": 0, "updated_at": datetime.utcnow().isoformat()}
