...
from app.models.ccnl_data import CCNLSector, CCNLAgreement
...
class DataSourceRegistry:
    ...
    def get_sources_by_type(self, source_type: DataSourceType) -> List[BaseDataSource]:
        ...
        return [
            ...
            if source.source_info.source_type == source_type
        ]
    ...
    def get_sources_for_sector(self, sector: CCNLSector) -> List[BaseDataSource]:
        ...
        return [
            source for source in self.sources.values()
            if sector in source.source_info.supported_sectors
        ]
    ...
    def get_prioritized_sources(self) -> List[BaseDataSource]:
        ...
        return sorted(
            self.sources.values(),
            key=lambda s: self.source_priority.get(s.source_info.source_id, 0),
            reverse=True
        )
