...
class FedermeccanicaDataSource(BaseDataSource):
    ...
    async def search_documents(self, query: DataSourceQuery) -> List[CCNLDocument]:
        ...
        relevant_sectors = [
            sector for sector in (query.sectors or self.source_info.supported_sectors)
...
class FederchimicaDataSource(BaseDataSource):
    ...
    async def _create_generic_document(
        self, link, base_url: str, sectors: List[CCNLSector]
    ) -> Optional[CCNLDocument]:
