...
class DatabaseEncryptionService:
    ...
    def __init__(self, master_key: Optional[str] = None, db_session: Optional[AsyncSession] = None):
        """
        ...
        self.current_key_version: Optional[int] = None
    ...
    def _validate_master_key(self) -> None:
        ...
        if len(self.master_key) != 44:
            raise ValueError(
                f"Invalid master key length: {len(self.master_key)}. "
                "Fernet keys must be exactly 44 characters."
            )
    ...
    async def encrypt_field(
        ...
        try:
            ...
            ciphertext = self._encrypt_aes_256_cbc(
    ...
    async def decrypt_field(
        ...
        try:
            ...
            plaintext_bytes = self._decrypt_aes_256_cbc(
                extracted_ciphertext or ciphertext,
                encryption_key.key_data
            )
    ...
    def _create_encrypted_result(
        ...
        return f"{metadata}{ciphertext_b64}".encode('utf-8')
    
    def _extract_metadata_from_ciphertext(self, ciphertext: bytes) -> Tuple[Optional[int], Optional[bytes]]:
    ...
    async def rotate_keys(self) -> None:
        ...
        try:
            ...
            if old_version and old_version in self.encryption_keys:
                ...
                self.encryption_keys[old_version].rotated_at = datetime.now(timezone.utc)
    ...
    async def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        avg_encryption_time = (
            (self.total_encryption_time / self.encryption_count * 1000) 
            if self.encryption_count > 0 else 0
        )
        avg_decryption_time = (
            (self.total_decryption_time / self.decryption_count * 1000) 
            if self.decryption_count > 0 else 0
        )
    ...
    async def migrate_unencrypted_data(
        ...
        try:
            ...
            while offset < stats["total_records"]:
                ...
                result = await self.db_session.execute(text(f"""
                    SELECT {field_list}
                    FROM {table_name}
                    ORDER BY id
                        ...
                        for i, field in enumerate(encrypted_fields):
                            ...
                            if field_value is not None:
                                encrypted_value = await self.encrypt_field(
                                    field_value, 
                                    FieldType.PERSONAL_DATA
                                )
                                update_values[field] = encrypted_value
                        
                        # Update record if there are values to encrypt
                        if update_values:
                            set_clause = ", ".join([f"{field} = :{field}" for field in update_values.keys()])
            ...
            await self._audit_log(
                ...
                success=stats["failed_records"] == 0
            )
...
def validate_encryption_config() -> Dict[str, Any]:
    ...
    config = {
        "master_key_set": bool(os.getenv("DB_ENCRYPTION_MASTER_KEY")),
...
ENCRYPTED_FIELDS_CONFIG = {
    ...
    "subscription_data": {
        ...
        "field_types": {
            "stripe_customer_id": FieldType.FINANCIAL_DATA,
            "invoice_data": FieldType.FINANCIAL_DATA
        }
    }
}