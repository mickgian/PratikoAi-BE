...
from app.models.document_simple import Document, DocumentType, ItalianDocumentCategory, ITALIAN_DOCUMENT_PATTERNS
...
class DocumentProcessor:
  ...
  async def _extract_f24_data(self, text: str) -> Dict[str, Any]:
    ...
    for match in tributi_matches:
      ...
      tributo = {
        'codice': codice,
        'importo': await self._parse_italian_number(importo)
      }
      if descrizione:
  ...
  async def _analyze_iva_register(self, df: pd.DataFrame) -> Dict[str, Any]:
    ...
    amount_cols = [col for col in df.columns if any(term in col.lower() 
                   for term in ['importo', 'totale', 'imponibile', 'imposta'])]
    ...
    if rate_cols and amount_cols:
      ...
      analysis['by_aliquota'] = {str(k): {'count': v['count'], 'imponibile': v['sum']} 
                                 for k, v in by_rate.items()}
    
    return analysis
  
  async def _parse_italian_number(self, value: str) -> Optional[float]:
  ...
  def _parse_italian_number_safe(self, value) -> Optional[float]:
