...
class EmbeddingManager:
    ...
    def __init__(
        self, pinecone_client, openai_client, db: AsyncSession | None = None, cache_service: CacheService | None = None
    ):
    ...
    async def generate_embeddings(
        self, texts: list[str], source_type: str = "general", use_cache: bool = True
    ) -> list[list[float]]:
        ...
        try:
            ...
            logger.info(
                f"Embedding generation completed in {processing_time:.2f}s: {len(valid_embeddings)}/{len(texts)} successful"
            )
    ...
    async def update_pinecone_embeddings(
        self, items: list[dict[str, Any]], source_type: str, namespace: str | None = None, batch_size: int = 100
        ...
        try:
            ...
            async def process_batch(batch_items: list[dict]) -> tuple[int, int, list[dict]]:
                async with semaphore:
                    return await self._process_embedding_batch(batch_items, source_type, target_namespace)
    ...
    async def _process_embedding_batch(
        self, batch_items: list[dict], source_type: str, namespace: str
        ...
        try:
            ...
            for item in batch_items:
                ...
                if not content:
                    failed += 1
                    failed_items.append({"id": item.get("id", "unknown"), "error": "No content provided"})
                    continue
            ...
            for item in batch_items:
                ...
                if not embedding:
                    failed += 1
                    failed_items.append({"id": item.get("id", "unknown"), "error": "Embedding generation failed"})
                    continue

                # Prepare metadata
                metadata = self._prepare_metadata(item, source_type)

                vector = {"id": str(item["id"]), "values": embedding, "metadata": metadata}
                vectors.append(vector)

            # Upsert to Pinecone
            if vectors:
                try:
                    upsert_response = self.pinecone.upsert(vectors=vectors, namespace=namespace)
                    ...
                    failed += len(vectors)
                    failed_items.extend([{"id": v["id"], "error": str(upsert_error)} for v in vectors])
    ...
    async def delete_embeddings(self, ids: list[str], namespace: str) -> dict[str, Any]:
        ...
        try:
            ...
            delete_response = self.pinecone.delete(ids=ids, namespace=namespace)
            ...
            return {"deleted_count": len(ids), "namespace": namespace, "success": True}

        except Exception as e:
            logger.error(f"Failed to delete embeddings: {e}")
            return {"deleted_count": 0, "namespace": namespace, "success": False, "error": str(e)}
    ...
    async def search_similar_content(
        ...
        if source_types is None:
            source_types = ["faq", "knowledge", "regulation"]
        try:
    ...
    async def get_embedding_statistics(self) -> dict[str, Any]:
        ...
        try:
            ...
            cost_per_embedding = self.stats["total_cost_usd"] / max(self.stats["total_embeddings_generated"], 1)
    ...
    async def optimize_embeddings(self, source_type: str, batch_size: int = 1000) -> dict[str, Any]:
        ...
        try:
            ...
            return {"source_type": source_type, "status": "failed", "error": str(e)}
