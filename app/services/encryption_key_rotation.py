...
from app.services.database_encryption_service import ENCRYPTED_FIELDS_CONFIG, DatabaseEncryptionService
...
class EncryptionKeyRotationService:
    ...
    def __init__(
        ...
        self.current_rotation: RotationProgress | None = None
    ...
    async def create_rotation_plan(
        ...
        rotation_id = f"rotation_{new_key_version}_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}"
    ...
    async def execute_rotation_plan(self, plan: RotationPlan) -> RotationResult:
        ...
        try:
            ...
            for table_name in plan.tables_to_rotate:
                ...
                try:
                    records_rotated, errors = await self._rotate_table_data(
                        table_name, plan.old_key_version, plan.new_key_version
                    )
                    ...
                    logger.info(f"Rotated {records_rotated} records in {table_name} ({errors} errors)")
                ...
                self.current_rotation.last_checkpoint = datetime.now(UTC)
            ...
            rollback_success = await self._rollback_rotation(
                plan.rotation_id, plan.old_key_version, plan.new_key_version
            )
    ...
    async def _create_new_encryption_key(self, new_key_version: int) -> None:
        ...
        try:
            ...
            if self.encryption_service.current_key_version != new_key_version:
                raise Exception(
                    f"Expected new key version {new_key_version}, got {self.encryption_service.current_key_version}"
                )
    ...
    async def _rotate_table_data(self, table_name: str, old_key_version: int, new_key_version: int) -> tuple[int, int]:
        ...
        count_result = await self.db.execute(text(f"SELECT COUNT(*) FROM {table_name}"))
        ...
        while offset < total_records:
            try:
                ...
                for record in batch_records:
                    try:
                        await self._rotate_record_encryption(
                            table_name, record, encrypted_fields, old_key_version, new_key_version
                        )
                        records_processed += 1
                ...
                if offset % (self.batch_size * 10) == 0:
                    ...
                    logger.info(f"Table {table_name}: {offset}/{total_records} records ({progress_pct:.1f}%)")
        ...
        logger.info(
            f"Completed table {table_name}: {records_processed} records processed, {errors_encountered} errors"
        )

        return records_processed, errors_encountered

    async def _rotate_record_encryption(
        self, table_name: str, record: Any, encrypted_fields: list[str], old_key_version: int, new_key_version: int
    ) -> None:
            ...
            if field_value is not None:
                try:
                    ...
                    decrypted_value = await self.encryption_service.decrypt_field(
                        field_value if isinstance(field_value, bytes) else field_value.encode(), old_key_version
                    )

                    # Re-encrypt with new key
                    encrypted_value = await self.encryption_service.encrypt_field(
                        decrypted_value,
                        field_type=ENCRYPTED_FIELDS_CONFIG[table_name]["field_types"].get(field_name, "string"),
                    )
    ...
    async def _rollback_rotation(self, rotation_id: str, old_key_version: int, new_key_version: int) -> bool:
    ...
    async def get_rotation_schedule(self) -> dict[str, Any]:
        ...
        if self.encryption_service.current_key_version:
            current_key = self.encryption_service.encryption_keys.get(self.encryption_service.current_key_version)
    ...
    async def emergency_rotation(self, reason: str) -> RotationResult:
        ...
        plan = await self.create_rotation_plan(priority=RotationPriority.EMERGENCY, scheduled_start=datetime.now(UTC))
...
async def schedule_automatic_rotation(db_session: AsyncSession, encryption_service: DatabaseEncryptionService) -> None:
    ...
    if await rotation_service.check_rotation_needed():
        ...
        plan = await rotation_service.create_rotation_plan(priority=RotationPriority.NORMAL)
...
async def get_rotation_status(
    db_session: AsyncSession, encryption_service: DatabaseEncryptionService
) -> dict[str, Any]:
