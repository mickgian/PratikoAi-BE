...
class EncryptionMonitoringService:
    ...
    async def collect_performance_metrics(self) -> PerformanceMetrics:
        ...
        try:
            ...
            total_encrypted_fields = sum(len(config["fields"]) for config in ENCRYPTED_FIELDS_CONFIG.values())
    ...
    async def collect_compliance_status(self) -> ComplianceStatus:
        ...
        try:
            ...
            for _table_name, config in ENCRYPTED_FIELDS_CONFIG.items():
                for _field_name, field_type in config.get("field_types", {}).items():
    ...
    async def collect_security_metrics(self) -> SecurityMetrics:
        ...
        try:
            ...
            key_age_days = 0
            if current_key:
                key_age = datetime.now(UTC) - current_key.created_at
    ...
    async def perform_health_check(self) -> SystemHealth:
        ...
        try:
            ...
            overall_status = self._calculate_overall_health(
                [encryption_status, key_mgmt_status, performance_status, compliance_health, security_health]
            )
            ...
            # Log health status
            logger.info(
                f"Health check completed: {overall_status.value} ({active_alerts} alerts, {critical_alerts} critical)"
            )
    ...
    async def create_alert(
        ...
        """
        alert_id = f"{category}_{component}_{datetime.now(UTC).timestamp()}"
        ...
        # Log alert
        logger.warning(f"ENCRYPTION ALERT [{severity.value.upper()}] {title}: {description}")
    ...
    async def get_monitoring_dashboard_data(self) -> dict[str, Any]:
        ...
        try:
            ...
            # Get top alerts
            top_alerts = sorted(self.active_alerts, key=lambda x: (x.severity.value, x.created_at), reverse=True)[:10]
    ...
    def _calculate_security_score(
        self, failed_decryptions: int, key_overdue_days: int, suspicious_patterns: int
    ) -> float:
        """Calculate security score (0-100)."""
        score = 100.0
    ...
    async def _get_last_key_rotation_date(self) -> str | None:
...
async def run_health_check(db_session: AsyncSession, encryption_service: DatabaseEncryptionService) -> dict[str, Any]:
