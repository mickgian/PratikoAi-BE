...
from langchain_core.messages import Message, SystemMessage, HumanMessage
...
class EnhancedQueryResponse:
    ...
    def __init__(
        ...
        context_used: Optional[QueryContext] = None,
...
class EnhancedQueryRouter:
        ...
        try:
            ...
            if enable_cache and cache_key and response.content:
                ...
                await self.cache_service.cache_llm_response(
                    cache_key, 
                    cache_data,
                    ttl_seconds=self._get_cache_ttl_for_classification(classification)
                )
    ...
    def _select_llm_provider(self, classification: DomainActionClassification, context: Optional[QueryContext]):
        ...
        return get_llm_provider(
            ...
            preferred_provider=self.settings.LLM_PREFERRED_PROVIDER
        )
    ...
    def _generate_cache_key(self, query: str, user_context: Optional[Dict[str, Any]]) -> str:
        ...
        if user_context:
            ...
            stable_context = {
                ...
                if k in ["profession", "region", "company_type"]
            }
