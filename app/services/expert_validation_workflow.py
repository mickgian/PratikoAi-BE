...
class ExpertValidationWorkflow:
    ...
    async def calculate_expert_trust_score(self, expert_profile: dict[str, Any]) -> float:
        ...
        try:
            ...
            total_score = (
                credentials_score + experience_score + performance_score + response_score + specialization_score
            )
            ...
            logger.debug(
                f"Trust score calculated: {trust_score:.3f} (credentials: {credentials_score:.3f}, experience: {experience_score:.3f}, performance: {performance_score:.3f})"
            )
    ...
    async def calculate_expert_consensus(self, expert_answers: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        try:
            ...
            for answer in expert_answers:
                ...
                weighted_answers.append({**answer, "weight": weight})
            ...
            consensus_reached = consensus_strength >= self.consensus_threshold and len(dominant_group["answers"]) >= 2
            ...
            result = {
                ...
                "disagreement_areas": self._identify_disagreement_areas(expert_answers)
                if not consensus_reached
                else [],
                "dominant_viewpoint": dominant_group.get("representative_answer", ""),
                "alternative_viewpoints": [
                    group["representative_answer"] for group in consensus_groups if group != dominant_group
    ...
    def _identify_consensus_groups(
        self, answers: list[dict[str, Any]], similarity_matrix: list[list[float]]
    ) -> list[dict[str, Any]]:
    ...
    async def _get_expert_profile(self, expert_id: str) -> dict[str, Any] | None:
    ...
    async def _store_feedback_loop_record(self, loop_result: dict[str, Any]) -> None:
        ...
        logger.info(
            f"Feedback loop completed: {loop_result.get('query_id', 'unknown')} - satisfaction: {loop_result.get('expert_satisfaction', 0):.2f}"
        )
    ...
    async def _log_learning_application(self, query_id: str, correction: dict[str, Any], iteration: int) -> None:
        ...
        logger.info(
            f"Learning applied: {query_id} - iteration {iteration} by expert {correction.get('expert_id', 'unknown')}"
        )
    ...
    async def get_validation_analytics(self, days: int = 30) -> dict[str, Any]:
        ...
        try:
            ...
            validations_query = (
                select(ExpertValidation)
                .where(ExpertValidation.requested_at >= start_date)
            ...
            return {
                ...
                "expert_participation_metrics": {
                    "total_experts_active": len(
                        {v.assigned_experts[0] if v.assigned_experts else "" for v in validations}
                    ),
                    "avg_experts_per_validation": sum(len(v.assigned_experts) for v in validations)
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "credential_weights": {cred_type.value: weight for cred_type, weight in self.credential_weights.items()},
            "performance_metrics": {
                "validation_success_rate": self.stats["validations_completed"]
                / max(self.stats["validations_requested"], 1),
                "consensus_success_rate": self.stats["consensus_reached"]
                / max(self.stats["validations_completed"], 1),
