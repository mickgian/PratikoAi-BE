...
from app.models.quality_analysis import (
    FailurePattern, ExpertFeedback, ItalianFeedbackCategory,
...
class FailurePatternAnalyzer:
    ...
    async def identify_patterns(self, feedback_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            filtered_patterns = [
                pattern for pattern in ranked_patterns 
            ...
            logger.info(f"Pattern analysis completed in {analysis_time:.1f}ms: {len(filtered_patterns)} patterns identified")
    ...
    def _analyze_category_patterns(self, categorized_feedback: Dict[str, List[Dict[str, Any]]]) -> List[Dict[str, Any]]:
        ...
        try:
            for pattern_data in patterns:
                ...
                if existing_pattern:
                    ...
                    existing_pattern.confidence_score = max(existing_pattern.confidence_score, pattern_data['confidence'])
    ...
    async def _find_similar_pattern(self, pattern_data: Dict[str, Any]) -> Optional[FailurePattern]:
        ...
        try:
            ...
            query = select(FailurePattern).where(
                and_(
                    FailurePattern.pattern_type == pattern_data['pattern_type'],
            ...
            for existing in existing_patterns:
                similarity = self._calculate_pattern_similarity(pattern_data, {
    ...
    def _calculate_pattern_similarity(self, pattern1: Dict[str, Any], pattern2: Dict[str, Any]) -> float:
        ...
        name_similarity = len(set(name1.split()) & set(name2.split())) / max(len(set(name1.split())), len(set(name2.split())), 1)
    ...
    async def _get_embeddings_matrix(self, texts: List[str]) -> Optional[np.ndarray]:
    ...
    def _get_tfidf_matrix(self, texts: List[str]) -> np.ndarray:
        ...
        vectorizer = TfidfVectorizer(
            max_features=1000,
            stop_words=['di', 'e', 'il', 'la', 'un', 'una', 'in', 'con', 'per', 'su', 'da', 'del', 'della'],  # Italian stop words
            ngram_range=(1, 2)
        )
    ...
    def _initialize_tax_patterns(self) -> Dict[str, List[str]]:
        ...
        return {
            'normativa_obsoleta': [
                'decreto', 'legge', 'circolare', 'normativa', 'aggiornamento',
                'modificata', 'cambiata', 'nuova', 'vecchia', 'obsoleta'
            ],
            'interpretazione_errata': [
                'interpretazione', 'sbagliata', 'errata', 'male', 'scorretto',
                'comprensione', 'significato', 'senso'
            ],
            'calcolo_sbagliato': [
                'calcolo', 'formula', 'numero', 'errore', 'sbagliato',
                'matematico', 'aritmetico', 'percentuale', 'aliquota'
            ],
            'caso_mancante': [
                'manca', 'mancante', 'assente', 'non considera', 'caso',
                'situazione', 'scenario', 'ipotesi'
            ],
            'troppo_generico': [
                'generico', 'vago', 'generale', 'specifico', 'dettaglio',
                'particolare', 'preciso', 'concreto'
            ]
        }
    ...
    async def get_pattern_analytics(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            patterns_query = select(FailurePattern).where(
                FailurePattern.first_detected >= start_date
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'performance_metrics': {
                'patterns_per_analysis': self.stats['patterns_identified'] / max(self.stats['failures_analyzed'] / 10, 1),
