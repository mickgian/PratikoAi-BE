...
class FAQBackgroundJobProcessor:
    ...
    async def process_pending_jobs(self) -> dict[str, Any]:
        ...
        try:
            ...
            jobs_query = (
                select(FAQGenerationJob)
                .where(FAQGenerationJob.status == "pending")
                .order_by(desc(FAQGenerationJob.priority), FAQGenerationJob.created_at)
            ...
            # Collect results
            results = {"processed": len(pending_jobs), "successful": 0, "failed": 0, "results": []}

            for i, result in enumerate(job_results):
                if isinstance(result, Exception):
                    results["failed"] += 1
                    results["results"].append(
                        {"job_id": str(pending_jobs[i].id), "status": "failed", "error": str(result)}
                    )
                else:
                    results["successful"] += 1
                    results["results"].append(result)

            logger.info(f"Job processing completed: {results['successful']} successful, {results['failed']} failed")
    ...
    async def _process_pattern_analysis_job(self, job: FAQGenerationJob) -> dict[str, Any]:
        ...
        analyzer = QueryPatternAnalyzer(self.db, self.embedding_service, self.normalizer)
    ...
    async def _process_faq_generation_job(self, job: FAQGenerationJob) -> dict[str, Any]:
        ...
        # Get candidate
        candidate_query = select(FAQCandidate).where(FAQCandidate.id == UUID(candidate_id))
    ...
    async def _process_rss_monitoring_job(self, job: FAQGenerationJob) -> dict[str, Any]:
        ...
        rss_integration = FAQRSSIntegration(self.db, self.llm_service, self.embedding_service)
    ...
    async def _process_batch_generation_job(self, job: FAQGenerationJob) -> dict[str, Any]:
        ...
        # Get candidates
        candidates_query = select(FAQCandidate).where(FAQCandidate.id.in_([UUID(cid) for cid in candidate_ids]))
    ...
    async def _cleanup_expired_candidates(self) -> dict[str, Any]:
        ...
        try:
            ...
            expired_query = select(FAQCandidate).where(
                and_(FAQCandidate.expires_at < datetime.utcnow(), FAQCandidate.status == "pending")
            )
            ...
            return {"candidates_expired": len(expired_candidates), "status": "completed"}
    ...
    async def _update_usage_statistics(self) -> dict[str, Any]:
        ...
        try:
            ...
            faqs_query = select(GeneratedFAQ).where(GeneratedFAQ.published is True)
            ...
            return {"faqs_updated": updated_count, "total_published": len(published_faqs), "status": "completed"}
    ...
    async def _archive_old_jobs(self) -> dict[str, Any]:
        ...
        try:
            ...
            old_jobs_query = select(FAQGenerationJob).where(
                and_(FAQGenerationJob.completed_at < cutoff_date, FAQGenerationJob.status.in_(["completed", "failed"]))
            )
    ...
    async def schedule_routine_jobs(self) -> list[str]:
        ...
        try:
            ...
            pattern_analysis_job = FAQGenerationJob(
                ...
                parameters={"scheduled": True, "analysis_window_days": 1},
                priority=7,
            )
            ...
            rss_monitoring_job = FAQGenerationJob(
                ...
                parameters={"scheduled": True, "check_all_feeds": True},
                priority=6,
            )
            ...
            maintenance_job = FAQGenerationJob(
                ...
                parameters={
                    "scheduled": True,
                    "tasks": ["cleanup_expired_candidates", "update_usage_statistics", "archive_old_jobs"],
                },
                priority=3,
            )
