...
class GDPRDocumentCleanup:
    ...
    async def _cleanup_retention_expired_documents(self) -> dict[str, Any]:
        ...
        query = select(Document).where(
            and_(Document.created_at < cutoff_date, Document.status != DocumentStatus.DELETED)
        )
        ...
        for document in expired_documents:
            try:
                ...
                self.logger.info(
                    f"Deleted expired document {document.id} (age: {(datetime.utcnow() - document.created_at).days} days)"
                )
        ...
        return {"expired_documents": cleaned_count, "storage_freed_mb": storage_freed_mb, "errors": errors}
    ...
    async def _cleanup_failed_processing_documents(self) -> dict[str, Any]:
        ...
        query = select(Document).where(
            and_(Document.status == DocumentStatus.FAILED, Document.created_at < cutoff_date)
        )
        ...
        return {"cleaned_count": cleaned_count, "storage_freed_mb": storage_freed_mb}
    ...
    async def delete_user_documents(self, user_id: UUID, reason: str = "USER_REQUEST") -> dict[str, Any]:
        ...
        query = select(Document).where(and_(Document.user_id == user_id, Document.status != DocumentStatus.DELETED))
    ...
    async def get_retention_status(self, user_id: UUID | None = None) -> dict[str, Any]:
        ...
        await self.db.execute(status_query)
