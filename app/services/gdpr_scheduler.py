...
from app.services.gdpr_deletion_service import DeletionStatus, GDPRDeletionService
...
class GDPRDeletionScheduler:
    ...
    async def initialize(self):
        ...
        try:
            self.engine = create_async_engine(self.settings.database_url, echo=False, pool_pre_ping=True)
    ...
    async def _process_deletion_batch(self, deletion_requests: list, gdpr_service: GDPRDeletionService) -> list:
        ...
        try:
            ...
            async with self.async_session() as session:
                ...
                for db_request in failed_requests:
                    ...
                    from app.services.gdpr_deletion_service import DeletionPriority, DeletionRequest
    ...
    async def get_scheduler_status(self) -> dict[str, Any]:
        ...
        try:
            ...
            if self.last_execution:
                next_execution = self.last_execution + timedelta(hours=self.job_config["execution_interval_hours"])
            ...
            return {"scheduler_status": "error", "error": str(e), "status_timestamp": datetime.now(UTC).isoformat()}
...
async def run_gdpr_deletion_job() -> dict[str, Any]:
    ...
    try:
        ...
        return {"job_status": "failure", "error": str(e), "execution_timestamp": datetime.now(UTC).isoformat()}
