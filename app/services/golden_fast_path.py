...
class GoldenFastPathService:
    ...
    async def is_eligible_for_fast_path(self, query_data: dict[str, Any]) -> EligibilityResult:
        ...
        try:
            ...
            rag_logging.rag_step_log(
                ...
                step_id=STEP_ID,
                node_label=NODE_LABEL,
                decision=error_result.decision
                if isinstance(error_result.decision, str)
                else error_result.decision.value,
    ...
    def _is_safe_quick_query(self, query: str) -> bool:
        ...
        return bool(re.search(r"\b(cos\'è|cosa è|quanto costa|come si)\b", query_lower))
    ...
    def _perform_safety_checks(self, query_data: dict[str, Any]) -> dict[str, Any]:
        ...
        if query_length > 500:
            return {"is_safe": False, "risk_level": "high", "reason": "query_too_long", "requires_doc_context": True}
        ...
        # Default: safe
        return {"is_safe": True, "risk_level": "low", "requires_doc_context": False}

    def can_serve_from_golden(
        self, confidence: float, kb_epoch: int, golden_epoch: int, confidence_threshold: float = 0.90
    ) -> bool:
        ...
        return not kb_epoch > golden_epoch
