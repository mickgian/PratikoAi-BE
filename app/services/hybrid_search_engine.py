...
class HybridSearchEngine:
    ...
    async def search(
        ...
        if search_types is None:
            search_types = ["faq", "knowledge", "regulation", "circular"]
        ...
        try:
            ...
            try:
                search_tasks = [
                    ...
                    asyncio.wait_for(
                        self._vector_search(query, search_types, date_filter), timeout=self.search_timeout_seconds
                    ),
                ]

                keyword_results, vector_results = await asyncio.gather(*search_tasks, return_exceptions=True)
    ...
    async def _keyword_search(
        self, query: str, expanded_queries: list[str], search_types: list[str], date_filter: datetime | None = None
    ) -> list[dict]:
        ...
        try:
            ...
            # Execute search
            params = {"tsquery": tsquery, "search_types": search_types, "max_results": self.max_results}
    ...
    async def _vector_search(
        self, query: str, search_types: list[str], date_filter: datetime | None = None
    ) -> list[dict]:
        ...
        try:
            ...
            if date_filter:
                metadata_filter["updated_at"] = {"$gte": date_filter.isoformat()}
            ...
            logger.debug(
                f"Vector search found {len(vector_results)} results above threshold {self.similarity_threshold}"
            )
    ...
    def _merge_results(self, keyword_results: list[dict], vector_results: list[dict]) -> list[SearchResult]:
        ...
        for result in vector_results:
            ...
            if result_id in merged:
                ...
                existing.relevance_score = (
                    existing.keyword_score * self.keyword_weight + result["vector_score"] * self.vector_weight
                )
                # Merge metadata
    ...
    async def _rerank_results(
        self, results: list[SearchResult], original_query: str, normalized_query: str
    ) -> list[SearchResult]:
    ...
    def _generate_cache_key(
        self, query: str, search_types: list[str], date_filter: datetime | None, limit: int
    ) -> str:
