...
class INPSINAILService:
    ...
    def _initialize_inail_rates(self) -> dict[RiskClass, INAILRate]:
        ...
        return {
            ...
            RiskClass.CLASS_5: INAILRate(
                ...
                sectors=[CCNLSector.TRASPORTI_LOGISTICA, CCNLSector.AUTOTRASPORTO_MERCI, CCNLSector.GOMMA_PLASTICA],
                valid_from=date(2024, 1, 1),
            ),
        }
    ...
    async def get_contribution_rates(self, sector: CCNLSector, worker_category: WorkerCategory) -> dict[str, Decimal]:
        ...
        if worker_category == WorkerCategory.QUADRO:
            rates["inps_employee"] = self.contribution_rates[
                f"{ContributionType.INPS_EMPLOYEE}_quadri"
            ].rate_percentage
            rates["inps_employer"] = self.contribution_rates[
                f"{ContributionType.INPS_EMPLOYER}_quadri"
            ].rate_percentage
    ...
    async def calculate_contributions(
        self, gross_salary: Decimal, sector: CCNLSector, worker_category: WorkerCategory, company_size: str = "medium"
    ...
    async def validate_contribution_calculation(self, calculation: ContributionCalculation) -> bool:
        ...
        recalc = await self.calculate_contributions(
            calculation.gross_salary, calculation.sector, calculation.worker_category
        )

        tolerance = Decimal("0.01")  # 1 cent tolerance

        return (
            abs(calculation.inps_employee - recalc.inps_employee) <= tolerance
            and abs(calculation.inps_employer - recalc.inps_employer) <= tolerance
            and abs(calculation.inail_employer - recalc.inail_employer) <= tolerance
        )

    async def get_annual_contribution_summary(
        self, sector: CCNLSector, worker_category: WorkerCategory, annual_salary: Decimal
    ) -> dict[str, Any]:
