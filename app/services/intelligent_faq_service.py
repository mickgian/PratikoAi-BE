...
class IntelligentFAQService:
    ...
    def __init__(self, db_session: AsyncSession, normalizer: ItalianQueryNormalizer | None = None):
    ...
    async def handle_query(self, query: str, user_id: str | None = None, context: str = "") -> FAQResponse:
        ...
        try:
            ...
            variation_result = await self.generate_variation(search_result.faq_entry, user_id, context)
    ...
    async def generate_variation(
        self, faq_entry: FAQEntry, user_id: str | None = None, context: str = ""
    ) -> dict[str, Any]:
        ...
        """
        # Generate cache key
        cache_key = generate_faq_cache_key(faq_entry.id, user_id or "anonymous", context)
        ...
        variation_text = await self._generate_gpt35_variation(faq_entry.answer, context)

        # Cache the variation
        await self._cache_variation(
            cache_key, faq_entry, user_id, faq_entry.answer, variation_text, "gpt-3.5-turbo", self.variation_cost
        )
    ...
    async def check_obsolescence(
        self, faq_entry: FAQEntry, recent_updates: list[dict[str, Any]]
        ...
        await self._log_obsolescence_check(
            faq_entry.id, is_obsolete, min(confidence_score, 1.0), reason, affecting_updates
        )
    ...
    async def collect_feedback(
        self, usage_log_id: str, was_helpful: bool, followup_needed: bool = False, comments: str | None = None
    ) -> bool:
        ...
        try:
            ...
            usage_log.feedback_submitted_at = datetime.now(UTC)
    ...
    async def get_analytics_summary(
        self, period_start: datetime, period_end: datetime, period_type: str = "daily"
    ) -> dict[str, Any]:
        ...
        usage_query = select(FAQUsageLog).where(FAQUsageLog.used_at >= period_start, FAQUsageLog.used_at <= period_end)
        ...
        cost_savings_data = calculate_cost_savings(
            total_queries, faq_responses, self.variation_cost, self.full_llm_cost
        )
        ...
        if faq_usage_count:
            popular_faq_ids = sorted(faq_usage_count.keys(), key=lambda x: faq_usage_count[x], reverse=True)[:10]
            ...
            for faq in popular_faqs:
                category_usage_count[faq.category] = (
                    category_usage_count.get(faq.category, 0) + faq_usage_count[faq.id]
                )

        top_categories = [
            {"category": cat, "usage_count": count}
            for cat, count in sorted(category_usage_count.items(), key=lambda x: x[1], reverse=True)[:5]
        ]

        top_faqs = [
            {"faq_id": faq_id, "usage_count": count}
            for faq_id, count in sorted(faq_usage_count.items(), key=lambda x: x[1], reverse=True)[:10]
        ]
    ...
    async def _get_cached_variation(self, cache_key: str) -> FAQVariationCache | None:
        """Get cached variation if it exists and hasn't expired."""
        query = select(FAQVariationCache).where(
            FAQVariationCache.cache_key == cache_key, FAQVariationCache.expires_at > datetime.now(UTC)
        )
    ...
    async def _update_cache_metrics(self, cached_variation: FAQVariationCache):
        ...
        cached_variation.last_used = datetime.now(UTC)
        await self.db.commit()
    ...
    async def _cache_variation(
        ...
        cache_entry = FAQVariationCache(
            ...
            expires_at=datetime.now(UTC) + timedelta(hours=self.variation_cache_hours),
        )
    ...
    async def _log_usage(
        self, faq_entry: FAQEntry, user_id: str | None, response_text: str, from_cache: bool, cost: float
    ):
        ...
        faq_entry.last_used = datetime.now(UTC)
    ...
    async def _update_faq_helpfulness(self, faq_id: str):
        ...
        helpfulness_query = select(func.avg(FAQUsageLog.was_helpful.cast(float))).where(
            FAQUsageLog.faq_id == faq_id, FAQUsageLog.was_helpful.is_not(None)
        )
...
async def get_faq_by_category(db: AsyncSession, category: str, limit: int = 50) -> list[FAQEntry]:
    """Get FAQ entries by category."""
    query = select(FAQEntry).where(FAQEntry.category == category, FAQEntry.needs_review is False).limit(limit)

    result = await db.execute(query)
    return result.scalars().all()


async def search_faqs_by_tags(db: AsyncSession, tags: list[str], limit: int = 20) -> list[FAQEntry]:
    """Search FAQ entries by tags."""
    # This would use PostgreSQL array operations in production
    query = select(FAQEntry).where(FAQEntry.needs_review is False).limit(limit)
