...
from app.models.subscription import Subscription, Invoice, SubscriptionPlan
...
class InvoiceData:
    ...
    # Customer data
    customer_name: str
    customer_partita_iva: Optional[str]
    customer_codice_fiscale: Optional[str]
    customer_address: str
    customer_cap: str
    customer_city: str
    customer_province: str
    customer_sdi_code: Optional[str]
    customer_pec_email: Optional[str]
...
class ItalianInvoiceService:
    ...
    async def _generate_pdf_invoice(self, invoice_data: InvoiceData) -> bytes:
        ...
        # Invoice lines
        lines_data = [
            ["Descrizione", "Qta", "Prezzo Unit.", "IVA%", "Totale"]
        ]
        ...
        while True:
            try:
                ...
                success = await self._transmit_to_sdi(
                    transmission_data["xml_content"],
                    transmission_data["invoice_number"]
                )
