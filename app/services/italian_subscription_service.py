...
class ItalianSubscriptionService:
    ...
    async def create_subscription(
        ...
        try:
            ...
            if not user:
                return SubscriptionResult(success=False, error_message="User not found")

            # Check if user already has active subscription
            existing_subscription = await self._get_user_subscription(user_id)
            if existing_subscription and existing_subscription.is_active:
                return SubscriptionResult(success=False, error_message="User already has an active subscription")

            # Validate Italian tax data
            validation_errors = self._validate_italian_tax_data(invoice_data)
            if validation_errors:
                return SubscriptionResult(
                    success=False, error_message=f"Dati non validi: {', '.join(validation_errors.values())}"
                )

            # Get subscription plan
            plan = await self._get_plan_by_type(plan_type)
            if not plan:
                return SubscriptionResult(success=False, error_message=f"Piano {plan_type} non trovato")
            ...
            return SubscriptionResult(success=True, subscription=subscription, stripe_subscription=stripe_subscription)

        except stripe.error.StripeError as e:
            logger.error(f"Stripe error creating subscription: {e}")
            return SubscriptionResult(success=False, error_message=f"Errore di pagamento: {str(e)}")
        except Exception as e:
            logger.error(f"Error creating subscription: {e}")
            await self.db.rollback()
            return SubscriptionResult(success=False, error_message="Errore interno del sistema")

    async def change_plan(self, subscription_id: str, new_plan_type: str, prorate: bool = True) -> PlanChangeResult:
        ...
        try:
            ...
            if not subscription:
                return PlanChangeResult(success=False, error_message="Abbonamento non trovato")

            # Get new plan
            new_plan = await self._get_plan_by_type(new_plan_type)
            if not new_plan:
                return PlanChangeResult(success=False, error_message=f"Piano {new_plan_type} non trovato")

            # Check if change is allowed
            can_change, reason = subscription.can_change_to_plan(new_plan)
            if not can_change:
                return PlanChangeResult(success=False, error_message=reason)
            ...
            stripe_subscription = await self._update_stripe_subscription(
                subscription.stripe_subscription_id, new_plan, prorate
            )
            ...
            return PlanChangeResult(success=False, error_message=f"Errore di pagamento: {str(e)}")
        except Exception as e:
            logger.error(f"Error changing plan: {e}")
            await self.db.rollback()
            return PlanChangeResult(success=False, error_message="Errore interno del sistema")
    ...
    async def cancel_subscription(
        ...
        try:
            subscription = await self._get_subscription(subscription_id)
            if not subscription:
                return CancellationResult(success=False, error_message="Abbonamento non trovato")

            if not subscription.is_active:
                return CancellationResult(success=False, error_message="Abbonamento già cancellato")
            ...
            if immediately:
                ...
                self.stripe.Subscription.modify(subscription.stripe_subscription_id, cancel_at_period_end=True)
            ...
            return CancellationResult(success=False, error_message=f"Errore di pagamento: {str(e)}")
        except Exception as e:
            logger.error(f"Error canceling subscription: {e}")
            await self.db.rollback()
            return CancellationResult(success=False, error_message="Errore interno del sistema")

    async def reactivate_subscription(
        self, subscription_id: str, payment_method_id: str | None = None
    ) -> SubscriptionResult:
        ...
        try:
            subscription = await self._get_subscription(subscription_id)
            if not subscription:
                return SubscriptionResult(success=False, error_message="Abbonamento non trovato")

            # Check if reactivation is possible
            if subscription.status not in [SubscriptionStatus.CANCELED, SubscriptionStatus.UNPAID]:
                return SubscriptionResult(success=False, error_message="Abbonamento non può essere riattivato")

            # Update payment method if provided
            if payment_method_id:
                self.stripe.PaymentMethod.attach(payment_method_id, customer=subscription.stripe_customer_id)
    ...
    async def get_subscription_plans(self) -> list[dict[str, Any]]:
        ...
        try:
            stmt = select(SubscriptionPlan).where(SubscriptionPlan.is_active is True)
    ...
    async def _get_plan_by_type(self, plan_type: str) -> SubscriptionPlan | None:
        """Get plan by billing period type"""
        billing_period = BillingPeriod.ANNUAL if plan_type == "annual" else BillingPeriod.MONTHLY
        stmt = select(SubscriptionPlan).where(
            and_(SubscriptionPlan.billing_period == billing_period, SubscriptionPlan.is_active is True)
        )
        result = await self.db.execute(stmt)
        return result.scalar_one_or_none()
    ...
    async def _create_stripe_customer(self, user: User, invoice_data: dict[str, Any]) -> stripe.Customer:
        ...
        if invoice_data.get("is_business") and invoice_data.get("partita_iva"):
            customer_data["tax_ids"] = [{"type": "eu_vat", "value": f"IT{invoice_data['partita_iva']}"}]
    ...
    async def _update_stripe_subscription(
        self, stripe_subscription_id: str, new_plan: SubscriptionPlan, prorate: bool
    ) -> stripe.Subscription:
        ...
        return self.stripe.Subscription.modify(
            ...
            items=[{"id": subscription.items.data[0].id, "price": new_plan.stripe_price_id}],
    ...
    async def _send_welcome_email(self, user: User, subscription: Subscription):
        ...
        template = (
            "subscription_welcome_annual"
            if subscription.plan.billing_period == BillingPeriod.ANNUAL
            else "subscription_welcome_monthly"
        )
        ...
        await self.email_service.send_email(
            ...
            context={
                ...
                "savings": subscription.plan.annual_savings
                if subscription.plan.billing_period == BillingPeriod.ANNUAL
                else None,
    ...
    async def _track_subscription_canceled(self, subscription: Subscription, reason: str | None):
