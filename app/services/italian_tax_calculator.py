...
class IrpefCalculator:
    ...
    def calculate_irpef_with_deductions(self, income: Decimal, deductions: dict[str, Decimal]) -> dict[str, Any]:
...
class VatCalculator:
    ...
    def calculate_eu_vat(
        self, net_amount: Decimal, origin_country: str, destination_country: str, service_type: str = "standard"
    ) -> dict[str, Any]:
...
class CorporateTaxCalculator:
    ...
    def calculate_irap(
        self, production_value: Decimal, region: str, business_type: str = "standard"
    ) -> dict[str, Any]:
    ...
    def calculate_combined_corporate_tax(
        ...
        effective_rate = (
            (total_tax / taxable_income * 100).quantize(Decimal("0.01")) if taxable_income > 0 else Decimal("0")
        )
...
class PropertyTaxCalculator:
    ...
    def __init__(self, regional_tax_service: RegionalTaxService | None = None):
        self.config = IMU_CONFIG_2024
        self.regional_service = regional_tax_service

    async def calculate_imu(
        self, cadastral_value: Decimal, cap: str, is_primary_residence: bool = False, property_type: str = "standard"
    ) -> dict[str, Any]:
...
class CapitalGainsTaxCalculator:
    ...
    def calculate_capital_gains_tax(
        self, purchase_price: Decimal, sale_price: Decimal, asset_type: str, holding_period_days: int = 0
    ) -> dict[str, Any]:
...
class TaxDeductionEngine:
    ...
    def calculate_family_deductions(
        ...
        if dependent_children > 0:
            ...
            for _i, age in enumerate(children_ages[:dependent_children]):
    ...
    def calculate_renovation_deductions(
        self, expenses: Decimal, renovation_type: str = "energy_efficiency"
    ) -> dict[str, Any]:
...
class FreelancerTaxCalculator:
    ...
    def calculate_flat_rate_tax(
        self, annual_revenue: Decimal, activity_category: str, is_new_business: bool = False
    ) -> dict[str, Any]:
        ...
        coefficient = self.flat_rate_config["activity_coefficients"].get(
            activity_category, self.flat_rate_config["activity_coefficients"]["other_services"]
        )
    ...
    def calculate_standard_regime_tax(
        self, annual_revenue: Decimal, business_expenses: Decimal, region: str, municipality: str
    ) -> dict[str, Any]:
        ...
        total_tax = irpef_result["gross_tax"] + regional_surcharge + municipal_surcharge + inps_result["contribution"]
    ...
    def calculate_quarterly_payments(
        self, previous_year_total_tax: Decimal, current_quarter_revenue: Decimal, quarter: int
    ) -> dict[str, Any]:
...
class TaxOptimizationEngine:
    ...
    def compare_employment_types(self, gross_income: Decimal) -> dict[str, Any]:
        ...
        return {
            ...
            "employee": {
                ...
                "tax_rate": (employee_irpef["gross_tax"] + employee_inps["employee_contribution"])
                / gross_income
                * 100,
            },
            "contractor": {
    ...
    def optimize_pension_contributions(self, annual_income: Decimal, current_contributions: Decimal) -> dict[str, Any]:
...
class ItalianTaxCalculator:
    ...
    async def calculate_net_salary(
        ...
        self.irpef_calculator.calculate_irpef(gross_salary)
    ...
    async def calculate_freelancer_taxes(
        ...
        if regime == "forfettario":
            ...
            tax_result = self.freelancer_calculator.calculate_flat_rate_tax(
                annual_revenue=annual_revenue, activity_category=activity_type, is_new_business=is_new
            )

            # INPS still applies in flat-rate regime
            inps_result = self.inps_calculator.calculate_self_employed_contribution(tax_result["taxable_income"])
    ...
    def process_natural_language_query(self, query: str) -> dict[str, Any]:
        ...
        if "net salary" in query_lower or "stipendio netto" in query_lower:
            ...
            return {
                ...
                "extracted_parameters": {"gross_income": gross_income, "location": location},
    ...
    async def _calculate_regional_municipal_taxes(self, income: Decimal, location: str) -> dict[str, Any]:
