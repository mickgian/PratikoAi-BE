...
class ItalianLegalDocumentAnalyzer:
    ...
    async def analyze_legal_document(
        ...
        try:
            ...
            analyzer_func = self.document_analyzers.get(
                document_category, 
                self._analyze_generic_legal
            )
            ...
            base_analysis["compliance_check"] = await self._check_legal_compliance(
                document_category,
                base_analysis
            )
            
            base_analysis["risk_assessment"] = self._assess_legal_risks(
                document_category,
                base_analysis
            )
    ...
    async def _analyze_citazione(self, text: str, data: Optional[Dict] = None) -> Dict[str, Any]:
    ...
    async def _analyze_ricorso(self, text: str, data: Optional[Dict] = None) -> Dict[str, Any]:
        """Analyze a ricorso (appeal/petition)"""
        analysis = {
            "document_type": LegalDocumentType.ATTO_INTRODUTTIVO,
            "procedural_stage": "Impugnazione/Ricorso"
        }
        ...
        atto_match = re.search(r"(?:impugna|contro|avverso) (?:il |la |l')(.+?)(?:notificat|del|emess)", text, re.IGNORECASE)
        if atto_match:
            analysis["atto_impugnato"] = atto_match.group(1).strip()
        
        # Extract notification date for deadline calculation
        notifica_match = re.search(r"notificat[oa] (?:il |in data )(\d{1,2}[/\-\.]\d{1,2}[/\-\.]\d{2,4})", text, re.IGNORECASE)
        if notifica_match:
            ...
            analysis["scadenze"] = self._calculate_ricorso_deadlines(
                notifica_date, 
                analysis.get("tipo_ricorso", "ricorso_generico")
            )
    ...
    async def _analyze_decreto_ingiuntivo(self, text: str, data: Optional[Dict] = None) -> Dict[str, Any]:
    ...
    async def _analyze_diffida(self, text: str, data: Optional[Dict] = None) -> Dict[str, Any]:
        """Analyze a diffida (formal notice/warning)"""
        analysis = {
            "document_type": LegalDocumentType.ATTO_STRAGIUDIZIALE,
            "procedural_stage": "Fase stragiudiziale"
        }
    ...
    async def _analyze_contratto(self, text: str, data: Optional[Dict] = None) -> Dict[str, Any]:
        """Analyze a contratto (contract)"""
        analysis = {
            "document_type": LegalDocumentType.CONTRATTUALE,
            "procedural_stage": "Negoziale"
        }
    ...
    async def _analyze_sentenza(self, text: str, data: Optional[Dict] = None) -> Dict[str, Any]:
        """Analyze a sentenza (judgment)"""
        analysis = {
            "document_type": LegalDocumentType.PROVVEDIMENTO_GIUDICE,
            "procedural_stage": "Decisione"
        }
    ...
    async def _analyze_precetto(self, text: str, data: Optional[Dict] = None) -> Dict[str, Any]:
        """Analyze an atto di precetto (formal demand before execution)"""
        analysis = {
            "document_type": LegalDocumentType.ATTO_STRAGIUDIZIALE,
            "procedural_stage": "Esecuzione forzata"
        }
        ...
        analysis["termine_adempimento"] = {
            "giorni": 10,
            "scadenza": (datetime.now() + timedelta(days=10)).isoformat()
        }
    ...
    async def _analyze_comparsa(self, text: str, data: Optional[Dict] = None) -> Dict[str, Any]:
        """Analyze a comparsa di risposta (response brief)"""
        analysis = {
            "document_type": LegalDocumentType.ATTO_DIFENSIVO,
            "procedural_stage": "Costituzione in giudizio"
        }
    ...
    async def _analyze_generic_legal(self, text: str, data: Optional[Dict] = None) -> Dict[str, Any]:
    ...
    def _extract_exceptions(self, text: str) -> List[str]:
        ...
        exception_keywords = [
            "eccepisce", "eccezione", "incompetenza", "prescrizione",
            "decadenza", "nullità", "difetto", "carenza"
        ]
    ...
    def _extract_ricorrente(self, text: str) -> Optional[str]:
        """Extract appellant/petitioner"""
        patterns = [
            r"ricorrente\s*[:]*\s*([A-Z][^,\n]{10,50})",
            r"([A-Z][^,\n]{10,50})\s*ricorre"
        ]
    ...
    def _extract_resistente(self, text: str) -> Optional[str]:
        """Extract respondent"""
        patterns = [
            r"resistente\s*[:]*\s*([A-Z][^,\n]{10,50})",
            r"contro\s+([A-Z][^,\n]{10,50})"
        ]
    ...
    async def _get_ai_legal_analysis(
        self,
        category: ItalianDocumentCategory,
        text: str,
...
Domanda specifica: {query}"""
        ...
        try:
            ...
            return {
                ...
                "tipo_analisi": "approfondimento_legale"
            }
            
        except Exception as e:
            logger.error(f"AI legal analysis failed: {e}")
            return {
                "errore": "Analisi AI non disponibile",
                "dettaglio": str(e)
            }
    ...
    def _extract_creditore(self, text: str) -> Optional[str]:
    ...
    def _extract_debitore(self, text: str) -> Optional[str]:
    ...
    def _extract_diffida_object(self, text: str) -> Optional[str]:
    ...
    def _extract_contract_object(self, text: str) -> Optional[str]:
    ...
    def _extract_contract_price(self, text: str) -> Optional[float]:
    ...
    def _extract_contract_duration(self, text: str) -> Optional[str]:
    ...
    def _extract_key_clauses(self, text: str) -> List[str]:
        ...
        clause_keywords = [
            "risoluzione", "recesso", "penale", "garanzia",
            "riserva", "esclusiva", "divieto", "obbligo"
        ]
    ...
    def _extract_convenuto(self, text: str) -> Optional[str]:
        """Extract defendant from comparsa"""
        patterns = [
            r"per\s+([A-Z][^,\n]{10,50})\s+convenut",
            r"nell'interesse di\s+([A-Z][^,\n]{10,50})"
        ]
    ...
    def _extract_counterclaims(self, text: str) -> List[str]:
        ...
        patterns = [
            r"domanda riconvenzionale(.{20,200})",
            r"in via riconvenzionale(.{20,200})"
        ]
