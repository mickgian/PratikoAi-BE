...
class LinkedInOAuthService:
    ...
    async def handle_callback(self, code: str, state: Optional[str] = None) -> Dict:
        ...
        try:
            ...
            user_info = {
                **profile_info,
                "email": email_info.get("emailAddress")
            }
    ...
    async def _exchange_code_for_token(self, code: str) -> Dict:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code != 200:
                logger.error("linkedin_oauth_token_exchange_failed", 
    ...
    async def _get_profile_info(self, access_token: str) -> Dict:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code != 200:
                logger.error("linkedin_oauth_profile_failed", 
                           status_code=response.status_code, 
                           response=response.text)
    ...
    async def _get_email_info(self, access_token: str) -> Dict:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code != 200:
                logger.error("linkedin_oauth_email_failed", 
                           status_code=response.status_code, 
                           response=response.text)
        ...
        with Session(database_service.engine) as session:
            ...
            existing_user = session.exec(
                select(User).where(
                    ...
                    User.provider_id == linkedin_user_id
                )
            ).first()
