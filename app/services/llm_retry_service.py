...
class RetryHandler:
    ...
    async def execute_with_retry(
        ...
        **kwargs
        ...
        logger.debug(
            f"[{request_id}] Starting retry execution, max_attempts={self.config.max_attempts}"
        )
        ...
        for attempt in range(self.config.max_attempts):
            ...
            try:
                ...
                logger.info(
                    f"[{request_id}] Success after {attempt + 1} attempts in {duration:.2f}s"
                )
                ...
                if self._is_retryable_http_error(e):
                    ...
                    logger.warning(
                        f"[{request_id}] HTTP {e.response.status_code} on attempt {attempt + 1}"
                    )
                    ...
                    logger.error(
                        f"[{request_id}] Non-retryable HTTP {e.response.status_code}: {e}"
                    )
        ...
        duration = time.time() - start_time
        logger.error(
            f"[{request_id}] All {self.config.max_attempts} attempts failed in {duration:.2f}s"
        )
    ...
    def _is_retryable_error(self, error: Exception) -> bool:
        ...
        retryable_keywords = [
            "rate limit", "timeout", "overloaded", "overwhelmed",
            "connection", "temporary", "unavailable", "busy",
            "throttled", "capacity", "server error"
        ]
    ...
    async def _handle_retry(
        ...
        logger.info(
            f"[{request_id}] Retry attempt {attempt + 1} after {error_type}, "
...
class CostAwareRetryHandler(RetryHandler):
    ...
    async def execute_with_retry(self, func: Callable[..., T], *args, **kwargs) -> T:
        ...
        # Check retry budget
        current_costs = await self.cost_tracker.get_retry_costs(
            user_id,
        ...
        if current_costs >= self.config.max_retry_cost:
            logger.warning(
                f"User {user_id} exceeded retry budget: "
                f"{current_costs:.3f} >= {self.config.max_retry_cost:.3f} EUR"
            )
...
class ErrorCategorizer:
    ...
    def is_transient(self, error: Exception) -> bool:
        ...
        transient_keywords = [
            "rate limit", "timeout", "overloaded", "overwhelmed",
            "service temporarily unavailable", "connection",
            "temporary", "busy", "throttled", "capacity"
        ]
        
        # Permanent error indicators
        permanent_keywords = [
            "invalid api key", "access denied", "unauthorized",
            "forbidden", "malformed request", "bad request",
...
class RetryMetrics:
    ...
    async def record_failure(self, provider: str, error_type: str):
        ...
        try:
            if self.retry_failure_counter:
                self.retry_failure_counter.labels(
                    provider=provider,
    ...
    async def get_health_score(self, provider: str, hours: int = 1) -> float:
        ...
        try:
            ...
            failure_count = sum(
                await self._get_redis_metric(key, hours)
