...
class ItalianLocationService:
    ...
    def __init__(self, db=None, cache_service: CacheService | None = None):
        self.db = db
    ...
    def _infer_location_from_cap_prefix(self, cap: str) -> dict[str, Any] | None:
    ...
    async def search_locations(
        self, query: str, limit: int = 10, region_filter: str | None = None
    ) -> list[dict[str, Any]]:
    ...
    def _get_sample_cap_for_provincia(self, provincia: str) -> str | None:
    ...
    async def get_distance_between_caps(self, cap1: str, cap2: str) -> float | None:
        ...
        try:
            ...
            if coord1.get("lat") and coord1.get("lng") and coord2.get("lat") and coord2.get("lng"):
                return self._haversine_distance(coord1["lat"], coord1["lng"], coord2["lat"], coord2["lng"])
        except Exception as e:
    ...
    async def validate_italian_address(self, address_components: dict[str, str]) -> dict[str, Any]:
        ...
        """
        result = {"is_valid": True, "errors": [], "warnings": [], "corrections": {}, "confidence": 1.0}
                ...
                try:
                    ...
                    if comune and comune.lower() != cap_location["comune"].lower():
                        result["warnings"].append(
                            f"Il comune '{comune}' non corrisponde al CAP {cap} (previsto: {cap_location['comune']})"
                        )
        ...
        if provincia and provincia not in self.PROVINCIA_TO_REGIONE:
            ...
            similar = [p for p in self.PROVINCIA_TO_REGIONE.keys() if abs(len(p) - len(provincia)) <= 1]
            if similar:
        ...
        if regione:
            ...
            if not found_regione:
                ...
                similar_regions = [
                    r for r in self.REGION_DATA.keys() if regione.lower() in r.lower() or r.lower() in regione.lower()
                ]
                if similar_regions:
    ...
    async def get_nearby_locations(self, cap: str, radius_km: float = 50, limit: int = 10) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for sample_cap, _comune in list(self.CAP_TO_CITY.items())[:100]:  # Limit search
                if sample_cap == cap:
                    continue
                ...
                try:
                    ...
                    if coords.get("lat") and coords.get("lng"):
                        distance = self._haversine_distance(
                            center_coords["lat"], center_coords["lng"], coords["lat"], coords["lng"]
                        )
    ...
    async def _get_locations_same_region(self, regione: str, limit: int) -> list[dict[str, Any]]:
        ...
        for cap, _comune in self.CAP_TO_CITY.items():
            try:
                location = await self.get_location_from_cap(cap)
    ...
    async def get_region_statistics(self, regione_name: str) -> dict[str, Any]:
        ...
        return {
            ...
            "is_autonomous": regione_name
            in ["Sicilia", "Sardegna", "Trentino-Alto Adige", "Valle d'Aosta", "Friuli-Venezia Giulia"],
        }
