...
class QualityMetricsDashboard:
    ...
    def __init__(self, db: AsyncSession, cache: CacheService | None = None):
        ...
        # Statistics tracking
        self.stats = {"metrics_calculated": 0, "alerts_generated": 0, "dashboards_served": 0, "cache_hit_rate": 0.0}
    ...
    async def get_failure_analysis_dashboard(self) -> dict[str, Any]:
        ...
        try:
            ...
            patterns_query = (
                ...
                .where(FailurePattern.is_resolved is False)
            ...
            categories_query = (
                select(ExpertFeedback.category, func.count().label("count"))
                .where(
                    and_(
            ...
            trending_query = (
                ...
                .where(FailurePattern.last_occurrence >= datetime.utcnow() - timedelta(days=7))
                .order_by(desc(FailurePattern.frequency_count))
                .limit(5)
            )
            ...
            ).where(ExpertFeedback.feedback_timestamp >= datetime.utcnow() - timedelta(days=30))

            result = await self.db.execute(intervention_query)
            intervention_data = result.one()

            intervention_rate = (
                intervention_data.interventions / intervention_data.total if intervention_data.total > 0 else 0
            )

            # Automated fixes applied
            fixes_query = (
                select(func.count())
                .select_from(SystemImprovement)
                .where(
                    and_(
                        SystemImprovement.created_at >= datetime.utcnow() - timedelta(days=30),
                        SystemImprovement.status == ImprovementStatus.COMPLETED,
                        SystemImprovement.requires_expert_validation is False,
                    )
                )
            )
    ...
    async def get_expert_performance_metrics(
        self, expert_id: str | None = None, time_period: timedelta = timedelta(days=30)
    ) -> dict[str, Any]:
        ...
        try:
            ...
            feedback_query = (
                select(
                    ...
                    func.count(case((ExpertFeedback.feedback_type == FeedbackType.CORRECT, 1))).label(
                        "correct_feedback"
                    ),
                    func.count(case((ExpertFeedback.feedback_type == FeedbackType.INCORRECT, 1))).label(
                        "incorrect_feedback"
                    ),
                )
            ...
            profiles_query = select(ExpertProfile).where(ExpertProfile.is_active is True)
            ...
            for profile in profiles:
                expert_data = next((fb for fb in feedback_data if fb.expert_id == profile.id), None)

                if expert_data:
                    accuracy_rate = (
                        expert_data.correct_feedback / expert_data.total_feedback
                        if expert_data.total_feedback > 0
                        else 0
                    )
                    ...
                    expert_metrics[str(profile.id)] = {
                        ...
                        "feedback_distribution": {
                            ...
                            "incomplete": expert_data.total_feedback
                            - expert_data.correct_feedback
                            - expert_data.incorrect_feedback,
                        },
            ...
            return {
                ...
                "aggregate_metrics": {
                    "avg_trust_score": sum(e["trust_score"] for e in expert_metrics.values())
                    / max(len(expert_metrics), 1),
                    "avg_accuracy_rate": sum(e["accuracy_rate"] for e in expert_metrics.values())
                    / max(len(expert_metrics), 1),
                    "avg_response_time": sum(e["average_response_time"] for e in expert_metrics.values())
                    / max(len(expert_metrics), 1),
                    "total_feedback_provided": sum(e["feedback_count"] for e in expert_metrics.values()),
                },
                "top_performers": sorted(expert_metrics.values(), key=lambda x: x["performance_score"], reverse=True)[
                    :5
                ],
            }
    ...
    async def track_system_improvements(self, start_date: datetime, end_date: datetime) -> dict[str, Any]:
        """Track system improvements over time"""
        try:
            # System improvements query
            improvements_query = (
                select(SystemImprovement)
                .where(and_(SystemImprovement.created_at >= start_date, SystemImprovement.created_at <= end_date))
    ...
    async def check_quality_alerts(self, current_metrics: dict[str, float]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            return [
                {"type": "system_error", "priority": "high", "message": f"Sistema di monitoraggio in errore: {str(e)}"}
            ]
    ...
    async def _calculate_accuracy_metrics(self, start_date: datetime, end_date: datetime) -> dict[str, Any]:
        ...
        ).where(and_(ExpertFeedback.feedback_timestamp >= start_date, ExpertFeedback.feedback_timestamp <= end_date))
        ...
        current_accuracy = (
            ...
            else self.target_accuracy_score
        )
    ...
    async def _calculate_expert_satisfaction_metrics(self, start_date: datetime, end_date: datetime) -> dict[str, Any]:
        ...
        validation_query = select(
            ...
            func.count(case((ExpertValidation.consensus_reached is True, 1))).label("successful_validations"),
        ).where(and_(ExpertValidation.requested_at >= start_date, ExpertValidation.requested_at <= end_date))
    ...
    async def _calculate_failure_rate_metrics(self, start_date: datetime, end_date: datetime) -> dict[str, Any]:
        ...
        ).where(FailurePattern.last_occurrence >= start_date)
        ...
        current_failure_rate = (
            ...
            if intervention_data.total_responses > 0
            else 0
        )
    ...
    async def _calculate_improvement_velocity_metrics(
        self, start_date: datetime, end_date: datetime
    ) -> dict[str, Any]:
        """Calculate improvement velocity metrics"""
        improvements_query = select(
            func.count().label("total_improvements"),
            func.count(case((SystemImprovement.status == ImprovementStatus.COMPLETED, 1))).label(
        ...
        improvements_per_week = (
            improvements_data.completed_improvements / time_period_weeks if time_period_weeks > 0 else 0
        )
        ...
        return {
            ...
            "completion_rate": (
                ...
                if improvements_data.total_improvements > 0
                else 0
            ),
        }
    ...
    async def _calculate_expert_engagement_metrics(self, start_date: datetime, end_date: datetime) -> dict[str, Any]:
        ...
        active_experts_query = select(func.count(func.distinct(ExpertFeedback.expert_id))).where(
            and_(ExpertFeedback.feedback_timestamp >= start_date, ExpertFeedback.feedback_timestamp <= end_date)
        )
        ...
        total_experts_query = select(func.count()).select_from(ExpertProfile).where(ExpertProfile.is_active is True)
    ...
    async def _get_quality_snapshot(self, date: datetime) -> dict[str, float]:
        ...
        return {"overall_quality": 0.82, "accuracy": 0.85, "satisfaction": 0.78}
    ...
    def _get_category_description(self, category: ItalianFeedbackCategory | None) -> str:
    ...
    def _calculate_expert_performance_score(self, metrics: dict[str, Any]) -> float:
        ...
        performance_score = accuracy * 0.4 + trust_score * 0.3 + normalized_response_time * 0.2 + activity_bonus * 0.1
