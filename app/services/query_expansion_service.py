...
class ItalianTaxQueryExpander:
    ...
    async def expand_query(
        ...
        try:
            ...
            if include_semantic and len(candidates) < max_expansions:
                semantic_candidates = await self._semantic_expansion(
                    query, 
                    max_expansions - len(candidates)
                )
    ...
    def _tokenize_query(self, query: str) -> List[str]:
        ...
        protected_phrases = [
            'partita iva', 'regime forfettario', 'dichiarazione dei redditi',
            'fattura elettronica', 'codice fiscale', 'imposta sul reddito',
            'valore aggiunto', 'modello 730', 'modello unico', 'f24 online'
        ]
    ...
    def _filter_candidates(
        ...
        for candidate in candidates:
            if (candidate.confidence >= min_confidence and 
                candidate.term.lower() not in seen_terms and
                candidate.term.lower() not in original_query.lower()):
        ...
        filtered.sort(
            key=lambda x: (x.score, x.confidence, source_priority.get(x.source, 0)),
            reverse=True
        )
    ...
    def _build_tax_expansions(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            'iva': [
                ...
                'detraibile iva', 'imponibile iva'
            ],
            'aliquota': [
                'aliquota iva', 'aliquota ordinaria', 'aliquota ridotta',
                'aliquota agevolata', 'percentuale iva'
            ],
            ...
            'reddito': [
                'reddito imponibile', 'reddito complessivo', 'reddito lordo',
                'reddito netto', 'dichiarazione redditi'
            ],
            ...
            'libero professionista': [
                'partita iva', 'professionista autonomo', 'lavoro autonomo',
                'attività professionale'
            ],
            
            # Fatturazione
            'fattura': [
                'fatturazione', 'fattura elettronica', 'e-fattura',
                'emissione fattura', 'registrazione fattura', 'sdi fattura'
            ],
            'fatturazione elettronica': [
                'fattura elettronica', 'e-fattura', 'xml fattura',
                'sistema interscambio', 'sdi', 'fatturazione digitale'
            ],
            
            # Modelli fiscali
            'f24': [
                'modello f24', 'f24 online', 'versamento f24',
                'delega f24', 'compensazione f24'
            ],
            '730': [
                'modello 730', '730 precompilato', 'dichiarazione 730',
                'caf 730', 'elaborazione 730'
            ],
            'unico': [
                'modello unico', 'unico persone fisiche', 'unico società',
                'dichiarazione unico'
            ],
            ...
            'forfettario': [
                'regime forfettario', 'coefficiente redditività',
                'soglia forfettario', 'requisiti forfettario'
            ],
            
            # Detrazioni e deduzioni
            'detrazione': [
                'detrazioni fiscali', 'detrazione irpef', 'spese detraibili',
                'oneri detraibili', 'rimborso detrazione'
            ],
            'deduzione': [
                'deduzioni fiscali', 'oneri deducibili', 'spese deducibili',
                'deduzione dal reddito'
            ],
            
            # Tributi locali
            'imu': [
                'imposta municipale unica', 'imposta municipale',
                'tassa casa', 'tassa immobili', 'tributo immobiliare'
            ],
            'tari': [
                'tassa rifiuti', 'tributo rifiuti', 'tassa ambientale',
                'tariffa rifiuti'
            ],
            'tasi': [
                'tassa servizi indivisibili', 'tributo servizi'
            ],
            
            # Contributi previdenziali
            'contributi': [
                'contributi inps', 'contributi previdenziali',
                'versamenti contributivi', 'gestione separata'
            ],
            'inps': [
                'istituto previdenza sociale', 'contributi inps',
                'gestione separata inps', 'cassetto previdenziale'
            ],
            
            # Società e forme giuridiche
            'srl': [
                'società responsabilità limitata', 's.r.l.',
                'società di capitali', 'srl semplificata'
            ],
            'spa': [
                'società per azioni', 's.p.a.',
                'società di capitali', 'consiglio amministrazione'
            ],
            'snc': [
            ...
            'sas': [
                ...
                'società di persone', 'soci accomandanti'
            ]
        }
    ...
    def _build_professional_terms(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            'consulente': [
                ...
                'consulente del lavoro', 'advisor fiscale'
            ],
            'studio': [
                'studio commercialista', 'studio fiscale',
                'studio tributario', 'ufficio fiscale'
            ],
            'cliente': [
                'contribuente', 'soggetto passivo',
                'titolare partita iva', 'impresa'
            ],
            'assistenza': [
                'consulenza fiscale', 'servizi fiscali',
                'supporto tributario', 'adempimenti fiscali'
            ],
            'dichiarazione': [
            ...
            'versamento': [
                'pagamento imposte', 'versamento tributi',
                'adempimento versamento', 'liquidazione imposte'
            ],
            'scadenza': [
                'termine versamento', 'scadenza fiscale',
                'termine adempimento', 'deadline fiscale'
            ]
        }
    ...
    def _build_regional_variations(self) -> Dict[str, List[str]]:
        ...
        return {
            'tasse': [
                'tasse', 'tributi', 'imposte', 'balzelli',
                'gabelle', 'contribuzioni'
            ],
            'soldi': [
                'denaro', 'quattrini', 'euro', 'importo',
                'somma', 'capitale'
            ],
            'pagare': [
                'versare', 'saldare', 'liquidare', 'corrispondere',
                'adempiere', 'estinguere'
            ],
            'guadagno': [
                'reddito', 'provento', 'utile', 'ricavo',
                'profitto', 'entrata'
            ],
            'spesa': [
                'costo', 'onere', 'esborso', 'uscita',
                'spettanza', 'erogazione'
            ],
            'lavoro': [
                'attività', 'prestazione', 'servizio',
    ...
    def _build_concept_relationships(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            'dichiarazione': [
                'modello fiscale', 'termine presentazione', 'rimborso',
                'accertamento', 'controllo fiscale'
            ],
            'iva': [
            ...
            'reddito': [
                'base imponibile', 'scaglioni tassazione', 'no tax area',
                'detrazioni personali', 'addizionali'
            ],
            'contributi': [
                'minimale contributivo', 'massimale contributivo',
                'aliquota contributiva', 'cassa previdenza'
            ]
        }
