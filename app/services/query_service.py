...
from app.core.llm import LLMFactory, get_llm_provider
...
from app.services.resilient_llm_service import LLMResult, ResilientLLMService, get_llm_service
...
class QueryService:
    ...
    async def process_query(
        self, request: QueryRequest, use_retry_mechanisms: bool = True
    ) -> QueryResponseSchema | QueryErrorResponse:
        ...
        try:
            ...
            query_response.completed_at = datetime.now(UTC)
            ...
            logger.error(f"[{query_id}] Query failed with retry error: {type(e).__name__}: {e}")
            ...
            logger.warning(f"[{query_id}] Query failed due to cost budget exceeded for user {e.user_id}")
            ...
            logger.error(f"[{query_id}] Query failed with unexpected error: {e}", exc_info=True)
    ...
    async def _process_with_retry_mechanisms(self, request: QueryRequest, query_id: str) -> LLMResult:
    ...
    async def _process_with_factory(self, request: QueryRequest, query_id: str) -> LLMResult:
        ...
        provider = get_llm_provider(
            messages=messages, strategy=RoutingStrategy.BALANCED, preferred_provider=request.preferred_provider
        )
        ...
        response = await provider.chat_completion(
            messages=messages, temperature=request.temperature or 0.7, max_tokens=request.max_tokens
        )
    ...
    async def get_query_status(self, query_id: str) -> QueryResponseSchema | None:
    ...
    async def get_user_query_metrics(self, user_id: str, hours: int = 24) -> QueryMetrics:
        ...
        try:
            ...
            return QueryMetrics(start_time=datetime.now(UTC), end_time=datetime.now(UTC))
...
async def get_query_service(db_session: AsyncSession | None = None) -> QueryService:
...
__all__ = ["QueryService", "get_query_service", "get_query_service_health"]
