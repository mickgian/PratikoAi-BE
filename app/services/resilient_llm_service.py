...
from app.models.query import LLMResponse, QueryResponse
...
class ResilientLLMService:
        ...
        for name, config in self.provider_configs.items():
            if config.enabled:
                ...
                self.circuit_breakers[name] = CircuitBreaker(
                    config.retry_config.circuit_breaker_threshold, config.retry_config.circuit_breaker_timeout
                )
                ...
                if hasattr(self.settings, "cost_tracking_enabled") and self.settings.cost_tracking_enabled:
                    ...
                    self.retry_handlers[name] = CostAwareRetryHandler(
                        config.retry_config, self.circuit_breakers[name], cost_tracker
                    )
                else:
                    self.retry_handlers[name] = RetryHandler(config.retry_config, self.circuit_breakers[name])
    ...
    async def _execute_request(self, request: LLMRequest) -> LLMResult:
        ...
        providers_to_try = self._get_provider_order(request.preferred_provider, request.allow_fallback)

        logger.info(f"[{request_id}] Starting LLM request for user {request.user_id}, providers: {providers_to_try}")
        ...
        for i, provider_name in enumerate(providers_to_try):
            ...
            try:
                ...
                await self.metrics.record_success(
                    provider_name, processing_time, was_fallback, request.preferred_provider if was_fallback else None
                )

                logger.info(
                    f"[{request_id}] Success with {provider_name} in {processing_time:.2f}s (fallback={was_fallback})"
                )
        ...
        logger.error(f"[{request_id}] All providers failed in {processing_time:.2f}s, last_error: {last_error}")
    ...
    def _get_provider_order(self, preferred_provider: str | None, allow_fallback: bool) -> list[str]:
        ...
        available_providers = [
            name for name, config in self.provider_configs.items() if config.enabled and name in self.providers
        ]
        ...
        if allow_fallback:
            ...
            remaining = sorted(available_providers, key=lambda name: self.provider_configs[name].priority)
            providers_to_try.extend(remaining)

        return providers_to_try

    def _estimate_cost(self, prompt: str, max_tokens: int | None) -> float:
    ...
    async def get_retry_statistics(self, hours: int = 1) -> dict[str, Any]:
        ...
        return {"providers": stats, "hours": hours, "generated_at": datetime.now(UTC).isoformat()}
...
class LLMServiceManager:
    ...
    async def _perform_health_check(self):
        ...
        for provider_name, status in health_status.items():
            ...
            if status["health_score"] < 50:
                logger.warning(f"Provider {provider_name} health score: {status['health_score']:.1f}%")

            # Automatically disable providers that are consistently failing
            if status["circuit_breaker_state"] == "open" and status["health_score"] < 25:
    ...
    async def get_service_status(self) -> dict[str, Any]:
        ...
        enabled_providers = [name for name, status in health_status.items() if status["enabled"]]

        if not enabled_providers:
            overall_health = "critical"
        else:
            avg_health = sum(health_status[name]["health_score"] for name in enabled_providers) / len(
                enabled_providers
            )
