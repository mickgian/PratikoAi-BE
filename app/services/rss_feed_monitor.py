...
This service monitors RSS feeds from Italian authorities and provides:
- Feed parsing with multiple source-specific parsers
- Health checking (response time, item counts, error tracking)
- DB-driven feed configuration (reads from feed_status table)

No hardcoded feed URLs - all feeds are read from the database.
"""
...
class RSSFeedMonitor:
    ...
    All feed URLs are read from the database (feed_status table).
    No hardcoded URLs.
    """
    ...
    def __init__(self, timeout: int = 30, db_session=None):
        ...
        Args:
            ...
            db_session: Database session for reading feed configuration (REQUIRED)
        """
    ...
    async def parse_agenzia_entrate_feed(self, feed_url: str, source_type: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                "agenzia_entrate_feed_parsed", feed_url=feed_url, source_type=source_type, items_count=len(results)
            )
    ...
    async def parse_inps_feed(self, feed_url: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info("inps_feed_parsed", feed_url=feed_url, items_count=len(results))
    ...
    async def parse_gazzetta_ufficiale_feed(self, feed_url: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info("gazzetta_ufficiale_feed_parsed", feed_url=feed_url, items_count=len(results))
    ...
    async def parse_feed_with_error_handling(
        ...
        Args:
            feed_url: RSS feed URL
            parser: Parser to use (agenzia_normativa, inps, gazzetta_ufficiale, generic).
                   If not provided, will auto-detect based on URL.

        Returns:
            List of parsed feed items (empty list on error)
        """
        try:
            # Use explicit parser if provided
            if parser:
                if parser == "agenzia_normativa":
            ...
            logger.error(
                "feed_parse_error_handling_failed", feed_url=feed_url, parser=parser, error=str(e), exc_info=True
            )
            return []
    ...
    async def get_all_italian_feeds(self) -> List[Dict[str, Any]]:
        ...
        Returns:
            List of feed dictionaries with keys: source, feed_type, feed_url, parser

        Raises:
            RuntimeError: If db_session is not set
        ...
        try:
            ...
            logger.info("feeds_loaded_from_database", feed_count=len(feeds))
    ...
    async def _parse_generic_feed(self, feed_url: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info("generic_feed_parsed", feed_url=feed_url, domain=domain, items_count=len(results))

            return results

        except Exception as e:
            logger.error("generic_feed_parse_failed", feed_url=feed_url, error=str(e), exc_info=True)
            return []
...
class FeedHealthMonitor:
    ...
    async def check_feed_health(self, feed_url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error("feed_health_check_failed", feed_url=feed_url, error=str(e), exc_info=True)
    ...
    async def check_all_italian_feeds_health(self) -> Dict[str, Dict[str, Any]]:
        ...
        async with RSSFeedMonitor() as monitor:
            ...
            for feed_name, feed_url in feeds.items():
                task = asyncio.create_task(self.check_feed_health(feed_url), name=f"health_check_{feed_name}")
            ...
            for feed_name, task in tasks:
                try:
                    ...
                    logger.error("feed_health_check_task_failed", feed_name=feed_name, error=str(e))
