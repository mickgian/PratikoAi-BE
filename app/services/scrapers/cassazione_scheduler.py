...
class ScheduledScrapingJob:
    ...
    last_run_result: Optional[ScrapingResult] = None
    ...
    def calculate_next_run_time(self, from_time: Optional[datetime] = None) -> datetime:
    ...
    def update_status(self, new_status: JobStatus, message: Optional[str] = None):
...
class CassazioneScheduler:
    ...
    async def execute_scraping_job(self, job_id: str) -> Optional[ScrapingResult]:
        ...
        try:
            ...
            async with scraper:
                result = await scraper.scrape_recent_decisions(
                    sections=job.court_sections,
                    days_back=job.days_back,
                    limit=job.max_decisions_per_run
                )
            ...
            job.update_status(
                JobStatus.COMPLETED, 
                f"Successfully processed {result.decisions_saved} decisions"
            )
            ...
            logger.info(
                f"Completed scraping job {job_id}: {result.decisions_found} found, "
                f"{result.decisions_saved} saved"
            )
            ...
            if job.retry_on_failure and job.retry_count <= job.max_retries:
                job.update_status(
                    JobStatus.SCHEDULED, 
                    f"Retry {job.retry_count}/{job.max_retries}: {error_message}"
                )
    ...
    def get_job_status(self, job_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            } if job.last_run_result else None,
