...
class SemanticFAQMatcher:
    ...
    async def _get_faq_details(self, faq_id: str) -> Optional[Dict]:
    ...
    def _extract_matched_concepts(
        ...
        tax_concepts = [
            'iva', 'irpef', 'partita iva', 'fattura', 'dichiarazione',
            'reddito', 'detrazione', 'deduzione', 'contributi',
            'regime forfettario', 'f24', 'modello 730', 'codice fiscale',
            'aliquota', 'imponibile', 'ritenuta', 'versamento'
        ]
        ...
        for concept in tax_concepts:
            if (concept in query_lower and 
                (concept in question_lower or concept in answer_lower)):
    ...
    def _apply_boosting_factors(
        ...
        professional_terms = [
            'dichiarazione', 'versamento', 'adempimento', 'normativa',
            'regolamento', 'circolare', 'risoluzione'
        ]
    ...
    async def get_matching_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "hit_rate_achievement": round(current_hit_rate / self.target_hit_rate, 3) if self.target_hit_rate > 0 else 0,
