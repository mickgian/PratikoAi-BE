...
class UserDataDeletor:
                ...
                try:
                    ...
                    if table_name == "users":
                        ...
                        result = await self.db.execute(
                            text(f"SELECT * FROM {table_name} WHERE {user_column} = :user_id"),
                            {"user_id": user_id}
                        )
                    else:
    ...
    async def delete_user_data(
        ...
        try:
            ...
            result = UserDataDeletionResult(
                ...
                audit_records_preserved=db_result.details.get("audit_records_preserved", 0) if db_result.details else 0,
    ...
    async def _delete_from_database(
        self,
        user_id: int,
        user_data_map: Dict[str, List[Dict[str, Any]]],
    ...
    async def _anonymize_audit_records(self, table_name: str, user_id: int):
        ...
        try:
            ...
            anonymized_id = f"deleted_user_{user_id}_{datetime.now(timezone.utc).strftime('%Y%m%d')}"
