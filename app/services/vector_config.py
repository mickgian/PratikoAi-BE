...
class VectorConfig:
    ...
    def get_provider_preference(self) -> str:
        ...
        if self.vec_provider_override in ["local", "pinecone"]:
            logger.info(
                "vector_provider_override", provider=self.vec_provider_override, environment=self.environment.value
            )
    ...
    def generate_index_name(self, dimension: int | None = None) -> str:
    ...
    def build_namespace(self, env: str, domain: str, tenant: str = "default") -> str:
        ...
        components = [f"env={env}", f"domain={domain}", f"tenant={tenant}"]
    ...
    def allow_fallback(self) -> bool:
        ...
        return not (self.environment in [Environment.PRODUCTION, Environment.PREPROD] and self.strict_mode)
