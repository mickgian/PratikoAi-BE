...
class LocalVectorProvider:
    ...
    def __init__(
        self, embedding_dimension: int = 384, embedding_model: str = "sentence-transformers/all-MiniLM-L6-v2"
    ):
        ...
        logger.info(
            "local_vector_provider_initialized",
    ...
    def upsert(self, vectors: list[dict[str, Any]]) -> bool:
        ...
        try:
            for item in vectors:
                ...
                self.metadata[doc_id] = {**metadata, "upserted_at": datetime.utcnow().isoformat(), "provider": "local"}
    ...
    def query(self, vector: list[float], top_k: int = 10, namespace: str = None) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for doc_id, stored_vector in self.vectors.items():
                ...
                similarities.append({"id": doc_id, "score": similarity, "metadata": self.metadata.get(doc_id, {})})
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            test_doc = {"id": "test_connection", "vector": test_vector, "metadata": {"test": True}}
    ...
    def load_from_disk(self, filepath: str) -> bool:
        ...
        try:
            ...
            if saved_dimension and saved_dimension != self.embedding_dimension:
                logger.warning("local_dimension_mismatch", current=self.embedding_dimension, saved=saved_dimension)

            logger.info(
                "local_vectors_loaded", filepath=filepath, count=len(self.vectors), saved_at=config.get("saved_at")
            )
            return True
