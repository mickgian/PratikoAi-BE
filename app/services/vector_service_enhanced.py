...
class EnhancedVectorService:
    ...
    def _initialize_provider(self):
        ...
        try:
            ...
            logger.info(
                "enhanced_vector_service_provider_initialized",
                provider=type(self.provider).__name__,
                environment=self.environment.value,
            )

        except Exception as e:
            logger.error("enhanced_vector_service_provider_failed", error=str(e), environment=self.environment.value)
    ...
    def _initialize_embedding_model(self):
        ...
        try:
            ...
            logger.info("enhanced_embedding_model_initialized", model=self.config.embedding_model)
        except Exception as e:
            logger.error("enhanced_embedding_model_failed", error=str(e), model=self.config.embedding_model)
    ...
    def _perform_startup_checks(self):
        ...
        try:
            ...
            # Log startup status
            logger.info(
                "enhanced_vector_service_startup_complete",
            ...
            # Log configuration
            logger.info("enhanced_vector_service_configuration", **self.config.to_safe_dict())
    ...
    def create_embedding(self, text: str) -> list[float] | None:
        ...
        if self.embedding_model:
            try:
                ...
                logger.debug("enhanced_embedding_created", text_length=len(text), dimension=len(embedding))
                return embedding
            except Exception as e:
    ...
    def store_document(self, document_id: str, text: str, metadata: dict[str, Any]) -> bool:
        ...
        if self.provider:
            try:
                ...
                vector_data = [{"id": document_id, "vector": embedding, "metadata": enhanced_metadata}]
                ...
                logger.error("enhanced_store_document_failed", error=str(e), document_id=document_id)
    ...
    def search_similar(
        self, query_text: str, top_k: int = 10, domain: str = None, filters: dict[str, Any] = None
    ) -> list[dict[str, Any]]:
        ...
        if self.provider:
            try:
                ...
                results = self.provider.query(
                    vector=query_embedding, top_k=top_k, namespace=namespace, filter_dict=filters
                )
    ...
    def store_ccnl_content(self, doc_id: str, content: str, metadata: dict[str, Any]) -> bool:
        ...
        enhanced_metadata = {**metadata, "domain": "ccnl", "language": "italian", "document_type": "ccnl"}
        return self.store_document(doc_id, content, enhanced_metadata)

    def search_italian_regulations(self, query: str, top_k: int = 10) -> list[dict[str, Any]]:
        """Search Italian regulations specifically."""
        return self.search_similar(
            query_text=query, top_k=top_k, domain="fiscale", filters={"document_type": "regulation"}
        )

    def search_ccnl_content(self, query: str, top_k: int = 10) -> list[dict[str, Any]]:
        """Search CCNL content specifically."""
        return self.search_similar(query_text=query, top_k=top_k, domain="ccnl", filters={"document_type": "ccnl"})
