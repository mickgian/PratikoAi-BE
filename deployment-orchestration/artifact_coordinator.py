        ...
        try:
            ...
            for obj in response.get("Contents", []):
                ...
                if len(key_parts) >= 6:
                    ...
                    # Get object metadata
                    obj_response = self.s3_client.head_object(Bucket=self.bucket_name, Key=obj["Key"])
...
class CrossRepoArtifactCoordinator:
    ...
    def __init__(self, storage_backend: StorageBackend = StorageBackend.LOCAL, storage_config: dict[str, Any] = None):
    ...
    async def create_deployment_manifest(
        self, deployment_id: str, environment: str, services: dict[str, str]
    ) -> DeploymentManifest:
    ...
    async def list_artifacts(
        self, deployment_id: str, artifact_type: ArtifactType = None, service: str = None
    ...
    async def share_artifact_cross_repo(
        self, deployment_id: str, artifact_id: str, target_repo: str, target_workflow: str
    ) -> dict[str, Any]:
        ...
        sharing_payload = {
            ...
            "artifact_content": content.decode("utf-8")
            if metadata.artifact_type
            in [ArtifactType.BUILD_INFO, ArtifactType.DEPLOYMENT_INFO, ArtifactType.VERSION_MANIFEST]
        ...
        await self.store_artifact(
            ...
            sharing_payload,
            shared_artifact_id,
            {"shared_with": target_repo, "target_workflow": target_workflow, "original_artifact": artifact_id},
        )
    ...
    async def receive_shared_artifact(self, shared_artifact_data: dict[str, Any]) -> ArtifactMetadata:
        ...
        metadata.metadata["received_at"] = datetime.now(UTC).isoformat()

        # Decode content
        if metadata.artifact_type in [
    ...
    async def cleanup_artifacts(self, deployment_id: str, older_than_days: int = 30) -> int:
        ...
        manifest = self.manifests.get(deployment_id)
        if manifest:
            manifest.artifacts = [a for a in manifest.artifacts if a.created_at >= cutoff_date]
            await self._store_manifest(manifest)
    ...
    async def _load_manifest(self, deployment_id: str) -> DeploymentManifest | None:
        ...
        try:
            ...
            for artifact in artifacts:
                if (
                    artifact.artifact_type == ArtifactType.DEPLOYMENT_INFO
                    and artifact.service == "coordinator"
                    and artifact.artifact_id.startswith("manifest_")
                ):
...
async def main():
    ...
    coordinator = CrossRepoArtifactCoordinator(
        storage_backend=StorageBackend.LOCAL, storage_config={"base_path": "/tmp/pratiko-artifacts-test"}
    )
    ...
    manifest = await coordinator.create_deployment_manifest(
        deployment_id=deployment_id, environment="staging", services={"backend": "1.2.0", "frontend-android": "2.1.0"}
    )
