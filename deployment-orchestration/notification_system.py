...
class NotificationFormatter:
    ...
    def format_for_slack(self, notification: NotificationMessage) -> dict[str, Any]:
        ...
        if notification.context.error_message:
            fields.append(
                {
                    "title": "Error",
                    "value": notification.context.error_message[:500] + "..."
                    if len(notification.context.error_message) > 500
                    else notification.context.error_message,
                    "short": False,
                }
            )
        ...
        if notification.context.started_at:
            end_time = notification.context.completed_at or datetime.now(UTC)
        ...
        return {"attachments": [attachment], "username": "PratikoAI Deploy Bot", "icon_emoji": ":rocket:"}
    ...
    def format_for_email(self, notification: NotificationMessage, html: bool = True) -> dict[str, str]:
        ...
        variables = {
            ...
            "environment_emoji": {"production": "🟢", "staging": "🟡", "development": "🔵"},
        }

        subject = f"[{notification.context.environment.upper()}] {notification.title}"
        body = template.render(**variables)

        return {"subject": subject, "body": body, "content_type": "text/html" if html else "text/plain"}
    ...
    def format_for_webhook(self, notification: NotificationMessage) -> dict[str, Any]:
        ...
        return {
            ...
            "context": {
                ...
                "completed_at": notification.context.completed_at.isoformat()
                if notification.context.completed_at
                else None,
...
class NotificationDispatcher:
    ...
    async def send_email_notification(
        self, smtp_config: dict[str, str], recipients: list[str], notification: NotificationMessage
    ) -> bool:
    ...
    async def send_webhook_notification(
        self, webhook_url: str, notification: NotificationMessage, headers: dict[str, str] = None
    ) -> bool:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                response = await client.post(webhook_url, json=payload, headers=request_headers, timeout=30)
    ...
    async def create_github_issue(
        self, repo_name: str, notification: NotificationMessage, assignees: list[str] = None
    ) -> bool:
...
class NotificationManager:
    ...
    def setup_default_rules(self):
        ...
        escalation_team = [
            NotificationRecipient(
                "Engineering Manager", NotificationChannel.EMAIL, "engineering-manager@pratiko.ai", escalation_level=2
            ),
        ...
        self.rules.append(
            NotificationRule(
                event_types=[NotificationType.DEPLOYMENT_FAILED, NotificationType.HEALTH_CHECK_FAILED],
    ...
    async def send_notification(
        ...
        message = self._create_notification_message(notification_type, context, severity, custom_message)
        ...
        for rule in matching_rules:
            ...
            for recipient in rule.recipients:
                if recipient.channel in rule.channels:
                    ...
                    self.last_notification_times[rule_key] = datetime.now(UTC)
    ...
    def _generate_message_content(
        self, notification_type: NotificationType, context: DeploymentContext, custom_message: str = None
    ) -> tuple[str, str]:
        ...
        if notification_type == NotificationType.DEPLOYMENT_STARTED:
            ...
            body = (
                custom_message
                or f"Cross-repository deployment started for {service_text} in {context.environment} environment."
            )
            ...
            body = (
                custom_message
                or f"CRITICAL: Rollback failed for {service_text} in {context.environment}. Manual intervention required."
            )
            ...
            body = (
                custom_message
                or f"Compatibility issues detected for {service_text} deployment to {context.environment}."
            )
            ...
            body = (
                custom_message
                or f"URGENT: Manual intervention required for {context.environment} deployment. Automated recovery failed."
            )
    ...
    def _generate_actions(
        self, notification_type: NotificationType, context: DeploymentContext
    ) -> list[dict[str, str]]:
        ...
        for service, url in context.workflow_urls.items():
            actions.append({"text": f"View {service.title()} Workflow", "url": url, "style": "default"})

        # Include health check links
        for service, url in context.health_urls.items():
            actions.append({"text": f"Check {service.title()} Health", "url": url, "style": "default"})
    ...
    def _find_matching_rules(
        self, notification_type: NotificationType, severity: Severity, context: DeploymentContext
    ...
    def _is_on_cooldown(self, rule_key: str, cooldown_minutes: int) -> bool:
        ...
        time_since_last = datetime.now(UTC) - last_notification
        return time_since_last.total_seconds() < (cooldown_minutes * 60)

    async def _send_to_recipient(
        self, recipient: NotificationRecipient, message: NotificationMessage, context: DeploymentContext
    ) -> bool:
        ...
        try:
            if recipient.channel == NotificationChannel.SLACK:
                ...
                return await self.dispatcher.send_email_notification(smtp_config, [recipient.address], message)

            elif recipient.channel == NotificationChannel.WEBHOOK:
                headers = recipient.preferences.get("headers", {})
                return await self.dispatcher.send_webhook_notification(recipient.address, message, headers)
                ...
                return await self.dispatcher.create_github_issue(repo_name, message, assignees)
...
async def main():
    ...
    context = DeploymentContext(
        ...
        health_urls={"backend": "https://api.pratiko.ai/health", "frontend-web": "https://pratiko.ai/health"},
    )
