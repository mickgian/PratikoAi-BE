...
class CrossRepoOrchestrator:
    ...
    def __init__(self, github_token: str, webhook_url: str | None = None):
        self.github = Github(github_token)
        self.webhook_url = webhook_url
        self.executions: dict[str, DeploymentExecution] = {}

        # Repository configuration
        self.repos = {"backend": "mickgian/PratikoAi-BE", "frontend": "mickgian/PratikoAi-KMP"}
    ...
    async def create_deployment_plan(
        ...
        notifications = {
            "slack": {"webhook_url": os.getenv("SLACK_WEBHOOK_URL"), "channel": "#deployments"},
            "email": {"enabled": True, "recipients": ["devops@pratiko.ai", "engineering@pratiko.ai"]},
            "github": {"create_issue_on_failure": True, "repository": self.repos["backend"]},
        }

        plan = DeploymentPlan(
            plan_id=plan_id, environment=environment, steps=steps, created_by=created_by, notifications=notifications
        )
    ...
    async def execute_deployment_plan(self, plan: DeploymentPlan) -> DeploymentExecution:
        ...
        execution = DeploymentExecution(
            plan_id=plan.plan_id, status=DeploymentStatus.RUNNING, started_at=datetime.now(UTC)
        )
        ...
        while remaining_steps:
            ...
            for step in remaining_steps:
                dependencies_met = all(
                    any(completed.service.value == dep for completed in ordered_steps) for dep in step.depends_on
                )
                if dependencies_met:
    ...
    async def _check_dependencies_completed(self, step: DeploymentStep, execution: DeploymentExecution) -> bool:
    ...
    async def _execute_step(self, step: DeploymentStep, plan: DeploymentPlan, execution: DeploymentExecution) -> bool:
    ...
    async def _trigger_workflow(self, step: DeploymentStep, plan: DeploymentPlan) -> str | None:
        ...
        try:
            ...
            if success:
                ...
                if latest_run:
                    logger.info(
                        f"Triggered workflow {step.workflow_name} for {step.service.value}, run ID: {latest_run.id}"
                    )
            ...
            async with httpx.AsyncClient(timeout=30) as client:
                for attempt in range(3):  # Retry up to 3 times
                    try:
                        ...
                        logger.warning(
                            f"Health check attempt {attempt + 1} failed for {step.service.value}: {response.status_code}"
                        )
    ...
    async def _update_version_registry(self, step: DeploymentStep, plan: DeploymentPlan) -> None:
        ...
        try:
            ...
            response = self.session.post(f"{self.version_registry_url}/api/v1/deployments", json=payload, timeout=30)
    ...
    async def _send_notification(self, event_type: str, plan: DeploymentPlan, execution: DeploymentExecution) -> None:
        ...
        try:
            ...
            if event_type in ["deployment_failed", "rollback_failed"] and plan.notifications.get("github", {}).get(
    ...
    async def _send_slack_notification(self, event_type: str, data: dict[str, Any], plan: DeploymentPlan) -> None:
        """Send Slack notification."""
        ...
        if data["error"]:
            message["attachments"][0]["fields"].append({"title": "Error", "value": data["error"], "short": False})

        async with httpx.AsyncClient() as client:
            await client.post(webhook_url, json=message)

    async def _send_email_notification(self, event_type: str, data: dict[str, Any], plan: DeploymentPlan) -> None:
...
Labels: deployment-failure, orchestrator, {data["environment"]}
            ...
            repo.create_issue(
                title=title, body=body.strip(), labels=["deployment-failure", "orchestrator", data["environment"]]
            )
    ...
    def _calculate_duration(self, execution: DeploymentExecution) -> str:
        ...
        end_time = execution.completed_at or datetime.now(UTC)
    ...
    async def get_deployment_status(self, plan_id: str) -> DeploymentExecution | None:
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        plan = await orchestrator.create_deployment_plan(
            Environment(args.environment), args.backend_version, frontend_versions, args.created_by
        )
