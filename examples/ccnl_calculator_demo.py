...
from app.services.ccnl_calculator_engine import CalculationPeriod, EnhancedCCNLCalculator
...
def create_metalmeccanici_ccnl() -> CCNLAgreement:
    ...
    ccnl.job_levels = [
        ...
        JobLevel(
            ...
            description="Operaio con qualifica specifica",
        ),
        JobLevel(
            "B1",
            ...
            description="Operaio altamente specializzato",
        ),
        JobLevel(
            "5",
            "Impiegato",
        ...
        JobLevel(
            ...
            description="Impiegato con responsabilità",
        ),
        JobLevel(
            "Q", "Quadro", WorkerCategory.QUADRO, minimum_experience_months=60, description="Quadro dirigenziale"
        ),
    ]
...
def demo_basic_calculations():
    ...
    monthly_comp = calculator.calculate_comprehensive_compensation(level_code="C2", period=CalculationPeriod.MONTHLY)
    annual_comp = calculator.calculate_comprehensive_compensation(level_code="C2", period=CalculationPeriod.ANNUAL)
    ...
    for category in categories:
        ...
        for months in seniority_examples:
            ...
            benefits = calculator.calculate_seniority_benefits(
                worker_category=category, hire_date=date.today() - timedelta(days=365 * years)
            )
...
def demo_geographic_differences():
    ...
    for area, diff_data in differences.items():
        print(
            f"{area.replace('_', ' ').title()}: {format_currency(diff_data['monthly_salary'])} "
...
def demo_career_progression():
    ...
    result = calculator.calculate_career_progression(
        starting_level="C1", progression_path=progression_path, starting_date=date(2004, 1, 1)
    )
...
def demo_complex_query():
    ...
    print("   This C2 metalworker in Northern Italy with 5 years experience would receive:")
