...
from app.services.ccnl_search_service import ccnl_search_service, SearchFilters
...
async def example_search_by_specific_criteria():
    ...
    geo_response = await ccnl_search_service.search_by_geographic_area(
        areas=[GeographicArea.SUD, GeographicArea.SUD_ISOLE],
        regions=["campania", "sicilia", "puglia"]
    )
    ...
    salary_response = await ccnl_search_service.search_by_salary_range(
        min_salary=Decimal('2500'),
        max_salary=Decimal('4000'),
    ...
    category_response = await ccnl_search_service.search_by_job_category(
        categories=[WorkerCategory.QUADRO],
        experience_years=5
    )
...
async def example_query_understanding():
    ...
    test_queries = [
        ...
        {
            "query": "Compare vacation days between commerce and manufacturing sectors",
            "expected": "Comparison query"
        }
    ]
