...
class FailureContext:
    ...
    deployment_phase: str | None = None  # build, test, deploy, post_deploy
    error_rate: float | None = None
    response_time_degradation: float | None = None
...
class CategorizedFailure:
    ...
    estimated_recovery_time: int | None = None  # minutes
...
class FailureCategorizer:
    ...
    def _load_configuration(self) -> dict[str, Any]:
        ...
        default_config = {
            ...
            "environment_risk_multipliers": {"production": 2.0, "staging": 1.2, "development": 0.8},
        }
        ...
        patterns = {
            FailureType.INFRASTRUCTURE: [
                FailureSignature(
                    ...
                    log_patterns=[r"container failed to start", r"pod scheduling failed", r"node not ready"],
                    metric_thresholds={
            ...
            FailureType.RESOURCE: [
                FailureSignature(
                    ...
                    log_patterns=[r"oom killed", r"no space left on device", r"resource quota exceeded"],
                    metric_thresholds={
            ...
            FailureType.APPLICATION: [
                FailureSignature(
                    ...
                    log_patterns=[r"traceback", r"stack trace", r"fatal error", r"segmentation fault"],
                    metric_thresholds={"error_rate": (">", 5.0), "response_time_p99": (">", 10000)},
            ...
            FailureType.DATA: [
                FailureSignature(
                    ...
                    log_patterns=[r"database error", r"sql error", r"migration rollback", r"constraint failed"],
                    metric_thresholds={"db_connection_errors": (">", 1.0), "db_response_time": (">", 5000)},
            ...
            FailureType.SECURITY: [
                FailureSignature(
                    ...
                    log_patterns=[r"auth failure", r"invalid credentials", r"access denied", r"security alert"],
                    metric_thresholds={"failed_auth_rate": (">", 10.0)},
            ...
            FailureType.NETWORK: [
                FailureSignature(
                    ...
                    log_patterns=[r"network error", r"timeout occurred", r"connection lost", r"dns lookup failed"],
                    metric_thresholds={"network_latency": (">", 1000), "packet_loss": (">", 1.0)},
            ...
            FailureType.TIMING: [
                FailureSignature(
                    ...
                    log_patterns=[r"timeout exceeded", r"deadlock detected", r"race condition", r"lock contention"],
                    metric_thresholds={"response_time_p95": (">", 30000), "lock_wait_time": (">", 10000)},
    ...
    def categorize_failure(
        ...
        failure_type, type_confidence = self._identify_failure_type(error_messages, log_entries, metrics, status_codes)
        ...
        affected_components = self._analyze_component_impact(failure_type, error_messages, log_entries, context)

        # Step 4: Recovery complexity determination
        recovery_complexity = self._determine_recovery_complexity(failure_type, severity, affected_components, context)

        # Step 5: Generate failure signature for future reference
        signature = self._generate_failure_signature(error_messages, log_entries, metrics, status_codes)

        # Step 6: Create reasoning explanation
        reasoning = self._generate_categorization_reasoning(
            failure_type, severity, affected_components, recovery_complexity, type_confidence, severity_confidence
        )
        ...
        estimated_recovery_time = self._estimate_recovery_time(failure_type, severity, recovery_complexity, context)

        # Step 9: Assess risks
        requires_rollback = self._assess_rollback_requirement(failure_type, severity, context)

        data_integrity_risk = self._assess_data_integrity_risk(failure_type, affected_components, error_messages)
        ...
        self.logger.info(
            f"Categorized failure {failure_id}: {failure_type.value} "
    ...
    def _identify_failure_type(
        self, error_messages: list[str], log_entries: list[str], metrics: dict[str, float], status_codes: list[int]
    ) -> tuple[FailureType, float]:
        """Identify the primary failure type using pattern matching."""
        type_scores = dict.fromkeys(FailureType, 0.0)
                ...
                for metric_name, (operator, threshold) in signature.metric_thresholds.items():
                    if metric_name in metrics:
                        ...
                        if (
                            ...
                            or (operator == "=" and abs(metric_value - threshold) < 0.1)
                        ):
                            score += 1.0
    ...
    def _heuristic_type_identification(
        self, error_messages: list[str], log_entries: list[str], metrics: dict[str, float]
    ) -> tuple[FailureType, float]:
    ...
    def _analyze_component_impact(
        self, failure_type: FailureType, error_messages: list[str], log_entries: list[str], context: FailureContext
    ) -> set[ComponentType]:
    ...
    def _determine_recovery_complexity(
        self,
        failure_type: FailureType,
        severity: FailureSeverity,
        affected_components: set[ComponentType],
    ...
    def _generate_categorization_reasoning(
        self,
        failure_type: FailureType,
        severity: FailureSeverity,
        affected_components: set[ComponentType],
    ...
    def _assess_rollback_requirement(
        self, failure_type: FailureType, severity: FailureSeverity, context: FailureContext
    ) -> bool:
    ...
    def _assess_data_integrity_risk(
        self, failure_type: FailureType, affected_components: set[ComponentType], error_messages: list[str]
    ) -> bool:
    ...
    def _update_statistics(self, categorized_failure: CategorizedFailure):
        ...
        self.categorization_stats["average_confidence"] = (current_avg * (total - 1) + new_confidence) / total
