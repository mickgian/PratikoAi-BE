...
class EvaluationContext:
    ...
    user_attributes: Dict[str, Union[str, int, float, bool]] = field(default_factory=dict)
...
class FeatureFlagClient:
    ...
    async def _get_cached_evaluation(self, flag_id: str, context: EvaluationContext) -> Optional[FlagEvaluation]:
    ...
    async def _invalidate_flag_cache(self, flag_id: str):
        ...
        with self.cache_lock:
            keys_to_remove = [
                ...
                if key.startswith(f"flag:{self.environment}:{flag_id}:")
            ]
        ...
        if asyncio.iscoroutinefunction(func):
            ...
            def sync_wrapper(*args, **kwargs):
                ...
                enabled = loop.run_until_complete(
                    client.is_enabled(flag_id, user_id, user_attributes, default)
                )
...
async def example_usage():
    ...
    client = FeatureFlagClient(
        ...
        fallback_config={
            "new_dashboard": {"value": False},
            "api_rate_limit": {"value": 100}
        }
    )
