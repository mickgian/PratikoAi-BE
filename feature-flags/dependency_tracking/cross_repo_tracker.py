...
class CrossRepositoryDependencyTracker:
    ...
    def __init__(
        ...
        self.http_client = httpx.AsyncClient(
            timeout=30, headers={"Authorization": f"Bearer {feature_flag_api_key}", "Content-Type": "application/json"}
        )
    ...
    async def load_dependencies_from_repositories(self):
        ...
        for repo_name, repo_info in self.repositories.items():
            try:
                ...
                repo_info.last_sync = datetime.now(UTC)
        ...
        for trans_dep in transitive_deps:
            if trans_dep != node_id:
                transitive_dependencies.append(
                    {"flag": trans_dep.split(":", 1)[1], "repository": trans_dep.split(":", 1)[0]}
                )
    ...
    async def create_coordinated_rollout(
        ...
        for dep in impact_analysis["dependent_flags"]:
            dependent_flags.append(
                {"flag_id": dep["flag"], "repository": dep["repository"], "dependency_type": dep["dependency_type"]}
            )
        ...
        for env in environments:
            for percentage in rollout_percentages:
                step = {
                    ...
                    "flags": [{"flag_id": primary_flag, "repository": primary_repository}] + dependent_flags,
        ...
        try:
            for step in rollout.rollout_steps:
                ...
                for flag_config in step["flags"]:
                    try:
                        result = await self._update_flag_rollout(
                            flag_config["repository"], flag_config["flag_id"], step["environment"], step["percentage"]
                        )
                        step_results.append(
                            {
                ...
                execution_results.append(
                    {"step_id": step["step_id"], "results": step_results, "executed_at": datetime.now(UTC).isoformat()}
                )
    ...
    async def _update_flag_rollout(self, repository: str, flag_id: str, environment: str, percentage: int):
        """Update flag rollout percentage via API."""
        config = {"flag_id": flag_id, "environment": environment, "rollout_percentage": percentage, "enabled": True}

        response = await self.http_client.put(
            f"{self.api_url}/api/v1/flags/{flag_id}/environments/{environment}", json=config
        )
    ...
    def _analyze_dependency_patterns(self) -> dict[str, Any]:
        ...
        patterns = {"dependency_types": {}, "repository_connections": {}, "complexity_metrics": {}}
    ...
    def _calculate_max_chain_length(self) -> int:
        ...
        try:
            return max(
                len(nx.shortest_path(self.dependency_graph, source, target)) - 1
    ...
    def _generate_recommendations(self, flag_analyses: dict[str, Any]) -> list[str]:
        ...
        orphaned = [node for node in self.dependency_graph.nodes() if self.dependency_graph.degree(node) == 0]
        if orphaned:
            recommendations.append(
    ...
    async def visualize_dependencies(
        self, output_file: str = "dependency_graph.png", repository_filter: list[str] = None
    ):
        ...
        repositories = {node.split(":", 1)[0] for node in graph.nodes()}
        ...
        # Add legend
        legend_elements = [
            plt.Line2D([0], [0], marker="o", color="w", markerfacecolor=repo_colors[repo], markersize=10, label=repo)
            for repo in repositories
        ]
...
def cli(ctx, github_token, api_url, api_key):
    ...
    ctx.obj["tracker"] = CrossRepositoryDependencyTracker(
        github_token=github_token, feature_flag_api_url=api_url, feature_flag_api_key=api_key
    )
