...
def create_warning_box(title, body, styles):
    ...
    data = [[Paragraph(f"<b>! {title}</b>", styles["WarningTitle"])], [Paragraph(body, styles["WarningBody"])]]
...
- Includi riferimenti a leggi, decreti, articoli specifici"""
    ...
    story.append(
        Paragraph(
            "Instructs the LLM to output structured XML tags for proactive suggestions:", styles["BodyTextCustom"]
        )
    )
...
[
  ...
  ...
    ...
    story.append(
        Paragraph("Location: app/core/prompts/synthesis_critical.py (Lines 68-144)", styles["BodyTextCustom"])
    )
    story.append(
        Paragraph("Used for technical_research routes. Defines 4 analysis tasks (Compiti):", styles["BodyTextCustom"])
    )
    story.append(
        Paragraph(
            "<bullet>&bull;</bullet> COMPITO 1: ANALISI CRONOLOGICA - Order documents by date, identify evolution",
            styles["BulletText"],
        )
    )
    story.append(
        Paragraph(
            "<bullet>&bull;</bullet> COMPITO 2: RILEVAMENTO CONFLITTI - Detect contradictions between sources",
            styles["BulletText"],
        )
    )
    story.append(
        Paragraph(
            "<bullet>&bull;</bullet> COMPITO 3: APPLICAZIONE GERARCHIA - Apply legal hierarchy (Legge > Decreto > Circolare)",
            styles["BulletText"],
        )
    )
    story.append(
        Paragraph(
            "<bullet>&bull;</bullet> COMPITO 4: VERDETTO OPERATIVO - Structured output with action/risk/deadline",
            styles["BulletText"],
        )
    )
...
RESPONSE: {response, suggested_actions, metadata}"""
    ...
    story.append(
        Paragraph("Location: app/core/langgraph/nodes/step_100__post_proactivity.py", styles["BodyTextCustom"])
    )
...
# Missing: Document context from earlier in conversation"""
    ...
    story.append(
        Paragraph(
            "<bullet>&bull;</bullet> system.md: Focuses on formatting/citations, not reasoning process",
            styles["BulletText"],
        )
    )
    story.append(
        Paragraph(
            "<bullet>&bull;</bullet> synthesis_critical.py: Has '4 Compiti' structure but no step-by-step instructions",
            styles["BulletText"],
        )
    )
    story.append(
        Paragraph(
            "<bullet>&bull;</bullet> No 'Let's think through this systematically' type prompts", styles["BulletText"]
        )
    )

    story.append(Paragraph("4.2 Document Priority Confusion", styles["SectionTitle"]))
    story.append(
        Paragraph("Two competing document analysis systems may send mixed signals:", styles["BodyTextCustom"])
    )
    story.append(
        Paragraph(
            "<bullet>&bull;</bullet> document_analysis.md - Comprehensive analysis framework", styles["BulletText"]
        )
    )
    story.append(
        Paragraph(
            "<bullet>&bull;</bullet> document_analysis_override.md - Forces focus ONLY on user docs",
            styles["BulletText"],
        )
    )
    story.append(
    ...
    story.append(
        Paragraph("<bullet>&bull;</bullet> Suggested actions instructions - ~500 tokens", styles["BulletText"])
    )
    story.append(
        create_warning_box(
            "GAP",
  ...
  +-- verdetto_parser.py              # Verdetto parsing"""
    ...
    story.append(
        Paragraph(
            "<bullet>&bull;</bullet> Create single ActionGenerationPrompt used by ALL routes", styles["BulletText"]
        )
    )
    story.append(
        Paragraph("<bullet>&bull;</bullet> Append to both synthesis and standard prompts", styles["BulletText"])
    )
    story.append(
        Paragraph("<bullet>&bull;</bullet> Parse same XML structure regardless of route", styles["BulletText"])
    )

    story.append(Paragraph("Fix 2: Add CoT Framework", styles["SubSectionTitle"]))
    story.append(
        Paragraph(
            "<bullet>&bull;</bullet> Create reasoning_framework.md with explicit step-by-step reasoning",
            styles["BulletText"],
        )
    )
    story.append(
        Paragraph("<bullet>&bull;</bullet> Inject before final answer generation in both routes", styles["BulletText"])
    )
    story.append(
        Paragraph("<bullet>&bull;</bullet> Include 'Ragiona passo dopo passo' instructions", styles["BulletText"])
    )

    story.append(Paragraph("Fix 3: Pass Context to Post-Proactivity", styles["SubSectionTitle"]))
    story.append(
        Paragraph("<bullet>&bull;</bullet> Modify Step 100 to receive state['context'] summary", styles["BulletText"])
    )
    story.append(Paragraph("<bullet>&bull;</bullet> Use context for action relevance scoring", styles["BulletText"]))
    story.append(
        Paragraph("<bullet>&bull;</bullet> Validate suggested actions against source material", styles["BulletText"])
    )

    story.append(Paragraph("Fix 4: HyDE Context Awareness", styles["SubSectionTitle"]))
    story.append(
        Paragraph("<bullet>&bull;</bullet> Pass last N conversation turns to HyDE generator", styles["BulletText"])
    )
    story.append(
        Paragraph("<bullet>&bull;</bullet> Include document filenames mentioned in conversation", styles["BulletText"])
    )
    story.append(
        Paragraph("<bullet>&bull;</bullet> Improve retrieval relevance for follow-up questions", styles["BulletText"])
    )
    ...
    story.append(
        Paragraph(
            "<bullet>&bull;</bullet> Clear prompt separation for different routes (synthesis vs. standard)",
            styles["BulletText"],
        )
    )
    story.append(
        Paragraph("<bullet>&bull;</bullet> Document-aware processing with specialized prompts", styles["BulletText"])
    )
    story.append(
        Paragraph(
            "<bullet>&bull;</bullet> Hierarchical source authority in synthesis prompt (law > decree > circular)",
            styles["BulletText"],
        )
    )
    story.append(
        Paragraph(
            "<bullet>&bull;</bullet> Two-tier proactivity system (pre-question + post-actions)", styles["BulletText"]
        )
    )
    story.append(
        Paragraph(
            "<bullet>&bull;</bullet> PII protection integrated throughout pipeline (DEV-007)", styles["BulletText"]
        )
    )
    ...
    story.append(
        Paragraph(
            "<bullet>&bull;</bullet> Missing explicit Chain-of-Thought guidance despite complex domain",
            styles["BulletText"],
        )
    )
    story.append(
        Paragraph("<bullet>&bull;</bullet> Context fragmentation in suggested action generation", styles["BulletText"])
    )
    story.append(
        Paragraph("<bullet>&bull;</bullet> Limited reasoning transparency in synthesis output", styles["BulletText"])
    )
    story.append(
        Paragraph("<bullet>&bull;</bullet> Dual action extraction paths causing inconsistency", styles["BulletText"])
    )
    story.append(
        Paragraph(
            "<bullet>&bull;</bullet> HyDE lacks conversation memory for follow-up questions", styles["BulletText"]
        )
    )
