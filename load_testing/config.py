...
class LoadTestConfig:
    ...
    italian_tax_types: list[str] = field(default_factory=lambda: ["IVA", "IRPEF", "IMU", "TASI", "TARI"])
    italian_document_types: list[str] = field(
        default_factory=lambda: ["fattura_elettronica", "f24", "dichiarazione_redditi", "contratto"]
    )
    italian_regions: list[str] = field(
        default_factory=lambda: ["Lombardia", "Lazio", "Campania", "Veneto", "Emilia-Romagna"]
    )
...
class LoadTestProfiles:
    ...
    BASELINE = LoadTestConfig(
        ...
        test_duration=60,
        ramp_up_time=0,
        scenarios={"simple_query": 0.4, "complex_query": 0.3, "tax_calculation": 0.2, "knowledge_search": 0.1},
    )
    ...
    TAX_DEADLINE = LoadTestConfig(
        ...
        ramp_up_time=120,
        scenarios={"tax_calculation": 0.4, "document_upload": 0.3, "complex_query": 0.2, "simple_query": 0.1},
    )
...
ITALIAN_ENDPOINTS = {
    "simple_query": EndpointConfig(
        path="/api/query", method="POST", body={"query": "Come calcolare l'IVA?"}, max_response_time=3.0, weight=0.3
    ),
    "complex_query": EndpointConfig(
        path="/api/query",
        method="POST",
        body={"query": "Analizza le implicazioni fiscali di una SRL che diventa SPA", "context": "detailed"},
    ...
    "tax_calculation": EndpointConfig(
        ...
        method="POST",
        body={"type": "IVA", "amount": 10000, "region": "Lombardia"},
        max_response_time=2.0,
        weight=0.2,
    ),
    "document_upload": EndpointConfig(path="/api/document/analyze", method="POST", max_response_time=30.0, weight=0.1),
    "knowledge_search": EndpointConfig(path="/api/knowledge/search", method="GET", max_response_time=3.0, weight=0.1),
    "user_profile": EndpointConfig(path="/api/user/profile", method="GET", max_response_time=1.0, weight=0.05),
}
