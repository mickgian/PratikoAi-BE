...
class LoadTestFramework:
    ...
    def __init__(
        ...
        # Session tracking
        self.current_session: LoadTestSession | None = None
            ...
            for user in users:
                try:
                    async with session.post(
                        f"{self.base_url}/api/auth/register", json=user, timeout=aiohttp.ClientTimeout(total=10)
                    ) as response:
                        if response.status in [200, 201] or response.status == 409:
    ...
    async def warmup_cache(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            login_response = await session.post(
                ...
                json={"email": "loadtest_user_1@pratikoai.it", "password": "TestPassword123!"},
            )
    ...
    async def run_test(
        self, users: int, duration: int, scenario: str = "mixed", ramp_up: int = 60, **kwargs
    ) -> LoadTestMetrics:
        ...
        # Start monitoring
        if self.monitor:
            self.current_session = await self.monitor.start_monitoring(session_id=session_id, config=asdict(config))
    ...
    async def run_spike_test(
        self, initial_users: int, spike_users: int, spike_duration: int, total_duration: int
    ) -> LoadTestMetrics:
    ...
    async def run_phased_test(self, config: dict[str, Any]) -> LoadTestMetrics:
        ...
        # Start monitoring
        if self.monitor:
            self.current_session = await self.monitor.start_monitoring(session_id=session_id, config=config)
    ...
    async def _run_locust_test(self, config: LoadTestConfig, **kwargs) -> LoadTestMetrics:
    ...
    async def _run_k6_test(self, config: LoadTestConfig, **kwargs) -> LoadTestMetrics:
        ...
        k6_config = {"VUS": config.target_users, "DURATION": f"{config.test_duration}s", "BASE_URL": self.base_url}
        ...
        cmd = [
            ...
            "run",
            "--out",
            f"json={self.results_dir}/{self.current_session.session_id}_k6.json",
        ...
        start_time = time.time()
        process = await asyncio.create_subprocess_exec(
            *cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE, env=env, cwd=Path.cwd()
        )
    ...
    async def _parse_locust_results(
        self, config: LoadTestConfig, start_time: float, end_time: float
    ) -> LoadTestMetrics:
    ...
    async def _parse_k6_results(self, config: LoadTestConfig, start_time: float, end_time: float) -> LoadTestMetrics:
    ...
    def _get_scenario_weights(self, scenario: str) -> dict[str, float]:
        ...
        scenarios = {
            ...
            "llm_heavy": {"complex_query": 0.6, "simple_query": 0.3, "knowledge_search": 0.1},
            "database_heavy": {"tax_calculation": 0.5, "user_operations": 0.3, "knowledge_search": 0.2},
            "cache_heavy": {"simple_query": 0.8, "knowledge_search": 0.2},
            "document_heavy": {"document_upload": 0.7, "complex_query": 0.3},
            "cacheable_queries": {"simple_query": 0.9, "user_operations": 0.1},
            "italian_tax": {"tax_calculation": 0.8, "simple_query": 0.2},
        }
    ...
    def generate_scaling_recommendations(self, metrics: LoadTestMetrics) -> list[ScalingRecommendation]:
    ...
    async def establish_baseline(self) -> dict[str, Any]:
        ...
        baseline_metrics = await self.run_test(users=1, duration=60, scenario="mixed")
        ...
        return {
            ...
            "resource_limits": {
                ...
                "memory_baseline": statistics.mean(baseline_metrics.memory_usage)
                if baseline_metrics.memory_usage
                else 0,
            },
        }
