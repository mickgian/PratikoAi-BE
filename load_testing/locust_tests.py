...
from load_testing.config import ITALIAN_ENDPOINTS, LoadTestConfig, LoadTestProfiles, TestDataGenerator
...
class PratikoAIUser(HttpUser):
    ...
    def on_start(self):
        ...
        response = self.client.post(
            "/api/auth/login",
            json={"email": f"loadtest_user_{user_num}@pratikoai.it", "password": "TestPassword123!"},
            catch_response=True,
        )
    ...
    def simple_query(self):
        ...
        with self.client.post(
            "/api/query", json={"query": query}, catch_response=True, name="/api/query [simple]"
        ) as response:
    ...
    def complex_query(self):
        ...
        with self.client.post(
            "/api/query",
            json={"query": query, "context": "detailed", "include_sources": True},
            catch_response=True,
            timeout=30,
    ...
    def document_upload(self):
        ...
        files = {"file": (f"test_{doc_type}.pdf", mock_pdf_content, "application/pdf")}

        data = {"document_type": doc_type, "extract_data": True}
    ...
    def knowledge_search(self):
        ...
        with self.client.get(
            ...
            params={"q": search_query, "limit": 10, "include_context": True},
            catch_response=True,
    ...
    def user_operations(self):
        ...
        if method == "GET":
            with self.client.get(endpoint, catch_response=True, name=endpoint) as response:
                if response.status_code == 200:
            ...
            with self.client.put(endpoint, json=data, catch_response=True, name=endpoint) as response:
