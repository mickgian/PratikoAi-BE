...
class LoadTestMonitor:
    ...
    def __init__(self, db_url: str, redis_url: str, prometheus_url: str | None = None, grafana_url: str | None = None):
        self.db_url = db_url
        ...
        self.session: LoadTestSession | None = None
    ...
    async def start_monitoring(self, session_id: str, config: dict[str, Any], interval: int = 5) -> LoadTestSession:
        """Start monitoring for a load test session"""
        if self.monitoring:
            raise RuntimeError("Monitoring already active")

        self.session = LoadTestSession(session_id=session_id, start_time=datetime.now(UTC), config=config)
    ...
    async def stop_monitoring(self) -> LoadTestSession:
        ...
        self.session.end_time = datetime.now(UTC)
    ...
    async def collect_snapshot(self) -> PerformanceSnapshot:
        ...
        return PerformanceSnapshot(
            system=system_metrics, database=db_metrics, redis=redis_metrics, application=app_metrics
        )
    ...
    async def _get_prometheus_metrics(self) -> ApplicationMetrics | None:
    ...
    def _generate_summary(self) -> dict[str, Any]:
        ...
        response_time_values = [
            s.application.response_time_p95 for s in self.snapshots if s.application.response_time_p95 > 0
        ]
    ...
    def export_grafana_dashboard(self) -> dict[str, Any]:
        ...
        return {
            "dashboard": {
                ...
                "panels": [
                    # CPU panel
                    {"title": "CPU Usage", "type": "graph", "targets": [{"expr": "cpu_percent"}]},
                    # Memory panel
                    {"title": "Memory Usage", "type": "graph", "targets": [{"expr": "memory_percent"}]},
                    # Database connections panel
                    {"title": "Database Connections", "type": "graph", "targets": [{"expr": "db_total_connections"}]},
                    # Response time panel
                    {
