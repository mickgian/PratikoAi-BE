...
class LoadTestRunner:
    ...
    async def run_full_suite(self) -> Dict[str, Any]:
        ...
        logger.info(f"Target: Validate system for 50-100 concurrent users (€25k ARR)")
    ...
    async def _normal_load_phase(self):
        ...
        logger.info(f"{'✅' if passed else '❌'} Normal load: P95={metrics.p95_response_time:.0f}ms, Errors={metrics.error_rate*100:.2f}%")
    ...
    async def _stress_test_phase(self):
        ...
        logger.info(f"{'✅' if passed else '❌'} Stress test: P95={metrics.p95_response_time:.0f}ms, Errors={metrics.error_rate*100:.2f}%")
    ...
    async def _spike_test_phase(self):
        ...
        logger.info(f"{'✅' if passed else '❌'} Spike test: P95={metrics.p95_response_time:.0f}ms, Errors={metrics.error_rate*100:.2f}%")
...
"""
    ...
    def _print_final_summary(self, report: Dict[str, Any]):
        ...
        logger.info(f"📊 Tests: {report['test_suite_summary']['passed_tests']}/{report['test_suite_summary']['total_tests']} passed")
