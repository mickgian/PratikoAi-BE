...
class GrafanaClient:
    ...
    def create_dashboard(self, dashboard_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            response.raise_for_status()
            logger.info(f"Dashboard created/updated successfully")
            return True
        except Exception as e:
...
class DashboardBackupManager:
        ...
        for uid, current in current_by_uid.items():
            if uid not in backup_dashboards:
                changes.append({
                    ...
                    "uid": uid,
                    "title": current["title"],
                    "description": f"New dashboard added"
                })
        ...
        for uid, backup in backup_dashboards.items():
            if uid not in current_by_uid:
                changes.append({
                    ...
                    "uid": uid,
                    "title": backup["title"],
                    "description": f"Dashboard deleted"
                })
    ...
    def _create_git_backup(self) -> Optional[str]:
