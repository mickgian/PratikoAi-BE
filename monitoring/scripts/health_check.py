...
class PratikoAIHealthChecker:
    ...
    def check_service_health(self, service_name: str, url: str, 
                           health_endpoint: str = "/health") -> HealthCheck:
    ...
    def check_prometheus_metrics(self) -> Dict[str, HealthCheck]:
        ...
        checks["prometheus_api"] = self.check_service_health("Prometheus", self.prometheus_url, "/api/v1/status/config")
                ...
                if unhealthy_targets == 0:
                    ...
                    return HealthCheck(
                        ...
                        details={
                            ...
                            "healthy_targets": healthy_targets,
                            "unhealthy_targets": unhealthy_targets,
                            "unhealthy_list": [t.get("labels", {}).get("job", "unknown") 
    ...
    def _check_revenue_threshold(self) -> HealthCheck:
        ...
        try:
            response = requests.get(
                f"{self.prometheus_url}/api/v1/query",
                params={"query": "monthly_revenue_eur"},
                timeout=self.timeout
            )
    ...
    def generate_recommendations(self, all_checks: Dict[str, HealthCheck]) -> List[str]:
        ...
        for issue in critical_issues:
            if "connection" in issue.message.lower():
                ...
                recommendations.append("🔴 CRITICAL: Implement immediate cost controls - user costs exceed safe limits")
        ...
        for issue in warning_issues:
            if "metric" in issue.name and "not available" in issue.message:
                recommendations.append(f"🟡 WARNING: Ensure {issue.name.replace('metric_', '')} metrics are being collected")
