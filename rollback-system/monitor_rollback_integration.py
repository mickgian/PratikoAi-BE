...
class HealthRollbackIntegration:
    ...
    def _setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[logging.FileHandler(self.config.integration_log_file), logging.StreamHandler()],
        )
    ...
    async def start_integrated_monitoring(self):
        ...
        try:
            ...
            monitoring_task = asyncio.create_task(self.health_monitor.start_monitoring(self.config.deployment_id))

            # Start integration supervision
            supervision_task = asyncio.create_task(self._supervise_integration())
    ...
    async def trigger_manual_rollback(
        self, reason: str, services: list[str] = None, preserve_logs: bool = True
    ) -> RollbackExecution:
        ...
        if services:
            targets = [RollbackTarget(service=service, environment=self.config.environment) for service in services]
        else:
            targets = [
        ...
        # Execute rollback
        execution = await self.rollback_orchestrator.initiate_rollback(trigger, targets, "manual_operator")
                ...
                self.rollback_history.append(
                    {
                        ...
                        "success_rate": len(execution.completed_steps) / len(execution.targets)
    ...
    async def _monitor_post_rollback_health(self, execution: RollbackExecution):
        ...
        try:
            monitoring_end = datetime.now(UTC).timestamp() + (self.config.post_rollback_monitoring_minutes * 60)

            consecutive_healthy_checks = 0
            required_healthy_checks = 5  # Need 5 consecutive healthy checks

            while datetime.now(UTC).timestamp() < monitoring_end:
                ...
                if health_report.overall_status == HealthStatus.HEALTHY:
                    ...
                    logger.info(
                        f"Post-rollback health check {consecutive_healthy_checks}/{required_healthy_checks} passed"
                    )
    ...
    async def _verify_post_rollback_health(self):
        ...
        await self.health_monitor.generate_health_report(self.config.deployment_id)
        ...
        for execution_id, execution in self.active_rollbacks.items():
            if execution.status == RollbackStatus.IN_PROGRESS:
                ...
                if execution.started_at:
                    duration_minutes = (datetime.now(UTC) - execution.started_at).total_seconds() / 60
                    if duration_minutes > self.config.verification_timeout_minutes:
                        logger.warning(
                            f"Rollback {execution_id} taking longer than expected ({duration_minutes:.1f} minutes)"
                        )
    ...
    async def _send_rollback_success_notification(self, execution: RollbackExecution, health_report: HealthReport):
        ...
        notification_data = {
            ...
            "rollback_duration": (execution.completed_at - execution.started_at).total_seconds() / 60
    ...
    async def get_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "last_report_time": datetime.now(UTC).isoformat(),
        }
    ...
    async def shutdown(self):
        ...
        if self.active_rollbacks:
            ...
            while self.active_rollbacks and (datetime.now(UTC).timestamp() - start_time) < timeout:
...
async def main():
    ...
    parser.add_argument(
        "--services", nargs="+", default=["backend", "frontend", "database"], help="Services to monitor"
    )
