...
class BackendRollback:
    ...
    async def _k8s_blue_green_rollback(self, target: RollbackTarget) -> bool:
        ...
        try:
            ...
            deployment_name = f"pratiko-backend"
            service_name = f"pratiko-backend-service"
    ...
    async def _s3_web_rollback(self, target: RollbackTarget) -> bool:
        ...
        try:
            ...
            response = self.s3_client.list_objects_v2(
                Bucket=bucket_name,
                Prefix=f"versions/{target.target_version}/"
            )
...
class RollbackOrchestrator:
    ...
    async def initiate_rollback(self, trigger: RollbackTrigger, targets: List[RollbackTarget],
                              initiated_by: str = "system") -> RollbackExecution:
    ...
    async def _execute_rollback(self, execution: RollbackExecution) -> bool:
        ...
        for target in sorted_targets:
            try:
                ...
                logger.info(f"Rolling back {target.service_type.value} from {target.current_version} to {target.target_version}")
                ...
                if success:
                    ...
                    if target.health_check_url:
                        ...
                        if not health_ok:
                            execution.logs.append(f"Health check failed for {target.service_type.value} after rollback")
    ...
    async def _rollback_single_target(self, target: RollbackTarget) -> bool:
        ...
        try:
            if target.service_type == ServiceType.DATABASE:
                if self.db_rollback:
                    return await self.db_rollback.rollback_migrations(
                        target.target_version, 
                        target.preserve_data
                    )
                else:
