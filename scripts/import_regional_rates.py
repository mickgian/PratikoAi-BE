...
from app.models.regional_taxes import (
    Regione, Comune, RegionalTaxRate, ComunalTaxRate,
    DEFAULT_REGIONAL_DATA, DEFAULT_TAX_RATES
)
    ...
    async def _import_comune(self, comune_data: Dict[str, Any]):
        ...
        try:
            ...
            if not comune_data.get("regione"):
                ...
                provincia_to_region = {
                    "RM": "Lazio", "MI": "Lombardia", "NA": "Campania",
                    "TO": "Piemonte", "PA": "Sicilia", "FI": "Toscana"
                }
    ...
    async def _create_comune_from_csv(self, row: pd.Series):
        ...
        try:
            ...
            provincia_to_region = {
                ...
                "GE": "Liguria", "BA": "Puglia", "CT": "Sicilia", "CA": "Sardegna"
            }
    ...
    async def validate_imported_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for city in major_cities:
                city_rates = (await self.session.execute(
            ...
            imu_rates = (await self.session.execute(
                select(ComunalTaxRate).where(ComunalTaxRate.tax_type == "IMU")
            )).scalars().all()
