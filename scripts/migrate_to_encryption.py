...
class EncryptionMigrator:
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.monitoring_service = EncryptionMonitoringService(self.db, self.encryption_service)
        ...
        for table_name, config in ENCRYPTED_FIELDS_CONFIG.items():
            try:
                ...
                if record_count > 100000:
                    plan.add_risk(
                        f"Large table {table_name} with {record_count} records may require extended maintenance window"
                    )
        ...
        try:
            ...
            for table_name, table_info in plan.tables.items():
                ...
                table_result = await self._migrate_table(
                    table_name, table_info["encrypted_fields"], table_info["record_count"]
                )
            ...
            results.update({"end_time": end_time.isoformat(), "duration_hours": duration, "success": True})
            ...
            if self.encryption_service:
                try:
                    key_info = {
                        ...
                        "active_keys": len(
                            [k for k in self.encryption_service.encryption_keys.values() if k.is_active]
                        ),
                    }
    ...
    async def _migrate_table(self, table_name: str, encrypted_fields: list[str], total_records: int) -> dict[str, Any]:
    ...
    async def _check_table_migration_status(self, table_name: str) -> dict[str, Any]:
        ...
        try:
            ...
            encrypted_records = (
                total_records if self.encryption_service and self.encryption_service.current_key_version else 0
            )
            ...
            return {"total_records": 0, "encrypted_records": 0, "encryption_percentage": 0}
