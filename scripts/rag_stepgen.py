            ...
            for pattern, node_type in node_patterns:
                ...
                for match in matches:
                    ...
                    if node_id not in seen:
                        ...
                        self.nodes[node_id] = {"node_id": node_id, "node_label": label, "type": node_type}
...
"""
        ...
        content += (
            "- **By Category**: " + ", ".join(f"{cat}: {c}" for cat, c in sorted(category_counts.items())) + "\n"
        )
...
class RAGStepGenerator:
    ...
    def run(self, write: bool = True, verbose: bool = False) -> dict:
        ...
        with open(self.mermaid_path) as f:
            content = f.read()
...
def main():
    ...
    parser.add_argument("--write", action="store_true", default=False, help="Write files (default is dry-run)")
