...
export default function ScadenzeFiscaliPage() {
  ...
  const categories = [
    { id: 'iva', name: 'IVA', icon: Receipt, color: 'bg-blue-100 text-blue-700' },
    { id: 'imposte', name: 'Imposte Dirette', icon: Building, color: 'bg-green-100 text-green-700' },
    { id: 'contributi', name: 'Contributi', icon: Users, color: 'bg-purple-100 text-purple-700' },
    { id: 'dichiarazioni', name: 'Dichiarazioni', icon: FileText, color: 'bg-orange-100 text-orange-700' },
    { id: 'versamenti', name: 'Versamenti', icon: CreditCard, color: 'bg-pink-100 text-pink-700' }
  ]
  ...
  const mockScadenze: ScadenzaFiscale[] = [
    ...
    },
    {
      ...
      description: 'Versamento ritenute IRPEF su lavoro dipendente e assimilati',
      date: '2025-02-16',
      category: 'imposte',
      priority: 'alta',
      completed: false,
  ...
  const filteredScadenze = mockScadenze.filter(scadenza => {
    const matchesCategory = selectedCategory === 'all' || scadenza.category === selectedCategory
    const matchesSearch = scadenza.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         scadenza.description.toLowerCase().includes(searchTerm.toLowerCase())
    return matchesCategory && matchesSearch
  })

  const upcomingScadenze = filteredScadenze.filter(scadenza => isUpcoming(scadenza.date))
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-[#2A5D67]">Scadenze Fiscali</h1>
                  <p className="text-[#1E293B] text-sm">Gestisci le tue scadenze fiscali e rimani sempre aggiornato</p>
                </div>
              </div>
            ...
            <div className="flex items-center">
              ...
              <div>
                <h3 className="font-semibold text-red-800">
                  {upcomingScadenze.length} scadenz{upcomingScadenze.length === 1 ? 'a' : 'e'} nei prossimi 7 giorni
                </h3>
            ...
            <div className="lg:col-span-1">
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-xl p-6 shadow-sm border border-[#C4BDB4]/20">
                  <h3 className="font-semibold text-[#2A5D67] mb-4">Cerca Scadenze</h3>
                  <div className="relative">
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-sm opacity-70">{mockScadenze.length}</span>
                      </div>
                    </button>
                    
                    {categories.map((category) => {
                      const count = mockScadenze.filter(s => s.category === category.id).length
                      return (
                        <motion.button
                          key={category.id}
                ...
                {/* Quick Stats */}
                <div className="bg-white rounded-xl p-6 shadow-sm border border-[#C4BDB4]/20">
                  <h3 className="font-semibold text-[#2A5D67] mb-4">Statistiche</h3>
                  <div className="space-y-3">
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-[#1E293B]">Scadenze Totali</span>
                      <span className="font-semibold text-[#2A5D67]">{mockScadenze.length}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-[#1E293B]">Imminenti (7gg)</span>
                      <span className="font-semibold text-red-600">{upcomingScadenze.length}</span>
                    </div>
                    <div className="flex justify-between items-center">
                  ...
                  >
                    <div className="text-center py-20">
                      ...
                      <p className="text-[#1E293B] mb-4">
                        Funzionalità in arrivo! Visualizza le tue scadenze in formato calendario
                      </p>
                      <Button
                        onClick={() => setViewMode('list')}
                        variant="outline"
                      ...
                      return (
                        <motion.div
                          ...
                          className={`bg-white rounded-xl shadow-sm border transition-all hover:shadow-md ${
                            isImminente ? 'border-red-200 bg-red-50' : 'border-[#C4BDB4]/20'
                          }`}
                        >
                          <div className="p-6">
                              ...
                              <div className="flex items-start space-x-4">
                                ...
                                <div className="flex-1">
                                  <div className="flex items-center space-x-3 mb-2">
                                    ...
                                    <span className={`px-2 py-1 text-xs font-medium rounded-full border ${getPriorityColor(scadenza.priority)}`}>
                                      {scadenza.priority === 'alta' ? 'Alta Priorità' : 
                                       scadenza.priority === 'media' ? 'Media Priorità' : 'Bassa Priorità'}
                                    </span>
                                  </div>
        ...
        >
          ...
          >
            ...
            <p className="text-[#1E293B] text-lg leading-relaxed mb-2">
              Stiamo perfezionando il sistema di gestione delle scadenze fiscali per offrirti la migliore esperienza possibile.
            </p>
