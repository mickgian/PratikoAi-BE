...
class FakeCacheService:
    ...
    def __init__(self, hit: bool = False, cached_response: dict | None = None):
        self.hit = hit
...
class FakeProviderFactory:
    ...
    def create_provider(self, provider_config: dict) -> dict:
        self.create_calls += 1
        return {"name": self.provider_name, "model": self.model, "api_key": "fake-key-123", "config": provider_config}
...
def fake_validate_request_orch(valid: bool = True) -> FakeOrchestrator:
    ...
    return FakeOrchestrator({"request_valid": valid, "validation_errors": [] if valid else ["Invalid request format"]})


def fake_privacy_check_orch(enabled: bool = True) -> FakeOrchestrator:
    """Fake for step_6__privacy_check orchestrator."""
    return FakeOrchestrator({"privacy_enabled": enabled, "anonymize_required": enabled})
...
def fake_llm_call_orch(success: bool = True, has_tools: bool = False) -> FakeOrchestrator:
    ...
    if success:
        ...
        if has_tools:
            response["tool_calls"] = [{"name": "kb_search", "args": {"query": "test query"}}]
        return FakeOrchestrator({"llm_success": True, "response": response})
    return FakeOrchestrator({"llm_success": False, "error": "LLM API error", "status_code": 500})
...
def fake_cost_estimate_orch(cost: float = 0.015, within_budget: bool = True) -> FakeOrchestrator:
    ...
    return FakeOrchestrator({"cost_estimate": cost, "within_budget": within_budget, "budget_limit": 0.50})
...
def fake_stream_setup_orch() -> FakeOrchestrator:
    ...
    return FakeOrchestrator({"sse_ready": True, "generator_created": True})
...
def fake_tool_execution_orch(tool_type: str = "kb") -> FakeOrchestrator:
    ...
    return FakeOrchestrator({"tool_success": True, "tool_type": tool_type, "results": results.get(tool_type, {})})
