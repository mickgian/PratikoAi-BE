...
class TestConsentManager:
    ...
    def test_grant_consent(self):
        ...
        consent_id = self.consent_manager.grant_consent(
            user_id=user_id, consent_type=ConsentType.FUNCTIONAL, ip_address="192.168.1.1"
        )
    ...
    def test_consent_history(self):
        ...
        personalization_records = [r for r in history if r.consent_type == ConsentType.PERSONALIZATION]
    ...
    def test_replace_existing_consent(self):
        ...
        history = self.consent_manager.get_consent_history(user_id)
        functional_records = [r for r in history if r.consent_type == ConsentType.FUNCTIONAL]
...
class TestDataProcessor:
    ...
    def test_cannot_process_without_consent(self):
        ...
        can_process = self.data_processor.can_process_data(
            user_id=user_id, data_category=DataCategory.BEHAVIORAL, processing_purpose=ProcessingPurpose.CONSENT
        )
    ...
    def test_should_delete_data(self):
        ...
        self.data_processor.record_processing(
            user_id=user_id,
...
class TestAuditLogger:
    ...
    def test_export_audit_log(self):
        ...
        self.audit_logger.log_consent_event(
            user_id=user_id, event_type="consent_granted", details={"consent_type": "functional"}
        )
...
class TestGDPRCompliance:
    ...
    def test_data_subject_access_request(self):
        ...
        response = self.gdpr.handle_data_subject_request(
            user_id=user_id, request_type="access", ip_address="192.168.1.1"
        )
    ...
    def test_data_subject_deletion_request(self):
        ...
        response = self.gdpr.handle_data_subject_request(
            user_id=user_id, request_type="deletion", ip_address="192.168.1.1"
        )
    ...
    def test_data_subject_portability_request(self):
        ...
        response = self.gdpr.handle_data_subject_request(
            user_id=user_id, request_type="portability", ip_address="192.168.1.1"
        )
    ...
    def test_unsupported_request_type(self):
        ...
        with pytest.raises(ValueError):
            self.gdpr.handle_data_subject_request(user_id=user_id, request_type="unsupported_type")
    ...
    def test_periodic_cleanup(self):
        ...
        self.gdpr.consent_manager.grant_consent(user_id=user_id, consent_type=ConsentType.MARKETING, expiry_days=0)
...
class TestGlobalGDPRInstance:
    ...
    def test_global_instance_functionality(self):
        ...
        consent_id = gdpr_compliance.consent_manager.grant_consent(
            user_id=user_id, consent_type=ConsentType.FUNCTIONAL
        )

        assert consent_id is not None
        assert gdpr_compliance.consent_manager.has_valid_consent(user_id, ConsentType.FUNCTIONAL)
