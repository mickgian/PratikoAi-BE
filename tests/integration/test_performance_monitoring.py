...
class TestPerformanceCompliance:
    ...
    async def test_response_time_sla_compliance(self, integration_test_environment):
        ...
        test_queries = [
            ...
            *[{"query": "IRPEF con addizionale regionale Lombardia", "expected_max": 2.0, "complexity": "medium"}]
            * 15,
            # Complex queries (should be <3s)
            *[{"query": "Consolidato fiscale holding internazionale", "expected_max": 3.0, "complexity": "complex"}]
            * 10,
            # Very complex (edge cases, should be <3s)
            *[
                {
                    "query": "Transfer pricing multinazionale CFC rules",
                    "expected_max": 3.0,
        ...
        for i, test_case in enumerate(test_queries):
            ...
            try:
                ...
                assert response["success"] is True, f"Query failed: {test_case['query']}"

            except Exception as e:
                failed_queries.append(
                    {"query": test_case["query"], "error": str(e), "complexity": test_case["complexity"]}
                )
    ...
    async def test_cost_efficiency_targets(self, integration_test_environment):
        ...
        for pattern in user_patterns:
            ...
            for query in pattern["daily_queries"]:
                response = await services["query_processor"].process_query(
                    {"query": query, "user_id": user_id, "user_type": pattern["user_type"]}
                )
            ...
            assert (
                daily_cost <= pattern["target_cost"]
            ...
            print(
                f"✅ {pattern['user_type']}: €{daily_cost:.4f}/day ({query_count} queries, €{avg_cost_per_query:.4f}/query)"
            )
    ...
    async def test_concurrent_load_performance(self, integration_test_environment):
        ...
        baseline_response = await services["query_processor"].process_query(
            {"query": baseline_query, "user_id": "baseline_user"}
        )

        baseline_time = time.time() - baseline_start
        assert baseline_response["success"] is True
        ...
        for test in concurrency_tests:
            ...
            for i in range(concurrent_users):
                task = asyncio.create_task(
                    services["query_processor"].process_query(
                        {"query": f"{baseline_query} user {i}", "user_id": f"concurrent_user_{i}"}
                    )
                )
                tasks.append(task)
            ...
            # Assertions
            assert (
                failed_count <= concurrent_users * 0.05
...
class TestSystemMonitoringIntegration:
    ...
    async def test_circuit_breaker_monitoring_alerts(self, integration_test_environment):
        ...
        for _i in range(6):  # Exceed failure threshold
        ...
        health_score = await health_scorer.calculate_health_score(
            provider, {"total_requests": 10, "success_count": 3, "failure_count": 7}
        )
    ...
    async def test_cost_monitoring_and_alerts(self, integration_test_environment):
        ...
        for scenario in spending_scenarios:
            ...
            cost_check = await cost_circuit.should_allow_request_cost_check(provider, estimated_cost=scenario["cost"])
...
class TestRealTimeMonitoringDashboard:
    ...
    async def test_dashboard_metrics_accuracy(self, integration_test_environment):
        ...
        for test_query in test_queries:
            ...
            response = await services["query_processor"].process_query(
                {"query": test_query["query"], "user_id": "dashboard_test_user", "track_metrics": True}
            )
        ...
        dashboard_metrics = {
            ...
            "avg_response_time": sum(metrics.get("response_times", []))
            / max(len(metrics.get("response_times", [])), 1),
        ...
        assert (
            abs(dashboard_metrics["total_cost"] - test_activity["total_cost"])
            <= test_activity["total_cost"] * tolerance
        )
        ...
        print("✅ Dashboard metrics accuracy verified:")
        ...
        print(
            f"   Avg Response: {dashboard_metrics['avg_response_time']:.3f}s, Errors: {dashboard_metrics['error_count']}"
        )
    ...
    async def test_real_time_health_dashboard(self, integration_test_environment):
        ...
        print(
            f"✅ Health dashboard: {health_metrics['total_providers']} providers, {health_metrics['overall_health']:.3f} overall health, {len(alerts)} alerts"
        )
