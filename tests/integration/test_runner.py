...
from .italian_tax_test_data import italian_tax_data, get_peak_load_scenario
...
class IntegrationTestRunner:
    ...
    async def run_comprehensive_tests(self, 
                                    ...
                                    run_stress_tests: bool = True) -> IntegrationTestReport:
    ...
    async def _run_stress_tests(self) -> TestResults:
        ...
        cost_under_load = {
            "total_cost_under_load": 1.2,
            "avg_cost_per_request": 0.0048
        }
    ...
    async def _generate_comprehensive_report(self, 
                                           ...
                                           suite_results: List[TestResults]) -> IntegrationTestReport:
    ...
    def _generate_recommendations(self, 
        ...
        if failed_suites:
            recommendations.append(
                f"🔧 Address test failures in: {', '.join(r.suite_name for r in failed_suites)}"
            )
    ...
    async def _print_summary(self, report: IntegrationTestReport) -> None:
        ...
        print(f"\n{'='*60}")
        print(f"🎯 PRATIKOAI INTEGRATION TEST SUMMARY")
        print(f"{'='*60}")
        ...
        print(f"   • P95 Response Time: {perf['p95_response_time']:.2f}s (Target: ≤{self.sla_targets['response_time_p95']}s)")
        ...
        print(f"   • Daily Cost Projection: €{cost['daily_cost_projection']:.2f} (Target: ≤€{self.sla_targets['daily_cost_per_user']})")
        print(f"   • Cost Target: {'✅ PASS' if cost['cost_efficiency_target_met'] else '❌ FAIL'}")
        
        print(f"\n🎯 QUALITY METRICS:")
        quality = report.quality_summary
        print(f"   • Avg Quality Score: {quality['avg_quality_score']:.2f} (Target: ≥{self.sla_targets['quality_score']})")
        ...
        if all_compliant and report.overall_success_rate >= 0.95:
            print(f"🎉 OVERALL ASSESSMENT: EXCELLENT - System ready for production")
        elif report.overall_success_rate >= 0.90:
            print(f"✅ OVERALL ASSESSMENT: GOOD - Minor optimizations recommended")
        elif report.overall_success_rate >= 0.80:
            print(f"⚠️  OVERALL ASSESSMENT: ACCEPTABLE - Several improvements needed")
        else:
            print(f"❌ OVERALL ASSESSMENT: NEEDS WORK - Major issues require attention")
