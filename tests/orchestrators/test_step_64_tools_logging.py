...
async def test_step_64_logs_tools_when_provided():
    ...
    with patch('app.observability.rag_logging.rag_step_log') as mock_log:
        with patch('app.observability.rag_logging.rag_step_timer'):
            result = await step_64__llmcall(ctx=ctx)

            # Find completion log call (processing_stage='completed')
            completion_calls = [
                call_obj for call_obj in mock_log.call_args_list
            ...
            assert completion_log['tools_provided'] == True, "tools_provided should be True"
            ...
            assert 'KnowledgeSearchTool' in completion_log['tool_names'], \
                "Should include KnowledgeSearchTool in names"
            assert 'CCNLTool' in completion_log['tool_names'], \
                "Should include CCNLTool in names"
...
async def test_step_64_logs_no_tools_when_none_provided():
    ...
    with patch('app.observability.rag_logging.rag_step_log') as mock_log:
        with patch('app.observability.rag_logging.rag_step_timer'):
            ...
            assert completion_log['tools_provided'] == False, "tools_provided should be False"
...
async def test_step_64_logs_tools_from_llm_params():
    ...
    with patch('app.observability.rag_logging.rag_step_log') as mock_log:
        with patch('app.observability.rag_logging.rag_step_timer'):
            ...
            assert completion_log['tools_provided'] == True
...
async def test_step_64_handles_dict_format_tools():
    ...
    with patch('app.observability.rag_logging.rag_step_log') as mock_log:
        with patch('app.observability.rag_logging.rag_step_timer'):
            ...
            assert 'search_kb' in completion_log['tool_names'] or \
                   'search_ccnl' in completion_log['tool_names']
...
async def test_step_64_tools_logging_survives_errors():
    ...
    ctx["provider_instance"].chat_completion = AsyncMock(
