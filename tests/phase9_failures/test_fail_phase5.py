...
class TestPhase5CostRejectionLoop:
    ...
    async def test_repeated_cost_rejection_eventually_fails(self):
        ...
        state = make_state(provider={"name": "anthropic", "model": "claude-3-5-sonnet-20241022"}, cheaper_attempts=0)

        # Attempt 1: over budget
        fake_cost1 = FakeOrchestrator({"cost_estimate": 0.80, "within_budget": False})
        ...
        fake_cheaper1 = FakeOrchestrator(
            {"cheaper_provider_found": True, "provider": {"name": "openai", "model": "gpt-4"}, "cheaper_attempts": 1}
        )
        ...
        fake_cost2 = FakeOrchestrator({"cost_estimate": 0.65, "within_budget": False})
        ...
        fake_cost3 = FakeOrchestrator({"cost_estimate": 0.55, "within_budget": False})
    ...
    async def test_all_providers_over_budget_fails_gracefully(self):
        ...
        fake_select = FakeOrchestrator(
            {"provider_selected": True, "provider": {"name": "openai", "model": "gpt-3.5-turbo"}}
        )
        ...
        fake_cost = FakeOrchestrator({"cost_estimate": 0.005, "within_budget": False, "budget_limit": 0.001})
        with patch("app.core.langgraph.nodes.step_055__estimate_cost.step_55__estimate_cost", fake_cost):
            state = await node_step_55(state)

        # No cheaper available
        fake_cheaper = FakeOrchestrator({"cheaper_provider_found": False, "error": "All providers exceed budget"})
...
class TestPhase5ProviderFactoryErrors:
    ...
    async def test_provider_creation_fails_retries_with_different(self):
        ...
        fake_create = FakeOrchestrator(
            {"provider_ready": False, "error": "Failed to initialize provider", "error_type": "initialization_error"}
        )
        ...
        fake_failover = FakeOrchestrator(
            {"failover_provider": {"name": "openai", "model": "gpt-4"}, "failover_triggered": True}
        )
    ...
    async def test_provider_invalid_api_key_error(self):
        ...
        # API key invalid
        fake_create = FakeOrchestrator(
            {"provider_ready": False, "error": "Invalid API key", "error_type": "auth_error", "retryable": False}
        )
        with patch("app.core.langgraph.nodes.step_057__create_provider.step_57__create_provider", fake_create):
            state = await node_step_57(state)
    ...
    async def test_provider_model_not_available_error(self):
        ...
        state = make_state(provider={"name": "anthropic", "model": "claude-3-opus-deprecated"})
...
class TestPhase5ProviderSelectionErrors:
    ...
    async def test_no_providers_available_for_strategy(self):
        ...
        fake_select = FakeOrchestrator(
            {"provider_selected": False, "error": "No providers available for strategy", "strategy": "NONEXISTENT"}
        )
        with patch("app.core.langgraph.nodes.step_048__select_provider.step_48__select_provider", fake_select):
            state = await node_step_48(state)
    ...
    async def test_provider_registry_empty_error(self):
        ...
        # Registry empty
        fake_select = FakeOrchestrator(
            {"provider_selected": False, "error": "Provider registry is empty", "registry_available": False}
        )
        with patch("app.core.langgraph.nodes.step_048__select_provider.step_48__select_provider", fake_select):
            state = await node_step_48(state)
...
class TestPhase5CostEstimationErrors:
    ...
    async def test_cost_estimation_invalid_model_data(self):
        ...
        state = make_state(provider={"name": "unknown_provider", "model": "unknown_model"})
