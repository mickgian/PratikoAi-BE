...
class TestPhase6ValidationFlow:
    ...
    async def test_valid_request_proceeds_to_privacy(self):
        ...
        with patch(
            "app.core.langgraph.nodes.step_001__validate_request.step_1__validate_request",
        ...
        fake_check = FakeOrchestrator({"validation_passed": True, "proceed_to_privacy": True})
    ...
    async def test_invalid_request_stops_early(self):
        ...
        fake_check = FakeOrchestrator({"validation_passed": False, "should_return_error": True})
        with patch("app.core.langgraph.nodes.step_003__valid_check.step_3__valid_check", fake_check):
            state = await node_step_3(state)
...
class TestPhase6PrivacyFlow:
    ...
    async def test_privacy_enabled_no_pii_proceeds(self):
        ...
        with patch(
            "app.core.langgraph.nodes.step_006__privacy_check.step_6__privacy_check",
        ...
        fake_pii = FakeOrchestrator({"pii_detected": False, "pii_entities": []})
    ...
    async def test_privacy_enabled_pii_detected_anonymizes(self):
        ...
        fake_anon = FakeOrchestrator(
            {"anonymized_input": "My email is [EMAIL]", "anonymization_map": {"[EMAIL]": "john@example.com"}}
        )
...
class TestPhase6AgentInitFlow:
    ...
    async def test_successful_validation_privacy_inits_agent(self):
        ...
        fake_init = FakeOrchestrator(
            {"agent_initialized": True, "agent_config": {"llm_enabled": True, "tools_available": ["kb_search"]}}
        )
    ...
    async def test_privacy_flow_preserves_request_context(self):
        ...
        state = make_state(
            request_id=request_id, session_id=session_id, messages=[{"role": "user", "content": "test"}]
        )
...
class TestPhase6GDPRLogging:
    ...
    async def test_gdpr_log_after_validation(self):
        ...
        # Step 4: GDPR log
        fake_gdpr = FakeOrchestrator({"gdpr_logged": True, "log_id": "gdpr-log-123"})
