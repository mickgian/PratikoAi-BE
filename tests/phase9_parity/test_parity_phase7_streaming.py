...
class TestPhase7StreamCheckParity:
    ...
    async def test_stream_check_enabled_delegates(self):
        ...
        state = make_state(streaming={"requested": True, "enabled": True})
    ...
    async def test_stream_check_preserves_response(self):
        ...
        state = make_state(streaming={"requested": True}, response={"content": "test response", "complete": True})
        fake_orch = FakeOrchestrator({"streaming_enabled": True})
...
class TestPhase7SSEWriteParity:
    ...
    async def test_sse_write_delegates_correctly(self):
        ...
        fake_orch = FakeOrchestrator({"chunks_written": 3, "write_success": True})
    ...
    async def test_sse_write_preserves_response_metadata(self):
        ...
        state = state_streaming_enabled()
        state["response"] = {"content": "test", "complete": True, "metadata": {"model": "claude-3-5-sonnet-20241022"}}
    ...
    async def test_non_streaming_path_delegates(self):
        ...
        state = make_state(streaming={"enabled": False}, response={"content": "complete response", "complete": True})

        fake_orch = FakeOrchestrator({"streaming_enabled": False, "response_complete": True})
