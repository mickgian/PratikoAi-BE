...
class TestAuditRolesAndWiring:
    ...
    def test_phase4_nodes_in_registry(self):
        ...
        for node_num in self.PHASE4_NODES:
            ...
            assert node_data['name'] == f"node_step_{node_num}", f"Node {node_num} name should be 'node_step_{node_num}'"
    ...
    def test_phase4_roles_are_node(self):
        ...
        for node_num in self.PHASE4_NODES:
            ...
            assert role_match.group(1) == 'Node', f"Step {node_num} should have Role: Node, found Role: {role_match.group(1)}"
    ...
    def test_phase4_status_is_wired(self):
        ...
        for node_num in self.PHASE4_NODES:
            ...
            assert "✅" in status or "Implemented & Wired" in status, \
    ...
    def test_wiring_connections_valid(self):
        ...
        for node_num, node_data in registry.items():
            ...
            for incoming_node in node_data['incoming']:
                ...
                assert node_num in registry[incoming_node]['outgoing'], \
            ...
            for outgoing_node in node_data['outgoing']:
                ...
                # Check reverse connection exists
                assert node_num in registry[outgoing_node]['incoming'], \
    ...
    def test_node_exports_exist(self):
        ...
        for node_num in self.PHASE4_NODES:
            node_name = f"node_step_{node_num}"

            # Check import statement
            assert f"from .step_{node_num:03d}" in content or f"node_step_{node_num}" in content, \
                f"Node {node_num} should be imported in __init__.py"

            # Check __all__ export
            assert f'"node_step_{node_num}"' in content, \
                f"Node {node_num} should be exported in __all__ list"
    ...
    def test_phase4_hot_path_connectivity(self):
        ...
        for node_num, expected_outgoing in expected_connections.items():
            ...
            assert set(actual_outgoing) == set(expected_outgoing), \
