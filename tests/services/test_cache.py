...
class TestCacheService:
    ...
    def test_generate_query_hash(self, sample_messages):
        ...
        hash1 = self.cache_service._generate_query_hash(sample_messages, "gpt-4o-mini", 0.2)
        hash2 = self.cache_service._generate_query_hash(sample_messages, "gpt-4o-mini", 0.2)
        hash3 = self.cache_service._generate_query_hash(sample_messages, "claude-3-haiku", 0.2)
    ...
    async def test_get_cached_response_disabled(self, sample_messages):
        ...
        result = await self.cache_service.get_cached_response(sample_messages, "gpt-4o-mini", 0.2)
    ...
    async def test_cache_response_disabled(self, sample_messages, sample_llm_response):
        ...
        result = await self.cache_service.cache_response(sample_messages, "gpt-4o-mini", sample_llm_response, 0.2)
        assert result is False

    async def test_get_conversation_cache_hit(self, mock_redis):
        """Test conversation cache hit."""
        session_id = "test-session"
        cached_messages = [{"role": "user", "content": "Hello"}, {"role": "assistant", "content": "Hi there!"}]
