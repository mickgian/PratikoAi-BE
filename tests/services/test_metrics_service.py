...
from app.services.metrics_service import Environment, MetricResult, MetricsReport, MetricsService, MetricStatus
...
class TestMetricsService:
    ...
    async def test_collect_technical_metrics_success(self, metrics_service):
        ...
        with (
            patch.object(metrics_service, "_get_api_response_time_p95", return_value=450.0),
    ...
    async def test_collect_technical_metrics_with_failures(self, metrics_service):
        ...
        with (
            patch.object(metrics_service, "_get_api_response_time_p95", return_value=750.0),
    ...
    async def test_collect_business_metrics_success(self, metrics_service):
        ...
        with (
            patch.object(metrics_service, "_get_average_cost_per_user", return_value=1.8),
    ...
    async def test_generate_metrics_report(self, metrics_service):
        ...
        with (
            patch.object(metrics_service, "collect_technical_metrics", return_value=technical_metrics),
    ...
    async def test_get_api_response_time_p95(self, metrics_service):
        ...
        with patch(
            "app.core.performance.performance_monitor.performance_monitor.get_performance_summary",
    ...
    async def test_get_cache_hit_rate(self, metrics_service):
        ...
        mock_stats = {"cache_hits": 80, "cache_misses": 20}

        with patch("app.services.cache.cache_service.get_cache_statistics", return_value=mock_stats):
