...
from app.models.usage import CostCategory, UsageEvent, UsageQuota, UsageType
from app.services.usage_tracker import CostBreakdown, UsageMetrics, UsageTracker, usage_tracker
...
class TestUsageTracker:
    ...
    async def test_cost_alert_creation(self, mock_db):
        ...
        await self.tracker._create_alert(
            user_id="test_user", alert_type="daily_threshold", threshold_eur=0.10, current_cost_eur=0.15
        )
    ...
    async def test_update_user_quota(self, mock_db):
        ...
        with patch.object(self.tracker, "get_user_quota", return_value=quota):
            await self.tracker._update_user_quota(user_id="test_user", cost_eur=0.002, tokens=100)
...
class TestUsageMetrics:
    ...
    def test_custom_values(self):
        ...
        metrics = UsageMetrics(total_requests=100, total_cost_eur=1.50, cache_hit_rate=0.75)
...
class TestCostBreakdown:
    ...
    def test_to_dict_conversion(self):
        ...
        breakdown = CostBreakdown(llm_inference=1.50, storage=0.30, compute=0.20, total=2.00)
