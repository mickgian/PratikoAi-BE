...
class TestBusinessPlanGenerator:
    ...
    def test_business_plan_generation_existing_company(self, plan_generator):
        ...
        result = plan_generator.generate_business_plan(
            company_data=company_data,
            projection_years=3,
            template=PlanTemplate.EXPANSION_FOCUSED
        )
    ...
    def test_financial_projections_revenue_growth_model(self, plan_generator):
        ...
        # Act
        projections = plan_generator.generate_financial_projections(
            base_data=base_data,
            projection_years=3
        )
    ...
    def test_cash_flow_projections_with_capex(self, plan_generator):
        ...
        # Act
        cash_flow = plan_generator.generate_cash_flow_projections(
            data=cash_flow_data,
            projection_years=5
        )
    ...
    def test_balance_sheet_projections(self, plan_generator):
        ...
        # Act
        balance_sheet = plan_generator.generate_balance_sheet_projections(
        ...
        assert year_1.total_assets > balance_sheet_data['starting_balance_sheet']['cash'] + \
    ...
    def test_risk_assessment_generation_comprehensive(self, plan_generator):
        ...
        customer_risk = next((r for r in risk_assessment.identified_risks 
                            if 'customer concentration' in r.description.lower()), None)
        ...
        regulatory_risk = next((r for r in risk_assessment.identified_risks 
                              if 'regulatory' in r.description.lower()), None)
    ...
    def test_risk_assessment_scenario_analysis(self, plan_generator):
        ...
        for scenario_name, scenario in scenario_analysis.scenarios.items():
    ...
    def test_business_plan_validation_comprehensive(self, plan_generator):
        ...
        financial_check = next((c for c in validation_result.validation_checks 
                              if 'financial' in c.check_name.lower()), None)
        assert financial_check is not None
        
        # Should validate market assumptions
        market_check = next((c for c in validation_result.validation_checks 
                           if 'market' in c.check_name.lower()), None)
    ...
    def test_business_plan_validation_financial_consistency(self, plan_generator):
        ...
        revenue_issue = next((i for i in financial_validation.inconsistencies 
                            if 'revenue' in i.issue_type.lower()), None)
        assert revenue_issue is not None
        
        # Should identify funding gap
        funding_issue = next((i for i in financial_validation.inconsistencies 
                            if 'funding' in i.issue_type.lower()), None)
    ...
    def test_business_plan_template_startup(self, plan_generator):
        ...
        assert template.required_financial_statements == [
            'P&L', 'Cash Flow', 'Balance Sheet', 'Break-even Analysis'
        ]
    ...
    def test_business_plan_italian_compliance(self, plan_generator):
        ...
        italian_plan = plan_generator.generate_business_plan(
            company_data=italian_company_data,
            template=PlanTemplate.ITALIAN_REGULATORY_COMPLIANT
        )
    ...
    def test_business_plan_validation_edge_cases(self, plan_generator):
        ...
        market_warning = next((w for w in validation.warning_flags 
                             if 'market share' in w.description.lower()), None)
