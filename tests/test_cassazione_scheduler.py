...
from app.services.scrapers.cassazione_scraper import CassazioneScraper, CourtSection, ScrapingResult
...
class TestScheduledScrapingJobModel:
    ...
    def test_create_job_with_full_config(self):
        ...
        job = ScheduledScrapingJob(
            ...
            scraping_config={"rate_limit_delay": 2.0, "timeout_seconds": 30, "respect_robots_txt": True},
        )
    ...
    def test_calculate_next_run_time(self):
        ...
        daily_job = ScheduledScrapingJob(job_id="daily_job", name="Daily Job", frequency=ScheduleFrequency.DAILY)
    ...
    def test_update_job_status(self):
        ...
        job = ScheduledScrapingJob(job_id="test_job", name="Test Job", frequency=ScheduleFrequency.WEEKLY)
...
class TestCassazioneScheduler:
    ...
    async def test_execute_scraping_job(self, scheduler, sample_job):
        ...
        mock_scraping_result = ScrapingResult(
            decisions_found=25, decisions_processed=24, decisions_saved=23, errors=1, duration_seconds=180
        )
    ...
    def test_get_all_jobs(self, scheduler):
        ...
        job1 = ScheduledScrapingJob(job_id="job1", name="Job 1", frequency=ScheduleFrequency.DAILY)
        job2 = ScheduledScrapingJob(job_id="job2", name="Job 2", frequency=ScheduleFrequency.WEEKLY)
    ...
    def test_get_jobs_by_status(self, scheduler):
        ...
        running_job = ScheduledScrapingJob(job_id="running_job", name="Running Job", frequency=ScheduleFrequency.DAILY)
        running_job.status = JobStatus.RUNNING

        completed_job = ScheduledScrapingJob(
            job_id="completed_job", name="Completed Job", frequency=ScheduleFrequency.WEEKLY
        )
...
class TestSchedulerNotifications:
    ...
    async def test_send_job_completion_notification(self, scheduler_with_notifications):
        ...
        result = ScrapingResult(decisions_found=50, decisions_saved=48, errors=2, duration_seconds=300)

        with patch("app.services.email_service.send_email") as mock_send_email:
            await scheduler_with_notifications._send_job_notification(job, JobStatus.COMPLETED, result)
    ...
    def test_notification_throttling(self, scheduler_with_notifications):
        ...
        should_send = scheduler_with_notifications._should_send_notification(job, JobStatus.COMPLETED)

        assert should_send is False

        # But should send for failures even if throttled
        should_send_failure = scheduler_with_notifications._should_send_notification(job, JobStatus.FAILED)
...
class TestSchedulerPersistence:
    ...
    async def test_load_job_configurations(self, scheduler_with_persistence):
        ...
        with patch("app.services.database.database_service") as mock_db:
            ...
            mock_job_records = [
                Mock(job_id="job1", name="Job 1", frequency="weekly", court_sections=["civile"], enabled=True),
                Mock(
                    job_id="job2",
                    name="Job 2",
                    frequency="daily",
    ...
    async def test_update_job_execution_history(self, scheduler_with_persistence):
        ...
        job = ScheduledScrapingJob(job_id="history_job", name="History Job", frequency=ScheduleFrequency.DAILY)

        result = ScrapingResult(decisions_found=30, decisions_saved=28, errors=2, duration_seconds=240)
...
class TestSchedulerIntegrationScenarios:
    ...
    async def test_full_weekly_update_scenario(self):
        ...
        mock_result = ScrapingResult(
            decisions_found=150, decisions_processed=148, decisions_saved=145, errors=3, duration_seconds=600
        )
