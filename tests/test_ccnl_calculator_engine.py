...
def sample_metalmeccanici_ccnl():
    ...
    ccnl.job_levels = [
        JobLevel(
            level_code="C1", level_name="Operaio Comune", category=WorkerCategory.OPERAIO, minimum_experience_months=0
        ),
        JobLevel(
            level_code="C2",
        ...
        JobLevel(
            level_code="5", level_name="Impiegato", category=WorkerCategory.IMPIEGATO, minimum_experience_months=0
        ),
        JobLevel(
            level_code="6",
    ...
    ccnl.salary_tables = [
        ...
        SalaryTable(
            ...
            thirteenth_month=True,
            fourteenth_month=True,
            company_size_adjustments={CompanySize.LARGE: Decimal("50.00")},
        ),
        SalaryTable(
            ccnl_sector=CCNLSector.METALMECCANICI_INDUSTRIA,
            level_code="B1",
...
class TestComprehensiveCompensationCalculation:
    ...
    def test_basic_annual_compensation(self, calculator):
        ...
        compensation = calculator.calculate_comprehensive_compensation(
            level_code="C2", geographic_area=GeographicArea.NAZIONALE, include_allowances=False
        )
    ...
    def test_compensation_with_geographic_adjustment(self, calculator):
        ...
        compensation = calculator.calculate_comprehensive_compensation(
            level_code="C2", geographic_area=GeographicArea.NORD, include_allowances=False
        )
    ...
    def test_compensation_with_allowances(self, calculator):
        ...
        compensation = calculator.calculate_comprehensive_compensation(
            level_code="C2", geographic_area=GeographicArea.NORD, working_days_per_month=22, include_allowances=True
        )
    ...
    def test_compensation_with_overtime(self, calculator):
        ...
        compensation = calculator.calculate_comprehensive_compensation(
            level_code="C2", overtime_hours_monthly=10, include_allowances=False
        )
    ...
    def test_compensation_with_deductions(self, calculator):
        ...
        compensation = calculator.calculate_comprehensive_compensation(level_code="C2")
...
class TestLeaveCalculations:
    ...
    def test_leave_with_usage(self, calculator):
        ...
        used_days = {LeaveType.FERIE: 10, LeaveType.PERMESSI_RETRIBUITI: 4}

        balances = calculator.calculate_leave_balances(seniority_months=36, used_days=used_days)
    ...
    def test_leave_accrual_rates(self, calculator):
        ...
        balances = calculator.calculate_leave_balances(seniority_months=36)
    ...
    def test_leave_expiry_dates(self, calculator):
        ...
        balances = calculator.calculate_leave_balances(seniority_months=36, calculation_date=calculation_date)
...
class TestSeniorityBenefits:
    ...
    def test_basic_seniority_benefits(self, calculator):
        ...
        benefits = calculator.calculate_seniority_benefits(
            worker_category=WorkerCategory.OPERAIO, hire_date=hire_date, calculation_date=date(2024, 1, 1)
        )
...
class TestGeographicDifferences:
    ...
    def test_geographic_salary_differences(self, calculator):
        ...
        differences = calculator.calculate_geographic_differences(level_code="C2", base_area=GeographicArea.NAZIONALE)
...
class TestOvertimeScenarios:
    ...
    def test_overtime_scenarios(self, calculator):
        ...
        scenarios = calculator.calculate_overtime_scenarios(base_monthly_salary=Decimal("2000.00"))
        ...
        assert (
            scenarios["weekend_overtime"]["examples"]["1_hour"] > scenarios["weekday_overtime"]["examples"]["1_hour"]
        )
...
class TestCCNLComparison:
    ...
    def test_compare_with_other_ccnl(self, calculator, sample_metalmeccanici_ccnl):
        ...
        other_ccnl = CCNLAgreement(
            sector=CCNLSector.COMMERCIO_TERZIARIO, name="CCNL Commercio", valid_from=date(2024, 1, 1)
        )
        ...
        other_ccnl.leave_entitlements = [
            LeaveEntitlement(
                ccnl_sector=CCNLSector.COMMERCIO_TERZIARIO, leave_type=LeaveType.FERIE, base_annual_days=24
            )
        ]

        comparisons = calculator.compare_with_other_ccnl(
            other_ccnl, level_code="C2", comparison_aspects=["salary", "leave"]
        )
...
class TestCareerProgression:
    ...
    def test_career_progression_calculation(self, calculator):
        ...
        result = calculator.calculate_career_progression(
            starting_level="C1", progression_path=progression_path, starting_date=date(2014, 1, 1)
        )
...
class TestComplexQuery:
    ...
    def test_answer_complex_metalworker_query(self, calculator):
        ...
        assert (
            result["seniority_benefits"]["notice_period_days"] == 15
        )  # 5 years = 60 months, which is in the 0-60 range
    ...
    def test_complex_query_different_scenarios(self, calculator):
        ...
        assert senior_result["summary"]["annual_gross_total"] > junior_result["summary"]["annual_gross_total"]
        assert senior_result["summary"]["total_leave_days"] > junior_result["summary"]["total_leave_days"]
        assert senior_result["summary"]["notice_period_days"] > junior_result["summary"]["notice_period_days"]
