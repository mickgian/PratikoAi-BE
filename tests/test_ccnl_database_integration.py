...
from app.models.ccnl_update_models import (
    CCNLDatabase, CCNLVersion, CCNLUpdateEvent, CCNLChangeLog,
    UpdateSource, UpdateStatus, ChangeType
)
from app.services.ccnl_rss_monitor import (
    RSSFeedMonitor, CCNLUpdateDetector, UpdateClassifier
)
...
class TestRSSFeedMonitoring:
    ...
    def test_ccnl_keyword_detection(self, rss_monitor):
        ...
        ccnl_titles = [title for title in test_titles 
                      if rss_monitor.contains_ccnl_keywords(title)]
...
class TestCCNLUpdateDetection:
    ...
    def test_priority_calculation(self, update_detector):
        ...
        low_priority_cases = [
            "Modifica minore accordo aziendale",
            "Chiarimenti interpretativi CCNL"
        ]
    ...
    async def test_ai_classification(self, update_detector):
        ...
        classification = await update_detector.ai_classify_update(
            "Accordo CCNL Metalmeccanici",
            sample_content
        )
...
class TestCCNLVersionManager:
    ...
    def test_rollback_capability(self, version_manager):
        ...
        current_version = version_manager.create_version(ccnl_id, {
            "version_number": "2024.1",
            "salary_data": {"minimum_wages": {"level_1": -1500}}  # Invalid negative wage
        })
        
        # Create previous good version
        previous_version = version_manager.create_version(ccnl_id, {
            "version_number": "2023.1", 
            "salary_data": {"minimum_wages": {"level_1": 1400}}
        })
...
class TestCCNLUpdateProcessor:
    ...
    async def test_cross_source_verification(self, update_processor):
        ...
        update_events = [
            CCNLUpdateEvent(
                id=uuid4(), ccnl_id=ccnl_id, source=UpdateSource.CGIL_RSS,
                title="CCNL Metalmeccanici rinnovato", detected_at=datetime.utcnow()
            ),
            CCNLUpdateEvent(
                id=uuid4(), ccnl_id=ccnl_id, source=UpdateSource.CONFINDUSTRIA_NEWS,
                title="Accordo Metalmeccanici siglato", detected_at=datetime.utcnow()
            )
        ]
...
class TestCCNLChangeDetection:
    ...
    def test_working_conditions_change_detection(self):
        ...
        changes = change_detector.detect_working_conditions_changes(
...
class MockRSSFeedMonitor:
    ...
    def contains_ccnl_keywords(self, title: str) -> bool:
        keywords = ["rinnovo ccnl", "contratto collettivo", "accordo siglato", 
                   "firma contratto", "minimi tabellari"]
