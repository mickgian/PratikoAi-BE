...
from app.models.ccnl_data import CCNLSector, WorkerCategory, AllowanceType
from app.services.ccnl_update_service import (
    ccnl_update_service, CCNLUpdate, CCNLAlert, UpdateType, AlertType
)
from app.services.inps_inail_service import inps_inail_service, RiskClass, ContributionType
...
class TestINPSINAILService:
    ...
    async def test_calculate_contributions(self):
        ...
        for salary in test_salaries:
            calc = await inps_inail_service.calculate_contributions(
                salary, sector, category
            )
...
class TestI18nService:
    ...
    def test_localized_ccnl_summary(self):
        ...
        required_fields = [
            "sector_name", "description", "typical_companies",
            "worker_category_label", "salary_label"
        ]
...
class TestIntegrationScenarios:
    ...
    async def test_comprehensive_sector_analysis(self):
        ...
        contribution_calc = await inps_inail_service.calculate_contributions(
            salary, sector, category
        )
        ...
        sector_updates = [
            u for u in ccnl_update_service.pending_updates 
            if u.sector == sector
        ]
        ...
        analysis = {
            ...
            "localized_info": {
                "italian": italian_info["sector_name"],
                "english": english_info["sector_name"]
            },
