...
class TestMetalmeccaniciIndustriaCCNL:
    ...
    def test_metalmeccanici_allowances(self):
        ...
        buoni_pasto = next((a for a in ccnl.special_allowances if a.allowance_type == AllowanceType.BUONI_PASTO), None)
...
class TestCommercioTerziarioCCNL:
    ...
    def test_commercio_job_levels_with_geographic_differences(self):
        ...
        has_geographic_differences = any(s.geographic_area != GeographicArea.NAZIONALE for s in ccnl.salary_tables)
...
class TestEdiliziaIndustriaCCNL:
    ...
    def test_edilizia_ccnl_with_risk_allowances(self):
        ...
        risk_allowances = [a for a in ccnl.special_allowances if a.allowance_type == AllowanceType.INDENNITA_RISCHIO]
        assert len(risk_allowances) >= 1

        # Check for transport allowances (common in construction)
        transport_allowances = [
            a for a in ccnl.special_allowances if a.allowance_type == AllowanceType.INDENNITA_TRASPORTO
        ]
...
class TestTrasportiLogisticaCCNL:
    ...
    def test_trasporti_driving_allowances(self):
        ...
        trasferta_allowances = [
            a for a in ccnl.special_allowances if a.allowance_type == AllowanceType.INDENNITA_TRASFERTA
        ]
...
class TestPriority1CCNLDataIntegration:
    ...
    def test_geographic_salary_differences(self):
        ...
        for ccnl in all_ccnl:
            has_diff = any(s.geographic_area != GeographicArea.NAZIONALE for s in ccnl.salary_tables)
            if has_diff:
