...
class TestTelecomunicazioniCCNL:
    ...
    def test_telecomunicazioni_ccnl_structure(self):
        ...
        assert agreement.working_hours.remote_work_allowed is True
        assert agreement.working_hours.shift_work is True
...
class TestICTSectorCCNL:
    ...
    def test_ict_sector_structure(self):
        ...
        assert len(agreement.job_levels) == 4
        assert agreement.working_hours.remote_work_allowed is True
        assert agreement.working_hours.flexible_hours is True
...
class TestServiceSectorsCCNL:
    ...
    @pytest.mark.parametrize(
        "sector_function,expected_sector",
        [
...
class TestSocialServicesCCNL:
    ...
    def test_cooperative_sociali_structure(self):
        ...
        operaio_leave = next(
            leave for leave in agreement.leave_entitlements if leave.worker_category == WorkerCategory.OPERAIO
        )
        impiegato_leave = next(
            leave for leave in agreement.leave_entitlements if leave.worker_category == WorkerCategory.IMPIEGATO
        )
...
class TestPriority2DataQuality:
    ...
    def test_notice_periods_progression(self):
        ...
        for agreement in all_agreements:
            ...
            for _category, notices in categories.items():
