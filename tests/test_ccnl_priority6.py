...
class TestPriority6CCNLDataStructures:
    ...
    def test_service_sector_ccnls(self):
        ...
        for ccnl in service_ccnls:
            ...
            assert ccnl.working_hours.ordinary_weekly_hours <= 40, f"Service sector should have standard hours: {ccnl.name}"
...
class TestPriority6CCNLCalculations:
    ...
    async def test_priority6_salary_calculations(self):
        ...
        for ccnl in priority6_ccnls:
            if ccnl.salary_tables:
                ...
                salary_table = next(
                    (table for table in ccnl.salary_tables 
                     if table.level_code == first_level.level_code), 
                    None
                )
                ...
                if salary_table:
                    ...
                    if ccnl.sector in [CCNLSector.DIRIGENTI_INDUSTRIA, CCNLSector.DIRIGENTI_COMMERCIO]:
                        assert salary_table.base_monthly_salary > Decimal('3000'), f"Management salary too low: {ccnl.name}"
                    elif ccnl.sector == CCNLSector.QUADRI:
                        assert salary_table.base_monthly_salary > Decimal('2500'), f"Quadri salary too low: {ccnl.name}"
                    else:
    ...
    async def test_priority6_leave_calculations(self):
        ...
        for ccnl in priority6_ccnls:
            annual_leaves = [
                leave for leave in ccnl.leave_entitlements 
                if leave.leave_type == LeaveType.FERIE
            ]
...
class TestPriority6CCNLBusinessLogic:
    ...
    def test_management_hierarchy_validation(self):
        ...
        for ccnl in [dirigenti_industria, dirigenti_commercio]:
            management_levels = [
                level for level in ccnl.job_levels 
                if level.category == WorkerCategory.DIRIGENTE
            ]
            assert len(management_levels) > 0, f"Missing management levels in {ccnl.name}"
        
        # Test quadri have appropriate category
        quadri_levels = [
            level for level in quadri.job_levels 
            if level.category == WorkerCategory.QUADRO
        ]
    ...
    def test_transport_specialization_requirements(self):
        ...
        specialized_levels = [
            level for level in autotrasporto.job_levels
    ...
    def test_service_sector_characteristics(self):
        ...
        part_time_levels = [
            level for level in acconciatura.job_levels
    ...
    def test_priority6_coverage_completeness(self):
        ...
        transport_sectors = [
            ccnl for ccnl in priority6_data 
            if ccnl.sector.value.startswith("auto")
        ]
        ...
        service_sectors = [
            ccnl for ccnl in priority6_data
            if ccnl.sector in [CCNLSector.POMPE_FUNEBRI, CCNLSector.ACCONCIATURA_ESTETICA, CCNLSector.IMPIANTI_SPORTIVI]
        ]
