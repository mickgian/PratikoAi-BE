...
from app.services.ccnl_search_service import (
    CCNLSearchService, SearchFilters, NaturalLanguageProcessor,
    SearchResult, FacetCount, SearchResponse
)
from app.models.ccnl_database import (
    CCNLAgreementDB, CCNLSectorDB, JobLevelDB,
    SalaryTableDB, WorkingHoursDB, LeaveEntitlementDB
)
...
class TestCCNLSearchService:
    ...
    async def test_salary_range_filter(self, search_service):
        ...
        filters = SearchFilters(
            min_salary=Decimal('1500'),
            max_salary=Decimal('3000')
        )
    ...
    async def test_geographic_filter(self, search_service):
        ...
        for result in response.results:
            assert any(area in ["nord", "lombardia", "piemonte"] 
                      for area in result.geographic_coverage)
    ...
    async def test_pagination(self, search_service):
        ...
        # First page
        response1 = await search_service.search(
            filters=SearchFilters(),
            page=1,
            page_size=10
        )
        ...
        # Second page
        response2 = await search_service.search(
            filters=SearchFilters(),
            page=2,
            page_size=10
        )
    ...
    async def test_search_statistics(self, search_service):
        ...
        response = await search_service.search(
            filters=SearchFilters(sectors=[CCNLSector.METALMECCANICI_INDUSTRIA])
        )
