...
class TestCCNLQueryFilters:
    ...
    def test_create_salary_range_filters(self):
        ...
        filters = CCNLQueryFilters(
            min_monthly_salary=Decimal("1500.00"), max_monthly_salary=Decimal("2500.00"), include_allowances=True
        )
    ...
    def test_create_experience_filters(self):
        ...
        filters = CCNLQueryFilters(min_experience_months=12, max_experience_months=60, supervision_roles_only=True)
    ...
    def test_filters_validation(self):
        ...
        # Valid filters
        valid_filters = CCNLQueryFilters(min_monthly_salary=Decimal("1000.00"), max_monthly_salary=Decimal("2000.00"))
        assert valid_filters.is_valid()

        # Invalid filters (min > max)
        invalid_filters = CCNLQueryFilters(
            min_monthly_salary=Decimal("2000.00"), max_monthly_salary=Decimal("1000.00")
        )
...
class TestCCNLSearchResult:
    ...
    def test_create_search_result(self):
        ...
        result = CCNLSearchResult(
            ...
            query_time_ms=45,
            filters_applied={"sectors": ["metalmeccanici_industria"], "geographic_area": "nord"},
        )
    ...
    def test_search_result_with_facets(self):
        ...
        result = CCNLSearchResult(
            ...
            query_time_ms=50,
            facets={
                "sectors": {"metalmeccanici_industria": 15, "commercio_terziario": 10, "edilizia_industria": 5},
                "worker_categories": {"operaio": 20, "impiegato": 8, "quadro": 2},
            },
        )
...
class TestCCNLService:
    ...
    def sample_ccnl_agreement(self):
        ...
        return CCNLAgreement(
            ...
            salary_tables=[
                SalaryTable(CCNLSector.METALMECCANICI_INDUSTRIA, "C1", Decimal("1450.00"), thirteenth_month=True),
                SalaryTable(
    ...
    async def test_get_ccnl_by_id(self, ccnl_service):
        ...
        with patch("app.services.database.database_service.get_session_maker") as mock_session:
            ...
            mock_agreement = CCNLAgreementDB(
                id=1, sector_code="metalmeccanici_industria", name="CCNL Test", valid_from=date(2024, 1, 1)
            )
    ...
    async def test_get_ccnl_by_sector(self, ccnl_service):
        ...
        with patch("app.services.database.database_service.get_session_maker") as mock_session:
            ...
            results = await ccnl_service.get_ccnl_by_sector(CCNLSector.METALMECCANICI_INDUSTRIA, current_only=True)
    ...
    async def test_update_ccnl_agreement(self, ccnl_service):
        ...
        updates = {"name": "Updated CCNL Name", "valid_to": date(2027, 12, 31), "renewal_status": "vigente"}
        ...
        with patch("app.services.database.database_service.get_session_maker") as mock_session:
            ...
            mock_existing = CCNLAgreementDB(
                id=1, sector_code="metalmeccanici_industria", name="Old Name", valid_from=date(2024, 1, 1)
            )
        ...
        with patch.object(ccnl_service, "get_current_ccnl_by_sector") as mock_get_ccnl:
            ...
            def mock_ccnl_side_effect(sector):
                if sector == sector1:
                    ...
                    mock_agreement.leave_entitlements = [Mock(leave_type="ferie", base_annual_days=24)]
                    mock_agreement.salary_tables = [Mock(level_code="C1", base_monthly_salary=Decimal("1450.00"))]
                    return mock_agreement
                else:
                    mock_agreement = Mock()
                    mock_agreement.sector = sector2
                    mock_agreement.name = "CCNL Commercio"
                    mock_agreement.leave_entitlements = [Mock(leave_type="ferie", base_annual_days=22)]
                    mock_agreement.salary_tables = [Mock(level_code="C1", base_monthly_salary=Decimal("1350.00"))]
                    return mock_agreement

            mock_get_ccnl.side_effect = mock_ccnl_side_effect

            result = await ccnl_service.compare_ccnl_provisions(
                sector1=sector1, sector2=sector2, comparison_aspects=["leave_entitlements", "salary_tables"]
            )
...
class TestCCNLDataImportService:
    ...
    async def test_import_ccnl_from_external_source(self, ccnl_service):
        ...
        with patch.object(ccnl_service, "save_ccnl_agreement") as mock_save:
            ...
            result = await ccnl_service.import_ccnl_from_external_data(
                external_data, validate_data=True, overwrite_existing=False
            )
    ...
    async def test_bulk_import_ccnl_data(self, ccnl_service):
        ...
        bulk_data = [
            ...
            {"sector": "commercio_terziario", "agreement_name": "CCNL Commercio", "valid_from": "2024-01-01"},
            {
        ...
        with patch.object(ccnl_service, "save_ccnl_agreement") as mock_save:
            ...
            result = await ccnl_service.bulk_import_ccnl_data(bulk_data, validate_each=True, stop_on_error=False)
...
class TestCCNLAnalyticsService:
    ...
    async def test_analyze_ccnl_trends(self, ccnl_service):
        ...
        result = await ccnl_service.analyze_ccnl_trends(start_date=date(2020, 1, 1), end_date=date(2024, 12, 31))
