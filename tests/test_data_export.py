...
class TestDataCollection:
    ...
    async def test_complete_user_profile_export(self, sample_user_data, db_session):
        ...
        export_request = DataExportRequest(
            user_id=sample_user_data.id, format=ExportFormat.JSON, include_sensitive=True
        )
    ...
    async def test_document_metadata_export_no_content(self, sample_user_data, db_session):
        ...
        export_request = DataExportRequest(user_id=sample_user_data.id, format=ExportFormat.JSON)
    ...
    async def test_subscription_billing_history_export(self, sample_user_data, db_session):
        ...
        export_request = DataExportRequest(user_id=sample_user_data.id, format=ExportFormat.JSON, include_fatture=True)
...
class TestFormatAndStructure:
    ...
    async def test_encrypted_field_decryption_during_export(self):
        ...
        with patch.object(service, "_get_user", return_value=user):
            export_request = DataExportRequest(user_id=uuid4(), format=ExportFormat.JSON, include_sensitive=True)
    ...
    async def test_file_size_handling_zip_large_exports(self):
        ...
        DataExportService(Mock(), Mock(), Mock())
        ...
        if len(json_content) > 10 * 1024 * 1024:  # 10MB
            zip_content = await generator.create_zip_export({"dati_completi.json": json_content})
    ...
    async def test_export_manifest_generation_metadata(self):
        ...
        manifest = await service.generate_export_manifest(
            export_request,
            {"profile": {"email": "test@test.it"}, "queries": [{"id": "1"}] * 100, "documents": [{"id": "1"}] * 50},
        )
        ...
        assert manifest["privacy_info"]["includes_sensitive_data"] is True
...
class TestPrivacySecurity:
    ...
    async def test_pii_anonymization_options(self):
        ...
        export_request = DataExportRequest(user_id=uuid4(), format=ExportFormat.JSON, anonymize_pii=True)
    ...
    async def test_exclusion_other_users_data(self):
        ...
        with patch.object(service, "get_user_queries") as mock_queries:
            ...
            DataExportRequest(user_id=user_id, format=ExportFormat.JSON)
    ...
    async def test_export_request_authentication_authorization(self, db_session):
        ...
        export_request = await service.create_export_request(user_id=user_id, format=ExportFormat.JSON)
    ...
    async def test_rate_limiting_max_5_exports_per_day(self, db_session):
        ...
        with pytest.raises(ExportLimitExceeded) as exc_info:
            await service.create_export_request(user_id=user_id, format=ExportFormat.JSON)
    ...
    async def test_audit_logging_export_requests(self, db_session):
        ...
        with patch("app.core.logging.logger") as mock_logger:
            ...
            export_request = await service.create_export_request(
                user_id=user_id, format=ExportFormat.JSON, options={"include_sensitive": True}
            )

            # Verify audit logging
            mock_logger.info.assert_called()
            log_calls = [call for call in mock_logger.info.call_args_list if "Data export requested" in str(call)]
            ...
            completion_logs = [
                call for call in mock_logger.info.call_args_list if "Data export completed" in str(call)
            ]
...
class TestItalianCompliance:
    ...
    async def test_partita_iva_included_business_exports(self, db_session):
        ...
        business_user = User(id=uuid4(), email="azienda@test.it", is_business=True, partita_iva="12345678903")
        db_session.add(business_user)
        await db_session.commit()

        export_request = DataExportRequest(user_id=business_user.id, format=ExportFormat.JSON, include_sensitive=True)
        ...
        assert business_info["is_business"] is True
    ...
    async def test_italian_invoice_data_export(self, db_session):
        ...
        export_request = DataExportRequest(user_id=user_id, format=ExportFormat.JSON, include_fatture=True)
    ...
    async def test_italian_language_labels_exports(self):
        ...
        italian_headers = ["Data", "Ora", "Domanda", "Tempo Risposta", "Token Utilizzati", "Costo", "Da Cache"]
    ...
    async def test_compliance_italian_data_protection_laws(self):
        ...
        assert compliance_info["includes_personal_data"] is True
        assert compliance_info["includes_financial_data"] is True
...
class TestExportProgressTracking:
    ...
    async def test_export_progress_updates(self):
        ...
        await tracker.update_progress(export_id=export_id, step="Raccolta dati profilo", current=1, total=5)
