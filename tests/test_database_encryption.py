...
from sqlalchemy import BYTEA, Boolean, Column, DateTime, Integer, MetaData, String, Table, create_engine, text
...
class TestEncryptedDataStorage:
    ...
    async def test_encrypted_data_unreadable_from_disk(self, async_db_session, encryption_service):
        ...
        async with async_db_session as session:
            ...
            result = await session.execute(text("SELECT email FROM test_users WHERE id = :id"), {"id": "test_user_1"})
    ...
    async def test_store_key_in_database(self, async_db_session):
        ...
        async with async_db_session as session:
            await session.execute(
                ...
                """),
                {"version": key_version, "key": encrypted_key, "algorithm": "AES-256-CBC"},
            )
            await session.commit()
...
class TestKeyRotation:
    ...
    async def test_multiple_key_versions_coexist(self, encryption_service):
        ...
        for data, encrypted, version in zip(data_list, encrypted_data, versions, strict=False):
...
class TestPerformanceImpact:
    ...
    async def test_bulk_encryption_performance(self, encryption_service):
        ...
        for original, encrypted in zip(test_emails, encrypted_emails, strict=False):
            decrypted = await encryption_service.decrypt_field(encrypted)
...
class TestBackupAndRestore:
    ...
    async def test_backup_contains_encrypted_data(self, async_db_session, encryption_service):
        ...
        # Arrange
        sensitive_data = {"email": "mario.rossi@example.com", "tax_id": "RSSMRA80A01H501U", "phone": "+39 02 1234567"}
        ...
        async with async_db_session as session:
            ...
            for field in sensitive_data:
                result = await session.execute(
                    text(f"SELECT {field} FROM test_users WHERE id = :id"), {"id": f"test_{field}"}
                )
    ...
    async def test_restore_from_encrypted_backup(self, async_db_session, encryption_service):
        ...
        async with async_db_session as session:
            ...
            result = await session.execute(text("SELECT email FROM test_users WHERE id = :id"), {"id": "restore_test"})
    ...
    async def test_backup_without_master_key_unreadable(self, async_db_session, encryption_service):
        ...
        async with async_db_session as session:
            ...
            result = await session.execute(
                text("SELECT phone FROM test_users WHERE id = :id"), {"id": "confidential_test"}
            )
    ...
    async def test_key_storage_audit_logging(self, async_db_session):
        ...
        async with async_db_session as session:
            await session.execute(
                ...
                """),
                {"version": key_version, "op": operation, "table": table_name, "user": user_id},
            )
            await session.commit()
...
class TestKeyRotationMechanism:
    ...
    async def test_quarterly_key_rotation_schedule(self, encryption_service):
        ...
        last_rotation = datetime.now(UTC) - timedelta(days=91)  # Overdue
...
class TestKeyVersioning:
    ...
    async def test_key_versioning_tracks_historical_access(self, encryption_service):
        ...
        for _i, (data, encrypted, version) in enumerate(
            zip(historical_data, encrypted_historical, key_versions, strict=False)
        ):
            decrypted = await encryption_service.decrypt_field(encrypted, version)
            assert decrypted == data
        ...
        async with async_db_session as session:
            ...
            await session.execute(
                text("""
                    ...
                    (3, :key3, 1)
                """),
                {"key1": b"old_key_1", "key2": b"old_key_2", "key3": b"current_active_key"},
            )
            await session.commit()

            # Query active key
            result = await session.execute(text("SELECT key_version FROM encryption_keys WHERE is_active = 1"))
...
class TestTransparentEncryption:
    ...
    async def test_orm_transparent_encryption(self, encryption_service):
        ...
        # Arrange
        user_data = {"email": "test@example.com", "tax_id": "RSSMRA80A01H501U", "phone": "+39 02 1234567"}
    ...
    async def test_existing_queries_work_without_modification(self, async_db_session, encryption_service):
        ...
        async with async_db_session as session:
            ...
            result = await session.execute(
                text("SELECT id, email FROM test_users WHERE id = :id"), {"id": "query_test"}
            )
            row = result.fetchone()
    ...
    async def test_migration_of_existing_unencrypted_data(self, async_db_session, encryption_service):
        ...
        async with async_db_session as session:
            ...
            result = await session.execute(text("SELECT email FROM test_users WHERE id = :id"), {"id": "legacy_user"})
    ...
    async def test_rollback_procedures_if_encryption_fails(self, async_db_session, encryption_service):
        ...
        async with async_db_session as session:
            ...
            try:
                ...
                result = await session.execute(
                    text("SELECT email FROM test_users WHERE id = :id"), {"id": "rollback_test"}
                )
...
class TestItalianDataProtectionCompliance:
    ...
    async def test_all_pii_fields_encrypted(self, async_db_session, encryption_service):
        ...
        async with async_db_session as session:
            ...
            for field in italian_pii:
                result = await session.execute(
                    text(f"SELECT {field} FROM test_users WHERE id = :id"), {"id": f"pii_{field}"}
                )
    ...
    async def test_database_dumps_are_encrypted(self, async_db_session, encryption_service):
        ...
        # Arrange
        sensitive_data = {"email": "gdpr.test@example.com", "tax_id": "RSSMRA80A01H501U"}
        ...
        async with async_db_session as session:
            ...
            for field in sensitive_data:
                result = await session.execute(
                    text(f"SELECT {field} FROM test_users WHERE id = :id"), {"id": f"dump_{field}"}
                )
    ...
    async def test_gdpr_right_to_be_forgotten_with_encryption(self, async_db_session, encryption_service):
        ...
        async with async_db_session as session:
            ...
            await session.execute(text("DELETE FROM test_users WHERE id = :id"), {"id": user_id})
            await session.commit()

            # Verify deletion
            result = await session.execute(text("SELECT COUNT(*) FROM test_users WHERE id = :id"), {"id": user_id})
            ...
            for entry in audit_entries:
                await session.execute(
                    text("""
                        ...
                        VALUES (:version, :op, :table, :user)
                    """),
                    {"version": 1, "op": entry["operation"], "table": entry["table"], "user": entry["user_id"]},
                )
            await session.commit()
        ...
        async with async_db_session as session:
            for i, entry in enumerate(audit_entries):
                ...
                await session.execute(
                    ...
                    """),
                    {"op": f"test_operation_{i}", "ts": entry_time.isoformat(), "version": 1},
                )
            await session.commit()
