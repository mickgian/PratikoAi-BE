...
class TestDocumentUploadValidation:
    ...
    async def test_validate_csv_file_success(self, document_uploader):
        ...
        upload_file = UploadFile(
            filename="registro_iva.csv", file=io.BytesIO(csv_bytes), content_type="text/csv", size=len(csv_bytes)
        )
    ...
    async def test_validate_malicious_filename(self, document_uploader):
        ...
        for filename in malicious_filenames:
            upload_file = UploadFile(
                filename=filename, file=io.BytesIO(b"%PDF-1.4\n"), content_type="application/pdf", size=20
            )
    ...
    async def test_virus_scan_simulation(self, document_uploader):
        ...
        with patch.object(document_uploader, "_scan_for_viruses") as mock_scan:
            ...
            upload_file = UploadFile(
                filename="clean_file.pdf", file=io.BytesIO(b"%PDF-1.4\n"), content_type="application/pdf", size=20
            )

            validation_result = await document_uploader.validate_file(upload_file)

            assert validation_result["virus_scan"]["clean"] is True
...
class TestPDFDocumentProcessing:
    ...
    async def test_extract_structured_data_from_fattura(self, pdf_processor):
        ...
        document = Document(id=uuid4(), extracted_text=mock_text, file_type=DocumentType.PDF.value)
    ...
    async def test_extract_f24_data(self, pdf_processor):
        ...
        document = Document(id=uuid4(), extracted_text=f24_text, file_type=DocumentType.PDF.value)
    ...
    async def test_classify_italian_document_type(self, pdf_processor):
        ...
        for case in test_cases:
            document = Document(id=uuid4(), extracted_text=case["text"], file_type=DocumentType.PDF.value)
    ...
    async def test_handle_scanned_pdf_ocr(self, pdf_processor):
        ...
        with patch("pytesseract.image_to_string") as mock_ocr:
            ...
            with patch("PyPDF2.PdfReader") as mock_pdf:
                ...
                document = Document(
                    id=uuid4(), filename="scanned_fattura.pdf", file_type=DocumentType.PDF.value, file_size=100000
                )

                extraction_result = await pdf_processor.extract_text(document)

                assert extraction_result["success"] is True
                assert extraction_result["ocr_used"] is True
                assert "FATTURA ELETTRONICA" in extraction_result["text"]
...
class TestExcelCSVProcessing:
    ...
    async def test_process_excel_bilancio(self, excel_processor):
        ...
        with patch("openpyxl.load_workbook") as mock_workbook:
            ...
            mock_ws_sp.iter_rows.return_value = [
                [Mock(value=cell) for cell in row] for row in mock_balance_sheet["Stato Patrimoniale"]
            ]
            ...
            mock_ws_ce.iter_rows.return_value = [
                [Mock(value=cell) for cell in row] for row in mock_balance_sheet["Conto Economico"]
            ]

            mock_workbook.return_value.worksheets = [mock_ws_sp, mock_ws_ce]

            document = Document(
                id=uuid4(), filename="bilancio_2024.xlsx", file_type=DocumentType.EXCEL_XLSX.value, file_size=25000
            )
    ...
    async def test_process_csv_registro_iva(self, excel_processor):
        ...
        with patch("pandas.read_csv") as mock_read_csv:
            ...
            df = pd.DataFrame(
                df_data, columns=["Data", "Numero", "Cliente", "Imponibile", "Aliquota", "Imposta", "Totale"]
            )
            mock_read_csv.return_value = df

            document = Document(
                id=uuid4(), filename="registro_iva.csv", file_type=DocumentType.CSV.value, file_size=500
            )
    ...
    async def test_detect_csv_encoding_with_bom(self, excel_processor):
        ...
        document = Document(id=uuid4(), filename="test.csv", file_type=DocumentType.CSV.value)
        ...
        with patch("builtins.open", create=True) as mock_open:
            ...
            assert encoding_result["has_bom"] is True
...
class TestDocumentAnalysisAI:
    ...
    async def test_analyze_fattura_compliance(self, doc_analyzer):
        ...
        with patch.object(doc_analyzer, "_analyze_with_llm") as mock_llm:
            ...
            analysis_result = await doc_analyzer.analyze_document(
                document_data=fattura_data, query=query, analysis_type="compliance_check"
            )
    ...
    async def test_analyze_bilancio_financial_health(self, doc_analyzer):
        ...
        bilancio_data = {
            ...
            "conto_economico": {"ricavi": 200000, "costi": 180000, "ebitda": 20000, "utile_netto": 12000},
        }
        ...
        with patch.object(doc_analyzer, "_analyze_with_llm") as mock_llm:
            mock_llm.return_value = {
                "analysis": {
                    ...
                    "health_score": 78,
                    "key_ratios": {"leverage_ratio": 0.5, "roe": 0.12, "ebitda_margin": 0.10},
            ...
            analysis_result = await doc_analyzer.analyze_document(
                document_data=bilancio_data, query=query, analysis_type="financial_analysis"
            )
    ...
    async def test_multi_document_comparison(self, doc_analyzer):
        ...
        documents_data = [
            {"filename": "bilancio_2023.xlsx", "data": {"utile_netto": 10000, "ricavi": 180000}},
            {"filename": "bilancio_2024.xlsx", "data": {"utile_netto": 12000, "ricavi": 200000}},
        ]
        ...
        with patch.object(doc_analyzer, "_analyze_with_llm") as mock_llm:
            ...
            comparison_result = await doc_analyzer.compare_documents(documents_data=documents_data, query=query)
    ...
    async def test_extract_actionable_insights(self, doc_analyzer):
        ...
        with patch.object(doc_analyzer, "_analyze_with_llm") as mock_llm:
            ...
            insights_result = await doc_analyzer.generate_insights(document_data=f24_data, query=query)
...
class TestSecureDocumentStorage:
    ...
    async def test_store_document_encrypted(self, secure_storage):
        ...
        document = Document(
            id=uuid4(), filename="test_doc.pdf", file_type=DocumentType.PDF.value, file_size=len(document_content)
        )

        storage_result = await secure_storage.store_document(document=document, content=document_content)
    ...
    async def test_retrieve_document_decrypted(self, secure_storage):
        ...
        with patch.object(secure_storage, "_encrypt_content") as mock_encrypt:
            ...
            with patch.object(secure_storage, "_decrypt_content") as mock_decrypt:
                ...
                assert retrieval_result["decrypted"] is True
    ...
    async def test_gdpr_compliant_deletion(self, secure_storage):
        ...
        document_id = uuid4()
        document = Document(id=document_id, filename="gdpr_test.pdf", user_id=uuid4())

        deletion_result = await secure_storage.gdpr_delete_document(
            document_id=document_id, user_id=document.user_id, reason="user_request"
        )
        ...
        assert deletion_result["gdpr_compliant"] is True
