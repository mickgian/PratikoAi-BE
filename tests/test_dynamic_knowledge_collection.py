...
class TestRSSFeedMonitor:
    ...
    async def test_agenzia_entrate_feed_parsing(self):
        ...
        <rss version="2.0">
            <channel>
                <title>Agenzia delle Entrate - Circolari</title>
                <item>
                    <title>Circolare n. 15/E del 25 luglio 2025</title>
                    <link>https://www.agenziaentrate.gov.it/portale/documents/20143/5501356/Circolare_15E_2025.pdf</link>
                    <description>IVA su servizi digitali - Nuove disposizioni per operatori B2B</description>
                    <pubDate>Thu, 25 Jul 2025 10:00:00 GMT</pubDate>
                    <guid>https://www.agenziaentrate.gov.it/portale/documents/20143/5501356/Circolare_15E_2025.pdf</guid>
                </item>
                <item>
                    <title>Circolare n. 14/E del 20 luglio 2025</title>
                    <link>https://www.agenziaentrate.gov.it/portale/documents/20143/5501356/Circolare_14E_2025.pdf</link>
                    <description>Dichiarazione dei redditi 2025 - Novità e chiarimenti</description>
                    <pubDate>Mon, 20 Jul 2025 14:30:00 GMT</pubDate>
                    <guid>https://www.agenziaentrate.gov.it/portale/documents/20143/5501356/Circolare_14E_2025.pdf</guid>
                </item>
            </channel>
        </rss>"""
        ...
        with patch('feedparser.parse') as mock_feedparser:
            ...
            assert results[0]['url'] == "https://www.agenziaentrate.gov.it/portale/documents/20143/5501356/Circolare_15E_2025.pdf"
    ...
    async def test_inps_feed_parsing(self):
        ...
        <rss version="2.0">
            <channel>
                <title>INPS - Circolari</title>
                <item>
                    <title>Circolare INPS n. 82 del 30 luglio 2025</title>
                    <link>https://www.inps.it/bussola/VisualizzaDoc.aspx?sVirtualURL=/Circolari/Circolare numero 82 del 30-07-2025.htm</link>
                    <description>Pensioni di vecchiaia - Nuovi requisiti e modalità di calcolo</description>
                    <pubDate>Tue, 30 Jul 2025 16:00:00 GMT</pubDate>
                </item>
            </channel>
        </rss>"""
    ...
    async def test_content_extraction_pdf(self):
        ...
        with patch('app.services.document_processor.extract_pdf_content') as mock_extract:
            ...
            content = await processor.extract_content_from_pdf(
    ...
    async def test_content_extraction_html(self):
        ...
        mock_html_content = """
        <html>
        <head><title>INPS Circolare</title></head>
        <body>
            <div class="content">
                <h1>Circolare INPS n. 82 del 30 luglio 2025</h1>
                <h2>Pensioni di vecchiaia - Nuovi requisiti</h2>
                <p>Con la presente circolare si comunica che...</p>
                <ul>
                    <li>Età minima: 67 anni</li>
                    <li>Contributi minimi: 20 anni</li>
                </ul>
            </div>
        </body>
        </html>
        ...
        with patch('app.services.document_processor.extract_html_content') as mock_extract:
            ...
            processor = DocumentProcessor()
            content = await processor.extract_content_from_html(
    ...
    async def test_knowledge_base_update(self, db_session: AsyncSession):
        ...
        query = select(RegulatoryDocument).where(
            RegulatoryDocument.url == document_data['url']
        )
        ...
        query = select(KnowledgeItem).where(
            KnowledgeItem.source_url == document_data['url']
        )
    ...
    async def test_cache_invalidation(self):
        ...
        with patch.object(cache_service, 'clear_cache') as mock_clear_cache:
            ...
            await integrator.invalidate_relevant_caches(
                topics=['IVA', 'servizi digitali'],
                source='agenzia_entrate'
            )
            ...
            expected_calls = [
                ('search:*iva*',),
                ('search:*servizi*digitali*',),
                ('search:*agenzia*entrate*',),
                ('suggestions:*',)
            ]
    ...
    async def test_parallel_feed_processing(self):
        ...
        with patch('app.services.rss_feed_monitor.RSSFeedMonitor.parse_feed') as mock_parse:
            mock_parse.return_value = [
                {'title': 'Test Document', 'url': 'https://example.com/test.pdf'}
            ]
    ...
    async def test_error_handling_failed_feeds(self, db_session: AsyncSession):
        ...
        with patch('feedparser.parse') as mock_feedparser:
            ...
            results = await monitor.parse_feed_with_error_handling(
        ...
        feed_tracker = FeedStatus(
            feed_url='https://test-feed.com/rss.xml',
        ...
        query = select(FeedStatus).where(
            FeedStatus.feed_url == 'https://test-feed.com/rss.xml'
        )
    ...
    async def test_document_version_tracking(self, db_session: AsyncSession):
        ...
        original_doc = RegulatoryDocument(
            ...
            content_hash=hashlib.sha256("Contenuto originale".encode()).hexdigest(),
            version=1
        )
        ...
        query = select(RegulatoryDocument).where(
            RegulatoryDocument.url == updated_data['url'],
            RegulatoryDocument.version == 2
        )
    ...
    async def test_feed_history_maintenance(self, db_session: AsyncSession):
        ...
        old_status = FeedStatus(
            feed_url='https://old-feed.com/rss.xml',
            last_checked=old_date,
            last_success=old_date,
            status='success'
        )
...
class TestDynamicKnowledgeCollectorIntegration:
    ...
    async def test_end_to_end_document_collection(self, db_session: AsyncSession):
        ...
        with patch('feedparser.parse') as mock_feedparser, \
            ...
            query = select(RegulatoryDocument).where(
                RegulatoryDocument.title.contains("Circolare n. 16/E")
            )
            result = await db_session.execute(query)
