...
from app.models.faq_automation import (
    FAQCandidate, GeneratedFAQ, FAQPattern, RSSImpact, QueryCluster
)
from app.services.query_pattern_analyzer import (
    QueryPatternAnalyzer,
    InsufficientDataError,
    ClusteringFailedError
)
from app.services.auto_faq_generator import (
    AutomatedFAQGenerator,
    QualityValidationError,
    GenerationFailedError
)
from app.services.faq_rss_integration import (
    FAQRSSIntegration,
    RSSUpdateProcessor,
    FAQImpactAssessor
)
from app.services.faq_quality_validator import (
    FAQQualityValidator,
    QualityMetrics
)
...
class TestFAQGeneration:
    ...
    async def test_basic_faq_generation(self):
        ...
        # Mock LLM response
        generator.llm.complete_cheap = AsyncMock(return_value='''
        {
    ...
    async def test_quality_threshold_enforcement(self):
        ...
        generator.llm.complete_cheap = AsyncMock(return_value='{"question": "Test?", "answer": "Basic answer", "category": "Test", "additional_tags": []}')
        generator.validator.validate_faq = AsyncMock(return_value=0.70)  # Below threshold
        
        # Mock high-quality retry with GPT-4
        generator.llm.complete_expensive = AsyncMock(return_value='{"question": "Test question?", "answer": "Detailed professional answer", "category": "Test", "additional_tags": ["detailed"]}')
    ...
    async def test_regulatory_reference_extraction(self):
        ...
        generator.llm.complete_cheap = AsyncMock(return_value='{"question": "Quali sono le detrazioni per spese mediche?", "answer": "Le spese mediche sono detraibili al 19%...", "category": "Detrazioni", "additional_tags": ["sanita"]}')
...
class TestRSSIntegration:
    ...
    async def test_faq_history_preservation(self):
        ...
        rss_update = {
            "id": uuid4(),
            "title": "Nuove aliquote IVA",
            "summary": "Cambio aliquote IVA dal 23%"
        }
        
        # Mock LLM response for update
        integration.llm.complete_expensive = AsyncMock(return_value='''
        {
            "question": "Come si calcola l'IVA?",
...
class TestSystemIntegration:
    ...
    async def test_end_to_end_faq_generation_workflow(self, sample_query_logs):
        ...
        generator.llm.complete_cheap = AsyncMock(return_value='{"question": "Come calcolare IVA?", "answer": "IVA = importo × 0.22", "category": "IVA", "additional_tags": ["calcolo"]}')
        generator.validator.validate_faq = AsyncMock(return_value=0.90)
        generator._extract_regulatory_references = AsyncMock(return_value=[])
    ...
    async def test_cost_tracking_accuracy(self):
        ...
        await cost_tracker.track_faq_generation(
            faq_id=uuid4(),
            candidate_id=uuid4(),
    ...
    async def test_concurrent_faq_generation_jobs(self):
        ...
        candidates = [
            FAQCandidate(id=uuid4(), canonical_query=f"query_{i}") 
            for i in range(5)
        ]
...
class TestBusinessLogic:
    ...
    async def test_priority_scoring_algorithm(self):
        ...
        high_priority = FAQCandidate(
            frequency=15,
            avg_cost_cents=200,
            quality_score=0.95,
            total_cost_saved=20.0
        )
        
        # Low frequency, low cost candidate  
        low_priority = FAQCandidate(
            frequency=5,
            avg_cost_cents=50,
            quality_score=0.85,
            total_cost_saved=2.0
        )
    ...
    async def test_prevention_of_duplicate_faq_generation(self):
        ...
        candidate1 = FAQCandidate(
            canonical_query="calcolo iva 22 percento",
            query_variations=["Come calcolare IVA 22%?"]
        )
        
        candidate2 = FAQCandidate(
            canonical_query="calcolo iva aliquota ordinaria", 
            query_variations=["Come calcolare IVA ordinaria?"]
        )
    ...
    async def test_seasonal_query_handling(self):
        ...
        seasonal_analyzer.detect_seasonal_pattern = Mock(return_value={
            "is_seasonal": True,
            "peak_months": [3, 4, 5],
    ...
    async def test_multi_language_query_clustering(self):
        ...
        multilang_analyzer.normalize_to_standard_italian = Mock(side_effect=lambda x: {
    ...
    async def test_cost_optimization_over_time(self):
        ...
        optimizer.analyze_cost_trend = Mock(return_value={
            "trend": "decreasing",
...
class TestPerformanceMonitoring:
    ...
    async def test_system_behavior_during_rss_update_floods(self):
        ...
        rss_flood = [
            {"id": uuid4(), "timestamp": datetime.utcnow() - timedelta(minutes=i*3)}
            for i in range(20)
        ]
    ...
    async def test_quality_metrics_tracking(self):
        ...
        quality_tracker.calculate_trend = Mock(return_value={
            "trend": "improving",
            "weekly_average": 0.90,
