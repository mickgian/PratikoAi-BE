...
class TestGDPRDeletionRequest:
    ...
    async def test_create_deletion_request_user_initiated(self, db_session: AsyncSession):
        ...
        request = await gdpr_service.create_deletion_request(
            user_id=user.id, initiated_by_user=True, reason="User requested account deletion"
        )

        assert request.user_id == user.id
        assert request.status == DeletionStatus.PENDING
        assert request.initiated_by_user is True
        assert request.deletion_deadline <= datetime.now(UTC) + timedelta(days=30)
    ...
    async def test_create_deletion_request_admin_initiated(self, db_session: AsyncSession):
        ...
        # Create test user
        user = EncryptedUser(email="inactive.user@example.com", hashed_password="test_hash", is_active=False)
        db_session.add(user)
        await db_session.commit()
        await db_session.refresh(user)
        ...
        assert request.initiated_by_user is False
    ...
    async def test_deletion_request_validation_nonexistent_user(self, db_session: AsyncSession):
        ...
        with pytest.raises(ValueError, match="User with ID 99999 does not exist"):
            await gdpr_service.create_deletion_request(user_id=99999, initiated_by_user=True, reason="Test")
    ...
    async def test_deletion_request_validation_already_pending(self, db_session: AsyncSession):
        ...
        # Create test user
        user = EncryptedUser(email="duplicate.test@example.com", hashed_password="test_hash")
        db_session.add(user)
        await db_session.commit()
        await db_session.refresh(user)

        # Create first deletion request
        await gdpr_service.create_deletion_request(user_id=user.id, initiated_by_user=True, reason="First request")
        ...
        with pytest.raises(ValueError, match="Deletion request already exists"):
            await gdpr_service.create_deletion_request(
                user_id=user.id, initiated_by_user=True, reason="Duplicate request"
            )
...
class TestGDPRDeletionScheduling:
    ...
    async def test_deletion_deadline_calculation(self, db_session: AsyncSession):
        ...
        # Create test user
        user = EncryptedUser(email="deadline.test@example.com", hashed_password="test_hash")
        db_session.add(user)
        await db_session.commit()
        await db_session.refresh(user)
        ...
        request = await gdpr_service.create_deletion_request(
            user_id=user.id, initiated_by_user=True, reason="Test deadline calculation"
        )
    ...
    async def test_get_overdue_deletion_requests(self, db_session: AsyncSession):
        ...
        overdue_deadline = datetime.now(UTC) - timedelta(days=1)
        future_deadline = datetime.now(UTC) + timedelta(days=29)

        request1 = await gdpr_service.create_deletion_request(
            user_id=user1.id, initiated_by_user=True, reason="Overdue test 1"
        )
        request1.deletion_deadline = overdue_deadline

        request2 = await gdpr_service.create_deletion_request(
            user_id=user2.id, initiated_by_user=True, reason="Overdue test 2"
        )
        request2.deletion_deadline = overdue_deadline

        request3 = await gdpr_service.create_deletion_request(
            user_id=user3.id, initiated_by_user=True, reason="Not due yet"
        )
    ...
    async def test_automatic_deletion_execution(self, db_session: AsyncSession):
        ...
        request = await gdpr_service.create_deletion_request(
            user_id=user.id, initiated_by_user=True, reason="Automatic deletion test"
        )
        request.deletion_deadline = datetime.now(UTC) - timedelta(days=1)
        await db_session.commit()
...
class TestUserDataIdentification:
    ...
    async def test_identify_user_data_in_all_tables(self, db_session: AsyncSession):
        ...
        session = Session(
            user_id=user.id, session_token="test_session_token", expires_at=datetime.now(UTC) + timedelta(hours=24)
        )
    ...
    async def test_identify_cascading_relationships(self, db_session: AsyncSession):
        ...
        user = EncryptedUser(email="cascade.test@example.com", hashed_password="test_hash")
        db_session.add(user)
        await db_session.commit()
        await db_session.refresh(user)
        ...
        for i in range(3):
            session = Session(
                user_id=user.id, session_token=f"session_token_{i}", expires_at=datetime.now(UTC) + timedelta(hours=24)
            )
...
class TestActualDataDeletion:
    ...
    async def test_delete_user_data_complete_removal(self, db_session: AsyncSession):
        ...
        # Create related data
        session = Session(
            user_id=user.id, session_token="delete_session_token", expires_at=datetime.now(UTC) + timedelta(hours=24)
        )
        db_session.add(session)
        await db_session.commit()
        ...
        session_check = await db_session.execute(select(Session).where(Session.user_id == user.id))
    ...
    async def test_delete_user_data_preserves_audit_trail(self, db_session: AsyncSession):
        ...
        # Create user
        user = EncryptedUser(
            email="audit.preserve@example.com", hashed_password="test_hash", tax_id="AUDPRS80A01H501U"
        )
        db_session.add(user)
        await db_session.commit()
        await db_session.refresh(user)
        ...
        deletion_result = await data_deletor.delete_user_data(user.id, preserve_audit_trail=True)
    ...
    async def test_delete_user_data_handles_large_datasets(self, db_session: AsyncSession):
        ...
        user = EncryptedUser(email="large.dataset@example.com", hashed_password="test_hash")
        db_session.add(user)
        await db_session.commit()
        await db_session.refresh(user)
        ...
        for i in range(1000):  # Large dataset
            session = Session(
                user_id=user.id, session_token=f"large_session_{i}", expires_at=datetime.now(UTC) + timedelta(hours=24)
            )
            sessions.append(session)
        ...
        remaining_sessions = await db_session.execute(select(func.count(Session.id)).where(Session.user_id == user.id))
...
class TestMultiSystemDeletion:
    ...
    async def test_delete_from_application_logs(self, db_session: AsyncSession):
        ...
        with patch("app.services.log_anonymizer") as mock_anonymizer:
            mock_anonymizer.anonymize_user_logs = AsyncMock(
                return_value={
    ...
    async def test_delete_from_stripe_integration(self, db_session: AsyncSession):
        ...
        with patch("stripe.Customer.delete") as mock_stripe_delete:
            ...
            stripe_result = await data_deletor.delete_from_stripe(stripe_customer_id="cus_test123")

            assert stripe_result.success is True
            assert stripe_result.stripe_customer_deleted is True
    ...
    async def test_delete_from_backup_systems(self, db_session: AsyncSession):
        ...
        with patch("app.services.backup_manager") as mock_backup:
            mock_backup.anonymize_user_in_backups = AsyncMock(
                return_value={
...
class TestDeletionVerification:
    ...
    async def test_verify_complete_deletion(self, db_session: AsyncSession):
        ...
        user = EncryptedUser(
            email="verify.deletion@example.com", hashed_password="test_hash", tax_id="VERDEL80A01H501U"
        )
        db_session.add(user)
        await db_session.commit()
        await db_session.refresh(user)
        ...
        assert verification_result.is_completely_deleted is True
        ...
        assert verification_result.gdpr_compliant is True
    ...
    async def test_verify_deletion_finds_remaining_data(self, db_session: AsyncSession):
        ...
        user = EncryptedUser(email="incomplete.deletion@example.com", hashed_password="test_hash")
        db_session.add(user)
        await db_session.commit()
        await db_session.refresh(user)
        ...
        assert verification_result.is_completely_deleted is False
        assert len(verification_result.remaining_data_found) > 0
        assert verification_result.verification_score < 100.0
        assert verification_result.gdpr_compliant is False

        # Check specific remaining data
        assert any(data["table"] == "users" for data in verification_result.remaining_data_found)
    ...
    async def test_generate_deletion_certificate(self, db_session: AsyncSession):
        ...
        verification_result.verification_timestamp = datetime.now(UTC)
        ...
        assert certificate.is_complete_deletion is True
        assert certificate.certificate_id is not None
        assert certificate.issued_at is not None
        assert certificate.compliance_attestation is True
    ...
    async def test_verify_multi_system_deletion(self, db_session: AsyncSession):
        ...
        with patch.multiple(
            ...
            assert verification_result.database_deletion_verified is True
            assert verification_result.redis_deletion_verified is True
            assert verification_result.log_anonymization_verified is True
            assert verification_result.backup_anonymization_verified is True
            assert verification_result.stripe_deletion_verified is True
            assert verification_result.overall_verification_passed is True
...
class TestGDPRComplianceReporting:
    ...
    async def test_generate_deletion_compliance_report(self, db_session: AsyncSession):
        ...
        for i in range(10):
            user = EncryptedUser(email=f"report.user{i}@example.com", hashed_password="test_hash")
            db_session.add(user)
            await db_session.commit()
            await db_session.refresh(user)
            ...
            # Create deletion request
            await gdpr_service.create_deletion_request(
                user_id=user.id, initiated_by_user=i % 2 == 0, reason=f"Test deletion {i}"
            )
    ...
    async def test_deadline_compliance_monitoring(self, db_session: AsyncSession):
        ...
        await gdpr_service.create_deletion_request(user1.id, True, "Within deadline")
        ...
        req2.deletion_deadline = datetime.now(UTC) + timedelta(days=2)  # Approaching
        req3.deletion_deadline = datetime.now(UTC) - timedelta(days=1)  # Overdue
        await db_session.commit()
...
class TestGDPRDeletionIntegration:
    ...
    async def test_complete_gdpr_deletion_workflow(self, db_session: AsyncSession):
        ...
        # Create related data
        session = Session(
            user_id=user.id, session_token="workflow_session", expires_at=datetime.now(UTC) + timedelta(hours=24)
        )
        db_session.add(session)
        await db_session.commit()
        ...
        deletion_request = await gdpr_service.create_deletion_request(
            user_id=user.id, initiated_by_user=True, reason="Complete workflow test"
        )
        ...
        deletion_request.deletion_deadline = datetime.now(UTC) - timedelta(hours=1)
        await db_session.commit()
    ...
    async def test_gdpr_deletion_performance_requirements(self, db_session: AsyncSession):
        ...
        user = EncryptedUser(email="performance.test@example.com", hashed_password="test_hash")
        db_session.add(user)
        await db_session.commit()
        await db_session.refresh(user)
        ...
        for i in range(500):
            session = Session(
                user_id=user.id, session_token=f"perf_session_{i}", expires_at=datetime.now(UTC) + timedelta(hours=24)
            )
            sessions.append(session)
        ...
        await gdpr_service.create_deletion_request(user_id=user.id, initiated_by_user=True, reason="Performance test")
        ...
        remaining_count = await db_session.execute(select(func.count(Session.id)).where(Session.user_id == user.id))
...
async def sample_user_with_data(db_session: AsyncSession):
    ...
    # Create related data
    session = Session(
        user_id=user.id, session_token="sample_session", expires_at=datetime.now(UTC) + timedelta(hours=24)
    )
    db_session.add(session)
    await db_session.commit()
...
class TestGDPRDeletionPerformance:
    ...
    async def test_concurrent_deletion_requests(self, db_session: AsyncSession):
        ...
        users = []
        for i in range(10):
            user = EncryptedUser(email=f"concurrent{i}@example.com", hashed_password="test_hash")
            users.append(user)
        ...
        for user in users:
            task = gdpr_service.create_deletion_request(
                user_id=user.id, initiated_by_user=True, reason=f"Concurrent test {user.id}"
            )
            tasks.append(task)
    ...
    async def test_batch_deletion_processing(self, db_session: AsyncSession):
        ...
        users = []
        for i in range(20):
            user = EncryptedUser(email=f"batch{i}@example.com", hashed_password="test_hash")
            users.append(user)
        ...
        for user in users:
            request = await gdpr_service.create_deletion_request(
                user_id=user.id, initiated_by_user=True, reason=f"Batch test {user.id}"
            )
            request.deletion_deadline = datetime.now(UTC) - timedelta(days=1)
