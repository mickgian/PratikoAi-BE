...
class TestGoldenEpochRule:
    ...
    def test_serve_golden_when_kb_epoch_equal(self):
        ...
        can_serve = self.service.can_serve_from_golden(confidence=0.95, kb_epoch=100, golden_epoch=100)

        assert can_serve is True

    def test_serve_golden_when_kb_epoch_older(self):
        """Test that Golden is served when kb_epoch < golden_epoch."""
        can_serve = self.service.can_serve_from_golden(confidence=0.95, kb_epoch=99, golden_epoch=100)

        assert can_serve is True

    def test_reject_golden_when_kb_epoch_newer(self):
        """Test that Golden is rejected when kb_epoch > golden_epoch."""
        can_serve = self.service.can_serve_from_golden(confidence=0.95, kb_epoch=101, golden_epoch=100)
    ...
    def test_zero_epochs(self):
        ...
        can_serve = self.service.can_serve_from_golden(confidence=0.95, kb_epoch=0, golden_epoch=0)
