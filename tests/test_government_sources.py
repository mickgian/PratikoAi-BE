...
class TestGovernmentSourcesSearch:
    ...
    async def test_sector_determination_from_content(self):
        ...
        assert source._determine_sector_from_content("CCNL Metalmeccanici Industria") == CCNLSector.METALMECCANICI_INDUSTRIA
        assert source._determine_sector_from_content("Contratto Edilizia e Costruzioni") == CCNLSector.EDILIZIA_INDUSTRIA
        assert source._determine_sector_from_content("Accordo Commercio al Dettaglio") == CCNLSector.COMMERCIO_TERZIARIO
...
class TestGovernmentSourcesPriority:
    ...
    async def test_government_sources_high_priority(self):
        ...
        for source in sources:
            ...
            if source.source_info.source_id == "ministry_labor":
                assert source.source_info.reliability_score >= 0.98
            elif source.source_info.source_id == "cnel":
                assert source.source_info.reliability_score >= 0.98
