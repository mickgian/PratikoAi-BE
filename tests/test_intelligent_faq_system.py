...
from app.models.faq import FAQCategory, FAQUsageLog, UpdateSensitivity
from app.models.faq import FAQEntry as FAQEntryModel
...
class TestFAQEntryCreation:
    ...
    def test_create_faq_entry_with_all_required_fields(self, faq_service):
        ...
        faq_request = FAQCreateRequest(
            ...
            language="it",
            regulatory_refs=[{"source": "DPR 633/72", "article": "1", "description": "Disciplina IVA"}],
            update_sensitivity="medium",
        )

        # Mock database creation
        faq_service.db.create_faq_entry = Mock(
            return_value=FAQEntryModel(
                id=str(uuid4()),
    ...
    def test_create_faq_entry_with_minimal_required_fields(self, faq_service):
        ...
        faq_request = FAQCreateRequest(
            question="Cosa è l'IRPEF?", answer="L'IRPEF è l'imposta sui redditi delle persone fisiche."
        )
    ...
    def test_create_faq_entry_validation_error(self, faq_service):
        ...
        with pytest.raises(ValueError, match="Question cannot be empty"):
            faq_request = FAQCreateRequest(question="", answer="Valid answer")
        ...
        with pytest.raises(ValueError, match="Answer cannot be empty"):
            faq_request = FAQCreateRequest(question="Valid question?", answer="")
            faq_service.create_faq_entry(faq_request)
...
class TestSemanticSearchMatching:
    ...
    async def test_semantic_search_with_query_normalization(self, faq_service):
        ...
        faq_service.query_normalizer.normalize = Mock(
            return_value=Mock(normalized_query="iva servizi digitali aliquota")
        )
    ...
    async def test_semantic_search_multiple_matches_returns_best(self, faq_service):
        ...
        mock_faqs = [
            ...
            FAQEntryModel(
                id=str(uuid4()), question="IMU calcolo rata?", answer="IMU rata si calcola...", similarity_score=0.87
            ),
        ]
...
class TestSharedKnowledgeBaseAccess:
    ...
    async def test_concurrent_faq_access_by_multiple_users(self, faq_service):
        ...
        tasks = [faq_service.find_best_match(query, user_id=user_id) for user_id in user_ids]
    ...
    def test_faq_popularity_ranking(self, faq_service):
        ...
        mock_faqs = [
            FAQEntryModel(id=str(uuid4()), question="Come calcolare IVA?", hit_count=150, avg_helpfulness=4.2),
            FAQEntryModel(id=str(uuid4()), question="Quando pagare F24?", hit_count=89, avg_helpfulness=4.5),
            FAQEntryModel(id=str(uuid4()), question="Calcolo IRPEF dipendenti?", hit_count=200, avg_helpfulness=4.1),
        ]
...
class TestFAQVersioningAndUpdateTracking:
    ...
    def test_create_faq_version_on_update(self, faq_service):
        ...
        update_request = FAQUpdateRequest(
            answer="IVA = importo × aliquota (es. 0.22 per 22%)", tags=["IVA", "calcolo", "aliquota"]
        )
    ...
    def test_track_faq_last_validated_date(self, faq_service):
        ...
        faq_id = str(uuid4())
        validation_date = datetime.now(UTC).date()
        ...
        faq_service.db.update_faq_validation.assert_called_once_with(
            faq_id, last_validated=validation_date, needs_review=False
        )
...
class TestGPTVariationIntegration:
    ...
    async def test_variation_avoids_recently_used_phrases(self, faq_service):
        ...
        faq_service.db.get_recent_variations_for_user = AsyncMock(return_value=recent_variations)
        ...
        faq_service.db.get_recent_variations_for_user.assert_called_once_with(user_id, days=7)
    ...
    async def test_variation_cost_tracking(self, faq_service):
        ...
        faq_service.db.log_variation_cost.assert_called_once_with(
            user_id=user_id, cost_euros=0.0003, model_used="gpt-3.5-turbo", tokens_used=result.tokens_used
        )
...
class TestVariationCacheSystem:
    ...
    async def test_variation_cache_hit_avoids_llm_call(self, faq_service):
        ...
        assert not hasattr(faq_service.llm, "generate_variation") or not faq_service.llm.generate_variation.called
    ...
    async def test_cache_key_generation_for_variations(self, faq_service):
        ...
        faq_service.cache.generate_variation_cache_key = Mock(return_value=f"faq_variation:{faq_id}:{user_id}:v1")

        await faq_service.get_or_generate_variation(faq_id, user_id)

        # Verify cache key was generated properly
        faq_service.cache.generate_variation_cache_key.assert_called_once_with(faq_id, user_id)
...
class TestObsolescenceDetection:
    ...
    async def test_check_faq_against_recent_updates(self, faq_service):
        ...
        faq = FAQEntryModel(
            ...
            last_validated=datetime.now(UTC).date() - timedelta(days=10),
        ...
        faq_service.knowledge.get_recent_updates = AsyncMock(return_value=[recent_update])
    ...
    async def test_faq_not_obsolete_no_relevant_updates(self, faq_service):
        ...
        faq = FAQEntryModel(
            ...
            last_validated=datetime.now(UTC).date() - timedelta(days=5),
            update_sensitivity="medium",
        )
        ...
        faq_service.knowledge.get_recent_updates = AsyncMock(return_value=recent_updates)
        faq_service.knowledge.check_content_overlap = AsyncMock(
    ...
    async def test_automatic_faq_invalidation_on_detection(self, faq_service):
        ...
        faq_service.db.mark_faq_needs_review.assert_called_once_with(
            faq_id, reason="Potentially obsolete due to regulatory changes", confidence_score=0.92
        )
    ...
    async def test_fallback_to_full_llm_when_faq_outdated(self, faq_service):
        ...
        outdated_faq = FAQEntryModel(
            ...
            last_validated=datetime.now(UTC).date() - timedelta(days=30),
        )
    ...
    def test_update_sensitivity_levels(self, faq_service):
        ...
        high_sensitivity_faq = FAQEntryModel(
            update_sensitivity="high", last_validated=datetime.now(UTC).date() - timedelta(days=1)
        )

        # Medium sensitivity - check weekly
        medium_sensitivity_faq = FAQEntryModel(
            update_sensitivity="medium", last_validated=datetime.now(UTC).date() - timedelta(days=5)
        )

        # Low sensitivity - check monthly
        low_sensitivity_faq = FAQEntryModel(
            update_sensitivity="low", last_validated=datetime.now(UTC).date() - timedelta(days=20)
        )
...
class TestUsageAnalytics:
    ...
    async def test_collect_user_feedback(self, faq_service):
        ...
        feedback_request = FAQFeedbackRequest(
            usage_log_id=usage_log_id, was_helpful=True, followup_needed=False, comments="Very clear explanation"
        )
...
class TestPerformanceRequirements:
    ...
    async def test_faq_search_performance_under_100ms(self, faq_service):
        ...
        mock_faq = FAQEntryModel(
            id=str(uuid4()), question="Come si calcola l'IVA?", answer="IVA = importo × 0.22", similarity_score=0.91
        )
    ...
    async def test_concurrent_faq_database_access(self, faq_service):
        ...
        mock_faqs = [
            FAQEntryModel(
                id=str(uuid4()), question=f"FAQ for: {query}", answer=f"Answer for: {query}", similarity_score=0.90
            )
    ...
    async def test_variation_cache_performance(self, faq_service):
        ...
        # Mock cached variation
        cached_variation = {"variation_text": "Cached response", "created_at": datetime.now(UTC), "cost_euros": 0.0003}
    ...
    async def test_overall_response_time_under_500ms(self, faq_service):
        ...
        faq_service.get_or_generate_variation = AsyncMock(
            return_value=VariationResponse(
