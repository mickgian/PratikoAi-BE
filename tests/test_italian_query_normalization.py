...
from app.services.italian_query_normalizer import (
    ItalianQueryNormalizer,
    NormalizationResult,
    NormalizationStats
)
...
class TestAccentedCharacterHandling:
    ...
    def test_accent_normalization_preserves_meaning(self, normalizer):
        ...
        for input_query, expected_normalized in test_cases:
            ...
            assert any('à' in result.normalized_query or 'è' in result.normalized_query or 
                      'ì' in result.normalized_query or 'ò' in result.normalized_query or 
                      'ù' in result.normalized_query for _ in [None])
    ...
    def test_accent_variations_normalization(self, normalizer):
        ...
        for input_query, expected_normalized in test_cases:
            ...
            assert expected_normalized in result.normalized_query or \
                   result.normalized_query.replace(expected_normalized.replace('à', 'a')
                                                                     .replace('è', 'e')
                                                                     .replace('ì', 'i')
                                                                     .replace('ò', 'o')
                                                                     .replace('ù', 'u'), 
                                                   expected_normalized) == result.normalized_query
...
class TestSynonymMapping:
    ...
    def test_tax_synonym_mapping(self, normalizer):
        ...
        synonym_groups = {
            "iva": [
                "IVA", "imposta valore aggiunto", "i.v.a.", 
                "Imposta sul Valore Aggiunto", "iva"
            ],
            "irpef": [
                "IRPEF", "imposta reddito persone fisiche", "i.r.p.e.f.",
                "imposta sui redditi delle persone fisiche"
            ],
            "fattura": [
                "fattura", "fatturazione", "documento fiscale",
                "nota di addebito", "ricevuta fiscale"
            ],
            "detrazione": [
...
class TestRegionalDialectHandling:
    ...
    def test_northern_italy_variations(self, normalizer):
        ...
        for regional_term, standard_term in northern_cases:
            ...
            assert standard_term in result.normalized_query or \
                   regional_term in result.normalized_query  # Accept either form
...
class TestPerformanceRequirements:
    ...
    def test_memory_efficiency(self, normalizer):
        ...
        base_queries = [
            "IVA calcolo", "IRPEF aliquote", "IMU scadenze", "TARI pagamento",
            "F24 compilazione", "730 detrazioni", "fattura elettronica",
            "codice fiscale", "partita iva", "dichiarazione redditi"
        ]
...
class TestCacheKeyGeneration:
    ...
    def test_different_semantic_queries_different_cache_keys(self, normalizer):
        ...
        assert len(cache_keys) == len(different_queries), \
    ...
    def test_semantic_hashing_effectiveness(self, normalizer):
        ...
        for query1, query2 in similar_query_pairs:
            ...
            assert cache_key1 == cache_key2, \
                f"Similar queries should have same cache key: '{query1}' vs '{query2}'"
...
class TestCostOptimizationImpact:
    ...
    def test_estimated_cost_reduction(self):
        ...
        cost_reduction_percentage = (len(pre_normalization_queries) - len(cache_keys)) / len(pre_normalization_queries) * 100
