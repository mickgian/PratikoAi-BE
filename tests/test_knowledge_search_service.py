...
class TestKnowledgeSearchService:
    ...
    async def test_retrieve_topk_hybrid_search(self, knowledge_service, sample_query_data, mock_bm25_results):
        ...
        with patch('app.services.knowledge_search_service.KnowledgeSearchService._perform_bm25_search') as mock_bm25, \
             patch('app.services.knowledge_search_service.KnowledgeSearchService._perform_vector_search') as mock_vector, \
            ...
            assert call_args[0][1] == "RAG.preflight.knowledgesearch.retrieve.topk.bm25.and.vectors.and.recency.boost"  # step_id
    ...
    async def test_vector_only_search_mode(self, knowledge_service, sample_query_data):
        ...
        with patch('app.services.knowledge_search_service.KnowledgeSearchService._perform_vector_search') as mock_vector:
            mock_vector.return_value = [
                SearchResult(
                    id="vec1",
                    title="Vector Result",
    ...
    async def test_recency_boost_calculation(self, knowledge_service):
        ...
        old_doc = datetime.now(timezone.utc) - timedelta(days=100)
        recent_doc = datetime.now(timezone.utc) - timedelta(days=5)
        very_recent_doc = datetime.now(timezone.utc) - timedelta(hours=2)
    ...
    async def test_result_deduplication(self, knowledge_service, sample_query_data):
        ...
        with patch('app.services.knowledge_search_service.KnowledgeSearchService._perform_bm25_search') as mock_bm25, \
            ...
            mock_vector.return_value = [SearchResult(
                id="duplicate_1",
                title="Duplicate Item", 
                content="Content...",
                category="tax",
...
async def test_various_query_types(test_queries):
    ...
    for query_type, test_data in test_queries.items():
        ...
        with patch('app.services.knowledge_search_service.KnowledgeSearchService._perform_bm25_search') as mock_bm25, \
             patch('app.services.knowledge_search_service.KnowledgeSearchService._perform_vector_search') as mock_vector_search:
