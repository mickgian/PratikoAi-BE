...
class TestLLMProviderChatCompletion:
    ...
    @pytest.mark.asyncio
    async def test_openai_chat_completion_with_structured_logging(
        self, mock_openai_provider, sample_messages, sample_llm_response
    ):
        """Test OpenAI chat completion with proper structured logging."""
        with (
            patch("app.core.llm.providers.openai_provider.rag_step_log"),
            ...
            result = await mock_openai_provider.chat_completion(
                messages=sample_messages, tools=[], temperature=0.2, max_tokens=100
            )
    ...
    async def test_anthropic_chat_completion_with_structured_logging(self, mock_anthropic_provider, sample_messages):
        ...
        with (
            patch("app.core.llm.providers.anthropic_provider.rag_step_log"),
    ...
    async def test_chat_completion_with_tools_logging(self, mock_openai_provider, sample_messages):
        ...
        with (
            ...
            mock_langchain_response.tool_calls = [
                {"id": "call_123", "name": "calculator", "args": {"expression": "22 + 10 + 4"}}
            ]
            ...
            # Sample tools
            mock_tools = [Mock(name="calculator", description="Perform calculations")]

            # Execute method
            result = await mock_openai_provider.chat_completion(
                messages=sample_messages, tools=mock_tools, temperature=0.2, max_tokens=100
            )
    ...
    async def test_rag_step_logging_parameters(self, mock_openai_provider, sample_messages):
        ...
        with (
            ...
            # Execute method
            await mock_openai_provider.chat_completion(
                messages=sample_messages, tools=None, temperature=0.7, max_tokens=200
            )
...
def llm_test_scenarios():
    ...
    return {
        ...
        "api_error": {"description": "API error during call", "expected_logs": 1, "should_succeed": False},
        "timeout_error": {"description": "Timeout during API call", "expected_logs": 1, "should_succeed": False},
    }
