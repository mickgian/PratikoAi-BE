...
class TestPerformanceBaselines:
    ...
    async def test_single_user_response_time(self, load_tester):
        ...
        # Assert
        assert (
            metrics.p95_response_time < 3.0
        ), f"Single user P95 response time {metrics.p95_response_time}s exceeds 3s SLA"
        assert metrics.error_rate < 0.01, f"Single user error rate {metrics.error_rate} exceeds 1% threshold"
    ...
    async def test_50_concurrent_users_response_time(self, load_tester):
        ...
        assert metrics.error_rate < 0.01, f"50 users error rate {metrics.error_rate} exceeds 1% threshold"
        assert metrics.throughput >= 1000, f"Throughput {metrics.throughput} req/min below 1000 req/min target"
    ...
    async def test_100_concurrent_users_response_time(self, load_tester):
        ...
        # Assert
        assert (
            metrics.p95_response_time < 8.0
        ), f"100 users P95 response time {metrics.p95_response_time}s exceeds 8s SLA"
        assert metrics.error_rate < 0.02, f"100 users error rate {metrics.error_rate} exceeds 2% threshold"
    ...
    async def test_sustained_throughput(self, load_tester):
        ...
        # Assert
        assert metrics.throughput >= 1000, f"Sustained throughput {metrics.throughput} req/min below target"
        ...
        for window_throughput in throughput_windows:
            assert window_throughput >= 900, "Throughput dropped below 900 req/min during test"
    ...
    async def test_cache_hit_rate_under_load(self, load_tester):
        ...
        # Assert
        assert metrics.cache_hit_rate > 0.7, f"Cache hit rate {metrics.cache_hit_rate} below 70% target"
    ...
    async def test_database_connection_pool_handling(self, load_tester):
        ...
        # Arrange
        metrics = await load_tester.run_test(users=100, duration=300, scenario="database_heavy")

        # Assert
        max_db_connections = max(metrics.db_connections)
        assert max_db_connections <= 100, f"Database connections {max_db_connections} exceeded 100 connection limit"
        ...
        assert len(connection_errors) == 0, f"Found {len(connection_errors)} connection pool exhaustion errors"

    @pytest.mark.asyncio
    async def test_redis_performance_under_concurrent_access(self, load_tester):
        """Test Redis performance under concurrent access"""
        # Arrange
        metrics = await load_tester.run_test(users=100, duration=300, scenario="cache_heavy")
        ...
        assert p95_redis_latency < 10, f"Redis P95 latency {p95_redis_latency}ms exceeds 10ms threshold"

    @pytest.mark.asyncio
    async def test_llm_api_rate_limit_handling(self, load_tester):
        """Test LLM API rate limit handling under load"""
        # Arrange
        metrics = await load_tester.run_test(users=50, duration=300, scenario="llm_heavy")

        # Assert
        rate_limit_errors = load_tester.get_errors_by_type(metrics, "rate_limit")
        assert (
            len(rate_limit_errors) < metrics.total_requests * 0.01
...
class TestScalability:
    ...
    async def test_linear_scaling_to_50_users(self, load_tester):
        ...
        for users in scaling_points:
            metrics_by_users[users] = await load_tester.run_test(users=users, duration=120, scenario="mixed")
        ...
        for users, metrics in metrics_by_users.items():
            ...
            assert (
                metrics.avg_response_time <= expected_max_response_time
        ...
        for users, metrics in metrics_by_users.items():
            ...
            assert metrics.throughput >= expected_min_throughput, f"Throughput at {users} users below linear scaling"

    @pytest.mark.asyncio
    async def test_graceful_degradation_at_100_users(self, load_tester):
        """Test system behavior at 100 users (graceful degradation)"""
        # Arrange
        metrics = await load_tester.run_test(users=100, duration=300, scenario="mixed")

        # Assert - System should degrade gracefully, not fail
        assert metrics.error_rate < 0.05, f"Error rate {metrics.error_rate} exceeds 5% graceful degradation threshold"
        ...
        for endpoint in critical_endpoints:
            ...
            assert endpoint_metrics.error_rate < 0.02, f"Critical endpoint {endpoint} error rate too high"

    @pytest.mark.asyncio
    async def test_auto_scaling_triggers(self, load_tester):
        """Test auto-scaling triggers (if configured)"""
        # Arrange
        metrics = await load_tester.run_test(users=50, duration=300, scenario="cpu_intensive", monitor_scaling=True)

        # Assert
        scaling_events = load_tester.get_scaling_events(metrics)
        if metrics.avg_cpu_usage > 70:
            assert len(scaling_events) > 0, "No scaling events triggered despite high CPU usage"
        ...
        if scaling_events:
            ...
            assert (
                post_scaling_metrics.avg_response_time < pre_scaling_metrics.avg_response_time
    ...
    async def test_load_balancer_distribution(self, load_tester):
        ...
        # Arrange
        metrics = await load_tester.run_test(users=50, duration=300, scenario="mixed", track_instances=True)
        ...
        if len(instance_distributions) > 1:
            ...
            for instance, count in instance_distributions.items():
                ...
                assert (
                    abs(count - expected_share) / expected_share < 0.2
    ...
    async def test_memory_usage_growth_under_sustained_load(self, load_tester):
        ...
        assert memory_growth < 0.2, f"Memory grew by {memory_growth * 100}% during sustained load"

    @pytest.mark.asyncio
    async def test_cpu_utilization_at_target_load(self, load_tester):
        """Test CPU utilization remains <80% at target load"""
        # Arrange
        metrics = await load_tester.run_test(users=50, duration=300, scenario="mixed")

        # Assert
        avg_cpu = statistics.mean(metrics.cpu_usage)
        max_cpu = max(metrics.cpu_usage)

        assert avg_cpu < 80, f"Average CPU usage {avg_cpu}% exceeds 80% threshold"
        assert max_cpu < 95, f"Max CPU usage {max_cpu}% exceeds 95% threshold"
    ...
    async def test_disk_io_patterns_during_peak_usage(self, load_tester):
        ...
        assert (
            disk_io_metrics.avg_queue_length < 10
        ), f"Disk queue length {disk_io_metrics.avg_queue_length} indicates I/O bottleneck"
        assert disk_io_metrics.avg_wait_time < 100, f"Disk wait time {disk_io_metrics.avg_wait_time}ms too high"
...
class TestStressAndSpike:
    ...
    async def test_sudden_spike_from_10_to_100_users(self, load_tester):
        ...
        assert spike_metrics.error_rate < 0.05, f"Error rate {spike_metrics.error_rate} during spike exceeds 5%"

        # Response time should stabilize after spike
        post_spike_metrics = load_tester.get_metrics_after_spike(metrics, spike_config)
        assert post_spike_metrics.p95_response_time < 10.0, "Response time did not stabilize after spike"
    ...
    async def test_sustained_maximum_load_for_1_hour(self, load_tester):
        ...
        assert metrics.error_rate < 0.02, f"Error rate {metrics.error_rate} during sustained load exceeds 2%"

        # Check for memory leaks
        memory_trend = load_tester.calculate_trend(metrics.memory_usage)
        assert memory_trend < 0.1, f"Memory shows upward trend of {memory_trend}GB/hour indicating leak"
        ...
        performance_degradation = (
            last_15min.avg_response_time - first_15min.avg_response_time
        ) / first_15min.avg_response_time
        assert performance_degradation < 0.3, f"Performance degraded by {performance_degradation * 100}% over 1 hour"
    ...
    async def test_recovery_after_overload_condition(self, load_tester):
        ...
        load_tester.get_phase_metrics(metrics, 1)
        ...
        assert recovery_ratio < 1.2, f"System did not recover properly, response time {recovery_ratio}x normal"

        # Error rate should return to normal
        assert (
            recovery_metrics.error_rate <= normal_metrics.error_rate * 1.5
    ...
    async def test_circuit_breaker_activation_under_stress(self, load_tester):
        ...
        assert len(circuit_breaker_events) > 0, "Circuit breaker did not activate despite failures"

        # Verify fallback mechanisms worked
        fallback_successes = load_tester.get_fallback_successes(metrics)
        assert len(fallback_successes) > 0, "No successful fallbacks after circuit breaker activation"
    ...
    async def test_retry_mechanism_behavior_under_load(self, load_tester):
        ...
        metrics = await load_tester.run_test(users=50, duration=300, scenario="mixed", inject_failures=failure_rate)
        ...
        assert retry_success_rate > 0.8, f"Retry success rate {retry_success_rate} below 80%"

        # Retry overhead should be reasonable
        retry_overhead = retry_metrics.total_retry_time / metrics.total_requests
        assert retry_overhead < 0.5, f"Retry overhead {retry_overhead}s per request too high"
    ...
    async def test_faq_system_performance_under_concurrent_queries(self, load_tester):
        ...
        # Arrange
        metrics = await load_tester.run_test(users=100, duration=300, scenario="faq_only")

        # Assert
        # FAQ queries should be fast even under load
        faq_metrics = load_tester.get_endpoint_metrics(metrics, "/api/faq/search")
        assert (
            faq_metrics.p95_response_time < 1.0
        ), f"FAQ P95 response time {faq_metrics.p95_response_time}s exceeds 1s"

        # High cache hit rate for FAQ
        assert faq_metrics.cache_hit_rate > 0.9, f"FAQ cache hit rate {faq_metrics.cache_hit_rate} below 90%"
    ...
    async def test_knowledge_update_system_during_high_traffic(self, load_tester):
        ...
        update_task = asyncio.create_task(load_tester.trigger_knowledge_updates(interval=60, count=5))
        ...
        performance_impact = (
            update_impact_metrics.avg_response_time - normal_metrics.avg_response_time
        ) / normal_metrics.avg_response_time
        assert (
...
class TestItalianMarketSpecific:
    ...
    async def test_concurrent_tax_calculations(self, load_tester):
        ...
        metrics = await load_tester.run_test(
            users=50, duration=300, scenario="italian_tax", scenario_weights=italian_tax_scenarios
        )
        ...
        assert (
            tax_endpoint_metrics.p95_response_time < 2.0
        ...
        assert tax_accuracy > 0.999, f"Tax calculation accuracy {tax_accuracy} below 99.9%"
    ...
    async def test_document_processing_under_load(self, load_tester):
        ...
        document_types = {"fattura_elettronica": 0.4, "f24": 0.3, "dichiarazione_redditi": 0.2, "contratto": 0.1}
        ...
        assert (
            doc_metrics.p95_response_time < 30.0
        ...
        assert max_queue_size < 100, f"Document processing queue reached {max_queue_size} items"
    ...
    async def test_italian_query_normalization_performance(self, load_tester):
        ...
        metrics = await load_tester.run_test(users=50, duration=300, scenario="custom", custom_queries=italian_queries)

        # Assert
        normalization_overhead = load_tester.get_normalization_overhead(metrics)
        assert normalization_overhead < 50, f"Query normalization adds {normalization_overhead}ms overhead"

        # Verify normalized queries still get correct results
        accuracy = load_tester.verify_query_accuracy(metrics)
        assert accuracy > 0.95, f"Query accuracy {accuracy} below 95% threshold"
    ...
    async def test_regulatory_knowledge_queries_at_scale(self, load_tester):
        ...
        metrics = await load_tester.run_test(
            users=50, duration=300, scenario="custom", custom_queries=regulatory_queries
        )
        ...
        assert (
            knowledge_metrics.p95_response_time < 3.0
        ...
        assert (
            index_metrics.avg_search_time < 100
    ...
    async def test_gdpr_operations_under_load(self, load_tester):
        ...
        metrics = await load_tester.run_test(
            users=20, duration=300, scenario="gdpr_operations", operation_weights=gdpr_operations
        )
        ...
        assert gdpr_metrics.success_rate > 0.99, f"GDPR operation success rate {gdpr_metrics.success_rate} below 99%"

        # Data export should be reasonably fast
        export_metrics = load_tester.get_endpoint_metrics(metrics, "/api/gdpr/export")
        assert (
            export_metrics.p95_response_time < 60.0
        ...
        assert consistency_check.is_consistent, f"Data consistency issues found: {consistency_check.issues}"
...
async def load_tester():
    ...
    tester = LoadTestFramework(base_url="http://localhost:8000", enable_monitoring=True)
...
class TestLoadTestReporting:
    ...
    async def test_performance_baseline_establishment(self, load_tester):
        ...
        assert 0.5 < baseline_metrics.single_user_p95 < 3.0, "Baseline response time outside expected range"

    @pytest.mark.asyncio
    async def test_bottleneck_identification(self, load_tester):
        """Test automatic bottleneck identification"""
        # Arrange
        metrics = await load_tester.run_test(users=100, duration=300, scenario="mixed", identify_bottlenecks=True)
        ...
        if metrics.avg_cpu_usage > 80:
            assert any(b.type == "CPU" for b in bottlenecks), "Failed to identify CPU bottleneck"

        if max(metrics.db_connections) > 80:
            assert any(b.type == "DATABASE" for b in bottlenecks), "Failed to identify database bottleneck"
...
class TestCIIntegration:
    ...
    async def test_performance_regression_detection(self, load_tester):
        ...
        current = await load_tester.run_test(users=50, duration=300, scenario="mixed")
