...
class TestPostgreSQLFullTextSearch:
    ...
    async def test_search_with_ranking(self, db_session: AsyncSession):
        ...
        assert results[0].rank_score > results[-1].rank_score if len(results) > 1 else True, "Results should be ordered by rank"
    ...
    async def test_partial_word_matching(self, db_session: AsyncSession):
        ...
        assert any("fattur" in r.title.lower() or "fattur" in r.content.lower() for r in results), \
