...
def sample_expert_feedback():
    ...
    return {
        ...
        "improvement_suggestions": ["Specificare l'aliquota corretta", "Aggiungere esempi pratici"],
    }
...
def sample_prompt_template():
    ...
    return {
        ...
        **Fonti:** {sources}
        ...
        "quality_metrics": {"clarity_score": 0.85, "completeness_score": 0.90, "accuracy_score": 0.88},
        "usage_count": 0,
        "success_rate": 0.0,
    }
    ...
    return [
        {
            ...
            "example_queries": ["Regime forfettario 2023 soglie", "Detrazioni fiscali vecchia normativa"],
            "impact_score": 0.8,
            "confidence": 0.9,
        },
        {
            ...
            "example_queries": ["Calcolo IRPEF scaglioni", "IVA regime misto"],
            "impact_score": 0.9,
            "confidence": 0.85,
        },
    ]
...
class TestExpertFeedbackCollection:
    ...
    async def test_feedback_validation_errors(self):
        ...
        with pytest.raises(ValueError, match="Missing required field: query_id"):
            await collector.collect_feedback({"expert_id": str(uuid4()), "feedback_type": "correct"})

        # Test invalid feedback type
        with pytest.raises(ValueError, match="Invalid feedback_type"):
            await collector.collect_feedback(
                {"query_id": str(uuid4()), "expert_id": str(uuid4()), "feedback_type": "invalid_type"}
            )
    ...
    async def test_feedback_storage_and_retrieval(self, sample_expert_feedback):
        ...
        # Retrieve feedback
        stored_feedback = await collector.get_feedback_by_query_id(sample_expert_feedback["query_id"])
...
class TestAdvancedPromptEngineering:
    ...
    async def test_prompt_generation_with_context(self):
        ...
        generated_prompt = await engineer.generate_enhanced_prompt(
            template_id="structured_reasoning_v1", context_data=context_data
        )
    ...
    async def test_prompt_improvement_based_on_feedback(self, sample_expert_feedback):
        ...
        improvement_data = {
            ...
            "performance_metrics": {"accuracy_score": 0.75, "user_satisfaction": 0.65},
        }
...
class TestAutomaticImprovementEngine:
    ...
    async def test_generate_improvement_recommendations(self):
        ...
        engine = AutomaticImprovementEngine(db=AsyncMock(), prompt_engineer=AsyncMock(), pattern_analyzer=AsyncMock())

        failure_analysis = {
            "primary_patterns": [{"type": "regulatory_outdated", "frequency": 15, "impact": 0.8}],
            "affected_areas": ["regime_forfettario", "iva_calculation"],
            "expert_suggestions": ["Aggiornare riferimenti normativi", "Includere esempi pratici"],
        }
    ...
    async def test_automatic_prompt_updates(self):
        ...
        update_request = {
            ...
            "specific_changes": ["Update regime forfettario references to 2024", "Add new IVA calculation examples"],
        }
    ...
    async def test_improvement_impact_measurement(self):
        ...
        before_metrics = {"accuracy_score": 0.75, "user_satisfaction": 0.70, "failure_rate": 0.15}

        after_metrics = {"accuracy_score": 0.85, "user_satisfaction": 0.80, "failure_rate": 0.08}

        impact_analysis = await engine.measure_improvement_impact(before_metrics, after_metrics, time_period_days=30)
...
class TestQualityMetricsDashboard:
    ...
    async def test_quality_metrics_calculation(self):
        ...
        metrics = await dashboard.calculate_quality_metrics(time_period=timedelta(days=30))
    ...
    async def test_expert_performance_metrics(self):
        ...
        expert_metrics = await dashboard.get_expert_performance_metrics(
            expert_id=str(uuid4()), time_period=timedelta(days=30)
        )
    ...
    async def test_system_improvement_tracking(self):
        ...
        improvement_data = await dashboard.track_system_improvements(
            start_date=datetime.utcnow() - timedelta(days=90), end_date=datetime.utcnow()
        )
...
class TestSystemIntegration:
    ...
    async def test_end_to_end_quality_workflow(self, sample_expert_feedback):
        ...
        workflow_result = await orchestrator.execute_quality_workflow(
            query_id=sample_expert_feedback["query_id"], expert_feedback=sample_expert_feedback
        )
