...
class TestRAGStep117FAQFeedback:
    ...
    async def test_step_117_processes_faq_feedback(self, mock_rag_log):
        ...
        with patch("app.services.intelligent_faq_service.IntelligentFAQService") as MockService:
            ...
            mock_service.collect_feedback.assert_called_once_with(
                usage_log_id="usage_123", was_helpful=True, followup_needed=False, comments="Very helpful answer!"
            )
    ...
    async def test_step_117_handles_missing_usage_log(self, mock_rag_log):
        ...
        ctx = {
            "feedback_data": {"usage_log_id": "missing_123", "was_helpful": False, "followup_needed": True},
    ...
    async def test_step_117_preserves_context(self, mock_rag_log):
        ...
        original_ctx = {
            "feedback_data": {"usage_log_id": "usage_456", "was_helpful": True},
    ...
    async def test_step_117_adds_feedback_metadata(self, mock_rag_log):
        ...
        ctx = {
            "feedback_data": {"usage_log_id": "usage_789", "was_helpful": True, "comments": "Great!"},
    ...
    async def test_step_117_handles_service_error(self, mock_rag_log):
        ...
        ctx = {"feedback_data": {"usage_log_id": "usage_error", "was_helpful": True}, "request_id": "test-117-error"}

        with patch("app.services.intelligent_faq_service.IntelligentFAQService") as MockService:
            mock_service = MockService.return_value
            mock_service.collect_feedback = AsyncMock(side_effect=Exception("Database connection error"))
    ...
    async def test_step_117_parity_feedback_collection(self):
        ...
        with patch("app.services.intelligent_faq_service.IntelligentFAQService") as MockService:
            ...
            for test_case in test_cases:
                mock_service.collect_feedback = AsyncMock(return_value=test_case["expected_success"])

                ctx = {**test_case, "request_id": f"parity-{test_case['feedback_data']['usage_log_id']}"}
...
class TestRAGStep117Integration:
    ...
    async def test_feedback_type_to_117_integration(self, mock_golden_log, mock_feedback_log):
        ...
        initial_ctx = {
            "feedback_type": "faq",
            "feedback_data": {"usage_log_id": "usage_integration", "was_helpful": True, "followup_needed": False},
