...
class TestRAGStep121TrustScoreOK:
    ...
    async def test_step_121_basic_trust_score_decision_high_trust(self):
        ...
        ctx = {
            ...
            "expert_feedback": {"feedback_type": "incorrect", "expert_data": {"name": "Dr. Marco Rossi"}},
    ...
    async def test_step_121_basic_trust_score_decision_low_trust(self):
        ...
        ctx = {
            ...
            "expert_feedback": {"feedback_type": "incomplete", "expert_data": {"name": "Dr. Giuseppe Verdi"}},
    ...
    async def test_step_121_trust_score_boundary_conditions(self):
        ...
        for case in test_cases:
            ctx = {
                ...
                "expert_feedback": {"feedback_type": "wrong", "expert_data": {"name": f"Expert {case['case_name']}"}},
    ...
    async def test_step_121_preserves_context_data(self):
        ...
        ctx = {
            ...
            "performance_tracking": {"step_120_duration": 0.045, "validation_start_time": "2023-10-01T10:00:00Z"},
    ...
    async def test_step_121_logs_decision_details(self):
        ...
        ctx = {
            "expert_validation": {"trust_score": 0.75, "expert_id": "expert_logging", "credentials_validated": True},
    ...
    async def test_step_121_decision_performance_tracking(self):
        ...
        ctx = {
            "expert_validation": {"trust_score": 0.88, "expert_id": "expert_performance"},
            "expert_feedback": {"feedback_type": "wrong"},
    ...
    async def test_step_121_decision_routing_consistency(self):
        ...
        for scenario in test_scenarios:
            ctx = {
                ...
                "expert_feedback": {"feedback_type": "incorrect"},
...
class TestRAGStep121Parity:
    ...
    async def test_step_121_parity_decision_structure(self):
        ...
        ctx = {
            "expert_validation": {"trust_score": 0.8, "expert_id": "expert_parity", "credentials_validated": True},
            "expert_feedback": {"feedback_type": "incomplete"},
...
class TestRAGStep121Integration:
    ...
    async def test_step_120_to_121_integration(self):
        ...
        step_120_output = {
            "expert_validation": {
                ...
                "validation_details": {"credentials_score": 0.9, "experience_score": 0.8, "track_record_score": 0.85},
            },
            "expert_feedback": {
                "feedback_type": "incorrect",
                "feedback_text": "Answer needs correction",
                "expert_data": {"name": "Dr. Integration Test", "certifications": ["Italian CPA"]},
            },
        ...
        with (
            patch("app.orchestrators.platform.step_120__validate_expert") as mock_step_120,
            patch("app.observability.rag_logging.rag_step_log"),
            patch("app.observability.rag_logging.rag_step_timer") as mock_timer,
        ):
    ...
    async def test_step_121_to_122_integration(self):
        ...
        step_121_ctx = {
            ...
            "expert_feedback": {"feedback_type": "wrong", "expert_data": {"name": "Dr. Low Trust"}},
            "request_id": "req_121_to_122_integration",
        }

        with (
            patch("app.orchestrators.feedback.step_122__feedback_rejected"),
            patch("app.observability.rag_logging.rag_step_log"),
            patch("app.observability.rag_logging.rag_step_timer") as mock_timer,
        ):
            mock_timer.return_value.__enter__ = Mock()
    ...
    async def test_step_121_to_123_integration(self):
        ...
        step_121_ctx = {
            ...
            "expert_feedback": {
                ...
                "expert_data": {"name": "Dr. High Trust", "certifications": ["Italian Tax Advisor"]},
            },
            "request_id": "req_121_to_123_integration",
        }

        with (
            patch("app.orchestrators.feedback.step_123__create_feedback_rec"),
            patch("app.observability.rag_logging.rag_step_log"),
            patch("app.observability.rag_logging.rag_step_timer") as mock_timer,
        ):
            mock_timer.return_value.__enter__ = Mock()
    ...
    async def test_full_trust_score_pipeline(self):
        ...
        test_scenarios = [
            {"trust_score": 0.85, "expected_final_step": "CreateFeedbackRec", "scenario_name": "high_trust_path"},
            {"trust_score": 0.55, "expected_final_step": "FeedbackRejected", "scenario_name": "low_trust_path"},
        ]
        ...
        scenarios = [
            {
                ...
                "trust_score": 0.93,
                "expert_data": {"certifications": ["Italian CPA", "Tax Advisor"], "location": "Milano"},
                "feedback_type": "incorrect",
            ...
            },
            {
                ...
                "trust_score": 0.45,
                "expert_data": {"certifications": [], "location": "Unknown"},
                "feedback_type": "wrong",
            ...
            },
            {
                ...
                "trust_score": 0.7,  # Exactly at threshold
                "expert_data": {"certifications": ["Basic Certification"], "location": "Roma"},
                "feedback_type": "incomplete",
                "expected_route": "CreateFeedbackRec",
            },
        ]
