...
class TestStep132RSSMonitorUnit:
    ...
    async def test_successful_rss_monitoring_initiation(self, sample_rss_monitor_context, sample_feed_results):
        ...
        with (
            patch("app.orchestrators.kb.rag_step_log") as mock_log,
            ...
            assert result["monitoring_result"]["has_new_items"] is True
    ...
    async def test_rss_monitoring_with_no_new_items(self):
        ...
        ctx = {"request_id": "rss-test-no-items", "monitor_type": "ccnl", "sources": ["ccnl_feeds"]}
    ...
    async def test_rss_monitoring_with_partial_failures(self):
        ...
        with (
            ...
            mock_monitor.return_value = {
                ...
                "has_new_items": True,
                "feeds": [
                    {"feed_name": "agenzia_entrate_circolari", "status": "success", "items_found": 3, "new_items": 1}
                ],
                "new_item_count": 1,
    ...
    async def test_missing_monitor_configuration(self):
        ...
        with (
            ...
            assert result["monitoring_result"]["has_new_items"] is False

    @pytest.mark.asyncio
    async def test_rss_monitoring_service_exception(self):
        """Test handling of RSS monitoring service exceptions."""
        ctx = {"request_id": "rss-test-exception", "monitor_type": "italian", "sources": ["agenzia_entrate"]}
    ...
    async def test_different_monitor_types(self):
        ...
        for monitor_type, sources in test_cases:
            ctx = {"request_id": f"rss-test-{monitor_type}", "monitor_type": monitor_type, "sources": sources}
...
class TestStep132RSSMonitorIntegration:
        ...
        with (
            ...
            mock_monitor.return_value = {
                ...
                "feeds": [
                    ...
                    {"feed_name": "ccnl_updates", "priority": "medium", "new_items": 2, "filtered_items": 2},
                ],
                "new_item_count": 7,
    ...
    async def test_manual_triggered_rss_monitoring(self):
        ...
        manual_ctx = {
            ...
            "user_initiated": True,
            "target_date_range": {"start_date": "2024-01-01", "end_date": "2024-01-15"},
        }
...
class TestStep132RSSMonitorParity:
    ...
    async def test_rss_service_behavior_preservation(self):
        ...
        ctx = {"request_id": "parity-test-001", "monitor_type": "italian", "sources": ["agenzia_entrate", "inps"]}

        with (
            patch("app.services.rss_feed_monitor.RSSFeedMonitor") as MockRSSMonitor,
            patch("app.orchestrators.kb.rag_step_log"),
            patch("app.orchestrators.kb.rag_step_timer") as mock_timer,
            patch("app.orchestrators.kb._coordinate_rss_monitoring") as mock_monitor,
        ):
    ...
    async def test_error_handling_consistency(self):
        ...
        ctx = {"request_id": "parity-error-test", "monitor_type": "invalid_type", "sources": ["unknown_source"]}
    ...
    async def test_output_structure_consistency(self):
        ...
        for scenario in test_scenarios:
            ...
            with (
                ...
                required_keys = ["step", "status", "monitoring_result"]
