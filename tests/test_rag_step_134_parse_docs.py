...
class TestStep134ParseDocsUnit:
    ...
    @pytest.mark.asyncio
    async def test_successful_document_parsing_and_extraction(
        self, sample_parsed_feeds_context, sample_extracted_documents
    ):
        """Test successful document parsing and text/metadata extraction."""
        with (
            patch("app.orchestrators.docs.rag_step_log") as mock_log,
    ...
    async def test_document_parsing_with_no_feeds_provided(self):
        ...
        ctx = {"request_id": "parse-test-no-feeds", "parsed_feeds": [], "total_items_parsed": 0, "feed_sources": []}
    ...
    async def test_document_parsing_with_extraction_failures(self):
        ...
        with (
            ...
            mock_extract.return_value = {
                ...
                "processing_summary": {"successful_extractions": 1, "failed_extractions": 1},
        ...
        for doc_type, doc_url in test_cases:
            ...
            with (
                ...
                mock_extract.return_value = {
                    ...
                    "processing_summary": {"successful_extractions": 1, "failed_extractions": 0},
                }
...
class TestStep134ParseDocsIntegration:
    ...
    async def test_background_scheduled_document_processing(self):
        ...
        scheduled_ctx = {
            ...
            "total_items_parsed": 2,
            "feed_sources": ["agenzia_entrate"],
            "processing_config": {"priority_processing": True, "max_concurrent_documents": 5, "timeout_seconds": 60},
        }
                ...
                "documents_extracted": [
                    {
                        ...
                        "metadata": {"legal_references": ["D.M. 123/2024"], "enhanced": True},
                        "success": True,
                    }
                ],
                "processing_summary": {"successful_extractions": 1},
...
class TestStep134ParseDocsParity:
    ...
    async def test_document_service_behavior_preservation(self):
        ...
        with (
            patch("app.services.document_processor.DocumentProcessor") as MockDocProcessor,
            patch("app.orchestrators.docs.rag_step_log"),
            patch("app.orchestrators.docs.rag_step_timer") as mock_timer,
            patch("app.orchestrators.docs._extract_text_and_metadata") as mock_extract,
        ):
        ...
        for scenario in test_scenarios:
            ...
            with (
                ...
                mock_extract.return_value = {
                    ...
                    "failed_extractions": 0,
                    "documents_extracted": [],
                    "processing_summary": {"successful_extractions": scenario["docs"], "failed_extractions": 0},
                }

                result = await step_134__parse_docs(ctx=ctx)

                # Verify consistent output structure
                required_keys = ["step", "status", "documents_processed", "successful_extractions"]
