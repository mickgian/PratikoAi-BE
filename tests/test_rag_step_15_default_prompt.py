...
class TestRAGStep15DefaultPrompt:
    ...
    async def test_step_15_successful_default_prompt(
        self, mock_logger, mock_rag_log, mock_classification_failed_context
    ):
    ...
    @patch("app.orchestrators.prompting.rag_step_log")
    @patch("app.core.logging.logger")
    async def test_step_15_no_classification_attempted(
        self, mock_logger, mock_rag_log, mock_no_classification_context
    ):
    ...
    async def test_step_15_system_prompt_selection(self, mock_logger, mock_rag_log):
        ...
        ctx = {"user_query": "What is 2+2?", "request_id": "req_prompt"}
    ...
    async def test_step_15_query_analysis(self, mock_logger, mock_rag_log):
        ...
        test_cases = [
            ...
            (
                "This is a more complex question about tax regulations that requires detailed analysis",
                "medium",
            ),  # 94 chars = medium
            ("What is VAT?", "simple"),
        ]
        ...
        for query, expected_complexity in test_cases:
            ctx = {"user_query": query, "request_id": f"req_{expected_complexity}"}
    ...
    async def test_step_15_workflow_routing(self, mock_logger, mock_rag_log):
        ...
        ctx = {"user_query": "Help with taxes", "workflow_mode": "standard", "request_id": "req_routing"}
    ...
    @patch("app.orchestrators.prompting.rag_step_log")
    @patch("app.core.logging.logger")
    async def test_step_15_ready_for_prompt_selection(
        self, mock_logger, mock_rag_log, mock_classification_failed_context
    ):
    ...
    async def test_step_15_comprehensive_logging(self, mock_logger, mock_rag_log, mock_classification_failed_context):
        ...
        required_fields = [
            ...
            "step_id",
            "node_label",
            "default_prompt_applied",
            "classification_bypassed",
            "prompt_strategy",
            "processing_stage",
            "next_step",
        ]
    ...
    async def test_step_15_performance_tracking(self, mock_logger, mock_rag_log, mock_classification_failed_context):
        ...
        with patch("app.orchestrators.prompting.rag_step_timer") as mock_timer:
            ...
            mock_timer.assert_called_with(
                15, "RAG.prompting.continue.without.classification", "DefaultPrompt", stage="start"
            )
