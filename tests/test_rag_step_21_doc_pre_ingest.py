...
class TestRAGStep21DocPreIngest:
    ...
    async def test_step_21_pdf_document(self, mock_rag_log):
        ...
        fingerprints = [
            {"hash": "abc123", "filename": "fattura_2024.pdf", "size": 2 * 1024 * 1024, "mime_type": "application/pdf"}
        ]

        ctx = {"fingerprints": fingerprints, "attachment_count": 1, "request_id": "test-21-pdf"}
    ...
    async def test_step_21_excel_document(self, mock_rag_log):
        ...
        ctx = {"fingerprints": fingerprints, "attachment_count": 1, "request_id": "test-21-excel"}
    ...
    async def test_step_21_xml_fattura(self, mock_rag_log):
        ...
        ctx = {"fingerprints": fingerprints, "attachment_count": 1, "request_id": "test-21-xml"}
    ...
    async def test_step_21_multiple_documents(self, mock_rag_log):
        ...
        ctx = {"fingerprints": fingerprints, "attachment_count": 3, "request_id": "test-21-multi"}
    ...
    async def test_step_21_key_field_hints(self, mock_rag_log):
        ...
        for filename, expected_hint in test_cases:
            ...
            fingerprints = [{"hash": "test", "filename": filename, "size": 1024, "mime_type": "application/pdf"}]

            ctx = {"fingerprints": fingerprints, "attachment_count": 1, "request_id": f"test-21-hint-{expected_hint}"}
    ...
    async def test_step_21_routes_to_step_22(self, mock_rag_log):
        ...
        fingerprints = [{"hash": "test", "filename": "test.pdf", "size": 1024, "mime_type": "application/pdf"}]

        ctx = {"fingerprints": fingerprints, "attachment_count": 1, "request_id": "test-21-route"}
...
class TestRAGStep21Parity:
    ...
    async def test_step_21_parity_mime_type_detection(self, mock_rag_log):
        ...
        for mime_type, doc_type in DOCUMENT_CONFIG["SUPPORTED_MIME_TYPES"].items():
            ...
            ctx = {"fingerprints": fingerprints, "attachment_count": 1, "request_id": f"test-parity-{doc_type.value}"}
...
class TestRAGStep21Integration:
    ...
    async def test_step_85_to_21_integration(self, mock_rag_log):
        ...
        from app.orchestrators.preflight import step_21__doc_pre_ingest, step_85__valid_attachments_check
        ...
        step_85_ctx = {
            ...
            "errors": [],
            "attachment_count": 1,
            "fingerprints": [{"hash": "abc", "filename": "invoice.pdf", "size": 1024, "mime_type": "application/pdf"}],
    ...
    async def test_step_21_prepares_for_step_22(self, mock_rag_log):
        ...
        fingerprints = [{"hash": "doc1", "filename": "contract.pdf", "size": 2048, "mime_type": "application/pdf"}]

        ctx = {"fingerprints": fingerprints, "attachment_count": 1, "request_id": "test-21-prep-22"}
