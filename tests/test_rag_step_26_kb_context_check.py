...
from app.services.knowledge_search_service import KnowledgeSearchConfig, KnowledgeSearchService, SearchResult
...
class TestRAGStep26KBContextCheck:
    ...
    def context_check_query_data(self):
        ...
        return {
            ...
            "trace_id": "trace_001",
            "golden_timestamp": datetime.now(UTC) - timedelta(days=30),  # Golden is 30 days old
    ...
    async def test_fetch_recent_kb_for_changes_basic(
        self, mock_timer, mock_log, knowledge_search_service, context_check_query_data, sample_kb_results
    ):
    ...
    async def test_fetch_recent_kb_no_recent_changes(
        self, mock_log, knowledge_search_service, context_check_query_data
    ):
    ...
    async def test_fetch_recent_kb_with_golden_timestamp_comparison(
        self, mock_log, knowledge_search_service, context_check_query_data, sample_kb_results
    ):
        """Test filtering based on Golden Set timestamp."""
        # Set Golden timestamp to 10 days ago
        context_check_query_data["golden_timestamp"] = datetime.now(UTC) - timedelta(days=10)
    ...
    @pytest.mark.asyncio
    @patch("app.services.knowledge_search_service.rag_step_log")
    async def test_detect_conflicting_information(self, mock_log, knowledge_search_service, context_check_query_data):
    ...
    @pytest.mark.asyncio
    @patch("app.services.knowledge_search_service.rag_step_log")
    async def test_error_handling_fetch_recent_kb(self, mock_log, knowledge_search_service, context_check_query_data):
    ...
    async def test_context_check_integration_with_graph_state(self, mock_db_session, mock_vector_service):
        ...
        state = GraphState(messages=[], session_id="test_session_123")
    ...
    async def test_structured_logging_format(
        ...
        with patch.object(knowledge_search_service, "_perform_hybrid_search", return_value=sample_kb_results):
            ...
            completed_log_calls = [
                ...
                if (len(call[1]) > 5 and call[1].get("step") == 26 and call[1].get("processing_stage") == "completed")
            ]
