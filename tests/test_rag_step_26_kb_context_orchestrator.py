...
class TestRAGStep26KBContextOrchestrator:
    ...
    async def test_step_26_no_recent_changes(self, mock_rag_log):
        ...
        ctx = {
            ...
            "golden_match": {"faq_id": "faq_002", "updated_at": (datetime.now(UTC) - timedelta(days=10)).isoformat()},
    ...
    async def test_step_26_includes_kb_metadata(self, mock_rag_log):
        ...
        with patch("app.services.knowledge_search_service.KnowledgeSearchService") as MockService:
            mock_service = MockService.return_value
            mock_service.fetch_recent_kb_for_changes = AsyncMock(
                return_value=[MagicMock(id="kb_1", score=0.9), MagicMock(id="kb_2", score=0.85)]
            )
    ...
    async def test_step_26_logs_kb_fetch_details(self, mock_rag_log):
        ...
        with patch("app.services.knowledge_search_service.KnowledgeSearchService") as MockService:
            mock_service = MockService.return_value
            mock_service.fetch_recent_kb_for_changes = AsyncMock(return_value=[MagicMock(id="kb_1")])

            await step_26__kbcontext_check(messages=[], ctx=ctx)
    ...
    async def test_step_26_handles_service_error(self, mock_rag_log):
        ...
        with patch("app.services.knowledge_search_service.KnowledgeSearchService") as MockService:
            mock_service = MockService.return_value
            mock_service.fetch_recent_kb_for_changes = AsyncMock(side_effect=Exception("KB service error"))
            ...
            error_logs = [call for call in mock_rag_log.call_args_list if call[1].get("processing_stage") == "error"]
    ...
    async def test_step_26_uses_golden_timestamp(self, mock_rag_log):
        ...
        golden_updated_at = (datetime.now(UTC) - timedelta(days=20)).isoformat()
        ctx = {
            "user_query": "Test",
            "golden_match": {"faq_id": "faq_001", "updated_at": golden_updated_at},
    ...
    async def test_step_25_to_26_high_confidence_integration(self, mock_golden_log, mock_kb_log):
        ...
        initial_ctx = {
            "golden_match": {
                "faq_id": "faq_001",
                "similarity_score": 0.95,
                "updated_at": (datetime.now(UTC) - timedelta(days=15)).isoformat(),
            },
            "match_found": True,
            "similarity_score": 0.95,
        ...
        with patch("app.services.knowledge_search_service.KnowledgeSearchService") as MockService:
            mock_service = MockService.return_value
            mock_service.fetch_recent_kb_for_changes = AsyncMock(
                return_value=[MagicMock(id="kb_new", updated_at=datetime.now(UTC) - timedelta(days=5))]
            )
