...
class TestRAGStep27KBDelta:
    ...
    def golden_metadata(self):
        ...
        return {
            ...
            "updated_at": datetime.now(UTC) - timedelta(days=30),  # 30 days old
            "confidence": 0.95,
            "source": "golden_set",
        }
    ...
    def older_kb_results(self):
        ...
        golden_time = datetime.now(UTC) - timedelta(days=30)
        return [
            SearchResult(
                id="kb_old",
    ...
    def decision_data_template(self):
        ...
        return {"trace_id": "test_trace_001", "user_id": str(uuid.uuid4()), "session_id": "test_session_456"}

    @pytest.mark.asyncio
    @patch("app.services.kb_delta_decision.rag_step_log")
    async def test_kb_newer_than_golden_decision_yes(
        self, mock_log, kb_delta_decision, golden_metadata, newer_kb_results, decision_data_template
    ):
        """Test decision when KB has newer information than Golden Set."""
        decision_data = {**decision_data_template, "kb_results": newer_kb_results, "golden_metadata": golden_metadata}
    ...
    async def test_kb_older_than_golden_decision_no(
        self, mock_log, kb_delta_decision, golden_metadata, older_kb_results, decision_data_template
    ):
        """Test decision when KB has no newer information than Golden Set."""
        decision_data = {**decision_data_template, "kb_results": older_kb_results, "golden_metadata": golden_metadata}
    ...
    async def test_kb_conflicting_tags_decision_yes(
        self, mock_log, kb_delta_decision, golden_metadata, conflicting_kb_results, decision_data_template
    ):
    ...
    async def test_empty_kb_results_decision_no(
        self, mock_log, kb_delta_decision, golden_metadata, decision_data_template
    ):
        """Test decision when no KB results are available."""
        decision_data = {**decision_data_template, "kb_results": [], "golden_metadata": golden_metadata}
    ...
    async def test_missing_golden_metadata_decision_yes(
        self, mock_log, kb_delta_decision, newer_kb_results, decision_data_template
    ):
        """Test decision when Golden metadata is missing."""
        decision_data = {**decision_data_template, "kb_results": newer_kb_results, "golden_metadata": None}
    ...
    async def test_mixed_kb_results_decision_yes(
        ...
        mixed_results = [
            SearchResult(
                ...
                score=0.85,
                source="kb_rss",
                updated_at=datetime.now(UTC) - timedelta(days=5),  # Newer
            ...
            SearchResult(
                ...
                score=0.65,
                source="kb_static",
                updated_at=datetime.now(UTC) - timedelta(days=60),  # Older
            ),
        ]

        decision_data = {**decision_data_template, "kb_results": mixed_results, "golden_metadata": golden_metadata}
    ...
    async def test_structured_logging_format(
        ...
        kb_delta_decision.evaluate_kb_vs_golden(decision_data)
        ...
        decision_log_calls = [
            ...
            if (len(call[1]) > 3 and call[1].get("step") == 27 and "decision" in call[1])
        ]
    ...
    async def test_decision_edge_cases(self, kb_delta_decision, decision_data_template):
        ...
        same_time = datetime.now(UTC) - timedelta(days=30)
        ...
        golden_same_time = {"updated_at": same_time, "tags": ["business_tax"], "category": "tax"}
