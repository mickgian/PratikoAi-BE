...
class TestRAGStep42ClassificationConfidenceCheck:
    ...
    def sample_messages(self):
        ...
        return [Message(role="user", content="I need help with tax calculations")]
    ...
    async def test_step_42_exact_threshold_uses_domain_prompt(
        self, mock_log, lang_graph_agent, sample_messages, exact_threshold_classification
    ):
    ...
    async def test_step_42_low_confidence_uses_default_prompt(
        ...
        default_prompt_logs = [
            ...
            if (
                len(call[1]) > 3
                and call[1].get("prompt_type") == "default"
                and call[1].get("confidence_below_threshold") is True
            )
        ]
    ...
    @pytest.mark.asyncio
    @patch("app.core.langgraph.graph.rag_step_log")
    async def test_step_42_no_classification_uses_default_prompt(self, mock_log, lang_graph_agent, sample_messages):
        ...
        step_42_logs = [
            call
            for call in mock_log.call_args_list
            if (len(call[1]) > 3 and call[1].get("step") == 42 and call[1].get("classification_exists") is False)
        ]
    ...
    @patch("app.core.langgraph.graph.rag_step_log")
    @patch("app.core.config.settings.CLASSIFICATION_CONFIDENCE_THRESHOLD", 0.6)
    async def test_step_42_edge_case_very_high_confidence(self, mock_log, lang_graph_agent, sample_messages):
    ...
    @patch("app.core.langgraph.graph.rag_step_log")
    @patch("app.core.config.settings.CLASSIFICATION_CONFIDENCE_THRESHOLD", 0.6)
    async def test_step_42_edge_case_very_low_confidence(self, mock_log, lang_graph_agent, sample_messages):
    ...
    @patch("app.core.langgraph.graph.rag_step_log")
    @patch("app.core.config.settings.CLASSIFICATION_CONFIDENCE_THRESHOLD", 0.6)
    async def test_step_42_different_domains_high_confidence(self, mock_log, lang_graph_agent, sample_messages):
