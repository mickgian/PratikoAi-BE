...
class TestRAGStep43DomainPromptGeneration:
    ...
    async def test_step_43_tax_information_request(self, mock_log, prompt_template_manager):
        ...
        step_43_logs = [call for call in mock_log.call_args_list if len(call[1]) > 3 and call[1].get("step") == 43]
    ...
    @pytest.mark.asyncio
    @patch("app.services.domain_prompt_templates.rag_step_log")
    async def test_step_43_legal_document_generation(self, mock_log, prompt_template_manager):
    ...
    @pytest.mark.asyncio
    @patch("app.services.domain_prompt_templates.rag_step_log")
    async def test_step_43_labor_calculation_request(self, mock_log, prompt_template_manager):
    ...
    @pytest.mark.asyncio
    @patch("app.services.domain_prompt_templates.rag_step_log")
    async def test_step_43_business_strategic_advice(self, mock_log, prompt_template_manager):
    ...
    @pytest.mark.asyncio
    @patch("app.services.domain_prompt_templates.rag_step_log")
    async def test_step_43_accounting_compliance_check(self, mock_log, prompt_template_manager):
    ...
    @pytest.mark.asyncio
    @patch("app.services.domain_prompt_templates.rag_step_log")
    async def test_step_43_tax_ccnl_query(self, mock_log, prompt_template_manager):
        """Test STEP 43: Generate prompt with context."""

        context = {"calculation_parameters": {"salary": 30000, "months": 12}}
    ...
    @pytest.mark.asyncio
    @patch("app.services.domain_prompt_templates.rag_step_log")
    async def test_step_43_prompt_customization_with_context(self, mock_log, prompt_template_manager):
    ...
    @pytest.mark.asyncio
    @patch("app.services.domain_prompt_templates.rag_step_log")
    async def test_step_43_error_handling_invalid_domain_action(self, mock_log, prompt_template_manager):
    ...
    @pytest.mark.asyncio
    @patch("app.services.domain_prompt_templates.rag_step_log")
    async def test_step_43_comprehensive_logging_format(self, mock_log, prompt_template_manager):
        """Test STEP 43: Comprehensive structured logging format."""

        prompt_template_manager.get_prompt(
            domain=Domain.TAX,
            action=Action.INFORMATION_REQUEST,
    ...
    @pytest.mark.asyncio
    @patch("app.services.domain_prompt_templates.rag_step_log")
    async def test_step_43_performance_tracking_with_timer(self, mock_log, prompt_template_manager):
    ...
    @pytest.mark.asyncio
    @patch("app.services.domain_prompt_templates.rag_step_log")
    async def test_step_43_all_domain_action_combinations(self, mock_log, prompt_template_manager):
        ...
        for domain, action, query in test_cases:
            ...
            result = prompt_template_manager.get_prompt(
                domain=domain, action=action, query=query, context=None, document_type=None
            )
...
class TestRAGStep43Orchestrator:
    ...
    async def test_step_43_orchestrator_success(self, mock_rag_log):
        ...
        classification = DomainActionClassification(
            domain=Domain.TAX, action=Action.INFORMATION_REQUEST, confidence=0.85, document_type=None
        )
    ...
    async def test_step_43_orchestrator_error_handling(self, mock_logger, mock_rag_log):
        ...
        # Create classification
        classification = DomainActionClassification(
            domain=Domain.LEGAL, action=Action.DOCUMENT_GENERATION, confidence=0.75, document_type="contratto"
        )
    ...
    async def test_step_43_orchestrator_no_classification(self, mock_rag_log):
        ...
        ctx = {"user_query": "Generate something", "request_id": "test-43-no-class"}
    ...
    async def test_step_43_orchestrator_creates_manager_if_missing(self, mock_rag_log):
        ...
        # Create classification
        classification = DomainActionClassification(
            domain=Domain.BUSINESS, action=Action.STRATEGIC_ADVICE, confidence=0.70, document_type=None
        )
    ...
    async def test_step_43_orchestrator_with_context(self, mock_rag_log):
        ...
        # Create classification
        classification = DomainActionClassification(
            domain=Domain.ACCOUNTING, action=Action.COMPLIANCE_CHECK, confidence=0.80, document_type=None
        )
    ...
    async def test_step_43_parity_behavior_preservation(self, mock_rag_log):
        ...
        # Create classification
        classification = DomainActionClassification(
            domain=Domain.TAX, action=Action.INFORMATION_REQUEST, confidence=0.90, document_type=None
        )
