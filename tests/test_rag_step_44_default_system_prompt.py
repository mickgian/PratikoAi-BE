...
class TestRAGStep44DefaultSystemPrompt:
    ...
    def sample_messages(self):
        ...
        return [Message(role="user", content="Can you help me with general questions?")]
    ...
    async def test_step_44_no_classification_uses_default_prompt(self, mock_log, lang_graph_agent, sample_messages):
    ...
    async def test_step_44_different_threshold_configuration(self, mock_log, lang_graph_agent, sample_messages):
    ...
    @pytest.mark.asyncio
    @patch("app.orchestrators.prompting.rag_step_log")
    async def test_step_44_prompt_content_validation(self, mock_log, lang_graph_agent, sample_messages):
    ...
    @pytest.mark.asyncio
    @patch("app.orchestrators.prompting.rag_step_log")
    async def test_step_44_edge_case_exactly_at_threshold(self, mock_log, lang_graph_agent, sample_messages):
    ...
    async def test_step_44_performance_tracking_with_timer(
        self, mock_timer, mock_log, lang_graph_agent, sample_messages, low_confidence_classification
    ):
    ...
    async def test_step_44_comprehensive_logging_format(
        ...
        lang_graph_agent._get_system_prompt(sample_messages, low_confidence_classification)
    ...
    @pytest.mark.asyncio
    @patch("app.orchestrators.prompting.rag_step_log")
    async def test_step_44_different_trigger_scenarios(self, mock_log, lang_graph_agent, sample_messages):
    ...
    @pytest.mark.asyncio
    @patch("app.orchestrators.prompting.rag_step_log")
    async def test_step_44_empty_messages_handled(self, mock_log, lang_graph_agent):
    ...
    @pytest.mark.asyncio
    @patch("app.orchestrators.prompting.rag_step_log")
    async def test_step_44_various_domains_with_low_confidence(self, mock_log, lang_graph_agent, sample_messages):
        """Test STEP 44: All domains with low confidence use default prompt."""

        domains_to_test = [Domain.TAX, Domain.LEGAL, Domain.LABOR, Domain.BUSINESS, Domain.ACCOUNTING]
