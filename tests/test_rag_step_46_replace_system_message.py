...
class TestRAGStep46ReplaceSystemMessage:
    ...
    def lang_graph_agent(self):
        ...
        agent._prompt_template_manager.get_prompt.return_value = (
            "Tax-specific system prompt for deductions and calculations."
        )
        return agent
    ...
    async def test_step_46_replace_system_message_with_tax_domain(
        self, mock_log, lang_graph_agent, messages_with_system, tax_classification
    ):
    ...
    async def test_step_46_replace_system_message_with_labor_domain(
        self, mock_log, lang_graph_agent, messages_with_system, labor_classification
    ):
        ...
        lang_graph_agent._prompt_template_manager.get_prompt.return_value = (
            "Labor law specialist prompt for calculations."
        )
        original_content = messages_with_system[0].content
    ...
    @pytest.mark.asyncio
    @patch("app.core.langgraph.graph.rag_step_log")
    async def test_step_46_no_replace_without_classification(self, mock_log, lang_graph_agent, messages_with_system):
        ...
        # Call the method
        result = lang_graph_agent._prepare_messages_with_system_prompt(
            messages_with_system, system_prompt="Some prompt", classification=None
        )
        ...
        step_45_logs = [call for call in mock_log.call_args_list if len(call[1]) > 3 and call[1].get("step") == 45]
    ...
    @pytest.mark.asyncio
    @patch("app.core.langgraph.graph.rag_step_log")
    async def test_step_46_replace_preserves_message_order(self, mock_log, lang_graph_agent, tax_classification):
        ...
        # Call the method
        result = lang_graph_agent._prepare_messages_with_system_prompt(
            messages, system_prompt="New tax-specific prompt", classification=tax_classification
        )
    ...
    @pytest.mark.asyncio
    @patch("app.core.langgraph.graph.rag_step_log")
    async def test_step_46_replace_with_different_message_types(self, mock_log, lang_graph_agent, tax_classification):
        ...
        # Call the method
        result = lang_graph_agent._prepare_messages_with_system_prompt(
            messages, system_prompt="Replaced system prompt", classification=tax_classification
        )
    ...
    async def test_step_46_replace_with_low_confidence_classification(
        ...
        # Call the method
        result = lang_graph_agent._prepare_messages_with_system_prompt(
            messages_with_system, system_prompt="Low confidence tax prompt", classification=low_conf_classification
        )
    ...
    async def test_step_46_replace_system_message_comprehensive_logging(
        ...
        lang_graph_agent._prepare_messages_with_system_prompt(
            messages_with_system, system_prompt="Comprehensive test prompt", classification=tax_classification
        )
    ...
    async def test_step_46_performance_tracking_during_replace(
        self, mock_timer, mock_log, lang_graph_agent, messages_with_system, tax_classification
    ):
        ...
        # Call the method
        lang_graph_agent._prepare_messages_with_system_prompt(
            messages_with_system, system_prompt="Performance test prompt", classification=tax_classification
        )
    ...
    @pytest.mark.asyncio
    @patch("app.core.langgraph.graph.rag_step_log")
    async def test_step_46_replace_multiple_scenarios(self, mock_log, lang_graph_agent):
