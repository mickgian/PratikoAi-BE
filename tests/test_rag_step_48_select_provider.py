...
class TestRAGStep48SelectProvider:
    ...
    def labor_classification_balanced(self):
        ...
        return DomainActionClassification(
            domain=Domain.LABOR, action=Action.CCNL_QUERY, confidence=0.85, reasoning="CCNL labor agreement query"
        )
    ...
    async def test_step_48_select_provider_with_tax_classification(
        ...
        step_48_logs = [call for call in mock_log.call_args_list if len(call[1]) > 3 and call[1].get("step") == 48]
    ...
    async def test_step_48_select_provider_without_classification(
        self, mock_log, mock_get_llm_provider, lang_graph_agent, messages_with_system, mock_openai_provider
    ):
        ...
        # Call the method
        lang_graph_agent._get_optimal_provider(messages_with_system)
    ...
    async def test_step_48_select_provider_with_preferred_provider(
        self, mock_log, mock_get_llm_provider, lang_graph_agent, messages_with_system, mock_anthropic_provider
    ):
    ...
    async def test_step_48_select_provider_fallback_on_error(
        self, mock_logger, mock_log, mock_get_llm_provider, lang_graph_agent, messages_with_system
    ):
    ...
    async def test_step_48_classification_aware_routing_strategies(
        ...
        for domain, action, expected_strategy, expected_cost in test_scenarios:
            ...
            classification = DomainActionClassification(
                domain=domain, action=action, confidence=0.8, reasoning=f"Test {domain.value} {action.value}"
            )
    ...
    async def test_step_48_performance_tracking(
        self, mock_timer, mock_log, mock_get_llm_provider, lang_graph_agent, messages_with_system, mock_openai_provider
    ):
        ...
        mock_timer.assert_called_with(
            48, "RAG.providers.langgraphagent.get.optimal.provider.select.llm.provider", "SelectProvider"
        )
    ...
    async def test_step_48_edge_cases_and_defaults(
        self, mock_log, mock_get_llm_provider, lang_graph_agent, mock_openai_provider
    ):
        ...
        # Call the method
        lang_graph_agent._get_optimal_provider(empty_messages)
