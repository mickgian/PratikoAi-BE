...
class TestRAGStep49ApplyRoutingStrategy:
    ...
    async def test_step_49_apply_cost_optimized_routing(
        self, mock_log, mock_messages, mock_openai_provider, mock_llm_factory
    ):
    ...
    async def test_step_49_apply_quality_first_routing(
        self, mock_log, mock_messages, mock_claude_provider, mock_llm_factory
    ):
        ...
        mock_llm_factory.get_optimal_provider.assert_called_once_with(
            messages=mock_messages, strategy=RoutingStrategy.QUALITY_FIRST, max_cost_eur=0.030, preferred_provider=None
        )
    ...
    async def test_step_49_apply_balanced_routing(
        ...
        # Execute
        result = apply_routing_strategy(
            messages=mock_messages, strategy=RoutingStrategy.BALANCED, max_cost_eur=0.020, llm_factory=mock_llm_factory
        )

        # Verify
        assert result == mock_openai_provider
        mock_llm_factory.get_optimal_provider.assert_called_once_with(
            messages=mock_messages, strategy=RoutingStrategy.BALANCED, max_cost_eur=0.020, preferred_provider=None
        )
    ...
    @pytest.mark.asyncio
    @patch("app.ragsteps.facts.step_49_rag_facts_llmfactory_get_optimal_provider_apply_routing_strategy.rag_step_log")
    async def test_step_49_apply_routing_factory_error_fallback(self, mock_log, mock_messages, mock_llm_factory):
    ...
    @pytest.mark.asyncio
    @patch("app.ragsteps.facts.step_49_rag_facts_llmfactory_get_optimal_provider_apply_routing_strategy.rag_step_log")
    async def test_step_49_edge_cases_and_defaults(self, mock_log, mock_openai_provider, mock_llm_factory):
        ...
        result = apply_routing_strategy(
            messages=[], strategy=RoutingStrategy.COST_OPTIMIZED, max_cost_eur=None, llm_factory=mock_llm_factory
        )

        # Verify
        assert result == mock_openai_provider
        mock_llm_factory.get_optimal_provider.assert_called_once_with(
            messages=[], strategy=RoutingStrategy.COST_OPTIMIZED, max_cost_eur=None, preferred_provider=None
        )
    ...
    @pytest.mark.asyncio
    @patch("app.ragsteps.facts.step_49_rag_facts_llmfactory_get_optimal_provider_apply_routing_strategy.rag_step_log")
    async def test_step_49_run_adapter_function(self, mock_log):
    ...
    async def test_step_49_routing_strategy_variations(
        ...
        for strategy, max_cost in strategies:
            ...
            # Execute
            result = apply_routing_strategy(
                messages=mock_messages, strategy=strategy, max_cost_eur=max_cost, llm_factory=mock_llm_factory
            )

            # Verify
            assert result == mock_openai_provider
            mock_llm_factory.get_optimal_provider.assert_called_once_with(
                messages=mock_messages, strategy=strategy, max_cost_eur=max_cost, preferred_provider=None
            )
    ...
    @pytest.mark.asyncio
    @patch("app.ragsteps.facts.step_49_rag_facts_llmfactory_get_optimal_provider_apply_routing_strategy.rag_step_log")
    async def test_step_49_performance_tracking(self, mock_log, mock_messages, mock_openai_provider, mock_llm_factory):
