...
class TestRAGStep49RouteStrategy:
    ...
    async def test_step_49_apply_routing_strategy_success(
        self, mock_get_factory, mock_logger, mock_rag_log, mock_openai_provider
    ):
        ...
        mock_factory.get_optimal_provider.assert_called_once_with(
            messages=messages, strategy=RoutingStrategy.COST_OPTIMIZED, max_cost_eur=0.5, preferred_provider="openai"
        )
    ...
    @patch("app.core.logging.logger")
    @patch("app.orchestrators.facts.get_llm_factory")
    async def test_step_49_quality_first_strategy(
        self, mock_get_factory, mock_logger, mock_rag_log, mock_openai_provider
    ):
        ...
        messages = [Message(role="user", content="Complex legal document analysis")]

        ctx = {"messages": messages, "routing_strategy": RoutingStrategy.QUALITY_FIRST, "max_cost_eur": 2.0}

        result = step_49__route_strategy(ctx=ctx)

        # Verify quality-first strategy was applied
        mock_factory.get_optimal_provider.assert_called_once_with(
            messages=messages, strategy=RoutingStrategy.QUALITY_FIRST, max_cost_eur=2.0, preferred_provider=None
        )
    ...
    async def test_step_49_handle_factory_error(self, mock_get_factory, mock_logger, mock_rag_log):
        ...
        ctx = {"messages": messages, "routing_strategy": RoutingStrategy.BALANCED}
    ...
    @patch("app.core.logging.logger")
    @patch("app.orchestrators.facts.get_llm_factory")
    async def test_step_49_balanced_strategy_with_budget(
        self, mock_get_factory, mock_logger, mock_rag_log, mock_openai_provider
    ):
        ...
        mock_factory.get_optimal_provider.assert_called_once_with(
            messages=messages, strategy=RoutingStrategy.BALANCED, max_cost_eur=1.0, preferred_provider="anthropic"
        )
    ...
    async def test_step_49_performance_tracking(
        self, mock_get_factory, mock_logger, mock_rag_log, mock_timer, mock_openai_provider
    ):
    ...
    async def test_step_49_ready_for_step_50(self, mock_get_factory, mock_logger, mock_rag_log, mock_openai_provider):
        ...
        ctx = {"messages": messages, "routing_strategy": RoutingStrategy.COST_OPTIMIZED, "max_cost_eur": 0.5}
    ...
    @patch("app.core.logging.logger")
    @patch("app.orchestrators.facts.get_llm_factory")
    async def test_step_49_comprehensive_logging(
        self, mock_get_factory, mock_logger, mock_rag_log, mock_openai_provider
    ):
        ...
        ctx = {"messages": messages, "routing_strategy": RoutingStrategy.BALANCED, "max_cost_eur": 1.0}
    ...
    async def test_step_49_parity_test(self, mock_get_factory, mock_logger, mock_rag_log, mock_openai_provider):
        ...
        mock_factory.get_optimal_provider.assert_called_once_with(
            messages=messages, strategy=RoutingStrategy.COST_OPTIMIZED, max_cost_eur=0.75, preferred_provider="openai"
        )
