...
class TestRAGStep50Orchestrator:
    ...
    async def test_step_50_quality_first_decision(self, mock_logger, mock_rag_log):
        ...
        messages = [Message(role="user", content="Complex legal analysis")]
    ...
    async def test_step_50_unsupported_strategy_fallback(self, mock_logger, mock_rag_log):
        ...
        ctx = {"routing_strategy": unsupported_strategy, "messages": messages, "provider": MagicMock()}
    ...
    async def test_step_50_performance_tracking(self, mock_logger, mock_rag_log, mock_timer):
        ...
        step_50__strategy_type(
            ctx={"messages": messages, "routing_strategy": RoutingStrategy.COST_OPTIMIZED, "provider": MagicMock()}
        )

        # Verify timer was used
        mock_timer.assert_called_with(50, "RAG.platform.routing.strategy", "StrategyType", stage="start")
