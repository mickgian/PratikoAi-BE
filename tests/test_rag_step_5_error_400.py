...
class TestRAGStep5Error400:
    ...
    async def test_step_5_validation_error_response(self, mock_logger, mock_rag_log, mock_validation_errors, mock_request_context):
    ...
    @patch('app.orchestrators.platform.rag_step_log')
    @patch('app.core.logging.logger')
    async def test_step_5_workflow_termination(self, mock_logger, mock_rag_log, mock_validation_errors, mock_request_context):
    ...
    @patch('app.orchestrators.platform.rag_step_log')
    @patch('app.core.logging.logger')
    async def test_step_5_comprehensive_logging(self, mock_logger, mock_rag_log, mock_validation_errors, mock_request_context):
        ...
        required_fields = [
            ...
            'status_code', 'error_returned', 'error_type',
            'processing_stage', 'workflow_terminated'
        ]
    ...
    async def test_step_5_performance_tracking(self, mock_logger, mock_rag_log, mock_validation_errors):
        ...
        with patch('app.orchestrators.platform.rag_step_timer') as mock_timer:
            ...
            mock_timer.assert_called_with(
                5,
                'RAG.platform.return.400.bad.request',
                'Error400',
                stage='start'
            )
    ...
    @patch('app.orchestrators.platform.rag_step_log')
    @patch('app.core.logging.logger')
    async def test_step_5_parity_preservation(self, mock_logger, mock_rag_log, mock_validation_errors, mock_request_context):
