...
class TestRAGStep77SimpleAIMsg:
    ...
    async def test_step_77_convert_simple_response(self, mock_rag_log):
        ...
        ctx = {"llm_response": llm_response, "request_id": "test-77-simple"}
    ...
    async def test_step_77_preserves_content(self, mock_rag_log):
        ...
        content = (
            "Based on the current regulations, the maximum deduction for home office expenses is €2,500 per year."
        )

        llm_response = LLMResponse(
            content=content, tool_calls=None, cost_estimate=0.001, model="gpt-4", provider="openai"
        )

        ctx = {"llm_response": llm_response, "request_id": "test-77-content"}
    ...
    async def test_step_77_no_tool_calls(self, mock_rag_log):
        ...
        ctx = {"llm_response": llm_response, "request_id": "test-77-no-tools"}
    ...
    async def test_step_77_routes_to_final_response(self, mock_rag_log):
        ...
        llm_response = LLMResponse(
            content="Test response", tool_calls=[], cost_estimate=0.001, model="gpt-4", provider="openai"
        )

        ctx = {"llm_response": llm_response, "request_id": "test-77-route"}
    ...
    async def test_step_77_empty_content(self, mock_rag_log):
        ...
        llm_response = LLMResponse(content="", tool_calls=[], cost_estimate=0.001, model="gpt-4", provider="openai")

        ctx = {"llm_response": llm_response, "request_id": "test-77-empty"}
...
class TestRAGStep77Parity:
    ...
    async def test_step_77_parity_simple_aimessage_creation(self, mock_rag_log):
        ...
        llm_response = LLMResponse(
            content=content, tool_calls=[], cost_estimate=0.001, model="gpt-4", provider="openai"
        )
    ...
    async def test_step_77_parity_langchain_import(self, mock_rag_log):
        ...
        llm_response = LLMResponse(
            content="Test", tool_calls=[], cost_estimate=0.001, model="gpt-4", provider="openai"
        )
...
class TestRAGStep77Integration:
    ...
    async def test_step_75_to_77_integration(self, mock_rag_log):
        ...
        step_75_ctx = {"llm_response": llm_response, "request_id": "test-integration-75-77"}
        ...
        step_77_ctx = {"llm_response": step_75_result["llm_response"], "request_id": step_75_result["request_id"]}
    ...
    async def test_step_77_prepares_for_final_response(self, mock_rag_log):
        ...
        llm_response = LLMResponse(
            content="Final answer to user query", tool_calls=[], cost_estimate=0.001, model="gpt-4", provider="openai"
        )
