...
class TestRAGStep78ExecuteTools:
    ...
    async def test_step_78_execute_single_tool(self, mock_rag_log):
        ...
        tools_by_name = {"search_knowledge_base": mock_tool}

        ctx = {"ai_message": ai_message, "tools_by_name": tools_by_name, "request_id": "test-78-single"}
    ...
    async def test_step_78_execute_multiple_tools(self, mock_rag_log):
        ...
        tools_by_name = {"search_kb": mock_kb, "search_faq": mock_faq, "ccnl_query": mock_ccnl}

        ctx = {"ai_message": ai_message, "tools_by_name": tools_by_name, "request_id": "test-78-multiple"}
    ...
    async def test_step_78_tool_with_complex_args(self, mock_rag_log):
            ...
            tool_calls=[
                {
                    ...
                    "args": {"query": "contract law", "limit": 5, "filters": {"type": "legal", "year": 2024}},
                    "id": "call_complex",
                }
            ],
        )
        ...
        ctx = {"ai_message": ai_message, "tools_by_name": tools_by_name, "request_id": "test-78-complex"}
        ...
        mock_tool.ainvoke.assert_called_once_with(
            {"query": "contract law", "limit": 5, "filters": {"type": "legal", "year": 2024}}
        )
    ...
    async def test_step_78_routes_to_chat_node(self, mock_rag_log):
        ...
        ai_message = AIMessage(content="Executing tool.", tool_calls=[{"name": "test_tool", "args": {}, "id": "t1"}])

        mock_tool = AsyncMock(return_value="Tool result")

        tools_by_name = {"test_tool": mock_tool}

        ctx = {"ai_message": ai_message, "tools_by_name": tools_by_name, "request_id": "test-78-route"}
    ...
    async def test_step_78_tool_not_found(self, mock_rag_log):
        ...
        ai_message = AIMessage(
            content="Calling unknown tool.", tool_calls=[{"name": "unknown_tool", "args": {}, "id": "unknown"}]
        )

        tools_by_name = {}

        ctx = {"ai_message": ai_message, "tools_by_name": tools_by_name, "request_id": "test-78-not-found"}
...
class TestRAGStep78Parity:
    ...
    async def test_step_78_parity_tool_execution(self, mock_rag_log):
        ...
        ai_message = AIMessage(content="Test", tool_calls=[{"name": "test_tool", "args": {"x": 1}, "id": "t1"}])

        mock_tool = AsyncMock()
        mock_tool.ainvoke = AsyncMock(return_value="Test result")
        tools_by_name = {"test_tool": mock_tool}

        ctx = {"ai_message": ai_message, "tools_by_name": tools_by_name, "request_id": "test-parity"}
    ...
    async def test_step_78_parity_langchain_import(self, mock_rag_log):
        ...
        ai_message = AIMessage(content="Test", tool_calls=[{"name": "test", "args": {}, "id": "1"}])

        mock_tool = AsyncMock()
        mock_tool.ainvoke = AsyncMock(return_value="Result")
        tools_by_name = {"test": mock_tool}

        ctx = {"ai_message": ai_message, "tools_by_name": tools_by_name, "request_id": "test-parity-import"}
...
class TestRAGStep78Integration:
    ...
    async def test_step_76_to_78_integration(self, mock_rag_log):
        ...
        step_76_ctx = {"llm_response": llm_response, "request_id": "test-integration-76-78"}
    ...
    async def test_step_78_prepares_for_next_llm_iteration(self, mock_rag_log):
        ...
        tools_by_name = {"tool1": mock_tool1, "tool2": mock_tool2}
