...
from app.core.langgraph.tools.document_ingest_tool import DocumentIngestInput, DocumentIngestTool
...
class TestDocumentIngestTool:
    ...
    async def test_document_ingest_process_single_pdf(self, document_ingest_tool, sample_pdf_attachment):
        ...
        with (
            patch("app.core.langgraph.tools.document_ingest_tool.rag_step_log") as mock_log,
            ...
            input_data = DocumentIngestInput(
                attachments=[sample_pdf_attachment], user_id="user_123", session_id="session_456"
            )

            # Process document
            result = await document_ingest_tool._arun(
                attachments=input_data.attachments, user_id=input_data.user_id, session_id=input_data.session_id
            )
    ...
    async def test_document_ingest_file_size_limit(self, document_ingest_tool):
        ...
        with patch("app.core.langgraph.tools.document_ingest_tool.rag_step_log") as mock_log:
            # Create input
            input_data = DocumentIngestInput(
                attachments=[large_attachment], user_id="user_123", session_id="session_456"
            )
    ...
    async def test_document_ingest_unsupported_file_type(self, document_ingest_tool):
        ...
        with patch("app.core.langgraph.tools.document_ingest_tool.rag_step_log") as mock_log:
            # Create input
            input_data = DocumentIngestInput(
                attachments=[unsupported_attachment], user_id="user_123", session_id="session_456"
            )
    ...
    async def test_document_ingest_performance_timing(self, document_ingest_tool):
        ...
        with (
            ...
            await document_ingest_tool._arun(
                attachments=test_input.attachments, user_id=test_input.user_id, session_id=test_input.session_id
            )
...
async def test_document_ingest_tool_full_flow():
    ...
    with (
        ...
        result = await tool._arun(
            attachments=attachments, user_id="user_italian_business_001", session_id="session_invoice_processing_2024"
        )
