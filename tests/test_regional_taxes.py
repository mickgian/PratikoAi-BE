...
class TestLocationDetection:
    ...
    async def test_cap_to_comune_mapping_roma(self, sample_comuni):
        ...
        roma_data = next(c for c in sample_comuni if c["nome"] == "Roma")
    ...
    async def test_multiple_cap_same_comune(self):
        ...
        service = ItalianLocationService(Mock(), Mock())
        service.db.query = AsyncMock(return_value=Mock(
            comune="Roma",
            provincia="RM",
            regione="Lazio"
        ))
...
class TestIMUCalculations:
    ...
    async def test_imu_rate_comparison_different_cities(self, sample_comuni, sample_imu_rates):
        ...
        roma_rates = next(r for r in sample_imu_rates if 
                         any(c["id"] == r["comune_id"] and c["nome"] == "Roma" for c in sample_comuni))
        milano_rates = next(r for r in sample_imu_rates if
                           any(c["id"] == r["comune_id"] and c["nome"] == "Milano" for c in sample_comuni))
        napoli_rates = next(r for r in sample_imu_rates if
                           any(c["id"] == r["comune_id"] and c["nome"] == "Napoli" for c in sample_comuni))
...
class TestIRAPCalculations:
    ...
    async def test_irap_regional_comparison(self, sample_regioni, sample_irap_rates):
        ...
        lazio_rates = next(r for r in sample_irap_rates if
                          any(regione["id"] == r["regione_id"] and regione["nome"] == "Lazio" 
                              for regione in sample_regioni))
        lombardia_rates = next(r for r in sample_irap_rates if
                              any(regione["id"] == r["regione_id"] and regione["nome"] == "Lombardia"
                                  for regione in sample_regioni))
...
class TestIRPEFAddizionali:
    ...
    async def test_addizionale_irpef_roma_full_calculation(self, sample_comuni, sample_addizionale_rates):
        ...
        result = await service.calculate_irpef_addizionali(
            reddito_imponibile=Decimal("50000"),
            cap="00100"
        )
    ...
    async def test_addizionale_irpef_milano_calculation(self, sample_comuni, sample_addizionale_rates):
        ...
        result = await service.calculate_irpef_addizionali(
            reddito_imponibile=Decimal("60000"),
            cap="20100"
        )
    ...
    async def test_addizionale_irpef_campania_higher_regional(self, sample_comuni, sample_addizionale_rates):
        ...
        result = await service.calculate_irpef_addizionali(
            reddito_imponibile=Decimal("40000"),
            cap="80100"
        )
    ...
    async def test_addizionale_municipal_threshold_exemption(self, sample_addizionale_rates):
        ...
        service.get_municipal_addizionale_irpef = AsyncMock()
        service.get_municipal_tax_info = AsyncMock(return_value={
            "rate": Decimal("0.9"),
            "threshold": 11000
        })
...
class TestRegionalTaxIntegration:
    ...
    async def test_complete_tax_calculation_roma(self, sample_comuni, sample_imu_rates, sample_addizionale_rates):
        ...
        service.calculate_imu = AsyncMock(return_value={
            "comune": "Roma",
            "imposta_dovuta": 0,  # Prima casa exempt
            "note": "Abitazione principale esente IMU"
        })
        
        service.calculate_irpef_addizionali = AsyncMock(return_value={
            "comune": "Roma",
            "regione": "Lazio",
        ...
        service.get_location_from_cap = AsyncMock(return_value={
            "comune": "Roma",
            "provincia": "RM",
            "regione": "Lazio"
        })
    ...
    async def test_business_tax_calculation_with_location(self):
        ...
        service.calculate_irap = AsyncMock(return_value={
            "regione": "Lazio",
            "aliquota": 4.82,
        ...
        result = await service.calculate_business_taxes(
            cap="00100",
            revenue=1000000,
            business_type="standard"
        )
...
class TestRegionalTaxDataManagement:
    ...
    async def test_tax_rate_historical_lookup(self):
        ...
        service.get_tax_rates = AsyncMock(return_value=Mock(
    ...
    async def test_rate_change_notification(self):
        ...
        service.detect_rate_changes = AsyncMock(return_value=[
            {
                "comune": "Roma",
                "tax_type": "IMU", 
    ...
    async def test_concurrent_rate_access(self):
        ...
        import asyncio
        tasks = [
            service.get_tax_rates(uuid4(), "IMU", date.today())
            for _ in range(10)
        ]
    ...
    async def test_api_response_caching(self):
        ...
        result1 = await service.get_all_rates_for_location("00100")
...
class TestRegionalTaxPerformance:
    ...
    async def test_fallback_to_default_rates(self):
        ...
        service.get_comune_by_cap = AsyncMock(return_value=None)
        service.get_default_rates = AsyncMock(return_value={
            "IMU": {"rate": Decimal("1.06"), "note": "Aliquota standard"}
        })
