...
import pytest
from app.core.rag.retrieval_gate import retrieval_gate, GateDecision
...
class TestRetrievalGate:
    ...
    def test_institution_reference_needs_retrieval(self):
        ...
        queries = [
            "Come dice l'Agenzia Entrate sul tema?",
            "Circolare INPS numero 10",
            "Risoluzione MEF del 2024"
        ]

        for query in queries:
            decision = retrieval_gate(query)
            assert decision.needs_retrieval == True, f"Failed for: {query}"
