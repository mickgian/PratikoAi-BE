...
from app.services.scheduler_service import SchedulerService, ScheduledTask, ScheduleInterval, scheduler_service
...
class TestRSSFeedActivation:
    ...
    async def test_rss_feed_monitor_initialization(self):
        ...
        for feed_key, feed_url in monitor.italian_feeds.items():
    ...
    async def test_inps_feed_collection(self):
        ...
        with patch('feedparser.parse') as mock_feedparser:
            ...
            async with RSSFeedMonitor() as monitor:
                results = await monitor.parse_inps_feed(
                    'https://www.inps.it/rss/circolari.xml'
                )
        ...
        for i in range(150):
            ...
            mock_documents.append({
                ...
                'metadata': {
                    ...
                    'document_number': f'{i+1}/2025'
                }
            })
        
        # Mock successful processing
        with patch('app.services.dynamic_knowledge_collector.DynamicKnowledgeCollector.process_document_batch') as mock_batch:
    ...
    async def test_parallel_feed_processing_performance(self):
        ...
        with patch('app.services.rss_feed_monitor.RSSFeedMonitor.parse_feed') as mock_parse:
            async def mock_parse_feed(url, *args):
                ...
                return [
                    {
                        'title': f'Test Document from {url}',
                        'url': f'{url}/test.pdf',
            ...
            assert total_processing_time < 300, f"Parallel processing took {total_processing_time:.2f}s, should be < 300s"
...
class TestEndToEndRSSActivation:
    ...
    async def test_system_readiness_checklist(self):
        ...
        monitor = RSSFeedMonitor()
        checklist_results['rss_feeds_configured'] = hasattr(monitor, 'italian_feeds') and len(monitor.italian_feeds) >= 7
