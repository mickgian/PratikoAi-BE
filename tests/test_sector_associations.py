...
class TestSectorAssociationsDataSources:
    ...
    async def test_federmeccanica_initialization(self):
        ...
        assert (
            source.source_info.name == "Federmeccanica - Federazione Sindacale dell'Industria Metalmeccanica Italiana"
        )
        ...
        expected_sectors = [CCNLSector.METALMECCANICI_INDUSTRIA, CCNLSector.METALMECCANICI_ARTIGIANI]
...
class TestSectorAssociationsConnectivity:
    ...
    async def test_search_without_connection(self):
        ...
        # Don't connect first
        query = DataSourceQuery(sectors=[CCNLSector.CHIMICI_FARMACEUTICI], max_results=5)
...
class TestSectorAssociationsSearch:
    ...
    async def test_sector_filtering(self):
        ...
        query_supported = DataSourceQuery(sectors=[CCNLSector.METALMECCANICI_INDUSTRIA], max_results=10)
    ...
    async def test_get_associations_for_sectors(self):
        ...
        sector_association_sources = [
            s for s in unknown_sources if s.source_info.source_type == DataSourceType.SECTOR_ASSOCIATION
        ]
...
class TestSectorAssociationsErrorHandling:
    ...
    async def test_network_timeout_handling(self):
        ...
        query = DataSourceQuery(sectors=[CCNLSector.ICT], max_results=5)
    ...
    async def test_malformed_response_handling(self):
        ...
        query = DataSourceQuery(sectors=[CCNLSector.EDILIZIA_INDUSTRIA], max_results=3)
...
class TestSectorAssociationsPerformance:
    ...
    async def test_concurrent_searches(self):
        ...
        for source in sources[:3]:  # Test first 3 sources
            if source.source_info.supported_sectors:
                query = DataSourceQuery(sectors=source.source_info.supported_sectors[:1], max_results=5)
    ...
    async def test_search_response_time(self):
        ...
        source = FederalimentareDataSource()
        query = DataSourceQuery(sectors=[CCNLSector.ALIMENTARI_INDUSTRIA], max_results=5)
