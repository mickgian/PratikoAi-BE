...
class TestConfigurationResolution:
    ...
    def test_namespace_prefix_configuration(self):
        ...
        with patch.dict(os.environ, {"APP_ENV": "production", "PINECONE_NAMESPACE_PREFIX": "prod-ns"}, clear=True):
            from app.services.vector_config import VectorConfig
    ...
    def test_vector_strict_mode_configuration(self):
        ...
        with patch.dict(
            os.environ, {"VECTOR_STRICT_EMBEDDER_MATCH": "true", "VECTOR_STRICT_MODE": "true"}, clear=True
        ):
            from app.services.vector_config import VectorConfig
    ...
    def test_embedding_model_configuration(self):
        ...
        with patch.dict(
            os.environ,
            {"EMBEDDING_MODEL": "sentence-transformers/all-MiniLM-L6-v2", "EMBEDDING_DIMENSION": "384"},
            clear=True,
        ):
            from app.services.vector_config import VectorConfig
...
class TestNamespaceGeneration:
    ...
    def test_namespace_with_all_components(self):
        ...
        namespace = config.build_namespace(env="staging", domain="legale", tenant="client-123")
    ...
    def test_namespace_domain_validation(self):
        ...
        for domain in valid_domains:
            namespace = config.build_namespace(env="dev", domain=domain, tenant="default")
    ...
    def test_namespace_invalid_domain(self):
        ...
        with pytest.raises(ValueError, match="Invalid domain"):
            config.build_namespace(env="dev", domain="invalid-domain", tenant="default")
    ...
    def test_namespace_environment_validation(self):
        ...
        for env in valid_envs:
            namespace = config.build_namespace(env=env, domain="fiscale", tenant="default")
...
class TestEnvironmentSpecificBehavior:
    ...
    def test_production_config_strictness(self):
        ...
        with patch.dict(
            os.environ,
            {"APP_ENV": "production", "PINECONE_API_KEY": "prod-key", "VECTOR_STRICT_MODE": "true"},
            clear=True,
        ):
            from app.services.vector_config import VectorConfig
...
class TestConfigurationSerialization:
    ...
    def test_safe_config_serialization(self):
        ...
        with patch.dict(
            os.environ,
            {"PINECONE_API_KEY": "pcsk_secret_key", "EMBEDDING_MODEL": "sentence-transformers/all-MiniLM-L6-v2"},
            clear=True,
        ):
            from app.services.vector_config import VectorConfig
    ...
    def test_config_logging_format(self):
        ...
        with patch.dict(
            os.environ,
            {"PINECONE_API_KEY": "test-key", "PINECONE_INDEX_NAME": "pratikoai-dev", "EMBEDDING_DIMENSION": "384"},
            clear=True,
        ):
            from app.services.vector_config import VectorConfig
