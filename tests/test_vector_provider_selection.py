...
class TestProviderSelection:
    ...
    def test_provider_selection_dev_explicit_pinecone_override(self):
        ...
        with (
            patch.dict(
                os.environ,
                {"APP_ENV": "development", "VEC_PROVIDER": "pinecone", "PINECONE_API_KEY": "test-key"},
                clear=True,
            ),
            patch("app.core.config.get_environment", return_value=Environment.DEVELOPMENT),
        ):
            from app.services.vector_provider_factory import VectorProviderFactory
    ...
    def test_provider_selection_staging_uses_pinecone_when_keys_present(self):
        ...
        with patch.dict(os.environ, {"APP_ENV": "staging", "PINECONE_API_KEY": "staging-key"}, clear=True):
...
class TestConfigResolution:
    ...
    def test_builds_namespace_with_env_and_domain(self):
        ...
        namespace = factory.build_namespace(env="dev", domain="fiscale", tenant="default")
    ...
    def test_builds_namespace_with_different_domains(self):
        ...
        for domain in domains:
            namespace = factory.build_namespace(env="staging", domain=domain, tenant="default")
...
class TestFallbackBehavior:
    ...
    def test_pinecone_unavailable_falls_back_to_local_and_logs_warn(self, mock_pinecone):
        ...
        with patch("app.core.logging.logger") as mock_logger:
            ...
            with patch.dict(os.environ, {"VEC_PROVIDER": "pinecone", "PINECONE_API_KEY": "test-key"}):
    ...
    def test_pinecone_failure_in_production_with_strict_mode(self, mock_pinecone):
        ...
        with patch.dict(
            os.environ, {"APP_ENV": "production", "PINECONE_API_KEY": "prod-key", "VECTOR_STRICT_MODE": "true"}
        ):
...
class TestAdapterContracts:
    ...
    def test_upsert_calls_adapter_with_ids_vectors_metadata(self):
        ...
        with patch.object(provider, "_upsert_vectors") as mock_upsert:
            test_data = [
                {"id": "doc-1", "vector": [0.1, 0.2, 0.3], "metadata": {"source": "test", "domain": "fiscale"}}
            ]
    ...
    def test_query_returns_hits_with_scores_and_metadata(self):
        ...
        # Mock query response
        mock_response = [{"id": "doc-1", "score": 0.95, "metadata": {"source": "test", "title": "Test Document"}}]

        with patch.object(provider, "_query_vectors", return_value=mock_response):
            results = provider.query(
                vector=[0.1, 0.2, 0.3], top_k=5, namespace="env=dev,domain=fiscale,tenant=default"
            )
...
class TestPipelineIntegration:
    ...
    def test_ccnl_search_returns_hits_after_upsert_mock(self, mock_factory):
        ...
        mock_provider.query.return_value = [
            {
                "id": "ccnl-1",
                "score": 0.85,
                "metadata": {"domain": "ccnl", "title": "Contratto Metalmeccanici", "article": "Art. 15"},
            }
        ]
        mock_factory.return_value.get_provider.return_value = mock_provider
        ...
        results = search_service.semantic_search(query="orario lavoro", domain="ccnl", limit=10)
