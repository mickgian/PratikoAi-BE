...
class TestHybridSearchEngine:
    ...
    def hybrid_search_engine(
        self, mock_postgres_service, mock_pinecone_service, mock_embedding_service, mock_normalizer
    ):
    ...
    async def test_deduplication_of_overlapping_results(
        ...
        mock_postgres_service.search.return_value = [
            {
                "id": "faq_1",
                "content": "Fattura elettronica obbligatoria",
                "source_type": "faq",
                "rank": 0.9,
                "metadata": {},
            },
            ...
            "matches": [
                {
                    "id": "faq_1",
                    "score": 0.88,
                    "metadata": {"content": "Fattura elettronica obbligatoria", "source_type": "faq"},
                },
                {
                    "id": "faq_3",
                    "score": 0.82,
                    "metadata": {"content": "Sistema di Interscambio SDI", "source_type": "faq"},
                },
            ]
        }
    ...
    async def test_performance_under_300ms_for_hybrid_search(
        ...
        def mock_pinecone_query(*args, **kwargs):
            ...
            return {
                "matches": [{"id": "test_2", "score": 0.85, "metadata": {"content": "test", "source_type": "faq"}}]
            }
            ...
            "matches": [
                {
                    "id": "med_vector",
                    "score": 0.7,
                    "metadata": {"content": "medium vector", "source_type": "regulation"},
                },
...
class TestSemanticFAQMatching:
    ...
    async def test_faq_hit_rate_improvement_target_70_percent(self, faq_matcher):
        ...
        faq_matcher.pinecone.query.return_value = {
            "matches": [
                {"id": "faq_1", "score": 0.88, "metadata": {"concepts": ["iva", "calcolo"]}},
                {"id": "faq_2", "score": 0.82, "metadata": {"concepts": ["aliquota", "22%"]}},
            ]
        }
    ...
    async def test_similarity_threshold_tuning(self, faq_matcher):
        ...
        def mock_get_faq(faq_id):
            score_map = {"exact_match": 0.96, "high_match": 0.86, "medium_match": 0.78, "low_match": 0.65}
    ...
    async def test_false_positive_prevention(self, faq_matcher):
        ...
        faq_matcher.pinecone.query.return_value = {
            "matches": [
                {"id": "relevant_faq", "score": 0.85, "metadata": {"concepts": ["iva", "fattura"]}},
                {
                    "id": "irrelevant_faq",
        ...
        def mock_get_faq(faq_id):
            if faq_id == "relevant_faq":
                return MockFAQMatch(
                    faq_id, "IVA su fattura", "L'IVA si calcola...", 0.85, "high", matched_concepts=["iva", "fattura"]
                )
            else:
                return MockFAQMatch(
                    faq_id,
    ...
    async def test_multi_faq_matching_for_complex_queries(self, faq_matcher):
        ...
        with patch.object(faq_matcher, "_check_faq_freshness", new_callable=AsyncMock) as mock_freshness:
            ...
            matches = await faq_matcher.find_matching_faqs("iva fattura elettronica regime forfettario", max_results=3)
    ...
    async def test_performance_with_1000_plus_faqs(self, faq_matcher):
        ...
        large_matches = [{"id": f"faq_{i}", "score": 0.8 - (i * 0.001), "metadata": {}} for i in range(1000)]
    ...
    async def test_italian_language_semantic_accuracy(self, faq_matcher):
        ...
        faq_matcher.pinecone.query.return_value = {
            "matches": [
                ...
                {"id": "faq_italian_2", "score": 0.87, "metadata": {"concepts": ["modello", "730"], "language": "it"}},
            ]
        }
        ...
        def mock_get_faq(faq_id):
            if faq_id == "faq_italian_1":
                ...
                return MockFAQMatch(faq_id, "Modello 730 precompilato", "Il modello 730...", 0.87, "high")
...
class TestKnowledgeBaseIntegration:
    ...
    @pytest.mark.asyncio
    async def test_regulatory_document_semantic_search(self, context_builder, mock_hybrid_search):
        ...
        with patch.object(context_builder, "_extract_relevant_portion", new_callable=AsyncMock) as mock_extract:
            ...
            with patch.object(context_builder, "_count_tokens", return_value=150):
                context = await context_builder.build_context("fatturazione elettronica obbligatoria 2024")
    ...
    @pytest.mark.asyncio
    async def test_multi_source_context_building(self, context_builder, mock_hybrid_search):
    ...
    @pytest.mark.asyncio
    async def test_relevance_scoring_across_source_types(self, context_builder, mock_hybrid_search):
    ...
    @pytest.mark.asyncio
    async def test_date_based_relevance_decay(self, context_builder, mock_hybrid_search):
        ...
        mock_hybrid_search.search.return_value = [
            MockSearchResult(
                "old_reg", "Old regulation", "regulation", 0.90, metadata={"updated_at": old_date.isoformat()}
            ),
            MockSearchResult(
                "recent_reg", "Recent regulation", "regulation", 0.85, metadata={"updated_at": recent_date.isoformat()}
            ),
        ]
    ...
    @pytest.mark.asyncio
    async def test_context_size_optimization(self, context_builder, mock_hybrid_search):
        ...
        with patch.object(context_builder, "_extract_relevant_portion", new_callable=AsyncMock) as mock_extract:
            ...
            with patch.object(context_builder, "_count_tokens", side_effect=mock_count_tokens):
                context = await context_builder.build_context("test query", max_tokens=2000)
...
class TestPerformanceRequirements:
    ...
    def test_memory_usage_efficiency(self):
        ...
        large_results = [MockSearchResult(f"id_{i}", f"content_{i}", "faq", 0.8) for i in range(1000)]
...
class TestItalianLanguageSpecifics:
    ...
    async def test_handling_tax_specific_terminology(self):
        ...
        for _abbreviation, expanded_terms in tax_terms.items():
    ...
    def test_professional_vs_casual_language(self):
        ...
        professional_terms = ["dichiarazione dei redditi", "partita iva", "codice fiscale", "regime forfettario"]

        casual_terms = ["tasse", "soldi", "pagare", "guadagno"]
