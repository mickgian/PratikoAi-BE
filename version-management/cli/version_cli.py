    ...
    def check_compatibility(self, service_type: str, version: str, environment: str = 'development'):
        ...
        try:
            ...
            result = loop.run_until_complete(
                checker.check_deployment_compatibility(
            ...
            if matrix:
                ...
                for key, level in matrix.items():
                    if ':' in key:
                        ...
                        elif level in ['backward_compatible', 'forward_compatible']:
                            level_display = f"{Fore.YELLOW}⚠️  {level}"
                        elif level == 'limited_compatible':
                            level_display = f"{Fore.YELLOW}⚠️  {level}"
...
Examples:
  ...
  %(prog)s register
    ...
    # Register command
    register_parser = subparsers.add_parser('register', help='Register version interactively')
