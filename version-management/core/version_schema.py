...
class ServiceVersion:
    ...
    def is_valid_version(version_str: str) -> bool:
        ...
        return bool(re.match(semantic_pattern, version_str) or re.match(date_pattern, version_str))
    ...
    def add_dependency(
        self, service_type: ServiceType, min_version: str, max_version: str | None = None, reason: str | None = None
    ):
        """Add a dependency on another service version."""
        dependency = VersionDependency(
            service_type=service_type, min_version=min_version, max_version=max_version, reason=reason
        )
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        data["deployments"] = {
            env.value if isinstance(env, Environment) else env: dt.isoformat() for env, dt in self.deployments.items()
        }
...
class VersioningScheme:
    ...
    def parse_version(version_str: str) -> dict[str, Any]:
        ...
        if date_match:
            ...
            return {"type": "date", "date": date_str, "commit": commit_hash, "suffix": suffix, "raw": version_str}
...
class CompatibilityRules:
    ...
    def check_compatibility(
        service1_type: ServiceType, service1_version: str, service2_type: ServiceType, service2_version: str
    ) -> CompatibilityLevel:
...
if __name__ == "__main__":
    ...
    backend_version.add_dependency(
        ServiceType.INFRASTRUCTURE, min_version="1.1.0", reason="Requires new database schema"
    )
    ...
    api_contract = APIContract(
        version="1.2.3",
        service_type=ServiceType.BACKEND,
        endpoints=[APIEndpoint(path="/api/v1/chat/stream", method="POST", added_in_version="1.2.0")],
    ...
    compatibility = CompatibilityRules.check_compatibility(
        ServiceType.BACKEND, "1.2.3", ServiceType.FRONTEND_ANDROID, "1.2.1"
    )
    print(f"\nCompatibility: {compatibility}")

    # Test version generation
    new_version = VersioningScheme.generate_version(
        ServiceType.FRONTEND_ANDROID, ChangeType.PATCH, "1.2.3", Environment.PRODUCTION
    )
