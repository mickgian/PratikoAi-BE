...
from ..core.version_schema import (
    ...
    CompatibilityLevel, APIContract, VersionDependency
)
...
class ServiceVersionModel(Base):
    ...
    def to_service_version(self) -> ServiceVersion:
        ...
        for dep_model in self.dependencies:
            dependencies.append(VersionDependency(
...
class VersionDependencyModel(Base):
    ...
    # Indexes
    __table_args__ = (
        Index('idx_service_version_dependency', 'service_version_id', 'dependency_service_type'),
    )
...
class VersionRegistryDB:
    ...
    def register_version(self, service_version: ServiceVersion) -> str:
        ...
        with self.get_session() as session:
            try:
                ...
                existing = session.query(ServiceVersionModel).filter(
                    ServiceVersionModel.service_type == service_version.service_type,
                    ServiceVersionModel.version == service_version.version
                ).first()
                
                if existing:
                    raise ValueError(f"Version {service_version.version} already exists for {service_version.service_type.value}")
    ...
    def get_version(self, service_type: ServiceType, version: str) -> Optional[ServiceVersion]:
        """Get a specific service version."""
        with self.get_session() as session:
            version_model = session.query(ServiceVersionModel).filter(
                ServiceVersionModel.service_type == service_type,
                ServiceVersionModel.version == version
            ).first()
    ...
    def get_latest_version(self, service_type: ServiceType, environment: Environment = None) -> Optional[ServiceVersion]:
        """Get the latest version for a service, optionally filtered by environment."""
        with self.get_session() as session:
            query = session.query(ServiceVersionModel).filter(
                ServiceVersionModel.service_type == service_type
            )
            
            if environment:
                # Filter by versions deployed to specific environment
                query = query.join(DeploymentModel).filter(
    ...
    def get_versions_by_service(self, service_type: ServiceType, limit: int = 50) -> List[ServiceVersion]:
        ...
        with self.get_session() as session:
            version_models = session.query(ServiceVersionModel).filter(
                ServiceVersionModel.service_type == service_type
            ).order_by(ServiceVersionModel.created_at.desc()).limit(limit).all()
            
            return [model.to_service_version() for model in version_models]
    
    def record_deployment(self, service_type: ServiceType, version: str, 
                         environment: Environment, deployed_by: str = "system",
                         deployment_id: str = None, deployment_strategy: str = None) -> str:
        """Record a deployment."""
        with self.get_session() as session:
            try:
                ...
                existing_deployment = session.query(DeploymentModel).filter(
                ...
                if existing_deployment:
                    ...
                    existing_deployment.deployed_at = datetime.now(timezone.utc)
    ...
    def check_compatibility(self, source_service: ServiceType, source_version: str,
                          target_service: ServiceType, target_version: str) -> CompatibilityLevel:
        """Check compatibility between two service versions."""
        with self.get_session() as session:
            # Look for existing compatibility check
            source_model = session.query(ServiceVersionModel).filter(
            ...
            compatibility_check = session.query(CompatibilityCheckModel).filter(
    ...
    def record_compatibility_check(self, source_service: ServiceType, source_version: str,
                                 target_service: ServiceType, target_version: str,
                                 compatibility_level: CompatibilityLevel,
                                 breaking_changes: List[str] = None,
                                 warnings: List[str] = None,
                                 check_details: Dict[str, Any] = None) -> str:
        ...
        with self.get_session() as session:
            try:
                ...
                existing_check = session.query(CompatibilityCheckModel).filter(
                    CompatibilityCheckModel.source_version_id == source_model.id,
                    CompatibilityCheckModel.target_service_type == target_service,
                    CompatibilityCheckModel.target_version == target_version
                ).first()
                ...
                if existing_check:
                    ...
                    existing_check.checked_at = datetime.now(timezone.utc)
    ...
    def get_deployment_status(self, environment: Environment) -> Dict[str, Any]:
        ...
        with self.get_session() as session:
            deployments = session.query(DeploymentModel).join(ServiceVersionModel).filter(
                DeploymentModel.environment == environment
            ).all()
    ...
    def cleanup_old_versions(self, service_type: ServiceType, keep_count: int = 50):
        ...
        with self.get_session() as session:
            try:
                ...
                versions_to_keep = session.query(ServiceVersionModel.id).filter(
                    ServiceVersionModel.service_type == service_type
                ).order_by(ServiceVersionModel.created_at.desc()).limit(keep_count).subquery()
                
                # Delete old versions
                deleted_count = session.query(ServiceVersionModel).filter(
                    ServiceVersionModel.service_type == service_type,
                    ~ServiceVersionModel.id.in_(versions_to_keep)
                ).delete()
...
if __name__ == "__main__":
    ...
    from ..core.version_schema import ServiceVersion, ServiceType, ChangeType
