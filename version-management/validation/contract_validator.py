...
from ..core.version_schema import APIContract, APIEndpoint, CompatibilityLevel, ServiceType
...
class APIContractValidator:
    ...
    async def validate_contract_evolution(
        ...
        result = ContractValidationResult(is_compatible=True, compatibility_level=CompatibilityLevel.FULLY_COMPATIBLE)
    ...
    async def _validate_openapi_specs(
        self, old_spec: dict[str, Any], new_spec: dict[str, Any], result: ContractValidationResult
    ):
        ...
        for path in old_paths:
            ...
            for method in common_methods:
                await self._compare_method_operation(
                    path, method, old_paths[path][method], new_paths[path][method], result
                )
    ...
    async def _compare_method_operation(
        self,
        path: str,
        method: str,
    ...
    async def _compare_parameters(
        ...
        for param_name in removed_params:
            ...
            severity = (
                ContractViolationSeverity.CRITICAL
                if old_param.get("required", False)
                else ContractViolationSeverity.MINOR
            )

            change = ContractChange(
                change_type=ContractChangeType.BREAKING
                if severity == ContractViolationSeverity.CRITICAL
                else ContractChangeType.NON_BREAKING,
                severity=severity,
    ...
    async def _compare_request_body(
        self,
        endpoint_path: str,
        old_operation: dict[str, Any],
        new_operation: dict[str, Any],
        ...
        if not old_body and new_body:
            severity = (
                ...
                else ContractViolationSeverity.INFO
            )
            change = ContractChange(
                change_type=ContractChangeType.BREAKING
                if severity == ContractViolationSeverity.CRITICAL
                else ContractChangeType.ADDITIVE,
                severity=severity,
                path=f"{endpoint_path}/requestBody",
    ...
    async def _compare_responses(
        ...
        for code in common_codes:
            ...
            if old_schema and new_schema:
                await self._compare_schemas_deep(
                    f"{endpoint_path}/responses/{code}", old_schema, new_schema, result, is_request=False
                )

    async def _compare_schemas_deep(
        self,
        path: str,
    ...
    async def _compare_schemas(
        ...
        for schema_name in common_schemas:
            await self._compare_schemas_deep(
                f"/components/schemas/{schema_name}", old_schemas[schema_name], new_schemas[schema_name], result
            )
    ...
    async def _test_live_endpoint(self, base_url: str, endpoint: APIEndpoint, result: ContractValidationResult):
