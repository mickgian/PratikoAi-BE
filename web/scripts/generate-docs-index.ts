 ...
 *
 * Usage:
 *     node scripts/generate-docs-index.ts
...
class DocumentationIndexer {
  ...
  private allDocs: MarkdownFile[] = [];
  private docsByDirectory: Map<string, MarkdownFile[]> = new Map();
  ...
  private scanDirectory(dir: string, excludePatterns: string[]): void {
  ...
  private parseMarkdownFile(filePath: string): MarkdownFile {
...
**Auto-generated:** ${new Date().toISOString().split('T')[0]} ${new Date().toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit' })}
...
**Last Updated:** ${new Date().toISOString().split('T')[0]} ${new Date().toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit' })}
**Generator:** \`scripts/generate-docs-index.ts\`
`;
  }
